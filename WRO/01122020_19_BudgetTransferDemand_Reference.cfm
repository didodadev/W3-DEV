<!-- Description : BUDGET_TRANSFER_DEMAND tablosuna referans alanı eklendi
...
<querytag>
    ...
    ELSE
    BEGIN
        IF NOT EXISTS (SELECT 'Y' FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'BUDGET_TRANSFER_DEMAND' AND TABLE_SCHEMA = '@_dsn_period_@' AND COLUMN_NAME = 'REFERENCE')
        BEGIN
            ALTER TABLE BUDGET_TRANSFER_DEMAND ADD REFERENCE nvarchar(50) NULL         
        END;
    END
</querytag>