<!---
    İlker Altındal
    Create : 090621
    Desc : Workcube botton custom tagi ile kullanılır. Adreslenen cfc' ye formData olarak gelen verileri convert edip istek oluşturur, geri dönüş yapar.
    Methods : basketConverter, formConverter, delActions
--->
<cfcomponent extends="AddOns.Plevne.infrastructure.resolver">
    ...
    <cffunction name="basketConverter" access="remote" returnformat="JSON" hint="Basket tarafından gelen datalar convert edip ilgili cfc adresine gönderilir">
        ...
        <cftry>
            <cfif data_packet.basketv eq 1 >
                <cfloop collection='#data_packet.form_data#' item='k'>
                    <cfif k is 'items'>
                        <cfloop from="1" to="#arrayLen(data_packet.form_data[k])#" index="satir">
                            <cfloop collection="#data_packet.form_data[k][satir]#" item="satir_item">
                                <cfif satir_item is 'tax_percent'>
                                    <cfset data_packet.form_data["tax#satir#"] = data_packet.form_data[k][satir][satir_item]>
                                <cfelseif satir_item is 'other_money'>
                                    <cfset data_packet.form_data["other_money_#satir#"] = data_packet.form_data[k][satir][satir_item]>
                                <cfelseif satir_item is 'other_money_value'>
                                    <cfset data_packet.form_data["other_money_value_#satir#"] = data_packet.form_data[k][satir][satir_item]>
                                <cfelseif satir_item is 'other_money_grosstotal'>
                                    <cfset data_packet.form_data["other_money_grosstotal#satir#"] = data_packet.form_data[k][satir][satir_item]>
                                    <cfset data_packet.form_data["other_money_gross_total#satir#"] = data_packet.form_data[k][satir][satir_item]>
                                <cfelseif satir_item is 'deliver_dept'>
                                    <cfset data_packet.form_data["basket_row_departman#satir#"] = data_packet.form_data[k][satir][satir_item]>
                                    <cfset data_packet.form_data["deliver_dept#satir#"] = data_packet.form_data[k][satir][satir_item]>
                                <cfelseif satir_item neq 'tax'>
                                    <cfset data_packet.form_data["#satir_item##satir#"] = data_packet.form_data[k][satir][satir_item]>
                                </cfif>
                            </cfloop>
                        </cfloop>
                    <cfelseif k is 'header' or k is 'hidden_values' or k is 'footer'>
                        <cfloop collection="#data_packet.form_data[k]#" item="satir_item">
                            <cftry>
                                <cfif satir_item contains 'CKEDITOR_'>
                                    <cfset data_packet.form_data[Replace(satir_item,'CKEDITOR_','')] = data_packet.form_data[k][satir_item]>
                                <cfelse>
                                    <cfset data_packet.form_data[satir_item] = data_packet.form_data[k][satir_item]>
                                </cfif>
                            <cfcatch>
                                <cfset data_packet.form_data[satir_item] = javaCast( "null", 0 )>
                            </cfcatch>
                            </cftry>
                        </cfloop>
                    <cfelse>
