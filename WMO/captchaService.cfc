...
$Id: captchaService.cfc 6064 2007-05-18 16:59:50Z pfarrell $
--->
<cfcomponent
	...
	<cffunction name="validateCaptcha" access="public" returntype="boolean" output="false"
		...
		<cfreturn NOT CompareNoCase(getHashReference(arguments.hash,FALSE), arguments.text) />
	</cffunction>
	...
	<cffunction name="writeToStream" access="private" returntype="void" output="false"
		...
		*************************************************************************	
		...
		<!--- <cfset graphics.drawString(JavaCast("string", "Workcube"), JavaCast("int", 4), JavaCast("int", top)) /> --->
		<!---
		*************************************************************************
		*	Draw attribution (please do not removed or comment out this code	* 
	...
	<cffunction name="loadXML" access="private" returntype="void" output="false"
		...
		<cfset var location = "" />
		<cfset var configBean = getConfigBean() />
		<cfset var name = "" />
		<cfset var value = "" />
		...
		<cfloop from="1" to="#ArrayLen(configNodes)#" index="i">
			<cfset name = Trim(configNodes[i].XmlAttributes['name']) />
			<cfset value = Trim(configNodes[i].XmlAttributes['value']) />
			<cfinvoke component="#configBean#" method="set#name#">
		...
		<!--- Set defined fonts --->
		<cfset configBean.setDefinedFonts(definedFonts) />
	</cffunction>
	...
	<cffunction name="randString" access="private" returntype="string" output="false"
		...
		<cfset var safe = "A,B,C,D,E,F,G,H,J,K,M,N,P,Q,R,S,T,R,X,Y,Z" />
		...
		<cfelseif arguments.type EQ "alphaNumLCase">
			...
			<cfset rangeMax = ListLen(useList) />
		<cfelseif arguments.type EQ "safe">
			<cfset useList = safe />
			<cfset rangeMax = ListLen(useList) />
