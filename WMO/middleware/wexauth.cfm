...
<cfif find( "wex.cfm", lcase( cgi.SCRIPT_NAME ) ) gt 0>
    <cfif len( cgi.PATH_INFO ) and cgi.PATH_INFO neq "/" and listLen( mid( cgi.PATH_INFO, 2, len( cgi.PATH_INFO ) - 1 ), '/' ) gt 0>
        ...
        <cfif find( ".cfm", query_wex.FILE_PATH ) gt 0>
            <cfloop array="#structKeyArray(url)#" item="u">
                <cfset attributes[u] = url[u]>
            </cfloop>
            <cfloop array="#structKeyArray(form)#" item="f">
                <cfset attributes[f] = form[f]>
            </cfloop>
            <cfinclude template="../../#query_wex.FILE_PATH#">
        <cfelseif find( ".json", query_wex.FILE_PATH ) gt 0>
            <cffile action = "read" file = "../../#query_wex.FILE_PATH#" variable = "content" charset = "utf-8">
        <cfelse>
            <cfscript>
                dottedpath = lcase( query_wex.FILE_PATH );
            ...
            <cfif isDefined("wexed_func")>
                ...
                <cfelse>
                    <cfscript>
                        wex_result = evaluate("wexed_object.#wexed_func_name#(argumentCollections=url)");
                    </cfscript>
                </cfif>
