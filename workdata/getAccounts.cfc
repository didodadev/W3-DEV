<cfcomponent>
    ...
    <cffunction name="getCompenentFunction">
		<cfif isdefined("arguments.use_period") and arguments.use_period eq true>
			<cfset dsn2 = dsn2>
		<cfelse>
			<cfset dsn2 = dsn>
		</cfif>
        <cfquery name="getAccounts_" datasource="#dsn#_#session.ep.company_id#">
			...
			<cfif isdefined("arguments.account_type") and arguments.account_type eq 1>
				AND ACCOUNTS.ACCOUNT_TYPE = 'Ticari'
			<cfelseif isdefined("arguments.account_type") and arguments.account_type eq 2>
				AND ACCOUNTS.ACCOUNT_TYPE = 'Kredili'
			</cfif>	
			ORDER BY
				BANK_BRANCH.BANK_NAME,
				ACCOUNTS.ACCOUNT_NAME
