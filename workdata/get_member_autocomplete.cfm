...
<cffunction name="get_member_autocomplete" access="public" returntype="query" output="no">
	...
	<cfargument name="company_firm_type" required="no" type="string" default="">
	...
	<cfif ListFind(arguments.select_list,1,',')>
		<cfquery name="get_1" datasource="#dsn#">
			...
			WHERE
				C.COMPANY_ID = CP.COMPANY_ID AND C.COMPANY_STATUS = 1 AND CP.COMPANY_PARTNER_STATUS = 1
				<cfif isDefined("arguments.company_firm_type") and len(arguments.company_firm_type)>
					AND C.FIRM_TYPE LIKE '%#arguments.company_firm_type#%'
				</cfif>
	...
	<cfif ListFind(arguments.select_list,2,',')>
		<cfquery name="get_2" datasource="#dsn#">
			SELECT DISTINCT top 10
				...
				<cfif database_type is 'MSSQL'>
					C.CONSUMER_NAME + ' ' + C.CONSUMER_SURNAME AS MEMBER_PARTNER_NAME,
				<cfelseif database_type is 'DB2'>
					C.CONSUMER_NAME || ' ' || C.CONSUMER_SURNAME AS MEMBER_PARTNER_NAME,	
				</cfif>
				<cfif database_type is 'MSSQL'>
	...
	<cfif ListFind(arguments.select_list,3,',')>
		...
		<cfquery name="get_3" datasource="#dsn#">
				SELECT
					...
					<cfif database_type is 'MSSQL'>
						EP.EMPLOYEE_NAME + ' ' + EP.EMPLOYEE_SURNAME + ' - ' + REPLACE(EP.POSITION_NAME,'&','') AS MEMBER_PARTNER_NAME,
					<cfelseif database_type is 'DB2'>
						EP.EMPLOYEE_NAME || ' ' || EP.EMPLOYEE_SURNAME || ' || ' || REPLACE(EP.POSITION_NAME,'&','') AS MEMBER_PARTNER_NAME,
					</cfif>
					<cfif database_type is 'MSSQL'>
		...
		<cfquery name="get_4" datasource="#dsn#">
			SELECT
				...
				'' AS ACCOUNT_CODE,
				<cfif database_type is 'MSSQL'>
					EP.EMPLOYEE_NAME + ' ' + EP.EMPLOYEE_SURNAME + ' - ' + REPLACE(POSITION_NAME,'&','') AS MEMBER_PARTNER_NAME,
				<cfelseif database_type is 'DB2'>
					EP.EMPLOYEE_NAME || ' ' || EP.EMPLOYEE_SURNAME || ' || ' || REPLACE(POSITION_NAME,'&','') AS MEMBER_PARTNER_NAME,
				</cfif>
				<cfif database_type is 'MSSQL'>
