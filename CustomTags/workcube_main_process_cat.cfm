...
<cfquery name="GET_USER_PROCESS_CAT" datasource="#CALLER.DSN#">
	SELECT
		DISTINCT
	...
	WHERE
		...
		SPCF.FUSE_NAME = '#caller.attributes.fuseaction#' AND
...
<cfif len(attributes.main_process_cat)>
	...
	<cfelse>
		<cfquery name="CHECK_PROCESS_CAT_ID" datasource="#CALLER.DSN#">
		...
		<cfif not check_process_cat_id.recordcount>
			...
			<script type="text/javascript">
				alert("<cfoutput>#caller.getLang('main',791)#</cfoutput>!");
				history.back();
			</script>		
		<cfelse>
			<!--- 566.Bu İşlem Tipine Yetkiniz Yok ! --->
			<script type="text/javascript">
				alert("<cfoutput>#caller.getLang('main',566)#</cfoutput>");
				history.back();
			</script>		
		</cfif>
		<cfabort>
...
<select name="main_process_cat" id="main_process_cat" style="width:<cfoutput>#attributes.slct_width#</cfoutput>px;" <cfif len(attributes.onclick_function)>onChange="<cfoutput>#attributes.onclick_function#</cfoutput>;"</cfif>>
	<option value="" selected><cfoutput>#caller.getLang('main',322)#</cfoutput></option>	
...
function check_accounts(myForm)
{
	...
	if ( (process_cat_array[temp_field1.selectedIndex] == 1) && ( (!temp_field2.value.length) || (temp_field2.value == '') || (temp_field2.value == ' ') ) ) /*muhasebeci kontrolü yapılmalı*/
		{
		alert("<cfoutput>#caller.getLang('main',2173)#</cfoutput>");
		return false;
		}
	return true;
}
