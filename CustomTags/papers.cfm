...
--->
<cfparam name = "attributes.increase" type="boolean" default="true"><!--- paper_number 1 artırılmış olarak getirilip getirilmeyeceğini belirler --->
	...
	<cfswitch expression="#attributes.paper_type#">
		<cfcase value="offer,order">
			<cfif not isDefined("attributes.paper_type2")>
            	<cf_get_lang dictionary_id="33274.Eksik">
<!---				<cfoutput>Eksik Parametre paper_type2!</cfoutput>--->
			...
			<cfset paper_number = evaluate('get_gen_paper.#attributes.paper_type#_number') + (attributes.increase ? 1 : 0)>
		</cfcase>
		<cfcase value="campaign,promotion,catalog,support,opportunity,service_app,prod_order,cat_prom,target_market,stock_fis,subscription,production_result,ship_fis,credit,credit_revenue,credit_payment,pro_material,internal_demand,cari_to_cari,debit_claim,cash_to_cash,cash_payment,virman,incoming_transfer,outgoing_transfer,purchase_doviz,sale_doviz,creditcard_revenue,creditcard_payment,expense_cost,income_cost,production_lot,budget_plan,expenditure_request,correspondence,quality_control,production_quality_control,creditcard_debit_payment,securefund,creditcard_cc_bank_action,buying_securities,securities_sale,tahakkuk_plan,system_paper,mkdad,travel_demand,budget_transfer_demand,waste_operation,sample_analysis,waste_collection,ship_internal,cashregister">
			<cfquery name="GET_GEN_PAPER" datasource="#CALLER.DSN3#">
		...
		<cfcase value="ship,revenue_receipt,invoice,e_invoice">
			...
			<cfif len(evaluate('get_paper.#attributes.paper_type#_NO')) and len(evaluate('get_paper.#attributes.paper_type#_number'))>
				...
				<cfset paper_number = evaluate('get_paper.#attributes.paper_type#_number') + (attributes.increase ? 1 : 0)>
			<cfelse>
				<cfset paper_code = ''>
				<cfset paper_number = ''>
			</cfif>
		</cfcase>
		<cfcase value="employee,emp_app,g_service_app,asset,fixtures,employee_healty,emp_notice,asset_failure,allowence_expense,health_allowence_expense,additional_allowance,work,package">
			...
			<cfset paper_number = evaluate('get_main_paper.#attributes.paper_type#_number') + (attributes.increase ? 1 : 0)>
		</cfcase>		
		<cfdefaultcase>
