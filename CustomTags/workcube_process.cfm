...
<cfparam name="attributes.select_id" default="#attributes.select_name#"><!---- Select ismi(listeleme sayfalarında toplu onay ile karışıyor) --->
...
<cfif len( attributes.type )>
	<cfif attributes.type eq 'color-status' and isDefined("attributes.process_stage") and len( attributes.process_stage )>
		...
		<cfset get_ProcessType = Cmp.get_ProcessType(
			...
			friendly_url 	: isdefined("attributes.caller.widget_load") ? attributes.caller.widget_load : ""
		) />
		...
		<cfset process_rows_color = {
			1 : { backgroundColor : "##0abb8712", color : "##0abb87" }, <!--- Yeşil --->
			2 : { backgroundColor : "##ffb8220d", color : "##ffb822" }, <!--- Turuncu --->
			3 : { backgroundColor : "##5d78ff0d", color : "##5d78ff" }, <!--- Mavi --->
			4 : { backgroundColor : "##9370d21f", color : "##673ab7" }, <!--- Mor --->
			5 : { backgroundColor : "##70dbe80f", color : "##0d99d8" }, <!--- Turkuaz --->
			6 : { backgroundColor : "##fff70014", color : "##f7f704" }, <!--- Sarı --->
			7 : { backgroundColor : "##f34b4b12", color : "##d01818" }, <!--- Kırmızı Koyu --->
			8 : { backgroundColor : "##f15e081a", color : "##f15e08" }, <!--- Turuncu Koyu --->
			9 : { backgroundColor : "##fd397a12", color : "##fd397a" }, <!--- Kırmızı --->
			10: { backgroundColor : "##ce28ec0d", color : "##de12d8" } <!--- Menekşe --->
		}/>
...
<cfelse>
	<cfif isdefined("caller.widget_load")>
		<cfset widget_load_path = caller.widget_load>
	<cfelseif isdefined("attributes.caller.widget_load")>
		<cfset widget_load_path = caller.widget_load>
	<cfelse>
		<cfset widget_load_path = "">
	</cfif>
	<cfif attributes.is_upd eq 0>
		...
		<cfif attributes.is_detail eq 0>
			...
			<cfelse>
				<select name="process_stage" id="process_stage" <cfif isdefined("attributes.process_cat_width")>style="width:<cfoutput>#attributes.process_cat_width#</cfoutput>px;"</cfif> <cfif len(attributes.tabindex)>tabindex="<cfoutput>#attributes.tabindex#</cfoutput>"</cfif>>
		...
		<cfelse>
			...
			<cfif get_faction.recordcount and (ListFind(ValueList(get_faction.process_row_id),attributes.select_value) or not get_Line_Number.recordcount)>
			...
			<cfelse>
				...
				<select name="<cfoutput>#attributes.select_name#</cfoutput>" id="process_stage" <cfif isdefined("attributes.process_cat_width")>style="width:<cfoutput>#attributes.process_cat_width#</cfoutput>px;"</cfif> <cfif len(attributes.tabindex)>tabindex="<cfoutput>#attributes.tabindex#</cfoutput>"</cfif>>
					<option value=""><cfoutput>#caller.getLang('main',561)#</cfoutput></option><!--- 561.Yetkisiz --->
				</select>
			</cfif>
	...
	<cfelseif attributes.is_upd eq 1>
		...
		<cfif get_Process_Type_1.Line_Number neq attributes.Old_Process_Line>
			...
			<!--- Süreç aşamasında form elemanının adı yazılmışsa position_code bilgisini oradan alarak süreci forma düşürülen kullanıcı üzerinden yürütür! --->
			<cfset formPositionCode = "" />
			<cfif len(get_process_type_1.position_code_argument_name)>
				<cfif isdefined(Evaluate("caller.attributes.#get_process_type_1.position_code_argument_name#")) and len(Evaluate("caller.attributes.#get_process_type_1.position_code_argument_name#"))>
					<cfset formPositionCode = Evaluate("caller.attributes.#get_process_type_1.position_code_argument_name#") />
				<cfelseif isdefined(Evaluate("caller.arguments.#get_process_type_1.position_code_argument_name#")) and len(Evaluate("caller.arguments.#get_process_type_1.position_code_argument_name#"))>
					<cfset formPositionCode = Evaluate("caller.arguments.#get_process_type_1.position_code_argument_name#") />
				</cfif>
			</cfif>
			<cfset get_Cau_Position_Type = Cmp.get_Cau_Position_Type(
				...
				is_approval_recording_chief: #iif(get_process_type_1.is_confirm_first_chief_recording eq 1 or get_process_type_1.is_confirm_second_chief_recording eq 1, DE(1), DE(0))#,
				is_confirm_first_chief_recording: #iif(get_process_type_1.is_confirm_first_chief_recording eq 1, DE(1), DE(0))#,
				is_confirm_second_chief_recording: #iif(get_process_type_1.is_confirm_second_chief_recording eq 1, DE(1), DE(0))#,
				action_table: attributes.action_table,
				action_column: attributes.action_column,
				action_id: attributes.action_id,
				position_code: #iif( attributes.position_code neq 0, attributes.position_code, DE(0))#,
				mandate_position_code: #iif( attributes.mandate_position_code neq 0, attributes.mandate_position_code, DE(0))#,
				form_position_code: len(formPositionCode) ? formPositionCode : 0
				) />
			...
			<cfif Len(attributes.action_page)><!--- Bu kontrol silme sayfalarindan calistiginda uyari/sms mail vb gitmemesi icin eklendi, defaultta durum degismeyecek FBS 20120302 --->
				...
				<cfif get_general_positions.recordcount>
					<cfoutput query="get_general_positions">
						<cfif get_Process_Type_1.is_email eq 1 or get_Process_Type_1.is_kep eq 1>
							<!--- Email --->
							<cfif len(get_general_positions.employee_email)>
								...
								<cfif get_Process_Type_1.is_email eq 1 and get_Process_Type_1.is_kep eq 1>
									<cfset attributes.mail_content_to = '#get_general_positions.employee_email#,#EMPLOYEE_KEP_ADRESS#'>
								<cfelseif get_Process_Type_1.is_email eq 1>
									<cfset attributes.mail_content_to = '#get_general_positions.employee_email#'>
								<cfelse>
									<cfset attributes.mail_content_to = '#EMPLOYEE_KEP_ADRESS#'>
								</cfif>
					...
					<cfif get_cau_position_type.recordcount>
						...
						<cfoutput query="get_cau_position_type">
							...
							<cfset add_Page_Warnings = Cmp.add_Page_Warnings(
								...
								is_confirm_first_chief_recording	: get_process_type_1.is_confirm_first_chief_recording,
								is_confirm_second_chief_recording	: get_process_type_1.is_confirm_second_chief_recording,
