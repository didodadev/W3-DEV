...
<cfelseif isDefined("session.wp")>
	<cfset my_userid = session.wp.userid>
	<cfset my_position_code = session.wp.position_code>
	<cfset my_our_company_id = session.wp.our_company_id>
	<cfset my_period_id = session.wp.period_id>
...
<cfparam name="attributes.wrkflow" default="">
...
<cfset Cmp = createObject("component","workcube.CustomTags.cfc.get_workcube_process_cat") />
...
<cfelseif not Len(attributes.action_page)>
	...
	<cfelse>
		...
		switch (caller.fusebox.circuit)
		{
			...
			case "hr" : module_id = '3,82'; break;//gercek module_id:3
			...
			case "health" : module_id = 70; break;
			case "pda" : module_id = '13,35,26'; break; 
			case "Myhome" : module_id = 82; break;
			case "retail" : module_id = 50; break;
		}
	</cfscript>
	</cfif>
		...
		<cfelse>
			...
			<cfif not get_Check_Process_Cat_Id.recordcount>
				...
				<script type="text/javascript">
					...
					alert("<cf_get_lang dictionary_id='58203.İşlem Kategorisi Ayarlardan Kritik Olarak Değişmiş veya Silinmiş'>");
					history.back();
				</script>		
			<cfelse>
...
<cfif Len(attributes.action_page)>
	...
	<cfif get_Process_Cat_Emp_Cau.recordcount>
		...
		<cfset Cmp_Type = createObject("component","workcube.CustomTags.cfc.get_workcube_process") />
		...
		<cfoutput query="get_Process_Cat_Emp_Cau">
		  <cfset max_warning_date = now()>
			...
			<cfset add_Page_Warnings = Cmp_Type.add_Page_Warnings(
				...
				action_process_cat_id : attributes.process_cat,
				is_confirm			: ""
				) />
		</cfoutput>
	</cfif>
