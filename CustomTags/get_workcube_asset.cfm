...
<cfif attributes.design_id is 0>
	...
	<cf_ajax_list>
		<tbody>
            <tr height="25">
                <cfoutput>
                    <td class="txtboldblue" width="100%"><cfif attributes.is_image eq 1>#caller.getLang('main',2165)#<cfelse>#caller.getLang('main',156)#</cfif></td> <!---İlişkili İmajlar---> <!--- Belgeler ---> 
            ...
            <cfoutput query="get_asset">
                ...
                <tr height="20">
                    ...
                    <cfif extention eq "flv">
                        <a href="javascript://" onClick="windowopen('#request.self#?fuseaction=objects.popup_flvplayer&video=#caller.file_web_path##assetcat_path#/#asset_file_name#','video','popup_flvplayer');" title="#ASSET_DESCRIPTION#">#asset_name#</a>
                    ...
                    <cfelseif listfindnocase(imageplayer_extensions, "." & extention)>
                        <a href="#request.self#?fuseaction=objects.popup_download_file&file_name=#folder_#/#asset_file_name#&file_control=asset.form_upd_asset&asset_id=#asset_id#&assetcat_id=#assetcat_id#"><i class="fa fa-paperclip"></i></a>
                        <a href="javascript://" onClick="windowopen('#request.self#?fuseaction=objects.popup_download_file&direct_show=1&file_name=#folder_#/#asset_file_name#&file_control=asset.form_upd_asset&asset_id=#asset_id#&assetcat_id=#assetcat_id#','list','popup_download_file');" title="#ASSET_DESCRIPTION#">#asset_name#</a>
                    <cfelse>
                        <a href="#request.self#?fuseaction=objects.popup_download_file&file_name=#folder_#/#asset_file_name#&file_control=asset.form_upd_asset&asset_id=#asset_id#&assetcat_id=#assetcat_id#" title="#ASSET_DESCRIPTION#">#asset_name#</a>
                    </cfif>
                ...
                <cfif attributes.is_add eq 1>
                    ...
                    <cfif not len(emp_del_buttons.is_delete) and emp_del_buttons.is_delete neq 1>
                        <cfif not listfindnocase(caller.denied_pages,'objects.emptypopup_del_asset')>
                            <td width="20"><a style="cursor:pointer;" onclick="javascript:if(confirm('#caller.getLang('main',1057)#')) windowopen('#request.self#?fuseaction=objects.emptypopup_del_asset&asset_id=#ASSET_ID#&module=#attributes.module#&file_name=#ASSET_FILE_NAME#&file_server_id=#ASSET_FILE_SERVER_ID#','small'); else return false;"><i class="fa fa-minus" title="#caller.getLang('main',51)#"></i></a> <!--- Sil --->		<!---Kayıtlı Belgeyi Siliyorsunuz, Emin Misiniz? ---></td>
                        </cfif>
                    </cfif>
                </cfif>
                </tr>
...
<cfelse>
	<cfsavecontent variable="baslik"><cfoutput>#caller.getLang('main',156)#</cfoutput></cfsavecontent> <!--- Belgeler --->
	<cfif isdefined("attributes.action_id_2")>
	...
	<cfif isdefined("attributes.is_add") and attributes.is_add eq 1>
        <cfsavecontent variable="add_drive">
            <li id="docs">
                <span href="javascript://" onclick="load_drive_list()"><i class="catalyst-plus"></i></span>
                <ul id="drive_list">
                    <li>
                        <a target="_blank" href="<cfoutput>#request.self#?fuseaction=asset.list_asset&event=add&module=#attributes.module#&module_id=#attributes.module_id#&action=#attributes.action_section#&action_id=#attributes.action_id#&asset_cat_id=#attributes.asset_cat_id#&action_type=#attributes.action_type##add_##info_#</cfoutput>"><img style="margin-right:4px;padding-left:5px;" width="20" height="30" src="css/assets/icons/catalyst-icon-svg/ctl-office-material-2.svg" /><cf_get_lang dictionary_id='63755.Workcube Arşive Ekle'></a>
                    </li>
                    <li>
                        <a target="_blank" href="https://docs.google.com/document"><img width="30" height="30" src="css/assets/icons/catalyst-icon-svg/google-docs.svg" /></i><cf_get_lang dictionary_id='63756.Google Docs'></a>
                    </li>
                    <li>
                        <a target="_blank" href="https://docs.google.com/spreadsheets"><img width="30" height="30" src="css/assets/icons/catalyst-icon-svg/google-forms.svg" /><cf_get_lang dictionary_id='63757.Google Tablolar'></a>
                    </li>
                    <li>
                        <a target="_blank" href="https://docs.google.com/presentation"><img width="30" height="30" src="css/assets/icons/catalyst-icon-svg/google-slides.svg" /></i><cf_get_lang dictionary_id='63758.Google Slaytlar'></a>       
                    </li>
                </ul>
            </li>
        </cfsavecontent>
		<cfset upd_ = "javascript:openBoxDraggable('#request.self#?fuseaction=objects.popup_asset_digital&module=#attributes.module#&module_id=#attributes.module_id#&action=#attributes.action_section#&action_id=#attributes.action_id#&asset_cat_id=#attributes.asset_cat_id#&action_type=#attributes.action_type##add_#&asset_archive=true&refresh_id=#attributes.id#');">
	<cfelse>
		<cfsavecontent variable="add_drive"></cfsavecontent>
		<cfset upd_ = ''>
	</cfif>
	<cfif attributes.is_image eq 1>
    ...
    <cfif isdefined("attributes.related_id") and len(attributes.related_id)><cfset url_str =url_str&'&related_id=#attributes.related_id#'></cfif>
	<cfif isdefined("attributes.related_section") and len(attributes.related_section)><cfset url_str =url_str&'&related_section=#attributes.related_section#'></cfif>
   ...
   <cf_box id="#attributes.id#" class="#maxi#" closable="0" add_href_size="wide" collapsed="#iif(attributes.style,1,0)#" info_href="#upd_#" title="#baslik#" style="width:#attributes.width#px;" box_page="#request.self#?fuseaction=objects.ajax_files&#url_str#" right_images="#add_drive#"></cf_box>
</cfif>
<script>
    function load_drive_list() {
        $('#drive_list').toggle();
    }
</script>