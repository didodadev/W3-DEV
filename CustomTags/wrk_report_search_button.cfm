	...
	<div id="buttons" style="display:none;"> 
   		<ul>
			<cfif attributes.is_excelbuton eq 1>
                <li>               		
                    <a href="javascript://" onclick="return (<cfif len(attributes.search_function)><cfoutput>#attributes.search_function#</cfoutput>&&</cfif> PROCTest(this,1));" title="<cf_get_lang_main no='1940.Excel Üret'>">&nbsp;<i class="fa fa-file-excel-o  fa-2x font-green"></i>&nbsp;</a>
                </li>
            </cfif>
            <cfif attributes.is_wordbuton eq 1>
                <li>                   
                    <a href="javascript://" onclick="return (<cfif len(attributes.search_function)><cfoutput>#attributes.search_function#</cfoutput>&&</cfif> PROCTest(this,2));" title="<cf_get_lang_main no='1941.Word Üret'>">&nbsp;<i class="fa fa-file-word-o fa-2x font-blue"></i>&nbsp;</a>
                </li>
            </cfif>
            <cfif attributes.is_pdfbuton eq 1>
                <li>                   
                    <a href="javascript://"  onclick="return (<cfif len(attributes.search_function)><cfoutput>#attributes.search_function#</cfoutput>&&</cfif> PROCTest(this,3));" title="<cf_get_lang_main no='1942.PDF Üret'>">&nbsp;<i class="fa fa-file-pdf-o fa-2x font-red"></i>&nbsp;</a>
                </li>           
            </cfif>
            <cfif attributes.is_mailbuton eq 1>
                <li> 
                    <a href="javascript://" onclick="return (<cfif len(attributes.search_function)><cfoutput>#attributes.search_function#</cfoutput>&&</cfif> PROCTest(this,4));" title="<cf_get_lang_main no='63.Mail Gönder'>">&nbsp;<i class="fa fa-envelope-o fa-2x font-blue-dark"></i>&nbsp;</a>       
                </li>
            </cfif>
...
<script>
	...
	{	
		...
		{
			if($("#listIcon").closest("table").attr('class').indexOf('report_list') != -1)
			{
				try{
                    ...
                    $("table.report_list").find('input').each(function(index,element){
                    ...
                    $("table.report_list").find('span').each(function(index,element){
				...
				{
					$("table.report_list").html($("table.report_list").html().replace(/<!-- del -->/g,''));
					$("table.report_list").find('th:hidden').each(function(index,element){
						$(element).before('<!-- del -->');
						$(element).after('<!-- del -->');
						})
					$("table.report_list").find('td:hidden').each(function(index,element){
						$(element).before('<!-- del -->');
						$(element).after('<!-- del -->');
						})
					var dataAll = JSON.stringify(encodeURIComponent('<!-- sil --><table>'+$("table.report_list").html().replace(/\n/g,'').replace(/\t/g,'')+'</table><!-- sil -->'));	
				}
				goster(working_div_main);
				console.log(dataAll);
				dark_mode_value = '<cfoutput>#listlast(session.dark_mode,":").trim()#</cfoutput>';
				dark_mode_value = dark_mode_value.trim();
				callURL("<cfoutput>#request.self#?fuseaction=objects.docExport&ajax=1&ajax_box_page=1&xmlhttp=1&_cf_nodebug=true&action_type=</cfoutput>"+type_id,handlerPost,{ data: $.toJSON(dataAll), '<cfoutput>#listfirst(session.dark_mode,":").trim()#</cfoutput>' : dark_mode_value});
