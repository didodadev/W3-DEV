<cfcomponent extends="cfc.queryJSONConverter">
    ...
    <cffunction name="upd_wrk" access="public">
        ...
        <cfargument name="moneyformat_style" >
        <cfargument name="is_eshipment">
            <cfquery name="upd_wrk_session_to_db" datasource="#this.dsn#">
                ...
                SET
                    ...
                    MONEYFORMAT_STYLE = <cfqueryparam cfsqltype="bit" value="#arguments.moneyformat_style#">,
	...
	<cffunction name="add_fnc" access="public" returntype="boolean">
        <cfargument name="userid" type="numeric" required="no">
        <cfargument name="employee_id" >
	...
	<cffunction name="upd_sett_4_fnc" access="public" >
        <cfargument name="revenue_receipt_no" type="string" default="" required="no">
        <cfargument name="revenue_receipt_number" type="string" default="" required="no">
        <cfargument name="invoice_no" type="string" default="" required="no">
        <cfargument name="invoice_number" type="string" default="" required="no">
        <cfargument name="e_invoice_no" type="string" default="" required="no">
        <cfargument name="e_invoice_number" type="string" default="" required="no">
        <cfargument name="ship_no" type="string" default="" required="no">
        <cfargument name="ship_number" type="string" default="" required="no">
        <cfargument name="userid" type="numeric" required="yes">
        ...
        <cfquery name="UPD_SETT_4" datasource="#this.dsn3#">
            UPDATE
                PAPERS_NO
            SET
                REVENUE_RECEIPT_NO = <cfif len(arguments.revenue_receipt_no)><cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.revenue_receipt_no#"><cfelse>''</cfif>,
                REVENUE_RECEIPT_NUMBER = <cfif len(arguments.revenue_receipt_number)><cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.revenue_receipt_number#"><cfelse>NULL</cfif>,
                INVOICE_NO = <cfif len(arguments.invoice_no)><cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.invoice_no#"><cfelse>''</cfif>,
                INVOICE_NUMBER = <cfif len(arguments.invoice_number)><cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.invoice_number#"><cfelse>NULL</cfif>,
                E_INVOICE_NO = <cfif len(arguments.e_invoice_no)><cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.e_invoice_no#"><cfelse>''</cfif>,
                E_INVOICE_NUMBER = <cfif len(arguments.e_invoice_number)><cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.e_invoice_number#"><cfelse>NULL</cfif>,
                SHIP_NO = <cfif len(arguments.SHIP_NO)><cfqueryparam cfsqltype="cf_sql_varchar" value="#ship_no#"><cfelse>NULL</cfif>,
                SHIP_NUMBER = <cfif LEN(arguments.ship_number)><cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.ship_number#"><cfelse>NULL</cfif>,
    ...
    <cffunction name="ONLINE_USER_EXTRANET" access="public" returntype="query">
        <cfset dsn = application.systemParam.systemParam().dsn>
        <cfquery name="GET_ONLINE_USER" datasource="#dsn#">
            SELECT 
                W.NAME + ' ' + W.SURNAME AS KULLANICI,
            ...
            FROM
                WRK_SESSION AS W
                LEFT JOIN COMPANY_PARTNER AS C ON C.PARTNER_ID = W.USERID
			WHERE
            	USER_TYPE <> 0
            ORDER BY
            	W.USER_TYPE ASC,
    ...
    <cffunction name="ONLINE_USER_INTRANET" access="public" returntype="query">
        <cfset dsn = application.systemParam.systemParam().dsn>
        <cfquery name="GET_ONLINE_USER" datasource="#dsn#">
            SELECT 
                W.NAME + ' ' + W.SURNAME AS KULLANICI,
    ...
    <cffunction name="GET_USER_GROUP" access="public" returntype="query">
        ...
        <cfquery name="GET_USER_GROUP" datasource="#dsn#">
			...
			FROM
                ...
                LEFT JOIN USER_GROUP_EMPLOYEE AS UGE ON E.POSITION_ID = UGE.POSITION_ID
                LEFT JOIN USER_GROUP AS UG ON UG.USER_GROUP_ID = UGE.USER_GROUP_ID
			WHERE
            	UG.USER_GROUP_ID IS NOT NULL AND
            	E.POSITION_CODE = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.position_code#">
        </cfquery>
		<cfreturn GET_USER_GROUP>
    </cffunction> 
    <cffunction name="GET_USER_GROUP_JSON" access="remote" returntype="any" returnformat="JSON">
        <cfargument name="position_code" type="numeric" required="yes">
        <cfreturn replace(SerializeJson(this.returnData(replace(SerializeJson(this.GET_USER_GROUP(arguments.position_code)),'//',''))),'//','') />
    </cffunction>
    <cffunction name="GET_USER_GROUP_MENU" access="public" returntype="numeric">
    	<cfargument name="userId" type="numeric" required="yes">
        <cfset dsn = application.systemParam.systemParam().dsn>
        <cfquery name="GET_USER_GROUP_MENU" datasource="#dsn#">
            SELECT
            	DISTINCT
            	USER_GROUP_ID
			FROM
		...
		<cfreturn GET_USER_GROUP_MENU.USER_GROUP_ID>
    </cffunction>
    <cffunction name="GET_USER_GROUP_MENU_INSIDE" access="public" returntype="numeric">
    	<cfargument name="userId" type="numeric" required="yes">
        <cfset dsn = application.systemParam.systemParam().dsn>
        <cfquery name="GET_USER_GROUP_MENU" datasource="#dsn#">
            SELECT
    ...
    <cffunction name="kickEmployee" access="remote" returntype="any">
    	<cfargument name="userId" type="numeric" required="yes">
        <cfargument name="userType" type="numeric" required="yes">
        ...
        <cfquery name="GET_NAME" datasource="#DSN#">
            SELECT
                ...
                SURNAME,
                POSITION_NAME,
                ISNULL(OUR_COMPANY_ID,COMPANY_ID) AS COMP_ID,
                CFTOKEN
            FROM
                WRK_SESSION
            WHERE
                USERID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.userid#"> AND 
                USER_TYPE = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.userType#"> AND 
                SESSIONID = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.sessionId#">
        </cfquery>
        <cfif GET_NAME.recordcount>
            <cftry>
                <cfif application.systemParam.systemParam().fusebox.use_stock_speed_reserve and len(get_name.comp_id)><!--- ürün seri rezerve kullanılıyorsa sistemden çıkısta herhangi işlemle ilişkilendirilmemiş rezerveler silinir  --->
                    <cfset ord_rr_db = '#dsn#_#get_name.comp_id#'>
                    <cfstoredproc procedure="DEL_ORDER_ROW_RESERVED" datasource="#ord_rr_db#">
                        <cfprocparam cfsqltype="cf_sql_varchar" value="#get_name.cftoken#">
                    </cfstoredproc>
                </cfif>
                <cfcatch></cfcatch>
            </cftry>
    ...
    <cffunction name="GET_WRK_MENU" access="public" returntype="query">
    	<cfargument name="menu_id" default="0">
        <cfset dsn = application.systemParam.systemParam().dsn>
        <cfquery name="GET_WRK_MENU" datasource="#dsn#">
            SELECT 
                    WRK_MENU_ID, 
                    WRK_MENU_NAME
                FROM 
		...
		<cfreturn GET_WRK_MENU>
    </cffunction> 
    <cffunction name="UPDATE_WORKTIPS_OPEN" access="remote">
    	<cfargument name="worktips_open" default="">
        <cfset dsn = application.systemParam.systemParam().dsn>
        <cfset session.ep.worktips_open = arguments.worktips_open>
        <cfquery name="update_worktips_open" datasource="#dsn#">
            UPDATE EMPLOYEES SET WORKTIPS_OPEN = #arguments.worktips_open# WHERE EMPLOYEE_ID =  <cfqueryparam cfsqltype="cf_sql_integer" value="#session.ep.userid#">
        </cfquery>
    </cffunction>
    <cffunction name="GET_ALL_POS_BY_USERID" access="remote">
        <cfargument name="userid" type="numeric" required="true">
        <cfset dsn = application.systemParam.systemParam().dsn>
        <cfquery name="GET_ALL_POS" datasource="#DSN#">
            SELECT POSITION_ID, POSITION_CODE, POSITION_NAME FROM EMPLOYEE_POSITIONS WHERE POSITION_STATUS = 1 AND EMPLOYEE_ID = <cfqueryparam value="#arguments.userid#" cfsqltype="cf_sql_integer">
        </cfquery>
        <cfreturn GET_ALL_POS />
    </cffunction>
</cfcomponent>
