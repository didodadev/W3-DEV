...
<cfcomponent displayname="cfGit" hint="I interact with git" output="false" extends="cfGit_helper">
	<!--- The setup --->
	<cfproperty
		name="git_path"
	...
	<cffunction name="rm" access="public" output="false" returntype="any">
		<cfargument name="option" type="struct" required="yes" hint="{ workTreeMode => boolean }"/>

		<cfscript>

			rmArguments = "--cached -r .";
			return execGit( "rm", rmArguments , option.workTreeMode );

		</cfscript>

	</cffunction>

	<cffunction name="reset" access="public" output="false" returntype="any">
		<cfargument name="option" type="struct" required="yes" hint="{ workTreeMode => boolean }"/>

		<cfscript>

			rmArguments = "--cached -r .";
			rmResponse = execGit( "rm", rmArguments , option.workTreeMode );
			if( rmResponse.status ){
				resetArguments = "--hard";
				return execGit( "reset", resetArguments , option.workTreeMode );
			}else return rmResponse;
