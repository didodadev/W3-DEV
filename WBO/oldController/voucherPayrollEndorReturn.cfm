...
<cf_xml_page_edit page_control_list="#xml_page_control_list#" default_value="1" fuseact="cheque.add_voucher_payroll_endor_return">
...
<script type="text/javascript">
	<cfif isdefined("attributes.event") and attributes.event is "upd">
		function delete_action()
		{
			...
			{
				alert("İşlem Görmüş Senet ler Var, Bordroyu Silemezsiniz !");
				return false;
			}
	...
	function check()
	{
		...
		if(get_process_cat.IS_CHEQUE_BASED_ACTION == 0)
		{
			...
			{
				alert("Ödeme Sözü Seçebilmeniz İçin İşlem Tipinde Senet Bazında Carileştirme Seçili Olmalıdır !");
				return false;
			}
			<cfif (isdefined("attributes.event") and attributes.event is "add") or not isdefined("attributes.event")>
				kontrol_status = 0;
				...
				if(kontrol_status == 1)
				{
					alert("Kısmi Tahsil Senet Seçebilmeniz İçin İşlem Tipinde Senet Bazında Carileştirme Seçili Olmalıdır !");
					return false;
				}
			</cfif>
		}
...
<cfscript>
	...
	WOStruct['#attributes.fuseaction#']['add']['filePath'] = 'cheque/form/add_voucher_payroll_endor_return.cfm';
	WOStruct['#attributes.fuseaction#']['add']['queryPath'] = 'cheque/query/add_voucher_payroll_endor_return.cfm';
	WOStruct['#attributes.fuseaction#']['add']['nextEvent'] = 'cheque.add_voucher_payroll_endor_return&event=upd';
	...
	WOStruct['#attributes.fuseaction#']['upd']['fuseaction'] = 'cheque.add_voucher_payroll_endor_return';
	WOStruct['#attributes.fuseaction#']['upd']['filePath'] = 'cheque/form/upd_voucher_payroll_endor_return.cfm';
	WOStruct['#attributes.fuseaction#']['upd']['queryPath'] = 'cheque/query/upd_voucher_payroll_endor_return.cfm';
	WOStruct['#attributes.fuseaction#']['upd']['nextEvent'] = 'cheque.add_voucher_payroll_endor_return&event=upd';
	...
	{		
		...
		tabMenuStruct['#attributes.fuseaction#']['tabMenus']['upd']['icons']['add']['href'] = "#request.self#?fuseaction=cheque.add_voucher_payroll_endor_return";
