...
<cfif isdefined('attributes.is_form_submitted') and attributes.is_form_submitted eq 1>
	...
	<cfif get_prod.recordcount>
				...
				<cfscript>
					...
					{	
						...
						if(not len(get_sarf.DIMM_CODE[ii]))
							muhasebe_uyari = "#muhasebe_uyari#;#get_sarf.PRODUCT_NAME[ii]# Ürünü İçin İlk Madde Malzeme Hesabı Eksik.";
						if(not len(get_sarf.MATERIAL_CODE[ii]))
							muhasebe_uyari = "#muhasebe_uyari#;#get_sarf.PRODUCT_NAME[ii]# Ürünü İçin Hammadde Eksik.";
					}
				...
				<cfscript>
					...
					{	
						...
						if(ListFind(str_alacakli_hesaplar,account_codes.PROD_LABOR_COST_CODE,",") eq 0) {
							...
							str_alacak_tutar = ListSetAt(str_alacak_tutar,ListFind(str_alacakli_hesaplar,get_sarf.PROD_LABOR_COST_CODE,","), ListGetAt(str_alacak_tutar,ListFind(str_alacakli_hesaplar,get_sarf.PROD_LABOR_COST_CODE,",")) + wrk_round(get_order_result_prods.LABOR_COST_SYSTEM[iii]));
							str_alacak_dovizli = ListSetAt(str_alacak_dovizli,ListFind(str_alacakli_hesaplar,get_sarf.PROD_LABOR_COST_CODE,","), ListGetAt(str_alacak_dovizli,ListFind(str_alacakli_hesaplar,get_sarf.PROD_LABOR_COST_CODE,",")) + wrk_round(get_order_result_prods.LABOR_COST_SYSTEM[iii]));
						}
					...
					<cfscript>
						...
						{	
							...
							if(not len(sarf_account_codes.DIMM_CODE))
								muhasebe_uyari = "#muhasebe_uyari#;#get_fire.PRODUCT_NAME[i]# Ürünü İçin İlk Madde Malzeme Hesabı Eksik.";
							if(not len(sarf_account_codes.ACCOUNT_LOSS))
								muhasebe_uyari = "#muhasebe_uyari#;#get_fire.PRODUCT_NAME[i]# Ürünü İçin Fire Hesabı Eksik.";
						}
						GET_NO_ = cfquery(datasource:"#dsn3#", sqlstring:"SELECT * FROM #dsn3_alias#.SETUP_INVOICE_PURCHASE");
						str_fark_gelir =GET_NO_.FARK_GELIR;
						str_fark_gider =GET_NO_.FARK_GIDER;
		...
		<cfoutput>
			<script>
				alert("Seçilen Tarih Aralığındaki Üretim Sonuçlarının Muhasebe Fişleri Oluşturuldu !\n Fiş Numaraları : \n #warning_txt#");
			</script>	
		</cfoutput>
	<cfelse>
		<script>
			alert("Seçilen Tarih Aralığındaki Üretim Sonucu Bulunamadı !");
		</script>	
	</cfif>
</cfif>
<cfscript>
	...
	WOStruct['#attributes.fuseaction#']['add']['filePath'] = 'account/display/production_result_account_card.cfm';
	WOStruct['#attributes.fuseaction#']['add']['queryPath'] = 'account/display/production_result_account_card.cfm';
