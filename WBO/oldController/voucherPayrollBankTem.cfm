...
<cfelseif isdefined("attributes.event") and attributes.event is "upd">
	<cfif isnumeric(attributes.id)>
        <cfquery name="GET_ACTION_DETAIL" datasource="#DSN2#">
            ...
            FROM
                VOUCHER_PAYROLL VP
                LEFT JOIN #dsn3_alias#.CREDIT_LIMIT CL ON CL.CREDIT_LIMIT_ID = VP.CREDIT_LIMIT
            ...
            WHERE
                VP.ACTION_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.id#"> AND
                VP.PAYROLL_TYPE = 100
...
<script type="text/javascript">
	<cfif isdefined("attributes.event") and attributes.event is "upd">
		function delete_action()
		{
			...
			{
				alert("İşlem Görmüş Senetler Var Bordroyu Silemezsiniz !");
				return false;
			}
...
<cfscript>
	...
	WOStruct['#attributes.fuseaction#']['add']['filePath'] = 'cheque/form/add_voucher_payroll_bank_tem.cfm';
	WOStruct['#attributes.fuseaction#']['add']['queryPath'] = 'cheque/query/add_voucher_payroll_bank_tem.cfm';
	WOStruct['#attributes.fuseaction#']['add']['nextEvent'] = 'cheque.form_add_voucher_payroll_bank_tem&event=upd';
	WOStruct['#attributes.fuseaction#']['add']['js'] = "javascript:gizle_goster_ikili('voucher_payroll_bank_tem','voucher_payroll_bank_tem_bask')";
	
	WOStruct['#attributes.fuseaction#']['upd'] = structNew();
	WOStruct['#attributes.fuseaction#']['upd']['window'] = 'normal';
	WOStruct['#attributes.fuseaction#']['upd']['fuseaction'] = 'cheque.form_add_voucher_payroll_bank_tem';
	WOStruct['#attributes.fuseaction#']['upd']['filePath'] = 'cheque/form/upd_voucher_payroll_bank_tem.cfm';
	WOStruct['#attributes.fuseaction#']['upd']['queryPath'] = 'cheque/query/upd_voucher_payroll_bank_tem.cfm';
	WOStruct['#attributes.fuseaction#']['upd']['nextEvent'] = 'cheque.form_add_voucher_payroll_bank_tem&event=upd';
	WOStruct['#attributes.fuseaction#']['upd']['parameters'] = 'id=##attributes.id##';
	WOStruct['#attributes.fuseaction#']['upd']['Identity'] = '##attributes.id##';
	WOStruct['#attributes.fuseaction#']['upd']['js'] = "javascript:gizle_goster_ikili('voucher_payroll_bank_tem','voucher_payroll_bank_tem_bask')";
	...
	{		
		...
		tabMenuStruct['#attributes.fuseaction#']['tabMenus']['upd']['icons']['add']['href'] = "#request.self#?fuseaction=cheque.form_add_voucher_payroll_bank_tem";
