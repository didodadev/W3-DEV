<cfscript>
	if(attributes.tabMenuController eq 0)
	{
		...
		if(isdefined("attributes.per_req_id"))
		{
			...
			WOStruct['#attributes.fuseaction#']['del']['fuseaction'] = '#request.self#?fuseaction=#ListFirst(attributes.fuseaction,'.')#.emptypopup_del_personel_requirement_form&per_req_id=#contentEncryptingandDecodingAES(isEncode:0,content:attributes.per_req_id,accountKey:'wrk')#&cat=##caller.get_per_req.per_req_stage##&head=##caller.get_per_req.personel_requirement_head##';
	...
	else
	{
		...
		else if(caller.attributes.event is 'upd')
		{
			...
			tabMenuStruct['#fuseactController#']['tabMenus']['upd']['menus'][0]['text'] = '#getLang('main',345)#';
			tabMenuStruct['#fuseactController#']['tabMenus']['upd']['menus'][0]['onClick'] = "windowopen('#request.self#?fuseaction=objects.popup_page_warnings&action=#attributes.fuseaction#&action_name=per_req_id&action_id=#attributes.per_req_id#','list');";
	...
	WOStruct['#attributes.fuseaction#']['systemObject']['pageSettings'] = "['item-req_head','item-our_company','item-position_cat','item-personel_count','item-personel_exp','item-vehicle_req','item-Gorus']";
</cfscript>
