<cfscript>
	if(attributes.tabMenuController eq 0)
	{
		...
		WOStruct['#attributes.fuseaction#']['add']['filePath'] = 'V16/cheque/form/add_voucher_payroll_entry.cfm';
		WOStruct['#attributes.fuseaction#']['add']['queryPath'] = '/V16/cheque/query/add_voucher_payroll_entry.cfm';
		WOStruct['#attributes.fuseaction#']['add']['nextEvent'] = 'cheque.form_add_voucher_payroll_entry&event=upd&id=';
		...
		if(isdefined("attributes.id"))
		{
			...
			WOStruct['#attributes.fuseaction#']['upd']['fuseaction'] = 'cheque.form_upd_voucher_payroll_entry';
			WOStruct['#attributes.fuseaction#']['upd']['filePath'] = '/V16/cheque/form/upd_payroll_voucher_entry.cfm';
			WOStruct['#attributes.fuseaction#']['upd']['queryPath'] = '/V16/cheque/query/upd_payroll_voucher_entry.cfm';
			WOStruct['#attributes.fuseaction#']['upd']['Identity'] = '#attributes.id#';
			WOStruct['#attributes.fuseaction#']['upd']['nextEvent'] = 'cheque.form_add_voucher_payroll_entry&event=upd&id=';
		...
		if(listFind('upd,del',attributes.event))
		{
			...
			WOStruct['#attributes.fuseaction#']['del']['fuseaction'] = '#request.self#?fuseaction=#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.del_voucher_payroll&id=#attributes.id#&head=##caller.get_action_detail.PAYROLL_NO##';
	...
	else
	{
		...
		else if(caller.attributes.event is 'upd')
		{
			...
			tabMenuStruct['#fuseactController#']['tabMenus']['upd']['icons']['add']['href'] = "#request.self#?fuseaction=cheque.form_add_voucher_payroll_entry&event=add";
			...
			tabMenuStruct['#fuseactController#']['tabMenus']['upd']['icons']['print']['onClick'] ="window.open('#request.self#?fuseaction=objects.popup_print_files&action=#attributes.fuseaction#&action_id=#attributes.id#&print_type=251','WOC');";
			tabMenuStruct['#fuseactController#']['tabMenus']['upd']['icons']['icn-md wrk-uF0134']['text'] = '#getLang('main',1040)#';
			tabMenuStruct['#fuseactController#']['tabMenus']['upd']['icons']['icn-md wrk-uF0134']['onClick'] = "windowopen('#request.self#?fuseaction=objects.popup_list_order_account_cards&voucher_payroll_id=#url.id#&action_id=#attributes.id#&process_cat=#get_action_detail.PAYROLL_TYPE#','wide');";
		}
	...
	WOStruct['#attributes.fuseaction#']['systemObject']['pageSettings'] = "['item-process_cat','item-payroll_revenue_date','item-revenue_collector_id','item-company_id']";
</cfscript>
