<!--- Eklendigi Asamanin Satirlarini Acik Yapar ve bu kaydi yapanın bulunduğu yetki grubuna uyarı yollar ve satırları açık hale getirir --->
<cfif len(caller.attributes.order_employee_id)>
	...
	<cfif GET_EMP_POS.RECORDCOUNT>
		...
		<cfif len(get_process_type.answer_hour)>
			<cfset max_warning_date = DateAdd("h", get_process_type.answer_hour, attributes.record_date)>
		</cfif>
		<cfif len(get_process_type.answer_minute)>
			<cfset max_warning_date = DateAdd("n", get_process_type.answer_minute, attributes.record_date)>
		</cfif>
