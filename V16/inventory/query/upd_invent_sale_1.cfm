...
<cfscript>
	...
	if(is_account eq 1)
	{
		...
		if (isdefined("attributes.row_kontrol#j#") and evaluate("attributes.row_kontrol#j#"))
		  {
			...
			str_alacak_tutar_list = ListAppend(str_alacak_tutar_list,wrk_round(value_new*evaluate("attributes.quantity#j#"),4),",");
			...
			{//sadece tevkifat için tevkifat tutarları alacak hesaplara ekleniyor
				...
				str_alacak_tutar_list = ListAppend(str_alacak_tutar_list,wrk_round(satir_tevk_tutar,4),",");
				if(listgetat(evaluate("attributes.money_id#j#"), 1, ',') neq session.ep.money)
					str_other_alacak_tutar_list = ListAppend(str_other_alacak_tutar_list,wrk_round((satir_tevk_tutar)/satir_currency_multiplier,4),",");
				else
					str_other_alacak_tutar_list = ListAppend(str_other_alacak_tutar_list,wrk_round((satir_tevk_tutar),4),",");
				str_other_alacak_currency_list = ListAppend(str_other_alacak_currency_list,listgetat(evaluate("attributes.money_id#j#"), 1, ','),",");
			...
			else if (indirimli_kdv neq 0)
			{
				...
				if(listgetat(evaluate("attributes.money_id#j#"), 1, ',') neq session.ep.money)
					str_other_alacak_tutar_list = ListAppend(str_other_alacak_tutar_list,wrk_round((indirimli_kdv)/satir_currency_multiplier,4),",");
				else
			...
			if(evaluate("attributes.last_diff_value#j#") gte 0)
			{
				...
				{
					str_other_alacak_tutar_list = listappend(str_other_alacak_tutar_list,wrk_round(evaluate("attributes.last_diff_value#j#")/satir_currency_multiplier,4),',');
					str_other_alacak_currency_list = listappend(str_other_alacak_currency_list,listgetat(evaluate("attributes.money_id#j#"), 1, ','),',');
				...
				str_other_borc_tutar_list = listappend(str_other_borc_tutar_list,wrk_round(abs(evaluate("attributes.last_inventory_value#j#")-evaluate("attributes.total_first_value#j#"))/satir_currency_multiplier,4),',');
			...
			{
				str_other_borc_tutar_list = listappend(str_other_borc_tutar_list,wrk_round(abs(evaluate("attributes.last_diff_value#j#"))/satir_currency_multiplier,4),',');
				str_other_borc_currency_list = listappend(str_other_borc_currency_list,listgetat(evaluate("attributes.money_id#j#"), 1, ','),',');
		...
		muhasebe_sil(action_id:attributes.invoice_id, process_type:form.old_process_type);
	...
	if(is_budget eq 1)
	{
		for(tt=1;tt lte attributes.record_num;tt=tt+1)
		{
			...
			other_money_value = (xml_total_budget eq 1) ? wrk_round(evaluate("attributes.row_other_total#tt#")/((evaluate("attributes.tax_rate#tt#")+100)/100),4) : wrk_round(abs(evaluate("attributes.last_diff_value#tt#"))/satir_currency_multiplier,4);
...
<cfif isdefined("form.cash")><!--- kasa seçili ise --->
	<cfif len(get_invoice.cash_id)>
		<cfquery name="UPD_ALISF_KAPA" datasource="#dsn2#">
			...
			SET
				...
				CASH_ACTION_FROM_COMPANY_ID = <cfif len(attributes.company_id)>#attributes.company_id#<cfelse>NULL</cfif>,
				CASH_ACTION_FROM_CONSUMER_ID = <cfif len(attributes.consumer_id)>#attributes.consumer_id#<cfelse>NULL</cfif>,
				CASH_ACTION_VALUE = #attributes.net_total_amount#,
				CASH_ACTION_CURRENCY_ID = '#SESSION.EP.MONEY#',
		...
		<cfquery name="ADD_ALISF_KAPA" datasource="#dsn2#">
			...
			VALUES
				(
					...
					1,
					'#FORM.INVOICE_NUMBER#',
					<cfif is_account>1,11,<cfelse>0,11,</cfif>
					#SESSION.EP.USERID#,
					'#CGI.REMOTE_ADDR#',
					#NOW()#,
					#form.process_cat#,
	...
	<cfscript>
		DETAIL_2 = "DEMİRBAŞ SATIŞ FATURASI KAPAMA İŞLEMİ";
		if(is_cari)//kasa cari
			...
			cari_sil(action_id:get_invoice.cash_id, process_type:35);
	</cfscript>
	...
	<cfif is_account><!--- kasa muhasebe --->
		...
		<cfscript>
			DETAIL_2 = attributes.comp_name & " " & "DEMİRBAŞ SATIŞ FATURASI"  & " KAPAMA İŞLEMİ";
			muhasebeci(
				action_id : act_id,
				workcube_process_type : 35,
				workcube_old_process_type : 35,
				account_card_type : 11,//0,
				islem_tarihi : attributes.invoice_date,
	...
	<cfelse>									
		<cfscript>
			if(len(get_invoice.cash_id))
				muhasebe_sil(action_id:get_invoice.cash_id, process_type:35);
		</cfscript>	
	</cfif>
