...
<cfset get_wo_file_path = cmp.GET_WO_FILE_PATH(full_fuseaction : attributes.fuseaction)>
<cfinclude template="product_export_xml_data.cfm">
<cfset attributes.mode = 3>
<cfparam name="attributes.keyword" default="">

<cfset get_xml_files=cmp.get_xml_files_f()>

<!--- <cf_box title="Product-Price-Stock Wizard">
	<cfinclude template="popup_product_export_wex.cfm">
</cf_box> --->

<!--- <cf_box title="Versions"> --->
	<cfform name="search" action="" method="post">
		...
		<div class="row form-inline">
			...
			<!--- <div class="form-group">
				<cfoutput>
				<a href="javascript://" onClick="windowopen('#request.self#?fuseaction=worknet.productexportwex&event=add','medium');" class="btn grey-cascade btn-small icon-pluss"></a>
				</cfoutput>
			</div> --->
		</div>
	</cfform>
	<cf_medium_list>
		...
		<tbody>
			<cfoutput query="get_xml_files">
				<tr data-id="#wex_file_id#">
					<td data-key>#wex_file_id#</td>
					<td><a href="/documents/wex_files/#wex_file_name#" class="tableyazi" target="_blank">#wex_file_name#</a></td>
					<td>#wex_file_id#</td>
					<td>/documents/wex_files/#replace(wex_file_name,".xml","","one")#</td>
					<td>#wex_detail#</td>
					<td>#record_ip#</td>
					<td style="text-align:center;">
						<a href="/documents/wex_files/#wex_file_name#" target="_blank"><i class="fa fa-file-code-o" style="font-size:16px" title="Ürün XML Dosyası !"></i></a>
						&nbsp;&nbsp;&nbsp;
						<cfsavecontent variable="message">Kayıtlı Wex Dosyasını Siliyorsunuz Emin misiniz!</cfsavecontent>
						<a href="javascript://" onClick="if (confirm('#message#')) windowopen('#request.self#?fuseaction=worknet.productexportwex&event=del&xml_file_id=#wex_file_id#','medium');"><i class="fa fa-minus-circle" style="font-size:16px" title="XML Sil !"></i> </a>
					</td>
					<td style="text-align:center;"><input type="button" value="WEX Export" href="javascript//" onclick="windowopen('#request.self#?fuseaction=objects.emptypopup_system&type=wxAdd&wex_file_id=#wex_file_id#&source_file_path=#get_wo_file_path.file_path#&source_wo=#attributes.fuseaction#','medium');"></td><!--- dev.workdev&fuseact=worknet.productexportwex##wx --->
				</tr>
			</cfoutput>			 
		</tbody>
	</cf_medium_list>	
