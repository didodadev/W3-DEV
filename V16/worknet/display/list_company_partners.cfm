...
<table class="ajax_list">
	<cfform name="form_partner" method="post" action="#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.popupajax_my_company_partners&cpid=#attributes.cpid#&maxrows=#session.ep.maxrows#&is_active=#attributes.partner_status#">
	<tr>
        <th colspan="8" align="right" style="text-align:right;">
			<select name="partner_status" style="width:60px;">
	...
	<cfif get_partner.recordcount>
	  <cfoutput query="get_partner">
		<tr <cfif company_partner_status eq 0>style="color:999999;"</cfif>>
			...
			<td><a href="#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.detail_partner&pid=#partner_id#" <cfif company_partner_status eq 0>style="color:999999;"<cfelse>class="tableyazi"</cfif>>#company_partner_name# #company_partner_surname#</a></td>
...
<script type="text/javascript">
	...
	{	
		AjaxFormSubmit(form_partner,'show_partner_message',0,' ',' ','<cfoutput>#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.popupajax_my_company_partners&cpid=#attributes.cpid#&maxrows=#session.ep.maxrows#&is_active=#attributes.partner_status#</cfoutput>','body_list_company_partner');
	}
</script>
