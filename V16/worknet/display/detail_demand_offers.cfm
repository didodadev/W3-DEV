    ...
    <cfif get_demand_offer.recordcount>
        <cfoutput query="get_demand_offer">
            <tr height="22">
                <td>
                    <cfif len(partner_id)>
                        <a href="#request.self#?fuseaction=worknet.detail_company&cpid=#company_id#" class="tableyazi" title="<cf_get_lang no ='15.Firma Detay'>">#get_par_info(partner_id,0,0,0)#</a>
...
<cfoutput>
	...
	function open_div_price(no,demand_offer_id)
	{
		...
		AjaxPageLoad('#request.self#?fuseaction=worknet.emptypopup_display_detail_offer&demand_offer_id=' + demand_offer_id, 'detail_offer' + no);
	}
	</script>
</cfoutput>
