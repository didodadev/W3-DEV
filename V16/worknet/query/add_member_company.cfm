...
<!--- <cfif len(attributes.partner_email)>
	<cfquery name="CHECK_USERNAME" datasource="#DSN#">
		...
		FROM 
			COMPANY_PARTNER
		WHERE 
			COMPANY_PARTNER_EMAIL = <cfqueryparam cfsqltype="cf_sql_varchar" value="#trim(attributes.partner_email)#">
	</cfquery>
...
<cflock name="#CREATEUUID()#" timeout="20">
	<cftransaction>
		...
		<cfset cmp.addCompany(
				...
				soyad:attributes.partner_surname,
				<!--- title:attributes.title, --->
				<!--- sex:attributes.sex, --->
				<!--- department:attributes.department, --->
				company_partner_email:attributes.partner_email,
				<!--- password:pass, --->
		...
		<cf_addressbook
			...
			active		= "1"
			name		= "#attributes.partner_name#"
			surname		= "#attributes.partner_surname#"
			<!--- sector_id	= "#attributes.company_sector#" --->
			company_name= "#attributes.fullname#"
			<!--- title		= "#attributes.title#" --->
			email		= "#attributes.partner_email#"
			telcode		= "#attributes.telcod1#"
			...
			address		= "#wrk_eval('attributes.adress')#"
			<!--- semt		= "#attributes.semt#" --->
			county_id	= "#attributes.county_id#"
			city_id		= "#attributes.city_id#"
		...
		<cfif isdefined("session.ep")>
			...
			<cf_workcube_process 
				...
				action_id='#get_max.max_company#'
				action_page='#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.form_list_company&event=upd&cpid=#get_max.max_company#' 
...
<script type="text/javascript">
	<cfoutput>
		window.location.href = '#request.self#?fuseaction=worknet.form_list_company&event=upd&cpid=#get_max.max_company#';
	</cfoutput>
</script>
<!---Ek Bilgiler--->
...
<cfelse>
	<cflocation url="#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.form_list_company&event=upd&cpid=#get_max.max_company#&pid=#get_max_partner.MAX_PART#" addtoken="no">
</cfif>
 --->