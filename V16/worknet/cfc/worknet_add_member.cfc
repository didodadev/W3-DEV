<cfcomponent>
	...
	<cffunction name = "getRelationWorknet" access = "public" hint = "Get Relation Worknet">
        ...
        <cfquery name="get_relation_worknet" datasource="#dsn#">
            ...
            FROM WORKNET_RELATION_COMPANY AS WRC
            JOIN COMPANY AS CMP ON CMP.COMPANY_ID = WRC.COMPANY_ID
            JOIN WORKNET AS WRK ON WRK.WORKNET_ID = WRC.WORKNET_ID
            WHERE
                1=1 
                AND WRC.COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.cpid#">
        </cfquery>
        <cfreturn get_relation_worknet>
	</cffunction>

	<!--- GET NOT WORKNET_RELATION_COMPANY --->
	<cffunction name = "getNotRelationWorknet" access = "public" hint = "Get Not Relation Worknet">
		<cfargument name="cpid" type="numeric" required="yes">
		<cfargument name="logo_list" type="numeric" required="no" default="0">
        <cfquery name="get_not_relation_worknet" datasource="#dsn#">
            ...
            WHERE
                1=1 
				AND WRK.WORKNET_ID <cfif arguments.logo_list eq 0>NOT</cfif> IN (
					SELECT WRC.WORKNET_ID FROM WORKNET_RELATION_COMPANY AS WRC WHERE WRC.COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.cpid#">
				)
	...
	<cffunction name = "insertRelationWorknet" access = "remote" hint = "Insert Relation Worknet" returnformat="JSON">
		...
		<cftry>
			<cfquery name="insert_relation_worknet" datasource="#dsn#" result="result">
				...
				VALUES
				(
					<cfqueryparam value = "#arguments.wrkid#" CFSQLType = "cf_sql_nvarchar">,
					<cfqueryparam value = "#arguments.cpid#" CFSQLType = "cf_sql_nvarchar">,
					#now()#,
					#session.ep.userid#,
					'#cgi.remote_addr#'
				)
	...
	<!--- UPD WORKNET_RELATION_COMPANY --->
	<cffunction name = "updateRelationWorknet" access = "remote" hint = "Update Relation Worknet" returnformat="JSON">
		<cfset attributes = arguments>
		<cfset response = structNew()>
		<cfset response.status = true>
		<cftry>
			<cfset getRelationCompany = getRelationCompany( wrkid: arguments.wid)>
			<cfif getRelationCompany.recordcount eq 0>
				<cfquery name="insert_relation_worknet" datasource="#dsn#" result="result">
					INSERT INTO
						WORKNET_RELATION_COMPANY
						(
					...
					VALUES
						(
							<cfqueryparam value = "#arguments.wid#" CFSQLType = "cf_sql_integer">,
			...
			<cfquery name="update_relation_worknet" datasource="#dsn#" result="result">
				UPDATE
					WORKNET_RELATION_COMPANY
				SET
					SELLER = <cfif isDefined('arguments.seller') and len(arguments.seller)><cfqueryparam value = "#arguments.seller#" CFSQLType = "cf_sql_nvarchar"><cfelse>NULL</cfif>,
					CLIENT_KEY = <cfif isDefined('arguments.client_key') and len(arguments.client_key)><cfqueryparam value = "#arguments.client_key#" CFSQLType = "cf_sql_nvarchar"><cfelse>NULL</cfif>,
					USER_NAME_MAIL = <cfif isDefined('arguments.user_name_mail') and len(arguments.user_name_mail)><cfqueryparam value = "#arguments.user_name_mail#" CFSQLType = "cf_sql_nvarchar"><cfelse>NULL</cfif>,
					STORE_URL = <cfif isDefined('arguments.store_url') and len(arguments.store_url)><cfqueryparam value = "#arguments.store_url#" CFSQLType = "cf_sql_nvarchar"><cfelse>NULL</cfif>,
					USE_WATALOGY_STORE = <cfif isDefined('arguments.use_watalogy_store') and len(arguments.use_watalogy_store)><cfqueryparam value = "#arguments.use_watalogy_store#" CFSQLType = "cf_sql_bit"><cfelse>NULL</cfif>
				WHERE
					WORKNET_ID = <cfqueryparam value = "#arguments.wid#" CFSQLType = "cf_sql_integer">
			</cfquery>
			<cfset response.MESSAGE = "Güncelleme başarılı">
			<cfset response.status = true>
            <cfset response.identity = arguments.wid>
			<cfcatch type = "any">
				<cfset response.status = false>
				<cfset response.message = "Güncelleme hatalı">
			</cfcatch>
		</cftry>
		<cfreturn response>
	</cffunction>

	<!--- SELECT WORKNET_RELATION_COMPANY --->
	<cffunction name="getRelationCompany" returntype="any">
		<cfargument name="wrkid" type="numeric" required="yes">
        <cfquery name="get_company_worknet" datasource="#dsn#">
            SELECT TOP 1 * FROM WORKNET_RELATION_COMPANY WHERE WORKNET_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.wrkid#"> AND OUR_COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.ep.company_id#">
        </cfquery>
		<cfreturn get_company_worknet>
    </cffunction>
	
	<!--- DELETE WORKNET_RELATION_COMPANY --->
	<cffunction name = "deleteRelationWorknet" access = "remote" hint = "Delete Relation Worknet" returnformat="JSON">
		...
		<cftry>
			<cfquery name="delete_relation_worknet" datasource="#dsn#">
				DELETE FROM WORKNET_RELATION_COMPANY WHERE WORKNET_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.wrkid#"> AND COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.cpid#">
			</cfquery>
	...
	<cffunction name="addCompany" access="public" returntype="string">
		...
		<cfset wat_member_code = memberCodeControl()>
		<cfquery name="ADD_COMPANY" datasource="#DSN#">
			...
			(
				...
				0,
				#wat_member_code#,
				<cfif len(arguments.domain)><cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.domain#"><cfelse>NULL</cfif>
			)
		</cfquery>
	...
	<!--- MEMBER_CODE CONTROL --->
	<cffunction name="memberCodeControl" access="private" returntype="any">
		<cfset wat_member_code = randRange(100000,999999) & randRange(100000,999999)>
		<cfquery name="memberCode_Control" datasource="#DSN#">
			SELECT
				*
			FROM
				COMPANY
			WHERE
				WATALOGY_MEMBER_CODE = <cfqueryparam cfsqltype="cf_sql_nvarchar" value="#wat_member_code#">
		</cfquery>
		<cfif memberCode_Control.recordCount gt 0>
			<cfset response = memberCodeControl()>
		<cfelse>
			<cfset response = wat_member_code>
		</cfif>
		<cfreturn response>
	</cffunction>
	...
	<!--- DEL COMPANY --->
	<cffunction name="delCompany" returntype="any">
		<cfargument name="cpid" type="numeric" required="yes">
        <cfquery name="del_company" datasource="#dsn#">
            DELETE FROM COMPANY WHERE COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.cpid#">
		</cfquery>
		<cfquery name="del_comp_period" datasource="#dsn#">
			DELETE FROM COMPANY_PERIOD WHERE COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.cpid#">
		</cfquery>
		<cfquery name="del_comp_proCat" datasource="#dsn#">
			DELETE FROM WORKNET_RELATION_PRODUCT_CAT WHERE COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.cpid#">
		</cfquery>
		<cfquery name="del_comp_par" datasource="#dsn#">
			DELETE FROM COMPANY_PARTNER WHERE COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.cpid#">
		</cfquery>
		<cfquery name="del_comp_branch" datasource="#dsn#">
			DELETE FROM COMPANY_BRANCH_RELATED WHERE COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.cpid#">
		</cfquery>
		<cfquery name="DEL_PRODUCT_WORKNET" datasource="#dsn#">
			DELETE FROM WORKNET_RELATION_COMPANY WHERE COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.cpid#">
		</cfquery>
    </cffunction>
	<!--- UPD COMPANY --->
	<cffunction name="updCompany" access="public" returntype="string">
		<cfargument name="process_stage" type="numeric" required="no">
		<cfargument name="domain" type="string" required="yes">
		<cfargument name="companycat_id" type="numeric" required="no">
		<cfargument name="nickname" type="string" required="yes">
		<cfargument name="fullname" type="string" required="yes">
		<cfargument name="vd" type="string" required="no">
		<cfargument name="vno" type="string" required="no">
		...
		<cfquery name="UPD_COMPANY" datasource="#DSN#">
			...
			SET
				...
				UPDATE_DATE = #now()#,
				DOMAIN = <cfif len(arguments.domain)><cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.domain#"><cfelse>NULL</cfif>
			WHERE
				COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.company_id#">
		</cfquery>
        <cf_wrk_get_history  datasource='#DSN#' source_table= 'COMPANY' target_table= 'COMPANY_HISTORY' record_id= '#arguments.company_id#' record_name='COMPANY_ID'>
        
		<!--- sertifikalar --->
		<cfif isdefined('arguments.req_type') and len(arguments.req_type)>
		...
		<cfif isdefined('arguments.product_category') and  len(arguments.product_category)>
        	<cfloop list="#arguments.product_category#" index="i">
				<cfquery name="ADD_PRODUTCT_CAT" datasource="#DSN#">
					INSERT INTO 
						WORKNET_RELATION_PRODUCT_CAT
		...
		<cfquery name="GET_COMPANY" datasource="#DSN#">
			...
			FROM 
				...
				<cfif len(arguments.marketplace)>
					LEFT JOIN WORKNET_RELATION_COMPANY WRC ON C.COMPANY_ID = WRC.COMPANY_ID
				</cfif>
			WHERE 
				WATALOGY_MEMBER_CODE IS NOT NULL
				<cfif len(arguments.marketplace)>
