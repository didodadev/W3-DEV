...
<!--- account.form_add_bill_collecting&eq=1 --->
...
<cflock name="#createUUID()#" timeout="20">
	<cftransaction>
		...
		<cfquery name="ADD_ACCOUNT_CARD" datasource="#DSN2#" result="MAX_ID">
			...
			VALUES
				(
					...
					<cfif isdefined('acc_process_type') and len(acc_process_type)><cfqueryparam cfsqltype="cf_sql_integer" value="#acc_process_type#"><cfelse><cfqueryparam cfsqltype="cf_sql_integer" value="11"></cfif>,
					<cfqueryparam cfsqltype="cf_sql_integer" value="#form.process_cat#">,
					<cfqueryparam cfsqltype="cf_sql_integer" value="#get_bill_no.TAHSIL_BILL_NO#">,
		...
		<cfquery name="ADD_ACCOUNT_CARD_ROW_KASA" datasource="#DSN2#">
			...
			VALUES
				(
					...
					<cfif isdefined("attributes.IS_OTHER_CURRENCY") and attributes.other_cash_amount gt 0><cfqueryparam cfsqltype="cf_sql_float" value="#attributes.other_cash_amount#">,<cfelse><cfqueryparam cfsqltype="cf_sql_float" value="#attributes.cash_debt#">,</cfif>
					...
					<cfqueryparam cfsqltype="cf_sql_float" value="#attributes.cash_debt#">,
		...
		<cfloop from="1" to="#attributes.rowCount#" index="i">
			<cfif acc_code_index lte listlen(attributes.acc_code,',') and (listgetat(attributes.claim,i,',') gt 0)>
				...
				<cfquery name="ADD_ACCOUNT_CARD_ROW_ALICI" datasource="#DSN2#">
					...
					(
						...
						<cfqueryparam cfsqltype="cf_sql_float" value="#ListGetAt(attributes.claim, i, ',')#">,<!---borc degeri--->
						<cfqueryparam cfsqltype="cf_sql_varchar" value="#session.ep.money#">,
		...
		<cfquery name="UPD_BILL_NO" datasource="#DSN2#">
			...
			SET
				BILL_NO = BILL_NO+1,
				TAHSIL_BILL_NO = TAHSIL_BILL_NO+1
		</cfquery>
		<cfscript>
        ...
        <cf_add_log employee_id="#session.ep.userid#" log_type="1" action_id="#MAX_ID.IDENTITYCOL#" action_name="#attributes.paper_no# Eklendi" paper_no="#attributes.paper_no#" period_id="#session.ep.period_id#" process_type="#get_process_type.PROCESS_TYPE#" data_source="#dsn2#">
	</cftransaction>
</cflock>
<cfset attributes.actionId = MAX_ID.IDENTITYCOL >

<script type="text/javascript">
	window.location.href='<cfoutput>#request.self#?fuseaction=account.form_add_bill_collecting&event=upd&var_=collecting_card&card_id=#MAX_ID.IDENTITYCOL#</cfoutput>';
</script>
