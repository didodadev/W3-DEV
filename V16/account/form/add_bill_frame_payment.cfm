...
<cf_grid_list sort="0" id="delete_tr">
    <thead>
        <tr> 
            ...
            <th id="other_money_1" <cfif (not isdefined("get_account_rows_A") or get_account_card.is_other_currency neq 1) and (not isdefined("get_comp_info.is_other_money") or get_comp_info.is_other_money neq 1)>style="display:none;"</cfif>><cf_get_lang dictionary_id='58121.Islem Dovizi'></th>            
    ...
    <tbody id="table_list">
		<cfif isdefined("get_account_rows_A")>
            ...
            <cfoutput query="get_account_rows_A">
                ...
                <tr id="tr_id#currentrow#">
                    <td><a href="javascript://" class="btnDelete"><i class="fa fa-minus" title="<cf_get_lang dictionary_id ='57463.Sil'>"  alt="<cf_get_lang dictionary_id ='57463.Sil'>" ></i></a></td>
                    ...
                    <td nowrap>
						<div class="form-group">
								<div class="input-group">
									...
									<span class="input-group-addon btnPointer icon-ellipsis" onClick="javascript:pencere_ac(#currentrow#);"></span> 
									<span onClick="ac_hesap_detay(#currentrow#);" class="input-group-addon btnPointer icon-chart"></span>									</div>
						</div>
					</td>
                    ...
                    <td>                      
                        <span class="input-group-addon btnPointer icon-ellipsis" onClick="javascript:ac_hesap_detay(#currentrow#);"></span>
                    <td>
						<div class="form-group" style="width:150px;">
                        ...
                        <td>
							<div class="form-group" style="width:150px;">
									<div class="input-group">
										...
										<span class="input-group-addon btnPointer icon-ellipsis" onclick="windowopen('#request.self#?fuseaction=objects.popup_list_departments&field_id=add_bill.acc_department_id#currentrow#' +'&field_dep_branch_name=add_bill.acc_department_name#currentrow#','list')" ></span>
									</div>
							</div>
						</td>
                    </cfif>
						...
						<td>
							<div class="form-group" style="width:150px;">
									<div class="input-group">
										...
										<span class="input-group-addon btnPointer icon-ellipsis"  onclick="openBoxDraggable('#request.self#?fuseaction=objects.popup_list_projects&project_id=add_bill.acc_project_id#currentrow#' +'&project_head=add_bill.acc_project_name#currentrow#')" ></span>
									</div>
							</div>
						</td>
					</cfif> 
                    <td>
						<div class="form-group" style="width:150px;">
								<input type="text" name="debt" id="debt" value="#TLFormat(AMOUNT)#" onchange="hesapla();" onBlur="f_kur_hesapla(1,'#currentrow#');hesapla();" onkeyup="return(FormatCurrency(this,event));" class="moneybox">
							</div>
					</td>
                    <cfif len(session.ep.money2)>
                        <td>
        ...
        <cfelse>
            <cfoutput> 
                <cfloop from="1" to="10" index="i">
                    <tr id="tr_id#i#"> 
                        <td><a href="javascript://" class="btnDelete"><i class="fa fa-minus" alt="<cf_get_lang dictionary_id ='57463.Sil'>" title="<cf_get_lang dictionary_id ='57463.Sil'>"></i></a></td>
                        <td>#i#</td>
                        <td nowrap>
							<div class="form-group">
							
									<div class="input-group">
										<input type="text" name="acc_code" id="acc_code#i#" value="" onFocus="auto_acc_code(#i#);" autocomplete="off">
										<span class="input-group-addon btnPointer icon-ellipsis" onClick="pencere_ac(#i#);"></span>	
                        ...
                        <td>
							<div class="form-group" style="width:150px;">
									<input type="text" name="debt" id="debt" value="" class="moneybox" onchange="hesapla();" onBlur="f_kur_hesapla(1,'#i#');hesapla();" onkeyup="return(FormatCurrency(this,event));">
								</div>
						</td>
                        <cfif len(session.ep.money2)>
                            <td>
        ...
        <input type="hidden" name="debt_total" id="debt_total" value="0">
...
<script type="text/javascript">
	rowCount = <cfif isdefined("get_account_rows_a.recordcount")><cfoutput>#get_account_rows_A.recordcount#</cfoutput><cfelse>10</cfif>;
	...
	function hesapla()
	{
		...
		for (var j=0; j < rowCount; j++)
		{
			if(rowCount == 1)
			{
				if(add_bill.debt.value.length)
					t_debt += parseFloat(filterNum(add_bill.debt.value));
				if(add_bill.amount_2 != undefined && add_bill.amount_2.value.length)
					t_amount_2 += parseFloat(filterNum(add_bill.amount_2.value));
			}
			else
			{
				if (add_bill.debt[j].value.length)
					t_debt += parseFloat(filterNum(add_bill.debt[j].value));
		...
		add_bill.cash_claim.value = commaSplit(t_debt);
		if(add_bill.amount_2 != undefined)
		{
			...
			if(add_bill.other_cash_currency[x].value == money2_js)
	...
	function kontrol2()
	{
		...
		if(add_bill.bill_detail.value.length > 150)
		{
			alert("<cf_get_lang dictionary_id='122.Açıklama Uzunluğunu Aştınız'>");
			return false;
		}	
		if (document.getElementById("cash_acc_code").value == '')
		{
			alert("<cf_get_lang_main dictionary_id='57471.Eksik veri'>:<cf_get_lang dictionary_id='39.Kasa hesap adı'>!");
			return false;
		}
		total_all_row=0;
		...
		if(rowCount==1)
		{
			if ((!add_bill.acc_code.value.length) && (add_bill.debt.value.length))
			{
			...
			if(add_bill.debt.value == '') add_bill.debt.value = 0;
			if(add_bill.other_amount.value == '') add_bill.other_amount.value = 0;
			if((add_bill.amount_2 != undefined) && (add_bill.amount_2.value == ''))
				add_bill.amount_2.value = 0;
			if((add_bill.ifrs_code != undefined) && (add_bill.ifrs_code.value == ''))
				add_bill.ifrs_code.value = 0;
			if(add_bill.account_code2.value == '') add_bill.account_code2.value = 0;
			total_all_row=add_bill.debt.value;
		}
		else
		{
			for (var j=0; j < rowCount; j++)
				if((!add_bill.acc_code[j].value.length) && (add_bill.debt[j].value.length))
				{
			...
			for (var j=0; j < rowCount; j++)
			{
				...
				if(add_bill.amount_2!= undefined && add_bill.amount_2[j] != undefined && add_bill.amount_2[j].value == '')
					add_bill.amount_2[j].value = 0;	
				if(add_bill.ifrs_code!= undefined && add_bill.ifrs_code[j] != undefined && add_bill.ifrs_code[j].value == '')
					add_bill.ifrs_code[j].value = 0;
				if(add_bill.account_code2[j].value == '') add_bill.account_code2[j].value = 0;
				total_all_row+=add_bill.debt[j].value;			
			}
		}
		add_bill.debt_total.value = total_all_row;
		if(add_bill.debt_total.value==0)
		{
			alert("<cf_get_lang dictionary_id ='59053.Bakiye Girmelisiniz'>!");
			return false;
		}
		add_bill.cash_claim.value = filterNum(add_bill.cash_claim.value);
		...
		if(document.add_bill.card_id.value){
			var parameter = document.add_bill.card_id.value + '*' + document.getElementById("paper_no").value + '*' + '12';
		}
		else
		{
			var parameter = document.getElementById("paper_no").value + '*' + '12';
		}
	...
	function pencere_ac(row)
	{
		...
		if(rowCount==1)
		{
			if (add_bill.acc_code.value.length != 0)
				openBoxDraggable('<cfoutput>#request.self#</cfoutput>?fuseaction=objects.popup_account_plan&field_id=add_bill.acc_code&field_ufrs_no=add_bill.ifrs_code&field_name=add_bill.acc_name&account_code=' + add_bill.acc_code.value);
			else
				openBoxDraggable('<cfoutput>#request.self#</cfoutput>?fuseaction=objects.popup_account_plan&field_id=add_bill.acc_code&field_ufrs_no=add_bill.ifrs_code&field_name=add_bill.acc_name');
		}
		else
		{
	...
	function addRow()
	{
		...
		newCell = newRow.insertCell(newRow.cells.length);
		newCell.innerHTML =  '<div class="form-group"><div class="input-group"><input type="text" name="acc_code" id="acc_code' + rowCount + '" style="width:90px;" onFocus="auto_addrow_acc_code(' + rowCount + ');" autocomplete="off"><span class="input-group-addon btnPointer icon-ellipsis" onClick="pencere_ac(' + rowCount + ');"></span><span class="input-group-addon btnPointer icon-chart" onClick="ac_hesap_detay(' + rowCount + ');"></span></div></div>';
		newCell = newRow.insertCell(newRow.cells.length);
		...
		newCell.innerHTML = '<input type="text" name="ifrs_code" value="" style="width:65px;">';
		newCell = newRow.insertCell(newRow.cells.length);
		...
		newCell = newRow.insertCell(newRow.cells.length);
		newCell.innerHTML = '<input type="text" name="acc_name" id="acc_name' + rowCount + '" style="width:100px;">';
		newCell = newRow.insertCell(newRow.cells.length);
		newCell.innerHTML = '<input type="text" name="detail_' + rowCount + '" style="width:100px;" onchange="hesapla();">';
		
		<cfif xml_acc_department_info and session.ep.isBranchAuthorization eq 0>
			newCell = newRow.insertCell(newRow.cells.length);
			newCell.innerHTML = '<div class="form-group" style="width:150px;"><div class="input-group"><input type="hidden" name="acc_department_id' + rowCount + '"><input type="text" name="acc_department_name' + rowCount + '" style="width:110px;"><span onClick="select_acc_department('+ rowCount +');" class="input-group-addon btnPointer icon-ellipsis" class= alt="<cf_get_lang dictionary_id='57734.seçiniz'>"></span>';
		</cfif>	
		<cfif xml_acc_project_info>
			newCell = newRow.insertCell(newRow.cells.length);
			newCell.innerHTML = '<div class="form-group" style="width:150px;"><div class="input-group"><input type="hidden" name="acc_project_id' + rowCount + '" id="acc_project_id' + rowCount + '"><input type="text" name="acc_project_name' + rowCount + '" id="acc_project_name' + rowCount + '" style="width:110px;" onFocus="auto_project('+ rowCount +');" autocomplete="off"><span onClick="select_acc_project('+ rowCount +');" class="input-group-addon btnPointer icon-ellipsis" alt="<cf_get_lang dictionary_id='57734.seçiniz'>" border="0" align="absmiddle"></span></div></div>';
		</cfif>	
		
		newCell = newRow.insertCell(newRow.cells.length);
		newCell.innerHTML = '<input type="text" name="debt" value="" onchange="hesapla();" onBlur="f_kur_hesapla(this.value,' + rowCount + ');hesapla();" onkeyup="return(FormatCurrency(this,event));" style="width:80px;" class="moneybox">';
	
	  if(money2_js != '')
	  {
		newCell = newRow.insertCell(newRow.cells.length);
		newCell.innerHTML = '<input type="text" name="amount_2" value="" onchange="hesapla();" onBlur="hesapla();" onkeyup="return(FormatCurrency(this,event));" class="moneybox" style="width:80px;">';
	  }
		...
		newCell.innerHTML = '<input type="text" name="other_amount" value="" onkeyup="return(FormatCurrency(this,event));" class="moneybox" style="width:80px;">';
	
		newCell = newRow.insertCell(newRow.cells.length);
		newCell.id = 'other_money_2';
		if(document.add_bill.is_other_currency.checked) newCell.style.display = ''; else newCell.style.display = 'none';
		newCell.innerHTML = '<div class="form-group" style="width:65px;"><select name="other_currency" style="width:65px;" onChange="f_kur_hesapla(this.value,' + rowCount + ');hesapla();"><cfoutput query="get_money_doviz"><option value="#money#" <cfif money eq session.ep.money>selected</cfif>>#money#</option></cfoutput></select></div>';
		
	}
	function auto_addrow_acc_code(no)
	{
		AutoComplete_Create('acc_code'+no,'ACCOUNT_CODE','ACCOUNT_CODE,ACCOUNT_NAME','get_account_code','0','CODE_NAME','acc_name'+no,'','3','250');
			...
			for(var i=1;i<=<cfoutput>#get_money_bskt.recordcount#</cfoutput>;i++)
			{
				if(eval('add_bill.hidden_rd_money_'+i).value == temp_act || eval('add_bill.hidden_rd_money_'+i).value == money2_js)
				{
					...
					if(eval('add_bill.hidden_rd_money_'+i).value == money2_js)
						add_bill.amount_2.value = commaSplit(filterNum(add_bill.debt.value)*rate1_eleman/rate2_eleman);
					if(eval('add_bill.hidden_rd_money_'+i).value == temp_act)
						add_bill.other_amount.value = commaSplit(filterNum(add_bill.debt.value)*rate1_eleman/rate2_eleman);
				}
			}
			if(temp_act == '<cfoutput>#session.ep.money#</cfoutput>')
				add_bill.other_amount.value = add_bill.debt.value;
		...
		else
		{
			...
			for(var i=1;i<=<cfoutput>#get_money_bskt.recordcount#</cfoutput>;i++)
			{
				if(eval('add_bill.hidden_rd_money_'+i).value == temp_act || eval('add_bill.hidden_rd_money_'+i).value == money2_js)
				{
					...
					rate2_eleman = filterNum(eval('add_bill.txt_rate2_' + i).value,session_money_round);
					if(eval('add_bill.hidden_rd_money_'+i).value == money2_js)
						add_bill.amount_2[row_no-1].value = commaSplit(filterNum(add_bill.debt[row_no-1].value)*rate1_eleman/rate2_eleman);
					if(eval('add_bill.hidden_rd_money_'+i).value == temp_act)
						add_bill.other_amount[row_no-1].value = commaSplit(filterNum(add_bill.debt[row_no-1].value)*rate1_eleman/rate2_eleman);
				}
			}
			if(temp_act == '<cfoutput>#session.ep.money#</cfoutput>')
				add_bill.other_amount[row_no-1].value = add_bill.debt[row_no-1].value;
			else if(temp_act == '' || temp_act == 0)
				...
				if(add_bill.debt[j].value != '')
				{
					for(var i=1;i<=<cfoutput>#get_money_bskt.recordcount#</cfoutput>;i++)
					{
						if(eval('add_bill.hidden_rd_money_'+i).value == add_bill.other_currency[j].value || eval('add_bill.hidden_rd_money_'+i).value == money2_js)
						{
							...
							rate2_eleman = filterNum(eval('add_bill.txt_rate2_' + i).value,session_money_round);
							if(eval('add_bill.hidden_rd_money_'+i).value == money2_js)
								add_bill.amount_2[j].value = commaSplit(filterNum(add_bill.debt[j].value)*rate1_eleman/rate2_eleman);
							if(eval('add_bill.hidden_rd_money_'+i).value == add_bill.other_currency[j].value)
								add_bill.other_amount[j].value = commaSplit(filterNum(add_bill.debt[j].value)*rate1_eleman/rate2_eleman);
						}
					}
				}
			}
	...
	function f_total_hesapla(temp_act)
	{
		...
		if(temp_act == '<cfoutput>#session.ep.money#</cfoutput>')
			add_bill.other_cash_amount.value =add_bill.cash_claim.value;
		else if(temp_act == '')
		...
		else
		{
			for(var i=1;i<=<cfoutput>#get_money_bskt.recordcount#</cfoutput>;i++)
			{
				if(eval('add_bill.hidden_rd_money_'+i).value == temp_act)
				{
					rate1_eleman = filterNum(eval('add_bill.txt_rate1_' + i).value,session_money_round);
					rate2_eleman = filterNum(eval('add_bill.txt_rate2_' + i).value,session_money_round);
					add_bill.other_cash_amount.value = commaSplit(filterNum(add_bill.cash_claim.value)*rate1_eleman/rate2_eleman);
				}
			}
		}
	}
