...
<cfquery name="get_acc_card_type" dbtype="query">
	SELECT PROCESS_TYPE,PROCESS_CAT_ID,PROCESS_CAT FROM GET_ALL_PROCESS_CAT WHERE 1=1 <cfif is_selected_account eq 0>AND IS_ACCOUNT = 1 </cfif>AND PROCESS_TYPE IN (11,12,13,14) ORDER BY PROCESS_TYPE
</cfquery>
...
<div class="col col-12 col-md-12 col-sm-12 col-xs-12">
	<cf_box>
		<cfform name="Add_Form_List_Bills" action="#request.self#?fuseaction=account.emptypopup_bills_process_group" method="post">
			<cf_box_elements vertical="1">
				<div class="col col-4 col-md-4 col-sm-6 col-xs-12" type="column" index="1" sort="true">
						...
						<div class="col col-9 col-md-9 col-sm-9 col-xs-12"> 
							<select name="process_type" id="process_type" multiple>
								<cfoutput query="get_process_cat_process_type">
									<option disabled="disabled" value="#process_type#-0" <cfif isdefined('x_selected_process_id') and len(x_selected_process_id) and x_selected_process_id eq process_type>selected</cfif>>#get_process_name(process_type)#</option>										
									...
									<cfloop query="get_pro_cat">
										<option value="#get_pro_cat.process_type#-#get_pro_cat.process_cat_id#" <cfif is_selected_account eq 1 and is_account eq 0>style="color:red"</cfif> >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;#get_pro_cat.process_cat#</option>
									</cfloop>
								</cfoutput>
							</select>
						</div>
					...
					<div class="form-group" id="item-code1">
						...
						<div class="col col-9 col-md-9 col-sm-9 col-xs-12"> 
							<div class="input-group">
								<cfinput type="Text" name="code1" id="code1" onFocus="AutoComplete_Create('code1','ACCOUNT_CODE','ACCOUNT_CODE,ACCOUNT_NAME','get_account_code','\'1\',0','','','add_sum_bills','3','250');" autocomplete="off">
					...
					<div class="form-group" id="item-code2">
						...
						<div class="col col-9 col-md-9 col-sm-9 col-xs-12"> 
							<div class="input-group">
								<cfinput type="Text" name="code2" id="code2" onFocus="AutoComplete_Create('code2','ACCOUNT_CODE','ACCOUNT_CODE,ACCOUNT_NAME','get_account_code','\'1\',0','','','add_sum_bills','3','250');" autocomplete="off">
					...
					<div class="form-group" id="item-is_what_group">
						<div class="col col-12">
							<label><cf_get_lang dictionary_id='47432.Hesap Bazında Grupla'> <input type="checkbox" name="is_account_group" id="is_account_group" value="1"></label>
							<label><cf_get_lang dictionary_id='47525.Gün Bazında Grupla'> <input type="checkbox" name="is_day_group" id="is_day_group" value="1"></label>
						</div>							
					</div>
				</div>
			</cf_box_elements>	
			<cf_box_footer>	
				<cf_workcube_buttons type_format='1' is_upd='0' add_function='Save()'>
			</cf_box_footer>
		</cfform>
	</cf_box>
</div>
<script type="text/javascript">
	...
	function Save()
	{
		...
		{
			alert("<cf_get_lang dictionary_id='30551.İşlem Tipini Girmelisiniz'>!");
			return false;
		}
		else 
		return true;
