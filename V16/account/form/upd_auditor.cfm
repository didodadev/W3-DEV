		...
		<cfform name="add_wizard" id="add_wizard" method="post">
			...
			<div class="col col-12 col-md-12 col-sm-12 col-xs-12">
				<cf_box_footer>
					...
					<div class="col col-6 col-md-6 col-sm-6 col-xs-12">
						<cf_workcube_buttons is_upd='1' add_function="kontrol()" delete_page_url = "#request.self#?fuseaction=#attributes.fuseaction#&event=del&wizard_id=#attributes.wizard_id#">
					</div>
				</cf_box_footer>
			</div>
		</cfform>
    ...
    $( document ).ready(function() {
        ...
        $.each(getJSON, function( index, value ) {
            ...
            if(index == 0) { // İlk gelenden wizard alanlarını doldurucaz.
               ...
               // $("#card_process_cat").val(value.CARD_PROCESS_CAT);
            }
            ...
            if($.inArray(value.WIZARD_BLOCK_ID,processedBlocks) == -1) { // İlk gelenden blok alanlarını dolduruyoruz.
                ...
                addRow();
                $("#block_name_left_" + $("#rowCount").val()).val(value.BLOCK_NAME_LEFT);
                $("#block_name_right_" + $("#rowCount").val()).val(value.BLOCK_NAME_RIGHT);
                $("#detail_counter_block_type_" + $("#rowCount").val()).val(value.BLOCK_OPERATION);
                $("#detail_counter_rate_type_" + $("#rowCount").val()).val(value.BLOCK_RATE);
            }
            if($.inArray(value.WIZARD_BLOCK_ROW_ID,processedRows) == -1) { // İlk gelenden row alanlarını dolduruyoruz.
				...
				ba_input = $("#ba_" + $("#rowCount").val() + '_' + value.BLOCK_COLUMN + '_' + $("#counter_" + $("#rowCount").val() + "_" + value.BLOCK_COLUMN).val());
