...
<cftransaction>
	 ...
	 <cfquery name="upd_perf" datasource="#dsn#">
		...
		SET
			...
			USER_POINT = <cfif len(puan) and IS_PERF_SALARY_RESULTING eq 1>#puan#<cfelse>NULL</cfif>,<!--- form puanı hesaplanmıyorsa puanlr yazılmasın --->
...
<cf_workcube_process 
	...
	action_page='#request.self#?fuseaction=hr.form_add_perf_emp_info'
	warning_description = 'Ölçme Değerlendirme : #get_perform.max_per_id#'>
<cflocation addtoken="no" url="#request.self#?fuseaction=hr.form_upd_perf_emp&per_id=#get_perform.max_per_id#">
<!--- <cflocation addtoken="no" url="#request.self#?fuseaction=hr.form_add_perf_emp_info"> --->
