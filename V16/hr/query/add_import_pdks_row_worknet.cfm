	...
	<cfif kont eq 1>
			...
			<cfif get_in_out.recordcount>
				<cfquery name="get_daily_in" datasource="#dsn#">
					SELECT * FROM EMPLOYEE_DAILY_IN_OUT WHERE EMPLOYEE_ID = #get_in_out.employee_id# AND FINISH_DATE IS NULL AND (START_DATE BETWEEN #createodbcdatetime(baslama_gun)# AND #createodbcdatetime(DATEADD("d",1,baslama_gun))#) AND ISNULL(FROM_HOURLY_ADDFARE,0) = 0
				</cfquery>					
				...
				<cfoutput>#i#.satır kişi (#pdks_no#) kayıt yapıldı!</cfoutput><br/>
			<cfelse>
				<cfoutput>#i#.satır kişi (#pdks_no#)bulunamadı!</cfoutput><br />
...
<cfoutput>Bulunamayanlar Listesi : #listdeleteduplicates(bulunamayan_listesi)#</cfoutput><br />
İşlem Tamamlandı!
