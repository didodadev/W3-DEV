	...
	<cfif isdefined("attributes.COEFFICIENT") and isdefined("attributes.REQ_TYPE_ID")>
        ...
        <cfif LEN_TYPE_ID NEQ LEN_COEF>
			<script type="text/javascript">
				alert("<cf_get_lang dictionary_id ='56825.oran girmediniz'>!");
				history.back();
            </script>
            <cfabort>
        </cfif>
...
<cfif isdefined("attributes.REQ_TYPE_ID")>
    ...
    <cfif (control.recordcount) and isdefined("attributes.ilk") and (attributes.ilk eq 0) >
        <cfquery name="DEL_ALL" datasource="#DSN#">
        	DELETE FROM POSITION_REQUIREMENTS WHERE POSITION_ID = #attributes.POSITION_ID# 
        </cfquery>
    </cfif>
    <cfset COUNTER = 1>
	<cfloop list="#attributes.REQ_TYPE_ID#" index="i">
		<cfif isDefined("attributes.COEFFICIENT") and len(attributes.COEFFICIENT)>
        	<cfset COEFFICIENT_ = LISTGETAT(attributes.COEFFICIENT,COUNTER)>
        </cfif>
