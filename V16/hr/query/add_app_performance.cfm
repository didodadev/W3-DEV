...
<cftransaction>
	...
	<cfset puan = 0>
		...
		<cfinclude template="../query/get_quiz_questions.cfm">
			...
			<!--- </cfif> --->
	</cfloop> <cfoutput>puan#puan#</cfoutput>
	<!--- // kağıdı gönder --->
	...
	<cfquery name="add_perform" datasource="#dsn#">
	 INSERT INTO
		...
		<cfelse>
			...
			<cfif isdefined("attributes.other_position_code") and len(attributes.other_position_code)>#attributes.other_position_code#<cfelse>NULL</cfif>,
			<cfif isdefined("attributes.is_view") and len(attributes.is_view)>1<cfelse>0</cfif>
		)
	 </cfquery> 
</cftransaction>
</cflock>
