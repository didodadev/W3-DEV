...
//if (attributes.sal_year lt 2022 or (attributes.sal_year gte 2022 and wrk_round(salary_ilk) neq wrk_round(control_daily_minimum_wage)) or get_hr_ssk.SSK_STATUTE eq 2)
{
	...
	if(attributes.sal_year gte 2022 and use_ssk neq 2 and (daily_minimum_wage_base gt 0 or max_daily_minimum_income_tax gt 0) and use_minimum_wage neq 1 and get_active_program_parameter.is_use_minimum_wage neq 1)
	{
        if(daily_minimum_wage_base lte 0)
        {
            daily_minimum_wage_base = gelir_vergisi_matrah + odenek_oncesi_gelir_vergisi_matrah;
        }
		...
		if (all_basic_wage gt 0)
			tax_ratio_daily = (daily_minimum_income_tax / daily_minimum_wage_base);
		else
			tax_ratio_daily = 0; // AK20040806 30 gün ücretsiz izni var ise
        
        if(isdefined("tutar_") and is_from_odenek eq 1)
            writeoutput('daily_minimum_income_tax:#daily_minimum_income_tax# gelir_vergisi:#gelir_vergisi# all_basic_wage:#all_basic_wage# daily_minimum_wage_base:#daily_minimum_wage_base#<br>')
		//Eğer ilk defa giriyorsa max alabilceği gv indirim tutarı
		if(count_2022 eq 0)
		{
			max_daily_minimum_income_tax = wrk_round(temp_daily_minimum_wage_base * tax_ratio_daily);
		}

		if(max_daily_minimum_income_tax gt 0 and max_daily_minimum_income_tax lt daily_minimum_income_tax)
			daily_minimum_income_tax = max_daily_minimum_income_tax;

        if(max_daily_minimum_income_tax eq 0)
            daily_minimum_income_tax = 0;
        
        if(daily_minimum_income_tax gt gelir_vergisi)
            gelir_vergisi = 0;
		else if(LSParseNumber(gelir_vergisi) gte LSParseNumber(daily_minimum_income_tax))
		{
