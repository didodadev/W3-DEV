<cfscript>
	...
	ext_total_hours_8 = 0;  //  Fazla Mesai Hafta Tatil Gün (--Muzaffer--İmbat için eklendi)
	ext_total_hours_9 = 0;  //  Fazla Mesai Akdi Tatil Gün  (--Muzaffer--İmbat için eklendi)
	ext_total_hours_10 = 0;  // Fazla Mesai Resmi Tatil Gün (--Muzaffer--İmbat için eklendi)
	ext_total_hours_11 = 0;  // Fazla Mesai Arefe Tatil Gün (--Muzaffer--İmbat için eklendi)
	ext_total_hours_12 = 0;  // Fazla Mesai Dini Bayram Gün (--Muzaffer--İmbat için eklendi)
...
<cfscript>
		...
		get_emp_worktimes_days =  cfquery(datasource : "#dsn#", sqlstring : "SELECT DISTINCT DAY(START_DATE) AS GUN FROM EMPLOYEE_DAILY_IN_OUT WHERE EMPLOYEE_ID = #attributes.EMPLOYEE_ID# AND IN_OUT_ID = #get_hr_ssk.in_out_id# AND START_DATE >= #last_month_1# AND FINISH_DATE < #last_month_30# AND IS_WEEK_REST_DAY IS NULL AND ISNULL(FROM_HOURLY_ADDFARE,0) = 0");
...
// ssk işçi ve işveren yüzdeler toplanır
	if (get_hr_ssk.use_ssk eq 3) /// yabancı veya ssk kullanmıyor
		...
		else if (get_hr_ssk.SSK_STATUTE eq 2 or get_hr_ssk.SSK_STATUTE eq 18) // emekli değilse işsizlik hissesi çarpanı
			{
		...
		if (get_hr_ssk.SSK_STATUTE eq 1 or get_hr_ssk.SSK_STATUTE eq 6) /// normal
			{
		...
		if (get_hr_ssk.SSK_STATUTE neq 2 and get_hr_ssk.SSK_STATUTE neq 18) // emekli değilse işsizlik hissesi çarpanı  Muzaffer İmbat Yeraltı emekli
			{
			issizlik_isci_carpan = 0;
			issizlik_isveren_carpan = 0;
...
else if(get_active_program_parameter.extra_time_style eq 2)
{
	...
	if(get_hr_salary.salary_type eq 1 and ssk_days eq aydaki_gun_sayisi)
		...
		{
		ssk_matrah_taban = (get_insurance.MIN_PAYMENT * ssk_days) / ssk_full_days;
