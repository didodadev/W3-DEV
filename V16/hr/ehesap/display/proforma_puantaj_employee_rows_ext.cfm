...
<cfquery name="get_salary_pay" datasource="#dsn#">
	SELECT
		...
		IN_OUT_ID,
		FROM_SALARY,
		ISNULL((SELECT SL.ITEM FROM SETUP_LANGUAGE_INFO AS SL WHERE SL.UNIQUE_COLUMN_ID= SP.COMMENT_PAY_ID AND SL.TABLE_NAME= 'SETUP_PAYMENT_INTERRUPTION' AND SL.LANGUAGE='#ucase(session.ep.language)#'),COMMENT_PAY) AS COMMENT_PAY_NAME
	FROM
		SALARYPARAM_PAY SP
	WHERE
...
<cfquery name="get_names" dbtype="query">
	SELECT DISTINCT COMMENT_PAY_ID,COMMENT_PAY,FROM_SALARY,COMMENT_PAY_NAME FROM get_salary_pay
</cfquery>

<cfoutput query="get_salary_pay">
	<cfif isdefined("ek_odenek_#COMMENT_PAY_ID#_#IN_OUT_ID#")>
		<cfset "ek_odenek_#COMMENT_PAY_ID#_#IN_OUT_ID#" = evaluate("ek_odenek_#COMMENT_PAY_ID#_#IN_OUT_ID#") + AMOUNT_PAY>
	<cfelse>
		<cfset "ek_odenek_#COMMENT_PAY_ID#_#IN_OUT_ID#" = AMOUNT_PAY>
	</cfif>
	
	<cfif isdefined("total_ek_odenek_#COMMENT_PAY_ID#")>
		<cfset "total_ek_odenek_#COMMENT_PAY_ID#" = evaluate("total_ek_odenek_#COMMENT_PAY_ID#") + AMOUNT_PAY>
	<cfelse>
		<cfset "total_ek_odenek_#COMMENT_PAY_ID#" = AMOUNT_PAY>
	</cfif>
</cfoutput>
...
<cf_ajax_list>
	<thead>
        <tr>
			...
			<cfoutput query="get_names">
				<th style="text-align:right;">#COMMENT_PAY_NAME# (<cfif FROM_SALARY eq 0><cf_get_lang dictionary_id='56257.Brüt'><cfelse><cf_get_lang dictionary_id='58083.Net'></cfif>)</th>
			</cfoutput>
		</tr>
	</thead>
	<tbody>
	...
	<cfoutput query="get_emps">
		...
		<tr class="color-row">
			...
			<cfloop query="get_names">
				<td style="text-align:right;"><cfif isdefined("ek_odenek_#COMMENT_PAY_ID#_#in_out_id_#")>#evaluate("ek_odenek_#COMMENT_PAY_ID#_#in_out_id_#")#<cfelse>-</cfif></td>
			</cfloop>
		</tr>
	</cfoutput>
	</tbody>
	<tfoot>
		<tr>
			...
			<cfoutput query="get_names">
				<th style="text-align:right;"><cfif isdefined("total_ek_odenek_#COMMENT_PAY_ID#")>#evaluate("total_ek_odenek_#COMMENT_PAY_ID#")#<cfelse>-</cfif></th>
			</cfoutput>
		</tr>
	</tfoot>
</cf_ajax_list>