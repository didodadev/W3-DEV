...
<cfif not isdefined('attributes.uploaded_file')>
	<div class="col col-12 col-md-12 col-sm-12 col-xs-12">
		<cf_box title="#getLang('','Kümülatif Vergi Matrahı Aktarım','42022')#">
			<cfform name="formimport" action="#request.self#?fuseaction=ehesap.import_cumulative_tax_total" enctype="multipart/form-data" method="post">
				<cf_box_elements>
					<div class="col col-4 col-md-4 col-sm-4 col-xs-12" type="column" index="1" sort="true">
						...
						<div class="form-group" id="item-example_file">
							...
							<div class="col col-6 col-md-8 col-sm-8 col-xs-12">
								<a href="/IEF/standarts/import_example_file/cumulative_tax_total.csv"><strong><cf_get_lang dictionary_id='43675.İndir'></strong></a>
							</div>
						</div>  						
					</div> 	
					<div class="col col-8 col-md-8 col-sm-8 col-xs-12" type="column" index="2" sort="true">		
						...
						<div class="form-group" id="item-exp4">
							<cf_get_lang dictionary_id ="59608.Kümüle vergi matrahı ve Kümüle vergi tutarı ondalık şekilde yazılacaksa (.) kullanılmalıdır, (,) kullanılmamalıdır.">
						</div>
						<div class="form-group" id="item-exp5">
							<cf_get_lang dictionary_id ="59609.Bordro Alanı 1/0 şeklinde girilmelidir.">
						</div>
						<div class="form-group" id="item-exp6">
							<cf_get_lang dictionary_id ="35596.Belgede toplam 7 sutun olacaktır alanlar sırasi ile">;
						</div>
						<div class="form-group" id="item-exp7">
							...
							5-<cf_get_lang dictionary_id ="59610.Kümüle Vergi Matrahı : 192.45 formatında olmalıdır."></br>
							6-<cf_get_lang dictionary_id ="59611.Kümüle Vergi Tutarı : 11.3 formatında olmalıdır."></br>
							7-<cf_get_lang dictionary_id ="55712.Bordro">*</br>
						</div>
					</div>
				</cf_box_elements>
				<cf_box_footer>
...
<cfelse>
	...
	<cfloop from="2" to="#line_count#" index="i">
		...
		<cftry>
			...
			kumule_vergi_matrahi = Listgetat(dosya[i],j,";");
			kumule_vergi_matrahi =trim(kumule_vergi_matrahi);
			j=j+1;
			//Kümüle Vergi Tutarı
			kumule_vergi_tutari = Listgetat(dosya[i],j,";");
			kumule_vergi_tutari =trim(kumule_vergi_tutari);
			j=j+1;
			//Bordro bilgisi
			bordro= Listgetat(dosya[i],j,";");
			bordro =trim(bordro);
			j=j+1;
			</cfscript>
 		...
 		<cfif len(tckimlik_no) and len(kumule_vergi_matrahi) and len(kumule_vergi_tutari) and len(bordro) and error_flag neq 1>
					...
					<cftry>
						<cfquery datasource="#dsn#" name="UPD_CUMULATIVE_TAX">
							UPDATE
								EMPLOYEES_IN_OUT
							SET 
								CUMULATIVE_TAX_TOTAL = #kumule_vergi_matrahi#,
								START_CUMULATIVE_TAX = #kumule_vergi_tutari#,
								IS_START_CUMULATIVE_TAX = #bordro#
							WHERE 
		...
		<cfelse>
			<cfset liste=ListAppend(liste,i&'. satırın tc kimlik numarası,kümüle gelir vergisi matrahı,kümüle gelir vergisi tutarı veya bordro alanlarından herhangi biri eksiktir <br/>',',')>	
		</cfif>
	</cfloop>
	...
	<script type="text/javascript">
		window.location.href='<cfoutput>#request.self#</cfoutput>?fuseaction=ehesap.import_cumulative_tax_total';
	</script><abort>
</cfif>
