...
// ssk matrahı bulunur
	...
	else if((gelen_salary+kazanc_tipli_odenek_tutar) gt SSK_MATRAH)
	{
		ssk_matrah_bu_ay_devreden = 0;
	}
...
// gelir vergisi matrahı hesaplanır
		...
		if(use_ssk neq 2 or (use_ssk eq 2 and (attributes.statue_type eq 6 or attributes.statue_type eq 7 or attributes.statue_type eq 9 or attributes.statue_type eq 10)))
		{
			if(get_hr_ssk.IS_TAX_FREE eq 1 and (get_hr_salary.gross_net eq 1 or (get_hr_salary.gross_net eq 0 and is_included_in_tax eq 0)))
			{
				...
				else
				{
					...
					if(isdefined("maas_ssk_isci_hissesi_") and isDefined("ext_salary_isci"))
					{
						gelir_vergisi_matrah = gelir_vergisi_matrah + maas_ssk_isci_hissesi_ + maas_ssk_isci_issizlik_ + ext_salary_isci + ext_salary_isci_issizlik;
					}
	...
	if(use_ssk neq 2 or (use_ssk eq 2 and (attributes.statue_type eq 6 or attributes.statue_type eq 7 or attributes.statue_type eq 9 or attributes.statue_type eq 10)))
	{
		damga_vergisi_matrah = salary-temp_vergi_istisna_damga_tutar_hs + vergi_istisna_damga_tutar - (damga_matraha_dahil_olmayan_kesinti_tutar - ozel_kesinti_damga_dahil_olmayan_tutar) - total_pay_unstamped - ext_salary_gross - total_pay_ssk_tax_notstamp_base;
	...
	if(get_hr_ssk.is_damga_free eq 1 and is_included_in_tax eq 0 and is_included_in_tax eq 0 and is_yearly_offtime eq 0 and included_in_tax_paid_amount_brut eq 0 and total_pay_ssk_tax eq 0 and total_pay_ssk_tax_net eq 0)
	{
		...
		if(get_hr_salary.gross_net eq 0 or (get_hr_salary.gross_net eq 1 and included_in_tax_hour_paid eq 0))
			daily_minimum_wage_stamp_tax = 0;
	}	
	if((get_hr_ssk.SSK_STATUTE eq 4 or get_hr_ssk.SSK_STATUTE eq 75) and is_damga_zero eq 1)
	{
	...
	*/
	if(not isdefined("is_devir_matrah_off") or is_devir_matrah_off eq 0)
	{
		...
		if(get_hr_salary.salary_type eq 1 and work_days eq 28 and izin eq 0 and aydaki_gun_sayisi eq 28)
		{
			//work_days = work_days + 2;
		}
		else if(get_hr_salary.salary_type eq 1 and aydaki_gun_sayisi eq 31 and ssk_days gt 30)
		{
			//work_days = work_days - 1;
		}
		else if(get_hr_salary.salary_type eq 1 and work_days eq 28 and izin eq 1)
		{
			work_days = work_days + 1;
		}
		else if(get_hr_salary.salary_type eq 1 and work_days eq 29 and izin eq 0 and aydaki_gun_sayisi eq 29)
		{
			//work_days = work_days + 1;
		}
