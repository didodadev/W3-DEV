<cfscript>
	...
	{
		for (i=1; i lte get_tax_exception_osi.recordcount; i = i+1) 
		{
			istisna_sifirlandi = 0;
			toplam_vergi_istisna_siniri = (vergi_brut_toplam_tutari / 100) * 15; //toplam_vergi_istisna_siniri = (vergi_brut_toplam_tutari / 100) * 10
			
			vergi_istisna_ssk_tutar_ = 0;
			satir_vergi_istisna_tutari = 0;
			satir_vergi_istisna_tutari = get_tax_exception_osi.amount[i];
			satir_vergi_istisna_tutari_ilk = get_tax_exception_osi.amount[i];
			...
			satir_vergi_istisna_siniri = (vergi_brut_toplam_tutari / 100) * 15; // sinir otomatik %5 // satir_vergi_istisna_siniri = (vergi_brut_toplam_tutari / 100) * 5; ****SG 20130130 ozel saglik sigortasi oranı degisiklik
				
			if(satir_vergi_istisna_tutari gt satir_vergi_istisna_siniri) 
				satir_vergi_istisna_tutari = satir_vergi_istisna_siniri;
				
			if(toplam_vergi_istisna_siniri lt vergi_istisna_osi)
				satir_vergi_istisna_tutari = 0;
			else if(toplam_vergi_istisna_siniri lt (vergi_istisna_osi  + satir_vergi_istisna_tutari))
				satir_vergi_istisna_tutari = toplam_vergi_istisna_siniri - (vergi_istisna_osi);
				
			if(old_except lt yillik_toplam_asgari_ucret)
				{
				if(old_except gt 0)
					{
					if((old_except + vergi_istisna_osi + satir_vergi_istisna_tutari) gt yillik_toplam_asgari_ucret)
						{
						satir_vergi_istisna_tutari = yillik_toplam_asgari_ucret - (old_except + vergi_istisna_osi);
			...
			{
				is_net_payment = 1;
				net_payment_temp = ilk_salary+this_vergi_istisna_vergi_tutar;
				net_total_brut = 0;
				gelir_vergisi_matrah = salary;
				include 'get_hr_compass_2022.cfm';
				include 'get_hr_compass_tax.cfm';
				...
				oran_damga_duzeltme = get_active_program_parameter.STAMP_TAX_BINDE/(1-tax_carpan);
				vergi_istisna_vergi_tutar_ = wrk_round(this_vergi_istisna_vergi_tutar/(1-tax_carpan));
			...
			puantaj_exts[puantaj_exts_index][1] = get_tax_exception_osi.TAX_EXCEPTION[i];
			...
			puantaj_exts[puantaj_exts_index][7] = get_tax_exception_osi.calc_days[i];
			...
			puantaj_exts[puantaj_exts_index][11] = get_tax_exception.yuzde_sinir[i]; //5;   SG 20130130 sınır degistirildi
			...
			puantaj_exts[puantaj_exts_index][30] = get_tax_exception_osi.amount[i];
			vergi_istisna_osi = vergi_istisna_osi + satir_vergi_istisna_tutari;
		}
	}
	vergi_istisna_osi = vergi_istisna_osi;
</cfscript>
