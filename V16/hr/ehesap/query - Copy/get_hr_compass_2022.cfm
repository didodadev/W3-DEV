...
<cfscript>
	if(use_minimum_wage neq 1 and get_active_program_parameter.is_use_minimum_wage neq 1 and year(last_month_1) gte 2022)
	{
		if(first_salary_temp lt temp_daily_minimum_wage or is_included_in_tax_hour_paid eq 1 or (get_hr_ssk.IS_DAMGA_FREE eq 1 and total_used_stamp_tax gte temp_stamp_tax_base))
		{
			...
			if((total_used_incoming_tax gte temp_gelir_vergisi_matrah and tax_carpan eq v1) or (total_used_incoming_tax gte temp_gelir_vergisi_matrah and tax_carpan gt v1 and gelir_vergisi eq 0))
			{
				total_used_incoming_tax = (total_used_incoming_tax) - (temp_gelir_vergisi_matrah);
				if(max_daily_minimum_income_tax gt income_tax_temp){
					max_daily_minimum_income_tax = max_daily_minimum_income_tax - income_tax_temp;
				}
			}
			else{
			...
			{
				daily_minimum_wage_base = total_used_incoming_tax - sakatlik_indirimi;
				first_exemption = 1;
			}
