...
<div class="col col-12 col-md-12 col-sm-12 col-xs-12">
	<cf_box>
		<cfform name="add_cheque_entry" method="post" action="#request.self#?fuseaction=finance.emptypopup_add_cheque_payment_exp" onsubmit="return (unformat_fields());">
			<cfoutput query="GET_MONEY">
				<cfif money eq session.ep.money2>
					<cfinput name="money_rate2" type="hidden" value="#rate2#" class="moneybox">
				</cfif>
			</cfoutput>
			<div class="row"><label class="pull-left bold"><cfoutput>#getLang('finance',226)#</cfoutput></label><label class="pull-right bold"><cfoutput>#right#</cfoutput></label></div>
			<cf_grid_list>
				<thead>
					<tr>
				...
				<cfloop from="1" to="15" index="i">
					<tbody>
						<tr>
							...
							<td>
								<div class="form-group"><cfinput type="text" name="CHEQUE_NO#i#" onkeyup="isNumber(this);" onblur='isNumber(this);'  message="#getLang('','çek no giriniz!',54614)#"></div>
							</td>
							<td>
								<div class="form-group"><cfinput type="text" name="CHEQUE_VALUE#i#" class="moneybox" onBlur="hesapla(#i#);" onkeyup="return(FormatCurrency(this,event));" message="#getLang('','tutar girmelisiniz',29535)#"></div>
							</td>
							<td>
								<div class="form-group">
							...
							<td nowrap="nowrap">
								<div class="form-group"><cfinput type="text" name="SYSTEM_CHEQUE_VALUE#i#" class="moneybox" onkeyup="return(FormatCurrency(this,event));" message="#getLang('','tutar girmelisiniz!',29535)#"></div>
							</td>
...
function kontrol()
{
	for(var i=1; i<=15; i++)
	{
		if(eval('add_cheque_entry.CHEQUE_NO'+i).value != "")
		{
			...
			{
				alert('<cfoutput>#getLang('ch',108)#</cfoutput> <cfoutput>#getLang('main',1096)#</cfoutput>' +i+'');
				return false;
			}
