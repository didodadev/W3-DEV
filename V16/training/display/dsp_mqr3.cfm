<cfif isdefined("get_quiz_question2") and ((not get_quiz_question2.recordcount) or (question_limit EQ attributes.question_row))>
...
<form method="post" name="make_quiz" id="make_quiz">
  ...
  <cfif isDefined("attributes.view_answer")>
    ...
    <table width="98%" align="center">
      ...
      <cfif (not get_quiz_question2.recordcount) or (ListLen(session.random_list) EQ attributes.question_row)>
		...
		<cfsavecontent variable="message"><cf_get_lang no='1.Sınavı Bitir'></cfsavecontent>
            <cf_workcube_buttons is_upd='0' insert_info='#message#'> <!--- add_function='#xfa.submit_quiz#'---> 
        </tr>
        <cfelse>
        <tr>
          <td width="600"><br/>
    ...
    <cfoutput query="get_quiz_question2">
      ...
      <table width="98%" align="center" class="txtbold" >
        <tr>
          <td><strong><cf_get_lang_main no='1398.Soru'> #attributes.question_row#:</strong> #question# <br/>
            <br/>
          </td>
        </tr>
        <cfif ANSWER_NUMBER NEQ 0>
          ...
          <input type="Hidden" name="open_question" id="open_question" value="1">
          <tr>
            <td><textarea name="user_answer_#attributes.question_row#" id="user_answer_#attributes.question_row#" cols="65" rows="4"></textarea>
            </td>
          </tr>
        </cfif>
