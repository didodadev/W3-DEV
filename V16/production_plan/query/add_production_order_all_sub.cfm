...
<cfif not isdefined("is_import_prod")>
	<table border="1">
	<tr>
		<td><cfif attributes.is_demand eq 1><cf_get_lang dictionary_id='57527.Talepler'><cfelse><cf_get_lang dictionary_id='51318.Üretim Emirleri'></cfif> <cf_get_lang dictionary_id='650.Oluşturuluyor..Lütfen Bekleyiniz...'></td>
	</tr>
...
<cfelse>
	...
	<cfquery name="#Main_Table#" datasource="#DSN3#">
		...
		WHERE 
			...
			<cfif not isdefined("attributes.is_listorder")>AND ORDER_ROW.SPECT_VAR_ID IS NOT NULL</cfif>
	</cfquery>
</cfif>
<cfscript>
	deep_level = 0;
	...
	<cfloop query="GET_AMOUNT">
		...
		<cfif isdefined("_spec_main_id_") and len(_spec_main_id_)>
			<cfloop from="1" to="#loop_count#" index="ii_indx">
				...
				<cfset attributes.from_all_sub=1>
...
<script type="text/javascript">
	<cfoutput>	window.location.href ="#request.self#?fuseaction=prod.order&is_submitted=1";</cfoutput>
</script>
<cfif not isdefined("is_import_prod")>
</table>
</cfif>
