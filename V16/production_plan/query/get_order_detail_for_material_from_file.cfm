...
<cfquery name="get_metarials" datasource="#dsn3#">
	SELECT
		...
		,ISNULL((SELECT ISNULL(REVIZE_PRICE, PRICE) FROM TEXTILE_SR_SUPLIERS WHERE REQ_ID = #attributes.req_id# AND STOCK_ID = S.STOCK_ID AND PRODUCT_ID = S.PRODUCT_ID), 0) AS LIST_PRICE
		,ISNULL((SELECT MONEY_TYPE FROM TEXTILE_SR_SUPLIERS WHERE REQ_ID = #attributes.req_id# AND STOCK_ID = S.STOCK_ID AND PRODUCT_ID = S.PRODUCT_ID), 0) AS MONEY_TYPE
	</cfif>
    ...
    FROM
		...
		PRODUCT_UNIT PU
	WHERE
		S.STOCK_ID = PMR.STOCK_ID 
        AND S.PRODUCT_UNIT_ID = PU.PRODUCT_UNIT_ID 
		AND PMR.RECORD_EMP = #session.ep.userid#
...
<cfquery name="get_metarials" datasource="#dsn3#">
SELECT * FROM ####get_metarials_get_order_#session.ep.userid#
</cfquery>
