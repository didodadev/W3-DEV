...
<cfif isdefined("attributes.expiration_date") and len(attributes.expiration_date)><cf_date tarih='attributes.expiration_date'></cfif>
<cfif (isdefined("attributes.is_delstock") and Len(attributes.is_delstock))>
	<cfif isdefined("session.ep.period_start_date")>
    	<cfset new_finish_date = createdate(year(attributes.finish_date),month(attributes.finish_date),day(attributes.finish_date))>
        <cfset date1 =  session.ep.period_start_date>
        <cfset date2 =  session.ep.period_finish_date>
		<cfif not len(new_finish_date) or (new_finish_date lt date1 or new_finish_date gt date2)>
    ...
    <cfelse>
        <cfif not len(attributes.finish_date) and year(attributes.finish_date) neq session_period_year>
            <script type="text/javascript">
                alert("<cf_get_lang no ='634.Üretim Sonuç Bitiş Tarihi Döneminiz ile Aynı Değil Stok Fişlerini Silemezsiniz'>!");
                history.go(-1);
            </script>
...
<cfif not form.del_pr_order_id eq 0>
    ...
    <cfquery name="UPD_PROD_ORDERS" datasource="#DSN3#">
        UPDATE PRODUCTION_ORDERS SET IS_GROUP_LOT = 0 WHERE P_ORDER_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#form.p_order_id#">
    </cfquery>
	<cfif session_our_company_info_is_cost eq 1><!--- sirket maliyet takip ediliyorsa not js le yonlenioyr cunku cost_action locationda calismiyor --->
		...
		<script type="text/javascript">
			<cfif isDefined("session.ep")><!--- Pda de Gelmemesi Icin --->
				window.location.href="<cfoutput>#request.self#?fuseaction=prod.order&event=upd&upd=#attributes.p_order_id#</cfoutput>";
			<cfelse>
...
<cfelse>
	 ...
	 <cftransaction>
		...
		<cfif len(attributes.station_id) and len(attributes.station_name)>
			<cfquery name="GET_WORKSTATION_DETAIL" datasource="#DSN3#">
				SELECT AVG_COST FROM WORKSTATIONS WHERE STATION_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.station_id#">
			</cfquery>
		...
		<cfquery name="ADD_PRODUCTION_ORDER" datasource="#DSN3#">
			...
			SET
				...
				EXPIRATION_DATE = <cfif len(attributes.expiration_date)>#attributes.expiration_date#<cfelse>NULL</cfif>
			WHERE
        ...
        <cfquery name="UPD_PRODUCTION_ORDERS" datasource="#DSN3#">
        	UPDATE PRODUCTION_ORDERS SET REFERENCE_NO = <cfif len(attributes.ref_no)>'#attributes.ref_no#'<cfelse>NULL</cfif> WHERE P_ORDER_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.p_order_id#">
        </cfquery>
		<cfquery name="GET_STOCK_FIS_KONTROL" datasource="#DSN3#">
			SELECT ISNULL(IS_STOCK_FIS,0) IS_STOCK_FIS FROM PRODUCTION_ORDER_RESULTS WHERE PR_ORDER_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.pr_order_id#">
		</cfquery>
        <cfif attributes.lot_no neq attributes.old_lot_no><!--- Eğer Lot no değişmiş ise üretim emrindeki lot numarasınıda değiştiriyoruz! --->
        	<cfquery name="UPD_PRO_ORDER_LOT_NUMBER" datasource="#DSN3#">
	        	UPDATE PRODUCTION_ORDERS SET  LOT_NO = <cfif len(attributes.lot_no)>'#attributes.lot_no#'<cfelse>NULL</cfif>  WHERE P_ORDER_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.pr_order_id#">
            </cfquery>
		</cfif>
		<cfif isdefined("attributes.is_delstock") and Len(attributes.is_delstock)>
		...
		<cfquery name="GET_FIS" datasource="#DSN3#">
			...
			WHERE
				PROD_ORDER_RESULT_NUMBER = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.pr_order_id#">
		</cfquery>
		<cfif isdefined("attributes.is_delstock") and Len(attributes.is_delstock)>
			<cfquery name="UPD_PROD_ORDERS" datasource="#DSN3#">
                UPDATE PRODUCTION_ORDERS SET IS_STAGE = 1 WHERE P_ORDER_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.p_order_id#"><!--- oluşmuş olan stok fişlerini siliyoruz yani durumu 2 den 1e yani başladı konumuna getiriyoruz. --->
			</cfquery>
			<cfloop query="get_fis">
				<cfquery name="DEL_MONEY" datasource="#DSN3#">
					...
					WHERE
						ACTION_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#get_fis.fis_id#">
				</cfquery>
				<cfquery name="DEL_ROW" datasource="#DSN3#">
					DELETE FROM
						#dsn2_alias#.STOCK_FIS_ROW 
					WHERE
						FIS_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#get_fis.fis_id#">
				</cfquery>
				<cfquery name="DEL_ROW_STOCK" datasource="#DSN3#">
					DELETE FROM
						#dsn2_alias#.STOCKS_ROW
					WHERE
						UPD_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#get_fis.fis_id#">
						AND PROCESS_TYPE = <cfqueryparam cfsqltype="cf_sql_integer" value="#get_fis.fis_type#">
				</cfquery>
                 <cfscript>
						del_serial_no(
				...
				<cfloop query="get_ship">
					<cfquery name="DEL_SHIP_ROW" datasource="#DSN3#">
						DELETE FROM
							#dsn2_alias#.SHIP_ROW
						WHERE
							SHIP_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#get_ship.ship_id#">
					</cfquery>
					<cfquery name="DEL_STOCK_ROW" datasource="#DSN3#">
						DELETE FROM
							#dsn2_alias#.STOCKS_ROW
						WHERE
							UPD_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#get_ship.ship_id#">
							AND PROCESS_TYPE = <cfqueryparam cfsqltype="cf_sql_integer" value="#get_ship.ship_type#">
					</cfquery>
                    <cfscript>
						del_serial_no(
				...
				<cfquery name="GET_P_O_R" datasource="#DSN3#">
					...
					WHERE
						...
						PRODUCTION_ORDER_RESULTS.P_ORDER_ID= <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.p_order_id#"> AND
			...
			<cfif len(attributes.record_num_exit) and attributes.record_num_exit neq "">
				<cfloop from="1" to="#attributes.record_num_exit#" index="i">
					<cfif evaluate("attributes.row_kontrol_exit#i#")>
						<cfif len(evaluate("attributes.expiration_date_exit#i#"))>
							<cfset form_expiration_date_exit = evaluate("attributes.expiration_date_exit#i#")>
						...
						<cfscript>
							...
							if(isdefined('attributes.labor_cost_exit#i#'))
								form_labor_cost_exit=evaluate("attributes.labor_cost_exit#i#");
							else
								form_labor_cost_exit ='';
							if(isdefined('attributes.reflection_cost_exit#i#'))
								form_reflection_cost_exit=evaluate("attributes.reflection_cost_exit#i#");
							else
								form_reflection_cost_exit ='';
							...
							if (isdefined("attributes.is_manual_cost_exit#i#") and evaluate("attributes.is_manual_cost_exit#i#") eq 1)
								form_is_manual_cost_exit = evaluate("attributes.is_manual_cost_exit#i#");
							else
                            ...
                            else
                            {
                                ...
                                else if(isDefined('session.pda.userid'))	
                                    form_wrk_row_id_exit='WRK#round(rand()*65)##dateformat(now(),'YYYYMMDD')##timeformat(now(),'HHmmssL')##session.pda.userid##round(rand()*100)##attributes.pr_order_id##form_stock_id_exit#_2';
                            }
							if(isdefined("attributes.wrk_row_relation_id_exit_#i#"))
								form_wrk_row_relation_id_exit = evaluate("attributes.wrk_row_relation_id_exit_#i#");
							else
								form_wrk_row_relation_id_exit = "";
							...
							if(isdefined("attributes.line_number_exit_#i#"))
								form_line_number_exit = evaluate("attributes.line_number_exit_#i#");
							else
                            ...
                            if(attributes.record_num eq 1 and attributes.is_changed_spec_main eq 1)//sarf kısmında 1 tane ürün varsa vede xml ayarlarından sarf değişikliklerinden spec seçilmiş denilmiş ise.
                            {
								...
								if(isdefined("attributes.width_exit#i#"))
								width_exit = evaluate("attributes.width_exit#i#");
								else
								width_exit = '';
								if(isdefined("attributes.height_exit#i#"))
								height_exit = evaluate("attributes.height_exit#i#");
								else
								height_exit = '';
								if(isdefined("attributes.length_exit#i#"))
								length_exit = evaluate("attributes.length_exit#i#");
								else
								length_exit = '';
								if(isdefined("attributes.specific_weight_exit#i#"))
								specific_weight_exit = evaluate("attributes.specific_weight_exit#i#");
								else
								specific_weight_exit = '';
								if(isdefined("attributes.weight_exit#i#"))
								weight_exit = evaluate("attributes.weight_exit#i#");
								else
								weight_exit = '';
                        </cfscript>
                        <cfquery name="ADD_ROW_ENTER" datasource="#DSN3#">
                            INSERT INTO
                                ...
                                (
                                    ...
                                    <cfif len(form_amount_exit2)>#form_amount_exit2#<cfelse>NULL</cfif>,
                                    ...
                                    <cfif len(form_wrk_row_relation_id_exit)>'#form_wrk_row_relation_id_exit#'<cfelse>NULL</cfif>,
                                    <cfif len(form_line_number_exit)>#form_line_number_exit#<cfelse>0</cfif> ,
                                    #form_is_manual_cost_exit#,
									<cfif len(form_expiration_date_exit)>#form_expiration_date_exit#<cfelse>NULL</cfif>,
									<cfif isdefined("form_labor_cost_exit") and len(form_labor_cost_exit)>#form_labor_cost_exit#<cfelse>0</cfif>,
									<cfif isdefined("form_reflection_cost_exit") and len(form_reflection_cost_exit)>#form_reflection_cost_exit#<cfelse>0</cfif>,
									<cfif isdefined("width_exit") and len(width_exit)>#width_exit#<cfelse>0</cfif>,
									<cfif isdefined("height_exit") and len(height_exit)>#height_exit#<cfelse>0</cfif>,
									<cfif isdefined("length_exit") and len(length_exit)>#length_exit#<cfelse>0</cfif>,
									<cfif isdefined("specific_weight_exit") and len(specific_weight_exit)><cfqueryparam cfsqltype="cf_sql_float" value="#specific_weight_exit#"><cfelse>0</cfif>,
									<cfif len(weight_exit)><cfqueryparam cfsqltype="cf_sql_float" value="#weight_exit#"><cfelse>0</cfif>
                                )				
                        </cfquery>
						<cfscript>
			   	...
			   	<cfif attributes.record_num eq 1 and attributes.is_changed_spec_main eq 1>
					...
					<cfelseif len(attributes.spect_id1)>
                        <cfquery name="SPECT_CHANGE_CONTROL" datasource="#DSN3#">
                            SELECT SPECT_MAIN_ID FROM SPECTS WHERE SPECT_VAR_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.spect_id1#">
                        </cfquery>
                    </cfif>
				...
				<cfloop from="1" to="#attributes.record_num_outage#" index="i">
					<cfif evaluate("attributes.row_kontrol_outage#i#")>
						...
						<cfscript>
							...
							if(isdefined('attributes.labor_cost_outage#i#'))
								form_labor_cost_outage=evaluate("attributes.labor_cost_outage#i#");
							else
								form_labor_cost_outage ='';
							if(isdefined('attributes.reflection_cost_outage#i#'))
								form_reflection_cost_outage=evaluate("attributes.reflection_cost_outage#i#");
							else
								form_reflection_cost_outage ='';
							...
							else
							{
								if(isDefined('session.ep.userid'))
									...
									form_wrk_row_id_outage='WRK#round(rand()*65)##dateformat(now(),'YYYYMMDD')##timeformat(now(),'HHmmssL')##session.pda.userid##round(rand()*100)##attributes.pr_order_id##form_stock_id_outage#_3';
							}
							if(isdefined("attributes.wrk_row_relation_id_outage_#i#"))
								form_wrk_row_relation_id_outage = evaluate("attributes.wrk_row_relation_id_outage_#i#");
							else
								form_wrk_row_relation_id_outage = "";
						...
						<cfquery name="ADD_ROW_ENTER" datasource="#DSN3#">
							INSERT INTO
								...
								(
									...
									<cfif len(form_amount_outage2)>#form_amount_outage2#<cfelse>NULL</cfif>,
                                    ...
                                    <cfif len(form_wrk_row_relation_id_outage)>'#form_wrk_row_relation_id_outage#'<cfelse>NULL</cfif>,
                                    <cfif len(form_line_number_outage)>#form_line_number_outage#<cfelse>0</cfif>,
                                    #form_is_manual_cost_outage#,
									<cfif len(form_expiration_date_outage)>#form_expiration_date_outage#<cfelse>NULL</cfif>,
									<cfif isdefined("form_labor_cost_outage") and len(form_labor_cost_outage)>#form_labor_cost_outage#<cfelse>0</cfif>,
									<cfif isdefined("form_reflection_cost_outage") and len(form_reflection_cost_outage)>#form_reflection_cost_outage#<cfelse>0</cfif>,
									<cfif isdefined("width_outage") and len(width_outage)>#width_outage#<cfelse>0</cfif>,
									<cfif isdefined("height_outage") and len(height_outage)>#height_outage#<cfelse>0</cfif>,
									<cfif isdefined("length_outage") and len(length_outage)>#length_outage#<cfelse>0</cfif>,
									<cfif isdefined("specific_weight_outage") and len(specific_weight_outage)><cfqueryparam cfsqltype="cf_sql_float" value="#specific_weight_outage#"><cfelse>0</cfif>,
									<cfif len(weight_outage)><cfqueryparam cfsqltype="cf_sql_float" value="#weight_outage#"><cfelse>0</cfif>
								)				
						</cfquery>
						<cfscript>
						...
						<cfscript>
							...
							else
							{
								if(isDefined('session.ep.userid'))
									...
									form_wrk_row_id='WRK#round(rand()*65)##dateformat(now(),'YYYYMMDD')##timeformat(now(),'HHmmssL')##session.pda.userid##round(rand()*100)##attributes.pr_order_id##form_stock_id#_1';

							}
							if(isdefined("attributes.wrk_row_relation_id_#i#"))
								form_wrk_row_relation_id = evaluate("attributes.wrk_row_relation_id_#i#");
							else
								form_wrk_row_relation_id = "";
							if(isdefined("attributes.cost_price_2#i#"))
							{
							...
							if(not (isdefined("attributes.is_free_amount_#i#") and len(evaluate("attributes.is_free_amount_#i#")) and evaluate("attributes.is_free_amount_#i#")) and form_stock_id eq get_prod_order.stock_id)
							{
								if(record_num_ eq 1)
								{
						...
						<cfquery name="ADD_ROW_ENTER" datasource="#DSN3#">
							...
							INTO
								...
								(
									...
									SPECIFIC_WEIGHT,
									WEIGHT
									<cfif len(work_id) and len(work_head)>
									,WORK_ID
									,WORK_HEAD
									</cfif>
								)
								...
								(
									...
									<cfif len(form_amount2) and len(form_amount2)>#form_amount2#<cfelse>NULL</cfif>,
									...
									<cfif isdefined("deger_value_total_price") and len(deger_value_total_price)>#deger_value_total_price#<cfelse>0</cfif>,
									...
									'#form_wrk_row_id#',
                                    <cfif len(form_wrk_row_relation_id)>'#form_wrk_row_relation_id#'<cfelse>NULL</cfif>,
									<cfif len(width)><cfqueryparam cfsqltype="cf_sql_float" value="#width#"><cfelse>NULL</cfif>,
									<cfif len(height)><cfqueryparam cfsqltype="cf_sql_float" value="#height#"><cfelse>NULL</cfif>,
									<cfif len(length)><cfqueryparam cfsqltype="cf_sql_float" value="#length#"><cfelse>NULL</cfif>,
									<cfif len(specific_weight)><cfqueryparam cfsqltype="cf_sql_float" value="#specific_weight#"><cfelse>NULL</cfif>,
									<cfif len(weight)><cfqueryparam cfsqltype="cf_sql_float" value="#weight#"><cfelse>NULL</cfif>
									<cfif len(work_id) and len(work_head)>
									,<cfqueryparam cfsqltype="cf_sql_integer" value="#work_id#">
									,<cfqueryparam cfsqltype="cf_sql_nvarchar" value="#work_head#">
									</cfif>
								)				
						</cfquery>
					</cfif>
                    ...
                    <cfloop from="1" to="#ListLen(attributes.order_row_id,',')#" index="i_in_d">
                        <cfquery name="UPD_ORDERS" datasource="#DSN3#">
                            ...
                            SET 
                            	SPECT_VAR_ID = #new_created_spect_id# 
                            WHERE 
                            	ORDER_ROW_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#ListGetAt(attributes.order_row_id,i_in_d,',')#"> 
                        </cfquery>
                    </cfloop>
                </cfif>
			</cfif>
            ...
            <cfif len(get_process_type.action_file_name)><!--- secilen islem kategorisine bir action file eklenmisse --->
				<cf_workcube_process_cat 
					...
					action_db_type = '#dsn3#'
					is_action_file = 1
					action_page='#request.self#?fuseaction=prod.list_results&event=upd&p_order_id=#attributes.p_order_id#&pr_order_id=#attributes.pr_order_id#'
	...
	<cf_workcube_process 
		...
		old_process_line='#attributes.pr_order_id#' 
		warning_description = 'Üretim Sonucu : #attributes.production_result_no#'>
        <cf_add_log  log_type="0" action_id="#attributes.pr_order_id#" action_name="#attributes.production_order_no#" paper_no="#attributes.production_result_no#" data_source="#dsn3#" process_stage="#attributes.process_stage#">
	<cfset attributes.actionId=attributes.pr_order_id>
	<cfif isdefined("attributes.from_order_operator")>
		<script type="text/javascript">
	...
	<cfelseif (isdefined("attributes.is_delstock") and Len(attributes.is_delstock)) and session_our_company_info_is_cost eq 1><!--- sirket maliyet takip ediliyorsa not js le yonlenioyr cunku cost_action locationda calismiyor --->
		<cfscript>cost_action(action_type:4,action_id:attributes.pr_order_id,query_type:3);</cfscript>
		<cfif isDefined("session.ep")><!--- Pda de Gelmemesi Icin --->
			<script type="text/javascript">
