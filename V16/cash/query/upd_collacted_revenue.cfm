<!--- eskiden tüm satırlar eklenip silinyordu şimdi duruma göre update ediliyor,cunku satırdaki ACTION_ID leri kaybetmek istemiyoruz,manuel belge kapamarda sıkıntı oluyor --->
...
<cfquery name="get_all_revenue" datasource="#dsn2#">
	...
	<cftransaction>
		...
		<cfif isdefined("attributes.record_num") and len(attributes.record_num)>
			<cfloop from="1" to="#attributes.record_num#" index="i">
				...
				<cfelseif isdefined("attributes.row_kontrol#i#") and evaluate("attributes.row_kontrol#i#") and isdefined("attributes.act_row_id#i#") and len(evaluate("attributes.act_row_id#i#"))><!--- güncellenecek satırlar --->
					<cfquery name="upd_revenue" datasource="#dsn2#">
						UPDATE
							CASH_ACTIONS
						SET
							...
							CASH_ACTION_FROM_COMPANY_ID = <cfif len(evaluate("action_company_id#i#")) and evaluate("member_type#i#") eq 'partner'>#evaluate("action_company_id#i#")#,<cfelse>NULL,</cfif>
							CASH_ACTION_FROM_CONSUMER_ID = <cfif len(evaluate("action_consumer_id#i#")) and evaluate("member_type#i#") eq 'consumer'>#evaluate("action_consumer_id#i#")#,<cfelse>NULL,</cfif>
							CASH_ACTION_FROM_EMPLOYEE_ID = <cfif len(evaluate("action_employee_id#i#")) and evaluate("member_type#i#") eq 'employee'>#evaluate("action_employee_id#i#")#,<cfelse>NULL,</cfif>
							CASH_ACTION_TO_CASH_ID = #listfirst(cash_action_to_cash_id,';')#,
							ACTION_DATE = #attributes.action_date#,
							OTHER_CASH_ACT_VALUE = #evaluate("attributes.action_value_other#i#")#,
							OTHER_MONEY = '#listfirst(evaluate("attributes.money_id#i#"),';')#',
							REVENUE_COLLECTOR_ID = #evaluate("attributes.employee_id#i#")#,
					...
					<cfscript>
						if(get_process_type.is_cari eq 1)
						{
							...
							cari_sil(action_id:evaluate("attributes.act_row_id#i#"),process_type:31);
					</cfscript>
				<cfelseif isdefined("attributes.row_kontrol#i#") and evaluate("attributes.row_kontrol#i#") eq 0 and isdefined("attributes.act_row_id#i#") and len(evaluate("attributes.act_row_id#i#"))><!--- satır silmeler --->
					...
					<cfscript>cari_sil(action_id:evaluate("attributes.act_row_id#i#"),process_type:31);</cfscript>
				</cfif>
                <cf_add_log employee_id="#session.ep.userid#" log_type="0" action_id="#form.multi_id#" action_name= "#wrk_eval("attributes.paper_number#i#")# Güncellendi" paper_no= "#wrk_eval("attributes.paper_number#i#")#" period_id="#session.ep.period_id#" process_type="#get_process_type.PROCESS_TYPE#" data_source="#dsn2#">
			</cfloop>
		</cfif>
		<cfscript>
		...
		<!--- <cfif len(get_process_type.action_file_name)> ---><!--- secilen islem kategorisine bir action file eklenmisse --->
			<cf_workcube_process_cat 
				...
				is_action_file = 1
				action_db_type = '#dsn2#'
				action_page='#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.form_add_cash_revenue&event=updMulti&multi_id=#attributes.multi_id#'
...
<script type="text/javascript">
	window.location.href='<cfoutput>#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.form_add_cash_revenue&event=updMulti&multi_id=#attributes.multi_id#</cfoutput>';
</script>
