<cfif form.active_period neq session.ep.period_id>
	<script type="text/javascript">
		...
		window.location.href='<cfoutput>#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.form_add_cash_rate_valuation</cfoutput>';
	</script>
	<cfabort>
</cfif>
	...
	<cftransaction>
		...
		<cfloop from="1" to="#attributes.total_record#" index="kk">
			<cfif isdefined("attributes.is_pay_#kk#")>				
				<cfquery name="ADD_CASH_ACT" datasource="#dsn2#">
					INSERT INTO
						CASH_ACTIONS
						(
							MULTI_ACTION_ID,
						...
						(
							...
							<cfif evaluate("attributes.control_amount_2_#kk#") gt 0>
								#evaluate("attributes.cash_id_#kk#")#
							<cfelse>
								NULL
							</cfif>,
				...
				<cfscript>
					if(is_budget eq 1 and len(attributes.expense_center) and len(attributes.expense_center_id) and len(attributes.expense_item_id) and len(attributes.expense_item_name))
					{
						...
						butceci(
							...
							detail : 'KASA KUR DEĞERLEME FİŞİ',
							branch_id : evaluate("attributes.branch_id_#kk#"),
							insert_type : 1
						);
					}
				</cfscript>
			...
			if(is_account eq 1)
			{
				...
				for(k=1; k lte attributes.total_record; k=k+1)
					if(isdefined("attributes.is_pay_#k#"))
					{
						...
						satir_detay_list[1][listlen(tutar_list)]='KASA KUR DEĞERLEME';
					}
				...
				{			
					muhasebeci (
						...
						fis_satir_detay:'KASA KUR DEĞERLEME İŞLEMİ',
						currency_multiplier : currency_multiplier,
						fis_detay:'KASA KUR DEĞERLEME İŞLEMİ',
		...
		<cf_workcube_process_cat 
			...
			action_page='#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.form_add_cash_rate_valuation'
			action_db_type = '#dsn2#'
