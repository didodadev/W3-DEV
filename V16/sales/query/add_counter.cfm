...
<cfif isDefined("uploaded_file") and len(form.uploaded_file)>
    ...
    <cftry>
        <cffile action="UPLOAD" filefield="uploaded_file" destination="#upload_folder#" mode="777" nameconflict="MAKEUNIQUE" accept="image/*">
...
<cfif response.status>
    <script type="text/javascript">
        <cfoutput>
            window.location.href = '#request.self#?fuseaction=sales.counter&event=upd&counter_id=#response.result.IDENTITYCOL#';
        </cfoutput>
    </script>
<cfelse>
    <script>
