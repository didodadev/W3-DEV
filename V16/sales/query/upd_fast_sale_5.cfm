...
<cfscript>
	...
	{
		GET_ACC_CODE=cfquery(datasource:"#dsn2#", sqlstring:"SELECT A_VOUCHER_ACC_CODE FROM CASH WHERE CASH_ID=#account_cash_id#");
		...
		for(i=1; i lte get_last_vouchers.recordcount; i=i+1)
		{
			if(get_last_vouchers.IS_PAY_TERM[i] == 0)
			{
				if(get_last_vouchers.CURRENCY_ID[i] neq session.ep.money)
					alacak_tutar = get_last_vouchers.OTHER_MONEY_VALUE[i];
				else
					alacak_tutar = get_last_vouchers.VOUCHER_VALUE[i];
