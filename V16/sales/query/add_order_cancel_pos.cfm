<cfif attributes.pos_count gt 0>
	<cfquery name="get_pos_process_cat" datasource="#dsn2#" maxrows="1">
		SELECT IS_ACCOUNT,IS_CARI,IS_ACCOUNT_GROUP FROM #dsn3_alias#.SETUP_PROCESS_CAT WHERE PROCESS_TYPE = 245
	</cfquery>
	...
	<cfloop from="1" to="#attributes.pos_count#" index="pos_sira">
		<cfscript>
			'attributes.pos_amount_#pos_sira#' = filterNum(evaluate('attributes.pos_amount_#pos_sira#'));
			'attributes.system_pos_amount_#pos_sira#' = filterNum(evaluate('attributes.system_pos_amount_#pos_sira#'));
		...
		<cfoutput query="get_money_bskt">
			<cfif get_money_bskt.money is attributes.order_money>
		...
		<cfquery name="GET_MAX" datasource="#dsn2#">
			SELECT MAX(CREDITCARD_PAYMENT_ID) AS MAX_ID FROM #dsn3_alias#.CREDIT_CARD_BANK_PAYMENTS WHERE WRK_ID = '#wrk_id#'
		</cfquery>
		<cfoutput query="get_money_bskt">
			...
			if(is_cari_pos eq 1)
			{
				if(account_currency_id is session.ep.money)
				{
					...
					(
						...
						due_date : attributes.cancel_date,
						...
						other_money :  attributes.order_money,			
			...
			if(is_account_pos eq 1)
			{
				...
				else
				{
					muhasebeci (
						...
						fis_satir_detay: 'KREDİ KARTI TAHSİLAT İPTAL',
						borc_hesaplar:  ACC,
						borc_tutarlar: evaluate("attributes.system_pos_amount_#pos_sira#"),
						other_amount_borc : temp_pos_tutar,
						other_currency_borc : account_currency_id,
						alacak_hesaplar: GET_CREDIT_PAYMENT.account_code,
						alacak_tutarlar: evaluate("attributes.system_pos_amount_#pos_sira#"),
