	...
	<cfif len(attributes.record_num) and attributes.record_num neq "">
	  <cfloop from="1" to="#attributes.record_num#" index="i">
		<cfif evaluate("attributes.row_kontrol#i#")>
		  <cfscript>
		  	if(isdefined('attributes.project_id#i#'))form_project_id=evaluate("attributes.project_id#i#");else form_project_id="";
		  	if(isdefined('attributes.relation_asset_id#i#'))form_asset_id=evaluate("attributes.relation_asset_id#i#");else form_asset_id="";
				...
				<cfif len(form_company_id) or len(form_consumer_id)>
				  <cfquery name="ADD_EVENT_PLAN_ROW" datasource="#DSN#">
						...
						SET
                            ...
                            ASSET_ID=<cfif len(form_asset_id)>#form_asset_id#<cfelseif  isdefined("attributes.relation_asset_id") and len(attributes.relation_asset_id)>#attributes.relation_asset_id#<cfelse>NULL</cfif>,
                            PROJECT_ID=<cfif len(form_project_id)>#form_project_id#<cfelseif  isdefined("attributes.project_id")and len(attributes.project_id)>#attributes.project_id#<cfelse>NULL</cfif>
						WHERE
