...
<cfif len(attributes.cheque_payroll_id)>
	<cfquery name="upd_payroll" datasource="#dsn2#">
		 UPDATE
			PAYROLL
		 SET
			PAYROLL_TOTAL_VALUE = #attributes.total_cheque_value#,
			PAYROLL_TYPE = 90,
			PAYROLL_OTHER_MONEY = <cfqueryparam cfsqltype="cf_sql_varchar" value="#session.ep.money#">,
			PAYROLL_OTHER_MONEY_VALUE = #attributes.total_cheque_value#,
			NUMBER_OF_CHEQUE = #row_count_cheque#,
			PAYROLL_REVENUE_DATE = #attributes.order_date#,
			PAYROLL_CASH_ID = #account_cash_id#,
			PAYMENT_ORDER_ID = #attributes.order_id#,
			PAYROLL_AVG_DUEDATE = <cfif isdefined("attributes.cheque_payroll_due_value_date_") and len(attributes.cheque_payroll_due_value_date_)>#attributes.cheque_payroll_due_value_date_#<cfelse>NULL</cfif>,
...
<cfelse>
	...
	<cfset attributes.cheque_payroll_id=get_bordro_id.P_ID>
</cfif>
...
<cfloop from="1" to="#attributes.record_num_4#" index="kk">
	<cfif len(evaluate("attributes.cheque_value#kk#")) and evaluate("attributes.cheque_value#kk#") gt 0 and evaluate("attributes.row_kontrol_4#kk#") eq 1>
		...
		<cfquery name="get_last_cheque" datasource="#dsn2#">
			SELECT MAX(CHEQUE_ID) AS MAX_ID FROM CHEQUE WHERE CHEQUE_PAYROLL_ID=#attributes.cheque_payroll_id#
		</cfquery>
		<cfloop from="1" to="#attributes.kur_say#" index="j">
			<cfquery name="ADD_MONEY_INFO" datasource="#dsn2#">
				INSERT INTO 
...
<cfquery name="get_last_cheques" datasource="#dsn2#">
	SELECT * FROM CHEQUE WHERE CHEQUE_PAYROLL_ID=#attributes.cheque_payroll_id#
</cfquery>
