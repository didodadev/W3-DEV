					...
					<cfif get_online_sales_types.recordcount>
						<cfoutput query="get_online_sales_types" startrow="#attributes.startrow#" maxrows="#attributes.maxrows#">
							...
							<tr>
								...
								<td>
									<cfif tip eq 0>
										<a href="javascript://" onclick="windowopen('#request.self#?fuseaction=objects.popup_com_det&company_id=#member_company_id#','list');" class="tableyazi">#sirket#</a>
									<cfelse>
										#sirket#
									</cfif>
								</td>
	...
	<script type="text/javascript">
		...
		function addorder(pre_id){
			if( confirm("İlgili Satır Siparişe Dönüştürülecektir. Onaylıyor musunuz ?") ){
				...
				AjaxControlPostDataJson('V16/objects2/sale/cfc/PreOrderAction.cfc?method=add_order_func',data,function(response) { 
					if( response.STATUS ){
						alert("Sipariş başarılı bir şekilde oluşturuldu");
					}else{
						alert("Sipariş oluşturulurken bir sorun oluştu");
					}
				});
			}
