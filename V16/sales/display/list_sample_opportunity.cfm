    ...
    $('input[name="opportunity_chech"]').each(function(){
        if($(this).is(':checked')){
            ...
            for (var i = 0; i < sample.length; i++) {
                 ...
                 var query_ = "SELECT PS.SALES_PRICE,PS.PRODUCT_SAMPLE_ID,ISNULL(P.P_SAMPLE_ID,0)as P_SAMPLE_ID,PS.PRODUCT_SAMPLE_NAME, PS.TARGET_PRICE ,P.PRODUCT_ID FROM PRODUCT_SAMPLE AS PS  LEFT JOIN <cfoutput>#DSN1#</cfoutput>.PRODUCT AS P ON P.P_SAMPLE_ID = PS.PRODUCT_SAMPLE_ID WHERE PS.PRODUCT_SAMPLE_ID = " +sample[i]  
		          
                 get_sales_price = wrk_query(query_,'dsn3');
