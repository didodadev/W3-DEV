...
function check_visit()
{
	...
	{
		alert("<cf_get_lang dictionary_id ='41218.Olay Ziyaret Başlığını Giriniz'> !");
		return false;
	}
	...
	<cfif x_form_visit eq 1>
		...
		{
			alert("<cf_get_lang dictionary_id='41429.Analiz Formu Giriniz'> !");
			return false;	
		}
	</cfif>
	...
	{
		alert("<cf_get_lang dictionary_id ='41032.Başlama Tarihini Doğru Giriniz'> !");
		return false;
	}
	if(document.getElementById('main_finish_date').value=='')
	{
		alert("<cf_get_lang dictionary_id ='30123.Bitiş Tarihini Doğru Giriniz'> !");
		return false;
	}
	if(datediff(document.getElementById('main_start_date').value,document.getElementById('main_finish_date').value ,0)<0)
	{
		alert("<cf_get_lang dictionary_id ='58862.Başlangıç Tarihi Bitiş Tarihinden Önce Olamaz'> !");
		return false;
	}
	if (document.getElementById('record_num').value == 0)
	{
		alert("<cf_get_lang dictionary_id ='41219.Lütfen Ziyaret Edilecek Seçiniz'> !");
		return false;
	}
	row_kontrol=0;
	...
	{	 
		...
		if(deger_row_kontrol == 1)
		{
			...
			{
				alert("<cf_get_lang dictionary_id ='41040.Lütfen Bitiş Tarihi Giriniz'> !");
				return false;
			}
...
function pencere_ac_date(no)
{
	if((document.add_event.main_start_date.value == "") && (document.add_event.main_finish_date.value == ""))
		alert("<cf_get_lang dictionary_id ='35064.Lütfen İlk Önce Olay Başlangıç ve Bitiş Tarihlerini Seçiniz'> !");
	else
...
function pencere_ac_company(no)
{
	if((document.getElementById('main_start_date').value=='') || (document.getElementById('main_finish_date').value == ''))
	{
		...
		alert("<cf_get_lang dictionary_id ='41220.Önce Ziyaret Tarihlerini Giriniz'> !");
		return false;	
	}
	else if(datediff(document.getElementById('main_start_date').value,document.getElementById('main_finish_date').value ,0)<0)
	{
		...
		openBoxDraggable('<cfoutput>#request.self#?fuseaction=objects.popup_list_multiuser_company&kontrol_startdate='+document.getElementById('main_start_date').value+'&record_num_=' + document.getElementById('record_num').value +'&is_first=1&startdate=' + document.getElementById('main_start_date').value +'&is_sales=1&is_close=0&is_choose_project=#is_choose_project#</cfoutput>');
}
