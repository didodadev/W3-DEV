...
<cf_xml_page_edit fuseact ="sales.popup_subscription_payment_plan">
    ...
    function camp_control()
    {
        <cfoutput>
            <cfif xml_control_camp eq 1>
                get_camp_info=wrk_query('SELECT DISTINCT SP.SUBSCRIPTION_ID,C.CAMP_HEAD FROM SUBSCRIPTION_PAYMENT_PLAN_ROW SP,CAMPAIGNS C WHERE SP.CAMPAIGN_ID = C.CAMP_ID AND C.PART_TIME IS NOT NULL AND SP.IS_BILLED = 0 AND SP.SUBSCRIPTION_ID=#attributes.subscription_id#','dsn3');
                if(get_camp_info.recordcount > 0)
                {
                ...
                windowopen('#request.self#?fuseaction=sales.popup_form_del_pay_plan_row&xml_del_camp_rows=#xml_del_camp_rows#&xml_del_ref_rows=#xml_del_ref_rows#&del_all=1&subscription_id=#attributes.subscription_id#','small');
            <cfelse>
                if(confirm('<cf_get_lang dictionary_id ="41307.Ödeme Planı Satırlarını Silmek İstediğinizden Emin misiniz">')) 
                    windowopen('#request.self#?fuseaction=sales.popup_form_del_pay_plan_row&xml_del_camp_rows=#xml_del_camp_rows#&xml_del_ref_rows=#xml_del_ref_rows#&del_all=1&subscription_id=#attributes.subscription_id#','small');
            </cfif>
    ...
    function camp_control_row(row_no,row_id,all_)
    {
        <cfoutput>
            ...
            <cfif xml_control_camp eq 1>
                get_camp_info=wrk_query('SELECT TOP 1 SP.SUBSCRIPTION_ID,C.CAMP_HEAD FROM SUBSCRIPTION_PAYMENT_PLAN_ROW SP,CAMPAIGNS C WHERE SP.CAMPAIGN_ID = C.CAMP_ID AND C.PART_TIME IS NOT NULL AND SP.IS_BILLED = 0 AND SP.SUBSCRIPTION_ID=#attributes.subscription_id# AND SP.SUBSCRIPTION_PAYMENT_ROW_ID='+row_id+'','dsn3');
                if(get_camp_info.recordcount > 0)
                {
