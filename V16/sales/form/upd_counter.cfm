	...
	<cf_box title="#iif(isDefined("attributes.draggable"),"getLang('sales','Sayaç Güncelle',41293)",DE(''))#" scroll="1" collapsable="1" resize="1" popup_box="#iif(isdefined("attributes.draggable"),1,0)#">
		<cfform name="upd_counter" method="post" action="#iif(isDefined("attributes.draggable"),DE('#request.self#?fuseaction=sales.emptypopup_upd_subscription_counter'),DE(''))#">
					...
					<div class="form-group" id="item-product">
						...
						<div class="col col-8">
							<div class="input-group">
								...
								<span class="input-group-addon btn_Pointer icon-ellipsis" href="javascript://" onclick="openBoxDraggable('<cfoutput>#request.self#</cfoutput>?fuseaction=objects.popup_product_price_unit&field_stock_id=upd_counter.stock_id&field_id=upd_counter.product_id&field_name=upd_counter.product&field_unit_id=upd_counter.unit_id&field_unit=upd_counter.unit_name');"></span>
							</div>
						</div>
					</div>
			...
			<cf_box_footer>
				...
				<div class="col col-6">
					<cf_workcube_buttons is_upd='1' is_delete='1' add_function='kontrol()' del_function="#isDefined('attributes.draggable') ? 'deleteFunc()' : ''#">
				</div>
			</cf_box_footer>
		</cfform>
	</cf_box>
