...
function kontrol()
{
	...
	<cfif xml_control_camp eq 1>
		...
		get_camp_info=wrk_query('SELECT DISTINCT SP.SUBSCRIPTION_ID,C.CAMP_HEAD FROM SUBSCRIPTION_PAYMENT_PLAN_ROW SP,CAMPAIGNS C WHERE SP.CAMPAIGN_ID = C.CAMP_ID AND C.PART_TIME IS NOT NULL AND SP.IS_BILLED = 0 AND SP.PAYMENT_DATE >'+paper_date_+' AND SP.SUBSCRIPTION_ID='+document.all.subscription_id.value,'dsn3');
