<cfquery name="get_position_cat" datasource="#DSN#">
	...
	FROM         
        ...
        INNER JOIN DEPARTMENT AS D ON EP.DEPARTMENT_ID = D.DEPARTMENT_ID 
        INNER JOIN BRANCH AS B ON D.BRANCH_ID = B.BRANCH_ID
	WHERE    
    	EP.EMPLOYEE_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.emp_id#">
</cfquery>
...
<cfquery name="get_trains" datasource="#DSN#">
   ...
   <cfif len(class_list)>
	...
	AND TCA.EMP_ID=<cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.emp_id#">
</cfquery>
<cfquery name="get_next_trains" datasource="#DSN#">
	SELECT
		...
		CAST(TC.START_DATE AS CHARACTER),
		CAST(TC.FINISH_DATE AS CHARACTER),
		</cfif>
		2 AS D_TYPE,
		TC.CLASS_NAME,
		TC.CLASS_PLACE,
