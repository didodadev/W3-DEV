<!--- <cfif ((not get_quiz_question.recordcount) or (question_limit EQ attributes.question_row))>
	<cfif isDefined("attributes.view_answer")><cfinclude template="../query/add_quiz_result.cfm"></cfif>
</cfif>  --->
<cfif (not get_quiz_question.recordcount) or (question_limit EQ attributes.question_row)>
...
<cfoutput>
   ...
   <input type="Hidden" name="page_type" id="page_type" value="#attributes.page_type#">
</cfoutput>
        ...
        <tr>
		  ...
		  <br/>
			...
			<cfif attributes.question_row gt 1>
				 <cfsavecontent variable="message1"><cf_get_lang no='73.Sınavı durdur'></cfsavecontent>
				...
				<cfsavecontent variable="message7"><cf_get_lang_main no="20.Geri"></cfsavecontent>
				<input type="button" onClick="history.go(-1);" value="<cfoutput>#message7#</cfoutput>" name="Back" id="Back" style="width:75px;">
			</cfif>
