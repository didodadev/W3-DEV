      ...
      <table width="100%" border="0">
        ...
        <cfloop query="get_quiz_questions">
          ...
          <tr class="color-list" height="20">
			...
			<cfif GET_TRAINING_PERF_DETAIL.RECORD_MEMBER neq session.ep.userid>
			...
			<td class="txtboldblue">
				<input type="checkbox" name="gd_#get_quiz_chapters.currentrow#_#get_quiz_questions.currentrow#" id="gd_#get_quiz_chapters.currentrow#_#get_quiz_questions.currentrow#" value="1"
				 ...
				 onClick="change_gd('#get_quiz_chapters.currentrow#','#get_quiz_questions.currentrow#');"
				> 
			</td>
			</cfif>
              ...
              <cfif len(evaluate("b#i#")) or len(evaluate("a#i#"))>
				...
				<cfelse>
                  <input type="Radio" name="user_answer_#get_quiz_chapters.currentrow#_#get_quiz_questions.currentrow#" id="user_answer_#get_quiz_chapters.currentrow#_#get_quiz_questions.currentrow#" value="#i#" 
					<cfloop query="GET_EMP_QUIZ_ANSWERS">
						<cfif isdefined("GET_EMP_QUIZ_ANSWERS") and isdefined("GET_EMP_QUIZ_ANSWERS.RESULT_ID") and GET_EMP_QUIZ_ANSWERS.QUESTION_ID is q_id and GET_EMP_QUIZ_ANSWERS.QUESTION_USER_ANSWERS is i>checked</cfif>
					</cfloop>
				   onClick="change_gd_radio('#get_quiz_chapters.currentrow#','#get_quiz_questions.currentrow#');"
				  >
				  </cfif>
          ...
          <cfloop from="1" to="20" index="i">
            <cfif len(evaluate("get_quiz_questions.answer#i#_photo")) or len(evaluate("get_quiz_questions.answer#i#_text"))>
              <tr class="color-list">
	            <td></td>
				<td align="left"><!--- <input type="checkbox" name="gd_#get_quiz_chapters.currentrow#_#get_quiz_questions.currentrow#" value="1" > ---></td>
                <td>
				<cfif GET_TRAINING_PERF_DETAIL.RECORD_MEMBER neq session.ep.userid>
					<cfloop query="GET_EMP_QUIZ_ANSWERS">
						<cfif IsDefined("GET_EMP_QUIZ_ANSWERS") AND IsDefined("GET_EMP_QUIZ_ANSWERS.RESULT_ID") AND GET_EMP_QUIZ_ANSWERS.QUESTION_ID IS q_id AND 
				...
				<cfelse>
                  <input type="Radio" name="user_answer_#get_quiz_chapters.currentrow#_#get_quiz_questions.currentrow#" id="user_answer_#get_quiz_chapters.currentrow#_#get_quiz_questions.currentrow#" value="#i#">
					<cfloop query="GET_EMP_QUIZ_ANSWERS">
						<cfif IsDefined("GET_EMP_QUIZ_ANSWERS") AND IsDefined("GET_EMP_QUIZ_ANSWERS.RESULT_ID") AND GET_EMP_QUIZ_ANSWERS.QUESTION_ID IS q_id AND 
							GET_EMP_QUIZ_ANSWERS.QUESTION_USER_ANSWERS IS i>checked
						</cfif>
					</cfloop>
				</cfif>
...
function change_gd(bolum,soru)
{
	if(eval("document.add_perform.gd_"+bolum+"_"+soru).checked == true)
		{
		eval("document.add_perform.user_answer_"+bolum+"_"+soru)[0].checked = false;
		eval("document.add_perform.user_answer_"+bolum+"_"+soru)[1].checked = false;
		eval("document.add_perform.user_answer_"+bolum+"_"+soru)[2].checked = false;
		eval("document.add_perform.user_answer_"+bolum+"_"+soru)[3].checked = false;
		eval("document.add_perform.user_answer_"+bolum+"_"+soru)[4].checked = false;
		}
}

function change_gd_radio(bolum,soru)
{
	eval("document.add_perform.gd_"+bolum+"_"+soru).checked = false;
}
</script>
