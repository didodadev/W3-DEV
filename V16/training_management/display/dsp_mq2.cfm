<!--- <cfif (isdefined("get_quiz_question.recordcount") and not get_quiz_question.recordcount) or (question_limit EQ attributes.question_row)>
	aaaaaaaaaa<cfinclude template="../query/add_quiz_result.cfm">
</cfif> --->
<!--- toplam süre yada soru süresi hesabı sonucu tek değişkene at --->
<cfif get_quiz.total_time is "">
  <cfset submit_time = get_quiz_question.question_time*60000>
  <cfelse>
        ...
        <tr>
          <td>
		   <cfsavecontent variable="message1">Sınavı Durdur</cfsavecontent>
            <input type="Reset" value="<cfoutput>#message1#</cfoutput>" style="width:75px;">
        ...
        <tr>
          <td>
		  	<cfif attributes.question_row gt 1>
				<cfsavecontent variable="message1">Sınavı Durdur</cfsavecontent>
				<input type="Reset" value="<cfoutput>#message1#</cfoutput>" onClick="is_stopped_quiz_function();"  style="width:75px;">
