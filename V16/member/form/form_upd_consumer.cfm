...
<cfelse>
	...
	<cfinclude template="../query/get_income_level.cfm">
	...
	<script type="text/javascript">
		...
		function kontrol()
		{
			...
			if(document.getElementById("earchive_sending_type").value == 1 && document.getElementById("consumer_email").value == "")
			{
			...
			<cfif xml_mobile_tel_required eq 1>
				...
				{
					alert("<cf_get_lang dictionary_id='58194.girilmesi zorunlu alan'>:<cf_get_lang dictionary_id='30254.Kod/ Mobil tel'>");
					return false;
				}
			</cfif>
			<cfif xml_only_numeric_password eq 1>
				...
				{
					alert("<cf_get_lang dictionary_id='30748.Lütfen Numeric Bir Şifre Değeri Giriniz'> !");
					return false;
				}
			</cfif>
			...
			if(is_dsp_category == 1)
			{
				...
				if (document.upd_consumer.consumer_cat_id[x].value == "")
				{ 
					alert ("<cf_get_lang dictionary_id='58194.girilmesi zorunlu alan'>:<cf_get_lang dictionary_id='30436.Bireysel Üye Kategori'>");
					return false;
				}
			}
			...
			if(is_dsp_reference_member == 1)
			{
				if(<cfoutput>#session.ep.admin#</cfoutput> == 1 && document.upd_consumer.ref_pos_code.value != '' && document.upd_consumer.ref_pos_code_name.value == '')
				{
					if((document.upd_consumer.ref_pos_code_row != undefined && document.upd_consumer.ref_pos_code_row.value == '' && document.upd_consumer.is_upper_member.checked == false) || document.upd_consumer.ref_pos_code_row == undefined)
					{
						...
						{
							alert("<cf_get_lang dictionary_id ='30653.Referans Üye Bağlantısını Kopardınız! İlgili Üyenin Referans Olduğu Üyeler İçin Referans Bilgisi Girmelisiniz'> !");
				...
				if(<cfoutput>#session.ep.admin#</cfoutput> == 1 && document.upd_consumer.hidden_reference_code.value != '' && document.upd_consumer.reference_code.value != document.upd_consumer.hidden_reference_code.value)
				{
					if((document.upd_consumer.ref_pos_code_row != undefined && document.upd_consumer.ref_pos_code_row.value == '' && document.upd_consumer.is_upper_member != undefined && document.upd_consumer.is_upper_member.checked == false && document.upd_consumer.is_ref_member != undefined && document.upd_consumer.is_ref_member.checked == false) || document.upd_consumer.ref_pos_code_row == undefined)
					{
						...
						if(get_consumer.recordcount)
						{
							alert("<cf_get_lang dictionary_id ='30652.Referans Üye Bağlantısını Değiştirdiniz! İlgili Üyenin Referans Olduğu Üyeler İçin Referans Bilgisi Girmelisiniz '>!");
			...
			*/
			...
			if(is_cari_working == 1)
			{
				...
				{
					alert("<cf_get_lang dictionary_id='30299.Çalışılabilir seçeneği seçili olmalıdır!'>");
					return false;
				}
			}	
			...
			if(document.upd_consumer.work_address != undefined)
			{
				...
				if ( y < 0 )
				{ 
					alert ("<cf_get_lang dictionary_id='30607.İş Adresi'><cf_get_lang dictionary_id='58210.Alanındaki Fazla Karakter Sayısı'>"+ ((-1) * y));
					return false;
				}
			}
			if(document.upd_consumer.home_address != undefined)
			{
				...
				if ( z < 0 )
				{ 
					alert ("<cf_get_lang dictionary_id='30606.Ev Adresi'><cf_get_lang dictionary_id='58210.Alanindaki Fazla Karakter Sayısı'>"+ ((-1) * z));
					return false;
				}
			}
			if(document.upd_consumer.tax_address != undefined)
			{
				...
				if ( v < 0 )
				{ 
					alert ("<cf_get_lang dictionary_id='30261.Fatura Adresi'><cf_get_lang dictionary_id='58210.Alanindaki Fazla Karakter Sayısı'>"+ ((-1) * v));
					return false;
				}
			}
			...
			<cfif xml_check_cell_phone eq 1>
				if(document.getElementById('mobilcat_id').value != "" && document.getElementById('mobiltel').value != "")
				{
					...
					{
						  alert("<cf_get_lang dictionary_id='30295.Girdiginiz Cep Telefonuna Kayitli Baska Temsilci Bulunmaktadir!'>");
		...
		function kontrol_ref_member(type)
		{
			if(type == 0)
			{
				...
				{
					alert("<cf_get_lang dictionary_id ='30651.Üyeyi Kendisi İçin Referans Üye Olarak Ekleyemezsiniz'> !");
					kontrol_inf = 1;
				}
				if(kontrol_inf == 0)
				{
					...
					{	
						...
						{
							alert("<cf_get_lang dictionary_id ='30650.Üyeyi Referans Olduğu Üyeye Bağlayamazsınız'> !");
							kontrol_inf = 1;
						}
					}
				}
