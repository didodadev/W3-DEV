...
<cfquery name="get_team_name" datasource="#dsn#">
	...
	FROM
		SALES_ZONES_TEAM
	WHERE
		TEAM_ID= #attributes.customer_team_id#
</cfquery>
...
<cfquery name="get_quote_teams" datasource="#dsn#">
	...
	FROM
		COMPANY,
		COMPANY_PARTNER
	<cfif isdefined("attributes.customer_team_id")>
	WHERE
		...
		(
			COMPANY.COMPANY_ID IN 
					...
					WHERE
						SALES_ZONES_TEAM_ROLES.TEAM_ID = #attributes.customer_team_id# AND
...
<cfquery name="GET_ROWS" datasource="#dsn#">
	SELECT 
		...
		SQGR.ROW_MONEY,
		SQGR.QUOTE_MONTH,
		SQGR.CUSTOMER_TEAM_ID,
		SQGR.CUSTOMER_COMP_ID
	FROM
		SALES_QUOTES_GROUP_ROWS SQGR
	WHERE
		SQGR.SALES_QUOTE_ID = #GET_SALES_QUOTE_ZONE.sales_quote_id#
		AND SQGR.CUSTOMER_TEAM_ID= #attributes.customer_team_id#
	ORDER BY
		POSITION_CODE,
		QUOTE_MONTH ASC	
</cfquery>
  ...
  <tr class="color-border">
    <td>
      <table cellspacing="1" cellpadding="2" border="0" width="100%" height="100%">
        <cfform name="form_basket" action="#request.self#?fuseaction=salesplan.emptypopup_upd_sales_quote_customer_based" method="post">
        ...
        <tr class="color-list" height="35">
          <td class="headbold"><cf_get_lang no='58.Müşteri Bazında Satış Hedefi'></td>
        </tr>
        <tr class="color-row">
          <td valign="top">
            <table>
                  ...
                  <select name="quote_year_select" id="quote_year_select" style="width:65px;" onchange="if (this.options[this.selectedIndex].value != 'null') { window.open(this.options[this.selectedIndex].value,'_self') }">
                    <cfoutput>
                      <cfloop from="#session.ep.period_year#" to="2020" index="i">
                        <option value="#request.self#?fuseaction=salesplan.popup_check_sales_quote_customer_based&quote_year=#i#&team_id=#GET_SALES_QUOTE_ZONE.sales_zone_id#-#attributes.customer_team_id#-#attributes.branch_id#" <cfif GET_SALES_QUOTE_ZONE.quote_year eq i>selected</cfif>>#i#</option>
                      </cfloop>
                    </cfoutput>
                  </select>
                </td>
              ...
              <tr>
                ...
                <td rowspan="3">
                  <textarea name="quote_detail" id="quote_detail" style="width:150px;height:65px;"><cfoutput>#GET_SALES_QUOTE_ZONE.quote_detail#</cfoutput></textarea>
                </td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td><input type="text" style="width:150px;" name="team_name" id="team_name" value="<cfoutput>#get_team_name.team_name#</cfoutput>" readonly>
                </td>
                <td>&nbsp;</td>
              </tr>
              <tr>
              ...
              <tr class="color-border">
                <td>
                  <table cellspacing="1" cellpadding="2" border="0" width="100%" height="100%">
                    ...
                    <tr class="color-list">
                      <td class="txtboldblue"><cf_get_lang_main no='1461.Satıcı'>
                        <input type="hidden" name="customer_ids" id="customer_ids" value="<cfoutput>#valuelist(get_quote_teams.COMPANY_ID)#</cfoutput>">
                        <input type="hidden" name="team_ids" id="team_ids" value="<cfoutput>#get_team_name.TEAM_ID#</cfoutput>">
                      </td>
                      <cfloop from="1" to="12" index="k">
                        <td align="center" class="txtboldblue"> <cfoutput>#Listgetat(aylar,k)#</cfoutput> </td>
                      </cfloop>
                    ...
                    <cfif GET_SALES_QUOTE_ZONE.recordcount>
                      ...
                      SELECT SQGR.QUOTE_MONTH, SQGR.SALES_INCOME, SQGR.ROW_MONEY
                      FROM SALES_QUOTES_GROUP SQ, SALES_QUOTES_GROUP_ROWS SQGR
                      WHERE SQ.QUOTE_TYPE = 2 AND SQ.QUOTE_YEAR=#GET_SALES_QUOTE_ZONE.quote_year#
                      AND SQ.SALES_ZONE_ID=#GET_SALES_QUOTE_ZONE.sales_zone_id#
                      AND SQGR.SALES_QUOTE_ID=SQ.SALES_QUOTE_ID AND SQGR.TEAM_ID=
                      #attributes.customer_team_id# ORDER BY SQGR.QUOTE_MONTH
                      ASC
                      </cfquery>
                      <tr class="color-row">
                        ...
                        <cfif get_sales_other_quotes.recordcount>
                          <cfoutput query="get_sales_other_quotes">
                            <td align="right" class="txtboldblue" style="text-align:right;">#tlformat(get_sales_other_quotes.SALES_INCOME,0)#</td>
                            <cfset toplam_other_quotes=toplam_other_quotes+get_sales_other_quotes.SALES_INCOME>
                          </cfoutput>
                          <td align="right" class="txtboldblue" style="text-align:right;"><cfoutput>#tlformat(toplam_other_quotes,0)#</cfoutput></td>
                          <cfelse>
                    ...
                    <cfoutput query="get_quote_teams">
                      <tr class="color-row">
                        <td>#FULLNAME#/ #get_team_name.TEAM_NAME#</td>
                        <cfset toplam = 0>
                        <cfloop from="1" to="12" index="k">
                          <cfquery name="get_team_target" dbtype="query">
                          SELECT SALES_INCOME FROM GET_ROWS WHERE	QUOTE_MONTH=#k# AND
                          CUSTOMER_COMP_ID=#COMPANY_ID# AND	CUSTOMER_TEAM_ID=#get_team_name.TEAM_ID#
                          </cfquery>
                          ...
                          <td>
                            <input tabindex="#currentrow#" onFocus="son_deger_degis(#get_team_name.TEAM_ID#,#COMPANY_ID#,#k#);" onkeyup="return(FormatCurrency(this,event,0));" onBlur="toplam_al(#get_team_name.TEAM_ID#,#COMPANY_ID#,#k#);" type="text" name="team_#get_team_name.TEAM_ID#_#COMPANY_ID#_#k#" style="width:60px;" class="box" value="#Tlformat(yazilacak_rakam,0)#" tabindex="#k#">
                          </td>
                        </cfloop>
                        <td>
                          <cfinput  type="text" name="toplam_#get_team_name.TEAM_ID#_#COMPANY_ID#" style="width:63px;" class="box" value="#tlformat(toplam,0)#">
                        </td>
                      </tr>
                    </cfoutput>
                    <tr class="color-row">
                      ...
                      <td><input type="text" name="son_toplam" id="son_toplam" style="width:63px;" class="box" value="<cfoutput>#tlformat(son_toplam,0)#</cfoutput>" readonly>
                      </td>
                    </tr>
                  </table>
                </td>
...
team=document.form_basket.team_ids.value;
...
{	
	...
	<cfoutput query="get_quote_teams">
		for(var i=1; i<=12; i++)			
		{ 
		if(eval("form_basket.team_" + team + "_#COMPANY_ID#_"+i).value == '')
			eval("form_basket.team_" + team + "_#COMPANY_ID#_"+i).value = 0;
		}
	</cfoutput>
	return true;
}
...
function UnformatFields()

	{
		<cfoutput query="get_quote_teams">
			for(var i=1; i<=12; i++)
			{
			eval("form_basket.team_" + team + "_#COMPANY_ID#_"+i).value = filterNum(eval("form_basket.team_" + team + "_#COMPANY_ID#_"+i).value);
			}
		</cfoutput>
...
function reformat()
{
	<cfoutput query="get_quote_teams">
			for(var i=1; i<=12; i++)
			{
			eval("form_basket.team_" + team + "_#COMPANY_ID#_"+i).value = commaSplit(eval("form_basket.team_" + team + "_#COMPANY_ID#_"+i).value);
			}
	</cfoutput>
