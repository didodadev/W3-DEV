...
<cfquery name="get_sales_other_quotes" datasource="#dsn#">
    ...
    WHERE
        ...
        SQGR.TEAM_ID= #attributes.CUSTOMER_TEAM_ID# 
    ORDER BY
        SQGR.QUOTE_MONTH ASC
</cfquery>
<cfparam name="attributes.quote_year" default= #session.ep.period_year#>
<cf_box title="#getLang('salesplan',58)#"><!--Müşteri Bazında Satış Hedefi-->
    <cfform name="form_basket" action="#request.self#?fuseaction=salesplan.emptypopup_add_sales_quote_customer_based" method="post">
    ...
    <cfquery name="get_consumers" datasource="#dsn#">
        SELECT COMPANY.COMPANY_ID, COMPANY.FULLNAME FROM COMPANY,
        COMPANY_PARTNER
        <cfif isdefined("attributes.customer_team_id")>
            WHERE COMPANY_PARTNER.PARTNER_ID = COMPANY.MANAGER_PARTNER_ID
            AND ( COMPANY.COMPANY_ID IN ( SELECT COMPANY_BRANCH_RELATED.COMPANY_ID
            FROM SALES_ZONES_TEAM_ROLES, COMPANY_BRANCH_RELATED WHERE
            SALES_ZONES_TEAM_ROLES.TEAM_ID = #attributes.customer_team_id#
            AND COMPANY_BRANCH_RELATED.SALES_DIRECTOR = SALES_ZONES_TEAM_ROLES.EMP_ID
            ) OR COMPANY.IMS_CODE_ID IN ( SELECT SALES_ZONES_TEAM_IMS_CODE.IMS_ID
            FROM SALES_ZONES_TEAM_IMS_CODE WHERE TEAM_ID = #attributes.customer_team_id#)
            )
        </cfif>
    </cfquery>
    <cf_basket>
    <cf_grid_list>
        ...
        <tr>
            <th><cf_get_lang dictionary_id='57457.Müşteri'>
                <input type="hidden" name="customer_ids" id="customer_ids" value="<cfoutput>#valuelist(get_consumers.COMPANY_ID)#</cfoutput>">
            ...
            <th><cf_get_lang_main dictionary_id='58170.Satır Toplam'></th>
        </tr>
        </thead>
        <tbody>
        <tr>
        ...
        <tr>
            <td nowrap>#FULLNAME#/ #get_team_name.TEAM_NAME#</td>
            <cfloop from="1" to="12" index="k">
                <td>
                <cfinput passThrough = "onkeyup=""return(FormatCurrency(this,event,0));""  onFocus=""son_deger_degis(#get_team_name.TEAM_ID#,#COMPANY_ID#,#k#);"" onBlur=""toplam_al(#get_team_name.TEAM_ID#,#COMPANY_ID#,#k#);""" type="text" name="team_#get_team_name.TEAM_ID#_#COMPANY_ID#_#k#" style="width:60px;" class="box" value="0" tabindex="#k#">
                </td>
            </cfloop>
            <td>
                <cfinput  type="text" name="toplam_#get_team_name.TEAM_ID#_#COMPANY_ID#" style="width:60px;" class="box" value="0">
            </td>
        </tr>
        </cfoutput>
        </tbody>
        <tfoot>
        <tr>
            <td class="txtbold"><cf_get_lang dictionary_id='53263.Toplamlar'></td>
            <cfloop from="1" to="12" index="m">
            <td>
                <input type="text" name="toplam_colon_<cfoutput>#m#</cfoutput>" id="toplam_colon_<cfoutput>#m#</cfoutput>" style="width:60px;" class="box" value="<cfoutput>#tlformat(evaluate("kolon_#m#"),0)#</cfoutput>" readonly>
            </td>
            </cfloop>
            <td><input type="text" name="son_toplam" id="son_toplam" style="width:60px;" class="box" value="0" readonly>
            </td>
        </tr>
        </tfoot>
    </cf_grid_list>
    </cf_basket>
    <cf_box_footer><cfif get_sales_other_quotes.recordcount><cf_workcube_buttons is_upd='0' add_function='upd_form()'></cfif></cf_box_footer>
    </cfform>
</cf_box>
