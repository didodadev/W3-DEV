...
<cfsavecontent variable="head"><cf_get_lang dictionary_id='41503.Müşteri Bazında Satış Hedefi'></cfsavecontent>
...
<cfelse>
	<cfquery name="GET_SALES_QUOTE_ZONE" datasource="#dsn#">
		...
		WHERE
			SQR.CUSTOMER_TEAM_ID = #listgetat(attributes.team_id,2,'-')# AND
	...
	<cfif GET_SALES_QUOTE_ZONE.recordcount>
		<cflocation url="#request.self#?fuseaction=salesplan.popup_upd_sales_quote_customer_based&SALES_QUOTE_ID=#GET_SALES_QUOTE_ZONE.SALES_QUOTE_ID#&branch_id=#listgetat(attributes.team_id,3,'-')#&customer_team_id=#listgetat(attributes.team_id,2,'-')#" addtoken="no">
	<cfelse>
		<cflocation url="#request.self#?fuseaction=salesplan.popup_add_sales_quote_customer_based&SALES_ZONE_ID=#listgetat(attributes.team_id,1,'-')#&branch_id=#listgetat(attributes.team_id,3,'-')#&quote_year=#attributes.quote_year#&customer_team_id=#listgetat(attributes.team_id,2,'-')#" addtoken="no">
	</cfif>
