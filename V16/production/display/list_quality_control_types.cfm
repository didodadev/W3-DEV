...
<cfform name="quality_definitions" method="post" action="V16/product/cfc/product_quality_control_types.cfc?method=add_quality_types">
	...
	<div class="ui-info-bottom">
		<cfoutput query="get_product_info">
			<div class="form-group col col-3 col md-3 col-sm-3 col-xs-6">
	...
	<cf_box_footer>
		...
		<cf_workcube_buttons extraButton="1"  extraButtonText="#getLang('','Ekle','57582')#"  extraFunction="loadPopupBox('quality_definitions','quality_box')" update_status="0">
	</cf_box_footer>
</cfform>
<cfform name="quality_definitions_params" method="post" action="V16/product/cfc/product_quality_control_types.cfc?method=add_quality_type_rows">
	...
	<cf_box_footer>
		<cf_workcube_buttons is_delete='0' add_function="loadPopupBox('quality_definitions_params','quality_box_')">
	</cf_box_footer>
</cfform>
...
function add_row_quality_type()
{
	...
	newCell = newRow.insertCell(newRow.cells.length);
	newCell.innerHTML = '<div class="form-group"><select name="quality_type_id'+row_count_qt+'" id="quality_type_id'+row_count_qt+'" onchange="change_process_cat('+row_count_qt+',this.value);"><option value=""><cf_get_lang dictionary_id="57734.Seçiniz"></option><cfoutput query="get_quality_control_type"><option value="#type_id#">#quality_control_type#</option></cfoutput></select></div>';
	newCell = newRow.insertCell(newRow.cells.length);
	newCell.innerHTML = '<div class="form-group"><select name="process_cat'+row_count_qt+'" id="process_cat'+row_count_qt+'"><option value=""><cf_get_lang dictionary_id="57734.Seçiniz"></option><option value="76">Mal Alım İrsaliyesi</option><option value="171">Üretim Sonucu</option><option value="811">İthal Mal Girişi</option><option value="-2"><cf_get_lang dictionary_id="37158.Servisler"></option></select></div>';
}
...
function change_process_cat(row,x)
{
	if(list_getat(x,2,';') != "" && list_getat(x,3,';') != "" && list_getat(x,4,';') != "")
		document.getElementById("process_cat"+row).value = list_getat(x,2,';');
}
