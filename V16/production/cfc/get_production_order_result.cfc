<cfscript>
    date_add= CreateObject("component","WMO.functions").date_add;
</cfscript>
<cffunction name="get_po_det_fnc" returntype="query" displayname="Üretim Sonuçlarını Listeler">
    ...
    <cfargument name="lotno" default="" displayname="Lot No'ya Göre Arama Yapıyor">
     ...
     WITH CTE1 AS (
        SELECT
            ...
            <cfif isdefined('arguments.position_code') and len(arguments.position_code) and isdefined('arguments.position_name') and len(arguments.position_name)>
                                            AND PRODUCT.PRODUCT_CATID IN 
                                            (SELECT 
                                                PC1.PRODUCT_CATID
                                            FROM 
                                                PRODUCT_CAT PC1
                                                JOIN #this.dsn1_alias#.PRODUCT_CAT_POSITIONS PC2 ON PC2.PRODUCT_CAT_ID = PC1.PRODUCT_CATID
                                            WHERE 
            ...
            ---> 
                        ...
                        <cfif isdefined('arguments.lotno') and len(arguments.lotno)>
               AND(
                                (PRODUCTION_ORDER_RESULTS.LOT_NO LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#arguments.lotno#%">)
            OR
            (PRODUCTION_ORDER_RESULTS.PR_ORDER_ID IN (SELECT PORR.PR_ORDER_ID FROM PRODUCTION_ORDER_RESULTS_ROW PORR WHERE PORR.LOT_NO LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#arguments.lotno#%">))
              )
                        </cfif>
            <cfif isdefined('arguments.station_id') and len(arguments.station_id)>
            AND(
                                (PRODUCTION_ORDER_RESULTS.STATION_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.station_id#">)
                                OR
                                (PRODUCTION_ORDER_RESULTS.STATION_ID IN (<cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.station_list#" list="yes">))
                                )
            </cfif>
            ...
            SELECT SF.PROD_ORDER_RESULT_NUMBER FROM #this.dsn#_#get_period.PERIOD_YEAR#_#get_period.OUR_COMPANY_ID#.STOCK_FIS SF WHERE SF.PROD_ORDER_RESULT_NUMBER = PRODUCTION_ORDER_RESULTS.PR_ORDER_ID
            ...
            AND PRODUCTION_ORDERS.RECORD_DATE < #DATE_ADD('d',1,arguments.finish_date)#
                     ...
                     CTE2 AS (
                            SELECT
                                CTE1.*,
                                ROW_NUMBER() OVER ( ORDER BY PR_ORDER_ID DESC ) AS RowNum,(SELECT COUNT(*) FROM CTE1) AS QUERY_COUNT
                            FROM
                                CTE1
                        )
                        SELECT
