...
<cfquery name="UPDTEMPLATE" datasource="#DSN#">
	UPDATE 
	  	TEMPLATE_FORMS
	SET 
		TEMPLATE_HEAD    = <cfqueryparam cfsqltype="cf_sql_varchar" value="#attributes.template_head#">,
		TEMPLATE_MODULE  = #attributes.module#,
		IS_LOGO = <cfif isdefined('attributes.is_logo')>1<cfelse>0</cfif>,
		TEMPLATE_CONTENT = <cfqueryparam cfsqltype="cf_sql_varchar" value="#cont#">,
        IS_PURSUIT_TEMPLATE = <cfif not isDefined("attributes.is_pursuit_template")>0<cfelse>1</cfif>,
        IS_LICENCE = <cfif isdefined('attributes.is_licence')>1<cfelse>0</cfif>,
		UPDATE_DATE = #now()#,
...
<cfif isdefined('attributes.is_licence')>
	<cfquery name="UPD_OTHER_LICENCE" datasource="#dsn#">UPDATE TEMPLATE_FORMS SET IS_LICENCE = 0 WHERE TEMPLATE_ID <> <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.template_id#"></cfquery>
</cfif>
