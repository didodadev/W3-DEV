<cfif cgi.referer contains 'form_copy_bskt_detail'>
	<cfset use_basket_id = attributes.MODULE_ID_>
<cfelse>
	<cfset use_basket_id = attributes.basket_id_>
</cfif>

<cfif listfind("1,5,6,7,8,11,15,17,20,31,32,33,35,36,37,39,40,41,42,43,47,49",use_basket_id)>
	<cfset bit_sale_purchase=0>
<cfelseif listfind("2,3,4,10,14,18,21,22,24,25,34,38,46,48,51,52",use_basket_id)>
	<cfset bit_sale_purchase=1>
<cfelseif listfind("12,13,19",use_basket_id)> 
	<cfset bit_sale_purchase=2>
<cfelseif listfind("26,28,29,44,45,50",use_basket_id)> 
...
<cfif int_kdv eq 0 or  int_price eq 0 or not isdefined("attributes.module_content")  or   ( not ListFind(attributes.module_content,'row_otvtotal') or isdefined("int_control_otv"))>
...
<cfif bool_kdv_all eq 0 or not isdefined("attributes.module_content") or not ListFind(attributes.module_content,'OTV') >
...
<script type="text/javascript">
	window.location.href="<cfoutput>#request.self#?fuseaction=settings.form_upd_bskt_detail&id=#use_basket_id#</cfoutput>";
</script>
