...
<cfelse>
	<cfquery name="UPD_GENERAL_PAPERS" datasource="#DSN3#">
		...
		SET 
			...
			ZONE_TYPE =	<cfif isDefined("attributes.zone_type")and len(attributes.zone_type)>#attributes.zone_type#<cfelse>0</cfif>
		WHERE 
			PAPER_TYPE = 0 AND
			ZONE_TYPE = 0
	</cfquery>
...
<cfelse>
	...
	SET
	   ...
	   ORDER_NUMBER = <cfif len(TRIM(attributes.g_order_number)) and isnumeric(attributes.g_order_number)>#attributes.g_order_number#<cfelse>NULL</cfif>,
	   ZONE_TYPE =	<cfif isDefined("attributes.zone_type") and len(attributes.zone_type)>#attributes.zone_type#<cfelse>0</cfif>
	WHERE 
		PAPER_TYPE = 1 AND 
		ZONE_TYPE = 0
