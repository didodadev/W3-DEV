<cffunction name="get_warehouse_rates_fnc" returntype="query">
	...
	<cfquery name="get_warehouse_rates" datasource="#this.DSN3#">
		SELECT 
			WT.*,
			CASE WHEN (C.NICKNAME = '' OR C.NICKNAME IS NULL) THEN C.FULLNAME ELSE C.NICKNAME END AS NICKNAME,
			C2.NICKNAME AS BILL_TO_COMPANY,
		...
		FROM
			WAREHOUSE_RATES WT
				...
				LEFT JOIN  #this.dsn_alias#.COMPANY C2 ON WT.BILL_TO_COMPANY_ID=C2.COMPANY_ID
		WHERE 
