<cfif not isdefined("attributes.row_id")>
	...
	<cffile action="read" file="#GetDirectoryFromPath(GetTemplatePath())#/V16/#folder_##dir_seperator#xml#dir_seperator##faction_#" variable="xmldosyam" charset="UTF-8">
	<cfscript>
		dosyam = XmlParse(xmldosyam);
		xml_dizi = dosyam.SETUP_SITE.XmlChildren;
		d_boyut = ArrayLen(xml_dizi);
	</cfscript>
	<cffile action="read" file="#GetDirectoryFromPath(GetTemplatePath())#/V16/add_options#dir_seperator#xml#dir_seperator##faction_#" variable="xmldosyam2" charset="UTF-8">
	<cfscript>
...
<table width="100%" cellpadding="2" cellspacing="1" border="0" height="100%" class="color-border">
  <tr class="color-list">
    <td height="35" class="headbold" colspan="3"><cf_get_lang no ='2498.Obje'> : <cfoutput>#get_menu_name.menu_name#</cfoutput></td>
  </tr>
  <tr class="color-row">
  	<cfif not isdefined("attributes.row_id")>
	<td width="220" valign="top">
		<div id="cont" style="width:220px;height:520px;z-index:1;overflow:auto;">
			...
			<table width="98%" align="center">
					...
					<tr style="display:none;" id="klasor_#count_#">
						<td><cfloop index="i" from="1" to="#d_boyut#">
								<cfif selectedElements[i].XmlText is '#mmk#'>
									...
									<cfoutput>
										<li><a href="#request.self#?fuseaction=settings.popup_change_object_property&link_xml=#link_xml#&object_folder=#folder_name#&file_name=#file_name#&name=#name#&menu_id=#attributes.menu_id#&faction=#attributes.faction#<cfif isDefined("attributes.is_pda")>&is_pda=1</cfif>" title="#link_detail#" class="tableyazi" target="change_window">#name#</a></li><br>
									</cfoutput>
								</cfif>
							 </cfloop>
						</td>
				...
				<tr>
					<td class="formbold"><cf_get_lang no='1647.Özel Dosyalar'></td>
				</tr>
	...
	<td valign="top" width="460"colspan="2">
		<iframe frameborder="0" src="<cfoutput>#request.self#?fuseaction=settings.popup_change_object_property<cfif isdefined("attributes.row_id")>&row_id=#attributes.row_id#</cfif><cfif isDefined("attributes.is_pda")>&is_pda=1</cfif></cfoutput>" width="100%" height="500" name="change_window" id="change_window" scrolling="auto"></iframe>
	</td>
  </tr>
</table>
<cfif not isdefined("attributes.row_id")>
