...
<cfform name="filter" action="#request.self#?fuseaction=settings.list_site_layouts" method="post">
        ...
        <cf_big_list_search_area>
            ...
            <table>
                <tr>			
                    ...
                    <td><cfinput type="text" name="keyword" id="keyword" value="#attributes.keyword#" maxlength="255"></td>
                    <td>
                    ...
                    <td>
                        <cfinput type="text" name="selected_link" id="selected_link" value="#attributes.selected_link#" style="width:150px;" maxlength="250">
                        <cfoutput><a href="javascript://" onClick="windowopen('#request.self#?fuseaction=settings.popup_list_select_switch&selected_link=filter.selected_link&is_faction=1','list');"><img src="/images/plus_thin.gif"></a></cfoutput>
                    </td>
...
<cf_big_list>
	<thead>
		<tr>
			...
			<th class="header_icn_none"><cfoutput><a href="javascript://" onClick="windowopen('#request.self#?fuseaction=settings.popup_add_site_objects','medium');"><img src="/images/plus_list.gif"></a></cfoutput></th>
	...
	<tbody>
		<cfif get_site_layouts.recordcount>
			...
			<cfoutput query="get_site_layouts" startrow="#attributes.startrow#" maxrows="#attributes.maxrows#">
                <tr>
                    <td><a href="javascript://" onClick="windowopen('#request.self#?fuseaction=settings.popup_select_site_objects&faction=#faction#&menu_id=#menu_id#<cfif get_menu_.site_type[listfind(menu_id_list,get_site_layouts.menu_id,',')] eq 4>&is_pda=1</cfif>','page');" class="tableyazi">#faction#</a></td>
                    <td width="250">
                    	<a href="#request.self#?fuseaction=list_main_menu&event=upd&menu_id=#menu_id#" class="tableyazi"><cfif len(menu_id)>#get_menu_.menu_name[listfind(menu_id_list,get_site_layouts.menu_id,',')]#</cfif></a>			  
                    </td>
                    <!-- sil -->
                    <td><a href="javascript://" onClick="windowopen('#request.self#?fuseaction=settings.popup_select_site_objects&faction=#faction#&menu_id=#GET_SITE_LAYOUTS.menu_id#<cfif get_menu_.site_type[listfind(menu_id_list,get_site_layouts.menu_id,',')] eq 4>&is_pda=1</cfif>','page');"><img src="/images/update_list.gif"></a></td>
                    <!-- sil -->
                </tr>
            </cfoutput>
		<cfelse>
	...
	<table cellpadding="0" cellspacing="0" border="0" width="98%" height="35" align="center">
		<tr>
			<td>
				...
				<cfset adres = "settings.list_site_layouts">
