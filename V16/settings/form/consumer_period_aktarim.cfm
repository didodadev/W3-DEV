...
<cfsavecontent variable = "title">
	<cf_get_lang no ='2072.Bireysel Üye Dönem Aktarımı'><cfif isdefined("attributes.is_transfer_all")>(<cf_get_lang_main no ='576.Geçişli'>)</cfif>
</cfsavecontent>
	...
	<cf_area width="50%">
		<table>
				...
				<tr>
					<td valign="top"> 
						<cftry>
							<cfinclude template="#file_web_path#templates/period_help/consumerPeriodTransfer_#session.ep.language#.html">
							<cfcatch>
								<script type="text/javascript">
	...
	<cftransaction>
        ...
        <cfquery name="aktar_" datasource="#dsn#">
            ...
            SELECT 
                ...
                <cfif fusebox.use_period>
					(SELECT AP.ACCOUNT_CODE FROM #dsn#_#get_hedef_period.period_year#_#get_hedef_period.our_company_id#.ACCOUNT_PLAN AP WHERE AP.ACCOUNT_CODE = CONSUMER_PERIOD.ACCOUNT_CODE),
					(SELECT AP.ACCOUNT_CODE FROM #dsn#_#get_hedef_period.period_year#_#get_hedef_period.our_company_id#.ACCOUNT_PLAN AP WHERE AP.ACCOUNT_CODE = CONSUMER_PERIOD.KONSINYE_CODE),
					(SELECT AP.ACCOUNT_CODE FROM #dsn#_#get_hedef_period.period_year#_#get_hedef_period.our_company_id#.ACCOUNT_PLAN AP WHERE AP.ACCOUNT_CODE = CONSUMER_PERIOD.ADVANCE_PAYMENT_CODE),
					(SELECT AP.ACCOUNT_CODE FROM #dsn#_#get_hedef_period.period_year#_#get_hedef_period.our_company_id#.ACCOUNT_PLAN AP WHERE AP.ACCOUNT_CODE = CONSUMER_PERIOD.SALES_ACCOUNT),
					(SELECT AP.ACCOUNT_CODE FROM #dsn#_#get_hedef_period.period_year#_#get_hedef_period.our_company_id#.ACCOUNT_PLAN AP WHERE AP.ACCOUNT_CODE = CONSUMER_PERIOD.PURCHASE_ACCOUNT),
					(SELECT AP.ACCOUNT_CODE FROM #dsn#_#get_hedef_period.period_year#_#get_hedef_period.our_company_id#.ACCOUNT_PLAN AP WHERE AP.ACCOUNT_CODE = CONSUMER_PERIOD.RECEIVED_GUARANTEE_ACCOUNT),
					(SELECT AP.ACCOUNT_CODE FROM #dsn#_#get_hedef_period.period_year#_#get_hedef_period.our_company_id#.ACCOUNT_PLAN AP WHERE AP.ACCOUNT_CODE = CONSUMER_PERIOD.GIVEN_GUARANTEE_ACCOUNT),
					(SELECT AP.ACCOUNT_CODE FROM #dsn#_#get_hedef_period.period_year#_#get_hedef_period.our_company_id#.ACCOUNT_PLAN AP WHERE AP.ACCOUNT_CODE = CONSUMER_PERIOD.RECEIVED_ADVANCE_ACCOUNT),
					(SELECT AP.ACCOUNT_CODE FROM #dsn#_#get_hedef_period.period_year#_#get_hedef_period.our_company_id#.ACCOUNT_PLAN AP WHERE AP.ACCOUNT_CODE = CONSUMER_PERIOD.EXPORT_REGISTERED_SALES_ACCOUNT),
					(SELECT AP.ACCOUNT_CODE FROM #dsn#_#get_hedef_period.period_year#_#get_hedef_period.our_company_id#.ACCOUNT_PLAN AP WHERE AP.ACCOUNT_CODE = CONSUMER_PERIOD.EXPORT_REGISTERED_BUY_ACCOUNT)
                <cfelse>
					NULL,
					NULL,
					NULL,
            ...
            WHERE
                ...
                <cfif fusebox.use_period>
                ACCOUNT_CODE IN (SELECT AP.ACCOUNT_CODE FROM #dsn#_#get_hedef_period.period_year#_#get_hedef_period.our_company_id#.ACCOUNT_PLAN AP WHERE AP.ACCOUNT_CODE = CONSUMER_PERIOD.ACCOUNT_CODE)
                </cfif>
                AND CONSUMER_ID NOT IN (SELECT CONSUMER_ID FROM CONSUMER_PERIOD CP2 WHERE PERIOD_ID = #attributes.aktarim_hedef_period# AND ACCOUNT_CODE IS NOT NULL)
        </cfquery>
		...
		<cfquery name="get_acc_kontrol" datasource="#dsn#">
			...
			FROM 
				...
				CONSUMER
			WHERE 
				CONSUMER.CONSUMER_ID = CONSUMER_PERIOD.CONSUMER_ID
				AND CONSUMER_PERIOD.PERIOD_ID = #attributes.aktarim_kaynak_period#
				AND ACCOUNT_CODE IS NOT NULL
				<cfif fusebox.use_period>
                AND (SELECT AP.ACCOUNT_CODE FROM #dsn#_#get_hedef_period.period_year#_#get_hedef_period.our_company_id#.ACCOUNT_PLAN AP WHERE AP.ACCOUNT_CODE = CONSUMER_PERIOD.ACCOUNT_CODE) IS NULL
				</cfif>
		</cfquery>
		<!--- eski donemdeki bilgiler, hedef donemde kriterlere uygun kayit yoksa aktarilir --->
		...
		<cfif isdefined("attributes.is_transfer_all")>
			<cfquery name="upd_default" datasource="#dsn#">
				UPDATE CONSUMER SET PERIOD_ID = #attributes.aktarim_hedef_period# WHERE PERIOD_ID = #attributes.aktarim_kaynak_period#
			</cfquery> 
		</cfif>
	</cftransaction>
	</cflock>
