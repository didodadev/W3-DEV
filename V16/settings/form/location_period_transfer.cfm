...
function basamak_1()
	{
	if(confirm("<cf_get_lang dictionary_id='43995.Dönem Yetki Aktarım İşlemi Yapacaksınız!!!Bu İşlem Geri Alınamaz!!!Emin misiniz'>"))
...
<div class="col col-12 col-md-12 col-sm-12 col-xs-12">
	<cf_box title="#getLang('','Depo Dönem Aktarımı','42011')#">
		<form action="" method="post" name="form_">
			<cf_box_elements>
				...
				<div class="col col-6 col-md-6 col-sm-6 col-xs-12" type="column" index="3" sort="true">
					...
					<label class="col col-12 col-md-12 col-sm-12 col-xs-12"><cf_get_lang dictionary_id='62994.Depo lokasyonu detayında tanımlı olan muhasebe kodunun yeni açılan dönemde de tanımlı olmasını sağlar.'><br/>
...
<cfif isdefined("attributes.aktarim_hedef_period")>	
	<cflock name="#CREATEUUID()#" timeout="70">
		<cftransaction>
			...
			<cfquery name="select_" datasource="#dsn#">
				SELECT 
    	            LOCATION_ACCOUNT_ID, 
                    PERIOD_ID
                FROM 
                ...
                WHERE 
                	PERIOD_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.aktarim_hedef_period#">
			</cfquery>
			<cfquery name="del_" datasource="#dsn#">
				DELETE FROM STOCKS_LOCATION_PERIOD WHERE PERIOD_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.aktarim_hedef_period#">
			</cfquery>
			
			<!--- eski donemdeki bilgiler aktarilir --->
			<cfquery name="GET_COLUMN" datasource="#dsn#">
				SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'STOCKS_LOCATION_PERIOD' AND COLUMN_NAME NOT IN ('PERIOD_ID','LOCATION_ACCOUNT_ID')  AND TABLE_SCHEMA = '#dsn#'
			</cfquery>
			
			<cfquery name="ADD_STOCKS_LOCATION_PERIOD" datasource="#dsn#">
				INSERT INTO 
					STOCKS_LOCATION_PERIOD 
				(
					PERIOD_ID,
