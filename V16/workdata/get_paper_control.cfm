...
<cffunction name="get_paper_control" access="public" returnType="query" output="yes">
	...
	<cfelseif get_paper_control.recordcount and arguments.paper_type eq 'TRAVEL_DEMAND' and listlen(arguments.no_value,'-') gt 1>
		<cfset new_number = listlast(arguments.no_value,'-')>
		<cfquery name="upd_paper" datasource="#DSN3#">
			UPDATE GENERAL_PAPERS SET TRAVEL_DEMAND_NUMBER = TRAVEL_DEMAND_NUMBER + 1  WHERE PAPER_TYPE IS NULL AND TRAVEL_DEMAND_NUMBER < #new_number#
		</cfquery>
	<cfelseif get_paper_control.recordcount and arguments.paper_type eq 'SHIP_INTERNAL' and listlen(arguments.no_value,'-') gt 1>
		<cfset new_number = listlast(arguments.no_value,'-')>
		<cfquery name="upd_paper" datasource="#DSN3#">
			UPDATE GENERAL_PAPERS SET SHIP_INTERNAL_NUMBER = SHIP_INTERNAL_NUMBER + 1  WHERE PAPER_TYPE IS NULL AND SHIP_INTERNAL_NUMBER < #new_number#
		</cfquery>
	...
	<cfelseif get_paper_control.recordcount and arguments.paper_type eq 'SHIP' and listlen(arguments.no_value,'-') gt 1>
		<cfset new_number = listlast(arguments.no_value,'-')>
		<cfquery name="kontrol_print" datasource="#dsn3#">
			SELECT * FROM PAPERS_NO WHERE EMPLOYEE_ID = #session.ep.userid# ORDER BY PAPER_ID DESC 
		</cfquery>
		<cfif kontrol_print.recordcount AND len(kontrol_print.SHIP_NUMBER)>
			<cfquery name="upd_paper" datasource="#DSN3#">
				UPDATE PAPERS_NO SET SHIP_NUMBER = SHIP_NUMBER + 1  WHERE EMPLOYEE_ID = #session.ep.userid# AND SHIP_NUMBER < #new_number#
			</cfquery>
		<cfelse>
			<cfquery name="kontrol_print" datasource="#dsn3#">
				SELECT
			...
			<cfif kontrol_print.recordcount>
				<cfquery name="upd_paper" datasource="#DSN3#">
					UPDATE PAPERS_NO SET SHIP_NUMBER = SHIP_NUMBER + 1 WHERE PAPER_ID=#kontrol_print.PAPER_ID# AND SHIP_NUMBER < #new_number#
				</cfquery>
			</cfif>	
		</cfif>	
	</cfif>
