<cfcomponent>
    ...
    <cffunction name="getComponentFunction">
		...
		<cfargument name="extra_params" type="any" required="no">
		<cfargument name="condition" type="any" required="no">
		...
		<cfquery name="GET_QUERY" datasource="#arguments.data_source#">
			SELECT 
				...
				<cfif len(arguments.extra_params)>
				,#UCase(arguments.extra_params)#
				</cfif>
			FROM 
				#UCase(arguments.table_name)#				
			<cfif len(arguments.condition)>
				WHERE 
					#UCase(arguments.condition)#
			</cfif>
			ORDER BY 
