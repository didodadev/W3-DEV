				...
				<tr class="color-row" style="vertical-align:top;">
			  		<td>
				 		<table align="center" cellpadding="0" cellspacing="0" border="0" style="width:98%">
						    <tr>
								<td colspan="2">
									<cfform name="emp_add_training" method="post" action="#request.self#?fuseaction=objects2.popup_add_edu_info">
									...
									<table border="0">
										...
										<tr>
											<td >&nbsp;</td>
											<td colspan="3"  style="text-align:right;"><input type="button" value="<cfif isdefined("attributes.ctrl_edu") and len(attributes.ctrl_edu) and attributes.ctrl_edu eq 1><cf_get_lang_main no='52.Güncelle'><cfelse><cf_get_lang_main no ='170.Ekle'></cfif>" onClick="gonder_training();"></td>
										</tr>
								 	</table> 
									</cfform>
							    </td>
...
<script type="text/javascript">
	...
	{   
		if(control()==true)
		{
			...
			if(document.getElementById('is_edu_continue').checked == true)
				is_edu_continue = 1;
			else
				is_edu_continue= 0;
			opener.gonder_edu(edu_type,ctrl_edu,count_edu,document.getElementById('edu_id').value,document.getElementById('edu_name').value,document.getElementById('edu_start').value,document.getElementById('edu_finish').value,document.getElementById('edu_rank').value,document.getElementById('edu_high_part_id').value,edu_part_id,document.getElementById('edu_part_name').value,is_edu_continue);
			window.close();
		}
		else
		{
