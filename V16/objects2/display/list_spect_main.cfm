        ...
        <tr>
        	<td colspan="2">
                <table align="right">
                    <tr>
                        <td>
                            <select name="spect_status" id="spect_status" style="width:50px;">
...
<table cellspacing="0" cellpadding="0" border="0" align="center" style="width:98%;">
	<thead>
        <tr class="color-header" style="height:22px;">
            ...
            <td class="form-title">Spect ID</td>
			...
			<cfif not isdefined('session.pp')>
                <th style="width:110px;"><cf_get_lang_main no ='487.Kaydeden'></th>
                <th style="width:60px;"><cf_get_lang_main no ='330.Tarih'></th>
                <!-- sil -->
                <!---<th style="width:15px;">
...
<cf_paging page="#attributes.page#" maxrows="#attributes.maxrows#" totalrecords="#attributes.totalrecords#" startrow="#attributes.startrow#" adres="#attributes.fuseaction##url_str#&keyword=#attributes.keyword#">
...
<script type="text/javascript">
	...
	{
		document.getElementById('spect_main_name').value=name_;//spect main name
		document.getElementById('spect_main_id').value=main_id;//spect main id
	...
	function go_page(type)
	{
		if(type!=1)
		{
			window.location.href='<cfoutput>#request.self#?fuseaction=objects2.popup_list_spect&only_list=1<cfif isdefined("attributes.field_main_id") and len(attributes.field_main_id)>&field_main_id=#attributes.field_main_id#</cfif><cfif isdefined("attributes.field_name") and len(attributes.field_name)>&field_name=#attributes.field_name#</cfif><cfif isdefined("attributes.stock_id") and len(attributes.stock_id)>&stock_id=#attributes.stock_id#</cfif></cfoutput>';
		}
	}
</script>
