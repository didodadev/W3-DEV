...
<cfif isdefined('session.pp.userid')>
	...
	<cfform name="change_pass" action="#request.self#?fuseaction=objects2.emptypopup_add_change_cons_pass" method="post">
		<table border="0">
        	<input type="hidden" name="mail_address" id="mail_address" value="<cfoutput>#get_username.company_partner_email#</cfoutput>" />
			<input type="hidden" name="partner_id" id="partner_id" value="<cfoutput>#get_username.partner_id#</cfoutput>" />
			<tr>
				<td style="width:60%"><cf_get_lang_main no='146.Üye No'></td>
				<td><cfinput type="text" name="member_code" id="member_code" value="#get_username.member_code#" maxlength="16" readonly="yes"></td>
			</tr>
			<tr>
				<td><cf_get_lang_main no='139.Kullanıcı Adı'></td>
				<td><cfinput type="text" name="username" id="username" value="#get_username.company_partner_username#" maxlength="16" readonly="yes"></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					<cf_workcube_buttons is_upd='0' insert_info='#update_btn#' is_cancel='0'>
					<!---<input type="button" name="gonder" id="gonder" value="<cf_get_lang_main no='52.Güncelle'>" onClick="return kontrol()"/>--->
				</td>
			</tr>
			<tr>
...
<cfif isDefined('attributes.is_mail_link') and attributes.is_mail_link eq 1>
	<cfif isdefined('url.ma') and len(url.ma)>
        ...
        <cfform name="change_pass" action="#request.self#?fuseaction=objects2.emptypopup_add_change_cons_pass" method="post">
        	<table border="0" style="margin-top:30px; width:55%;">
                ...
                <input type="hidden" name="mt" id="mt" value="<cfoutput>#attributes.mt#</cfoutput>" />
                <tr>
                    <td>&nbsp;<div class="uyegirisi_1" style="width:200px;"><span style=" text-align:left"><cf_get_lang_main no='139.Kullanıcı Adı'></span></div></td>
                    <td><cfinput type="text" name="username" id="username" value="#get_username.member_username#" maxlength="16" readonly="yes" class="txt_1"></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="button" class="btn_1" name="gonder" id="gonder" value="<cf_get_lang_main no='52.Güncelle'>" onClick="return kontrol()"/>
                    </td>
                </tr>
                <tr>
                    <td>&nbsp;</td>
    ...
    <cfelse>
        <script>
            alert("<cf_get_lang no='120.Yetkidışı Erişim'>");
            history.back(-1);
        </script>
        <cfabort>
    </cfif>
...
<script type="text/javascript">
	function kontrol()
	{
		...
		{
			alert("<cf_get_lang no='112.Şifre En Az 6 Karakterli ve Sadece Rakamlardan Oluşmalıdır'>");
			return false;
		}
		if(document.getElementById('new_password').value != document.getElementById('new_password2').value)
		{
			alert("<cf_get_lang no='111.Lütfen her iki şifreyide aynı giriniz'>.");
			return false;
		}	
		AjaxFormSubmit('change_pass','show_info',1,'<cf_get_lang_main no="1926.Güncelleniyor">','<cf_get_lang_main no="1927.Güncellendi">');
