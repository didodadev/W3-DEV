...
<cfquery name="GET_ORDER_ASSET" datasource="#DSN#">
	...
	WHERE
    	<cfif isdefined('session.pp.userid')>
	    	A.COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.pp.our_company_id#"> AND
		<cfelseif isDefined('session.ww.userid')>
	    	A.COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.ww.our_company_id#"> AND       
        </cfif>
		A.ASSETCAT_ID = ASSET_CAT.ASSETCAT_ID AND
		A.ACTION_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.order_id#"> AND
		A.ACTION_SECTION = 'ORDER_ID' AND
...
<cfset action_section = 'ORDER_ID'>
<cfset action_id = attributes.order_id>
<table border="0" width="98%" class="color-border" cellpadding="2" cellspacing="1">
	<tr class="color-header" height="22">
		...
		<td align="right" style="text-align:right;"><a href="javascript://" onclick="windowopen('<cfoutput>#request.self#?fuseaction=objects2.popup_add_asset&action_id=#attributes.order_id#&action_section=#action_section#&module_id=11&asset_cat_id=-12&module=salespur<cfif isdefined('attributes.is_file_upload_size')>&is_file_upload_size=#attributes.is_file_upload_size#</cfif></cfoutput>','small')"><img src="/images/pod_add.gif" title="" border="0" align="absmiddle"></a></td>
	</tr>
