<cfif isDefined('attributes.total_record_count') and len(attributes.total_record_count)>
	<cfset maxrows = attributes.total_record_count>
</cfif>

<cfif isdefined("attributes.paging_count") and isnumeric(attributes.paging_count)>
	<cfset main_max = attributes.paging_count>
<cfelse>
	<cfset main_max = 5>
</cfif>

<cfif isDefined('attributes.total_record_count') and len(attributes.total_record_count)>
    <cfquery name="GET_CONTENT" datasource="#DSN#" maxrows="#maxrows#">
        SELECT
            C.CONTENT_ID,
            C.CONT_HEAD,
            C.CONT_BODY,
...
<cfset attributes.totalrecords = get_content.recordcount>
...
<table align="center" cellpadding="2" cellspacing="2" border="0" style="width:100%">
	<cfif get_content.recordcount>
		<cfoutput query="get_content" startrow="#attributes.startrow#" maxrows="#attributes.max#" group="#group_#">
            ...
            <td style="vertical-align:top;" class="color-list">
                <table class="color-inner-table">
                    <cfif isdefined("attributes.is_content_main_image") and attributes.is_content_main_image neq 0>
                        <cfquery name="GET_IMAGE_CONT" datasource="#DSN#" maxrows="1">
                            ...
                            WHERE 
                                ...
                                <cfif attributes.is_content_main_image eq 3>
                                    AND IMAGE_SIZE = 2
                                <cfelseif  attributes.is_content_main_image eq 2>
                                    AND IMAGE_SIZE = 1
                                <cfelseif  attributes.is_content_main_image eq 1>
                                    AND IMAGE_SIZE = 0
                                </cfif>
                        </cfquery>
                    <cfelse>
                        <cfset get_image_cont.recordcount = 0>
                    </cfif>
                    <!--- Imaj poziyon ust ve yan--->
                    <cfif get_image_cont.recordcount and isdefined("attributes.is_content_main_image") and attributes.is_content_main_image neq 0 and isdefined('attributes.is_main_image_position') and listfind('0,1',attributes.is_main_image_position)>
                        <tr>
                            <td colspan="2" <cfif attributes.is_main_image_position eq 0>style="width:#my_width_#%;"<cfelseif attributes.is_main_image_position eq 1>rowspan="4" style="width:75px; vertical-align:top;"</cfif>>
                    ...
                    <!--- Imaj poziyon orta--->
                    <cfif get_image_cont.recordcount and isdefined('attributes.is_main_image_position') and (attributes.is_main_image_position eq 2)>
                    <tr>
                        <td colspan="2" style="width:#my_width_#%;">
                            <cfif len(main_image_width) or len(main_image_height)>
                                <cf_get_server_file output_file="content/#get_image_cont.contimage_small#" output_server="#get_image_cont.image_server_id#" image_width="#main_image_width#" image_height="#main_image_height#" output_type="0" title="#cont_head#" alt="#cont_head#">
                            <cfelse>
                                <cf_get_server_file output_file="content/#get_image_cont.contimage_small#" output_server="#get_image_cont.image_server_id#" output_type="0" title="#cont_head#" alt="#cont_head#">
                    ...
                    <cfif isdefined("attributes.is_content_continue") and attributes.is_content_continue neq 0>
                        <tr> 
                        	<td align="right">
                            	<cfif attributes.is_content_continue eq 2 and isDefined('attributes.continue_buton') and len(attributes.continue_buton)>
                                	<cfinclude template="#attributes.continue_buton#">
    							<cfelse>
									<a href="#url_friendly_request('objects2.detail_content&cid=#content_id#','#user_friendly_url#')#" class="tableyazi"> <cf_get_lang_main no='714.Devam'></a>                                
                                </cfif>
                            </td>
                            <cfif isdefined('attributes.is_content_record_date') and attributes.is_content_record_date eq 1>
                                <td style="text-align:right;width:30%;"><font class="txtbold"><cf_get_lang no ='784.Yayın Tarihi'> : </font>#dateformat(record_date,'dd/mm/yyyy')#<br></td>
                            <cfelse>
                                <td></td>
                            </cfif>
                        </tr>
			...
			<cfif isdefined('attributes.is_content_asset') and attributes.is_content_asset eq 1>
                ...
                <td width="82"><cfif len(get_ass.asset_id)><a href="#url_friendly_request('#employee_domain#/documents/#get_ass.module_name#/#get_ass.asset_file_name#','#get_ass.asset_file_real_name#')#" title="PDF Link" ><img width="33" height="33" border="0" src="../../documents/content/Image/pdf_icon.png" title="<cf_get_lang_main no ='1936.PDF'> <cf_get_lang no ='1669.Link'>" alt="<cf_get_lang_main no ='1936.PDF'> <cf_get_lang no ='1669.Link'>"/></a></cfif></td>
            <cfelse>
                <!--- <td style="width:15px;">&nbsp;</td> --->
            </cfif>
            <cfif ((currentrow mod attributes.content_main_mode eq 0)) or (currentrow eq recordcount)>
                </tr>
            </cfif>
...
<cfif isdefined('attributes.is_paging') and attributes.is_paging eq 1>
