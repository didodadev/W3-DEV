<cfquery name="GET_MONTHLY_PAGE_WARNINGS" datasource="#dsn#">
	...
	WHERE
		(
			...
			AND
			SMS_WARNING_DATE < <cfqueryparam cfsqltype="cf_sql_timestamp" value="#DATEADD('m',1,attributes.TO_DAY)#">
			)
			OR
			(
			EMAIL_WARNING_DATE >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#attributes.TO_DAY#">
			AND
			EMAIL_WARNING_DATE < <cfqueryparam cfsqltype="cf_sql_timestamp" value="#DATEADD('m',1,attributes.TO_DAY)#">
			)
		)
		AND
		(
