	...
	WHERE
		...
		(
            ...
            <cfif isDefined("session.agenda_user_id")>
                RECORD_AUTHOR = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.agenda_user_id#"> OR
                UPDATE_AUTHOR = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.agenda_user_id#">
            <cfelse>
		...
		(
		 	(
		 		...
		 		TARGET_START < <cfqueryparam cfsqltype="cf_sql_timestamp" value="#DateAdd("ww",1,attributes.to_day)#">
            ) OR
            (
            	TARGET_FINISH >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#attributes.to_day#"> AND
            	TARGET_FINISH < <cfqueryparam cfsqltype="cf_sql_timestamp" value="#DateAdd("ww",1,attributes.to_day)#">
            )
		)
</cfquery>
