	...
	WHERE
		...
		(
		 	(
		 		...
		 		TARGET_START < <cfqueryparam cfsqltype="cf_sql_timestamp" value="#DATEADD("D",1,attributes.to_day)#">
		 	) 
		 	OR
		 	(
		 		TARGET_FINISH >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#attributes.to_day#"> AND
				TARGET_FINISH < <cfqueryparam cfsqltype="cf_sql_timestamp" value="#DATEADD("D",1,attributes.to_day)#">
		 	)
		)
</cfquery>
