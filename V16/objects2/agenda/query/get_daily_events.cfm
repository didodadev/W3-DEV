...
<cfquery name="GET_DAILY_EVENTS" datasource="#DSN#">
	...
	WHERE
		...
		AND
		(
			(EVENT.STARTDATE >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#attributes.to_day#"> AND EVENT.STARTDATE < <cfqueryparam cfsqltype="cf_sql_timestamp" value="#DATEADD("D",1,attributes.to_day)#">)
			OR
			(EVENT.FINISHDATE >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#attributes.to_day#"> AND EVENT.FINISHDATE > <cfqueryparam cfsqltype="cf_sql_timestamp" value="#DATEADD("D",1,attributes.to_day)#">)
		) 
		AND 
		(
		<cfif isDefined("session.agenda_userid")>
		...
		<!--- KENDINDE --->
			...
			<cfelse>	
				...
				EVENT.VALIDATOR_PAR =  <cfqueryparam cfsqltype="cf_sql_integer" value="#session.pp.userid#">
			</cfif>
			<cfloop list="#grps#" index="grp_i">
				OR EVENT.EVENT_TO_GRP LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%,#grp_i#,%">
			</cfloop>
