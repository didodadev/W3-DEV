...
<cfif not GET_PROCESS.recordcount>
	<script type="text/javascript">
		alert("<cf_get_lang no ='33.Islem Tipleri Tanimli Degil! Ltfen Msteri Temsilcinize Basvurun'>.");
		history.back();
	</script>
	<cfabort>
</cfif>
...
<cflock name="#CreateUUID()#" timeout="20">
	...
	<cfquery name="ADD_OFFER" datasource="#DSN3#" result="MAX_ID">
		...
		(
			SALES_PARTNER_ID,
			<cfif len(attributes.member) and ((#listgetat(attributes.member,3,',')# eq 3) or (#listgetat(attributes.member,3,',')# eq 4))>
				PARTNER_ID,
				COMPANY_ID,		
		...
		(
			...
			<cfif len(attributes.member) and ((#listgetat(attributes.member,3,',')# eq 3) or (#listgetat(attributes.member,3,',')# eq 4))><!--- yenin partnermi consumermi oldugunu anlamak iin --->
				#listgetat(attributes.member,1,',')#,
				#listgetat(attributes.member,2,',')#,		
			<cfelseif len(attributes.member) and (#listgetat(attributes.member,3,',')#) eq 5>
				#listgetat(attributes.member,1,',')#,
			</cfif>
			1,
...
<script type="text/javascript">
	alert("<cf_get_lang no ='1430.Teklif Talebiniz Basariyla Alinmistir Sizinle En Kisa Zamanda Temasa Geilecektir'>!");
