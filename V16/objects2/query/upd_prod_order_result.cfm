...
<cfelse>
	<cfset session_userid = session.pp.userid>
	<cfset session_money = session.pp.money>
	<cfset session_money2 = session.pp.money2>
	<cfset session_our_company_info_spect_type = session.pp.our_company_info.spect_type>
	<cfset session_period_year = session.pp.period_year>
	<cfset session_period_id = session.pp.period_id>
	<!---<cfset session_our_company_info_is_cost = session.pp.our_company_info.is_cost>--->
</cfif>
...
<!---<cfif (isdefined("attributes.is_delstock") and Len(attributes.is_delstock)) and not len(attributes.finish_date) or year(attributes.finish_date) neq session_period_year>
	<script type="text/javascript">
		alert("Üretim Sonuç Bitiş Tarihi Döneminiz ile Aynı Değil Stok Fişlerini Silemezsiniz!");
		history.go(-1);
	</script>
	<cfabort>
</cfif>--->
...
<cfif not form.del_pr_order_id eq 0>
    ...
    <cfquery name="UPD_PROD_ORDERS" datasource="#DSN3#">
        UPDATE PRODUCTION_ORDERS SET IS_STAGE = 0,IS_GROUP_LOT = 0 WHERE P_ORDER_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#form.p_order_id#">
    </cfquery>
	<cfif session_our_company_info_is_cost eq 1><!--- sirket maliyet takip ediliyorsa not js le yonlenioyr cunku cost_action locationda calismiyor --->
		...
		<script type="text/javascript">
			<cfif isDefined("session.ep")><!--- Pda de Gelmemesi Icin --->
				window.location.href="<cfoutput>#request.self#?fuseaction=prod.form_upd_prod_order&upd=#attributes.p_order_id#</cfoutput>";
			<cfelse>
	...
	<cfelse>
		<cfif isDefined("session.ep")><!--- Pda de Gelmemesi Icin --->
			<cflocation url="#request.self#?fuseaction=prod.form_upd_prod_order&upd=#attributes.p_order_id#" addtoken="no">
		<cfelse>
...
<cfelse>
	...
	<cflock name="#CreateUUID()#" timeout="60">
	 <cftransaction>
		...
		<cfif isdefined("attributes.is_delstock") and Len(attributes.is_delstock)>
            <cfquery name="UPD_PROD_ORDERS" datasource="#DSN3#">
                UPDATE PRODUCTION_ORDERS SET IS_STAGE = 1 WHERE P_ORDER_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.pr_order_id#"><!--- oluşmuş olan stok fişlerini siliyoruz yani durumu 2 den 1e yani başladı konumuna getiriyoruz. --->
            </cfquery>
			...
			<cfif attributes.record_num eq 1 and attributes.is_changed_spec_main eq 1>
				<cfscript>
					...
					if(isDefined('attributes.cost_price1'))
					{
			...
			<cfif len(attributes.record_num_exit) and attributes.record_num_exit neq "">
			    ...
			    if(isDefined('attributes.cost_price_system_exit#i#'))
			    {
			    ...
			    {
				    form_cost_price_system_exit='';
				    form_purchase_extra_cost_system_exit='';
				    form_purchase_extra_cost_exit='';
				    form_money_system_exit='';
				    form_cost_price_exit='';
				    form_money_exit='';
			    }
				...
				<cfloop from="1" to="#attributes.record_num_outage#" index="i">
					<cfif evaluate("attributes.row_kontrol_outage#i#")>
						<cfscript>
							...
							form_kdv_amount_outage=evaluate("attributes.kdv_amount_outage#i#");
							if(isDefined('attributes.cost_price_system_outage#i#'))
							{
							...
							{
								form_cost_price_system_outage='';
								form_purchase_extra_cost_system_outage='';
								form_purchase_extra_cost_outage='';
								form_money_system_outage='';
								form_cost_price_outage='';
								form_money_outage='';
							}
					...
					<cfif evaluate("attributes.row_kontrol#i#")>
						<cfscript>
							...
							if(isdefined("attributes.amount2_#i#"))
								...
								form_unit_id = '';
							if(isdefined("attributes.unit#i#"))
							...
							if(isDefined('attributes.cost_price#i#'))
							{
            ...
            <cfif len(get_process_type.action_file_name)><!--- secilen islem kategorisine bir action file eklenmisse --->
				<cf_workcube_process_cat 
					...
					action_db_type = '#dsn3#'
					is_action_file = 1
					action_page='#request.self#?fuseaction=prod.upd_prod_order_result&p_order_id=#attributes.p_order_id#&pr_order_id=#attributes.pr_order_id#'
	...
	<cf_workcube_process 
		...
		record_date='#now()#' 
		action_page='#request.self#?fuseaction=prod.upd_prod_order_result&p_order_id=#attributes.p_order_id#&pr_order_id=#attributes.pr_order_id#' 
		action_id='#attributes.p_order_id#'
		action_table='PRODUCTION_ORDER_RESULTS'
		action_column='PR_ORDER_ID'
		old_process_line='#attributes.p_order_id#' 
	...
	<cfif (isdefined("attributes.is_delstock") and Len(attributes.is_delstock)) and session_our_company_info_is_cost eq 1><!--- sirket maliyet takip ediliyorsa not js le yonlenioyr cunku cost_action locationda calismiyor --->
		...
		<cfif isDefined("session.ep")><!--- Pda de Gelmemesi Icin --->
			<script type="text/javascript">
				window.location.href="<cfoutput>#request.self#?fuseaction=prod.upd_prod_order_result&p_order_id=#attributes.p_order_id#&pr_order_id=#attributes.pr_order_id#</cfoutput>";
			</script>
		</cfif>
	<cfelse>
		<!---- <cfif isDefined("session.ep")>---> <!--- Pda de Gelmemesi Icin --->
			<cflocation url="#request.self#?fuseaction=objects2.form_upd_prod_order_result&p_order_id=#attributes.p_order_id#&pr_order_id=#attributes.pr_order_id#" addtoken="no">
		<!--- </cfif> --->
 	</cfif> 
</cfif>
