...
<cfelse>
	<cfset session_userid = session_base.userid>
	<cfset session_money = session_base.money>
	<cfset session_money2 = session_base.money2>
	<cfset session_our_company_info_spect_type = session_base.our_company_info.spect_type> 
</cfif>
...
<cflock name="#CreateUUID()#" timeout="60">
	...
	<cfstoredproc procedure="ADD_PRODUCTION_ORDER_RESULT" datasource="#DSN3#">
            ...
            <cfprocparam cfsqltype="cf_sql_bit" value="0"><!--- WRK#round(rand()*65)##dateformat(now(),'YYYYMMDD')##timeformat(now(),'HHmmssL')##session_base.userid##round(rand()*100)# --->
            <cfprocparam cfsqltype="cf_sql_varchar" value="167">
        </cfstoredproc>
            ...
            <cfloop from="1" to="#attributes.record_num_exit#" index="st">
                <cfif evaluate("attributes.row_kontrol_exit#st#")>
                    ...
                    <!---<cfif isDefined('session.ep.userid')>--->
                        <cfset wrk_row_id_sarf = 'WRK#round(rand()*65)##dateformat(now(),'YYYYMMDD')##timeformat(now(),'HHmmssL')##session.pp.userid##round(rand()*100)##get_max2.max_id##form_stock_id_exit#_2'>
	...
	<cfif len(attributes.record_num_outage) and attributes.record_num_outage neq "">
		<cfloop from="1" to="#attributes.record_num_outage#" index="i">
			<cfif evaluate("attributes.row_kontrol_outage#i#")>
                ...
                <!---<cfif isDefined('session.ep.userid')>--->
					<cfset wrk_row_id_fire = 'WRK#round(rand()*65)##dateformat(now(),'YYYYMMDD')##timeformat(now(),'HHmmssL')##session.pp.userid##round(rand()*100)##get_max2.max_id##form_stock_id_outage#_3'>
				<!---<cfelseif isDefined('session.pda.userid')>
	...
	<cfquery name="GET_PROD_ORDER" datasource="#DSN3#">
		SELECT STOCK_ID FROM PRODUCTION_ORDERS WHERE P_ORDER_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.p_order_id#">
	</cfquery> 
	<cfif len(attributes.record_num) and attributes.record_num neq "">
		...
		<cfloop from="1" to="#attributes.record_num#" index="i">
			<cfif evaluate("attributes.row_kontrol#i#")>
                ...
                <!---<cfif isDefined('session.ep.userid')>--->
					<cfset wrk_row_id_ = 'WRK#round(rand()*65)##dateformat(now(),'YYYYMMDD')##timeformat(now(),'HHmmssL')##session.pp.userid##round(rand()*100)##get_max2.max_id##form_stock_id#_1'>
				<!---<cfelseif isDefined('session.pda.userid')>
	...
	<cfquery name="GET_RESULT_AMOUNT" datasource="#DSN3#">
     ...
     <cfquery name="UPD_PROD_ORDERS" datasource="#DSN3#">
        UPDATE PRODUCTION_ORDERS SET IS_STAGE = 1,RESULT_AMOUNT=#get_result_amount.RESULT_AMOUNT# WHERE P_ORDER_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.p_order_id#"><!--- 1 ATIYORUZ ÇÜNKÜ ÜRETİM BAŞLAMIŞ OLUYOR. --->
    </cfquery>
...
<!---<cfif isDefined("session.ep")>---><!--- Pda de Gelmemesi Icin --->
	<cflocation url="#request.self#?fuseaction=objects2.form_upd_prod_order_result&p_order_id=#attributes.p_order_id#&pr_order_id=#get_max2.max_id#" addtoken="no">
<!---</cfif>---> 
