...
<cfif isdefined("attributes.action_id") and len(attributes.action_id) and isdefined("attributes.action_section") and action_section eq 'PROJECT_ID'>
	<cfset attributes.action_id=contentEncryptingandDecodingAES(isEncode:0,content:attributes.action_id,accountKey:'wrk')>
</cfif>
<cfset events = getComponent.GET_RELATED_EVENTS(action_id:iif(isdefined('attributes.action_id') and len(attributes.action_id), 'attributes.action_id', ''),action_section:iIf(isdefined('attributes.action_section') and len(attributes.action_section), 'attributes.action_section', ''))>
<cfset par_events = getComponent.PAR_EVENTS(action_id:iif(isdefined('attributes.action_id') and len(attributes.action_id), 'attributes.action_id', ''),action_section:iIf(isdefined('attributes.action_section') and len(attributes.action_section), 'attributes.action_section', ''))>
...
<script>
  $(document).ready(function() {
	...
	$("<div>")   
		...
		<cfif isdefined("attributes.action_section") and len(attributes.action_section) and action_section eq 'PROJECT_ID'>
			<cfset project_head=get_project_name(attributes.action_id)>
				$('<div>').addClass('calendar_head').text("<cf_get_lang dictionary_id='57416.Proje'> : <cfoutput>#left(project_head,50)#</cfoutput>").appendTo("div.fc-left");
			<cfelse>
				$('<div>').addClass('calendar_head').text("<cf_get_lang dictionary_id='62597.Proje Takvimi'>").appendTo("div.fc-left");
		</cfif>
		...
		.append(
			$('<span>')
				.addClass('iconic fa fa-cube')
				.attr({
					onclick :"window.location.href='<cfoutput>#site_language_path#/projectDetail?id=#contentEncryptingandDecodingAES(isEncode:1,content:attributes.action_id,accountKey:"wrk")#</cfoutput>'",
					title		:"<cf_get_lang dictionary_id='57416.Proje'>"
				})
				.text("Proje")
