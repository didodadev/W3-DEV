    ...
    <cffunction name="getList" access="remote" returntype="any" output="no">
        ...
        <cftry>
            <cfquery name="get_list" datasource="#session.databaseName#">
                ...
                FROM
                    REPORT R,
                    #session.defaultDatabaseName#.EMPLOYEES E,
                    #session.defaultDatabaseName#.EMPLOYEE_POSITIONS EP
                WHERE
                	...
                	(
                        ...
                        R.REPORT_ID IN (SELECT REPORT_ID FROM REPORT_PERMISSIONS RP, #session.defaultDatabaseName#.EMPLOYEE_POSITIONS EP WHERE EP.POSITION_CODE = #session.positionCode# AND (RP.POSITION_CODE = EP.POSITION_CODE OR RP.POSITION_CAT_ID = EP.POSITION_CAT_ID)) OR
                        R.RECORD_EMP = #session.userID# /*OR
                        (SELECT ADMIN_STATUS FROM #session.defaultDatabaseName#.EMPLOYEE_POSITIONS WHERE EMPLOYEE_ID = #session.userID#) = 1  --1 'den fazla pozisyonu olanlarda sıkıntılı, ayrıca admin olması raporu görebileceği anlamına gelmemeli */
                    )
