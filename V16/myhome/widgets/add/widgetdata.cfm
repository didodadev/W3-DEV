...
<cftry>
    ...
    <cfscript>
        if(isdefined("start_hour") and len(start_hour))
        attributes.startdate = date_add('h', start_hour, attributes.startdate);
        if(isdefined("start_hour_") and len(start_hour_))
        attributes.startdate = date_add('h', start_hour_, attributes.startdate);
        if(len(start_min))
        attributes.startdate = date_add('n', start_min, attributes.startdate);
        if(isdefined("end_hour") and len(end_hour))
        attributes.finishdate = date_add('h', end_hour, attributes.finishdate);
        if(isdefined("end_hour_") and len(end_hour_))
        attributes.finishdate = date_add('h', end_hour_, attributes.finishdate);
        if(len(end_min))
        attributes.finishdate = date_add('n', end_min, attributes.finishdate);
    </cfscript>   
<cfscript>
    mandate_ref = myhome_employee_mandate_component.mandate_add(attributes.process_stage, attributes.employee_id, attributes.mandate_id, attributes.detail?:"", attributes.startdate?:"", attributes.finishdate?:"");
</cfscript>
