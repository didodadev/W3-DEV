...
<cfquery name="UPD_RESULT" datasource="#dsn#">
	...
	SET
		...
		UPDATE_DATE = #now()#,
		UPDATE_IP = '#CGI.REMOTE_ADDR#'<!--- ,  BURASI BİR KEZ DAHA DÜŞÜNÜLECEK !!!! ASELAM
...
<cfquery name="upd_perform" datasource="#dsn#">
	...
	<cfif isdefined('attributes.IS_VALID') and len(attributes.IS_VALID)>
		...
		OTHER_POSITION_CODE=<cfif len(other_position_code)>#attributes.other_position_code#<cfelse>NULL</cfif>,
		UPDATE_EMP = '#SESSION.EP.USERID#',
