	...
	<cftransaction>
		<cfquery name="UPD_EXPENSE_TOTAL_COST" datasource="#dsn2#">
			...
			SET
				...
				OTHER_MONEY_AMOUNT = <cfif isdefined("attributes.other_total_amount") and len(attributes.other_total_amount)>#attributes.other_total_amount#,<cfelse>NULL,</cfif>
				OTHER_MONEY_KDV = <cfif isdefined("attributes.other_kdv_total_amount") and len(attributes.other_kdv_total_amount)>#attributes.other_kdv_total_amount#,<cfelse>NULL,</cfif>
				OTHER_MONEY_NET_TOTAL = <cfif isdefined("attributes.other_net_total_amount") and len(attributes.other_net_total_amount)>#attributes.other_net_total_amount#,<cfelse>NULL,</cfif>
				OTHER_MONEY = <cfif isdefined("rd_money_value") and len(rd_money_value)>#sql_unicode()#'#rd_money_value#',<cfelse>NULL,</cfif>
				...
				EMP_ID = <cfif len(attributes.expense_employee_id)>#attributes.expense_employee_id#,<cfelse>NULL,</cfif>
				EXPENSE_DATE = <cfif isdefined("attributes.expense_date") and len(attributes.expense_date)>#attributes.expense_date#,<cfelse>NULL,</cfif>
				...
				DETAIL = <cfif isdefined("attributes.detail") and len(attributes.detail)>#sql_unicode()#'#attributes.detail#',<cfelse>NULL,</cfif>
				UPDATE_DATE = #now()#,
				UPDATE_EMP = #session.ep.userid#,
				UPDATE_IP = #sql_unicode()#'#cgi.remote_addr#',
				TOTAL_AMOUNT =<cfif isdefined("attributes.total_amount")> #attributes.total_amount#<cfelse>NULL</cfif>,
				NET_TOTAL_AMOUNT = <cfif isdefined("attributes.kdv_total_amount")>#attributes.kdv_total_amount#<cfelse>NULL</cfif>,
				NET_KDV_AMOUNT = <cfif isdefined("attributes.net_total_amount") and len(attributes.net_total_amount)>#attributes.net_total_amount#<cfelse>NULL</cfif>,
				EXPENSE_STAGE =<cfif isdefined("attributes.process_stage")> #attributes.process_stage#<cfelse>NULL</cfif>,
				...
				DUE_DATE = <cfif isdefined("expense_due_date") and len(expense_due_date)>#expense_due_date#<cfelse>NULL</cfif>,
				EXPENSE_HR_ALLOWANCE = <cfif isdefined("attributes.EXPENSE_HR_ALLOWANCE") and len(attributes.EXPENSE_HR_ALLOWANCE)>#attributes.EXPENSE_HR_ALLOWANCE#<cfelse>NULL</cfif>,
					...
					<cfquery name="ADD_EXPENSE_ROWS" datasource="#dsn2#">
						INSERT INTO
							...
							(
								...
								<cfif isdefined("form_expense_center_id") and len(form_expense_center_id)>#form_expense_center_id#,<cfelse>NULL,</cfif>
								<cfif isdefined("form_expense_item_id") and len(form_expense_item_id)>#form_expense_item_id#,<cfelse>NULL,</cfif>
								<cfif isdefined("attributes.form_row_detail") and len(attributes.expense_paper_type)>#sql_unicode()#'#attributes.expense_paper_type#',<cfelse>NULL,</cfif>
								<cfif isdefined("form_expense_center_id") and len(form_row_detail)>#sql_unicode()#'#form_row_detail#',<cfelse>NULL,</cfif>
								...
								0,
								0,
								<cfif isdefined("attributes.employee") and len(attributes.employee) and len(attributes.expense_employee_id)>#attributes.expense_employee_id#,<cfelse>NULL,</cfif>
								<cfif form_member_type is 'partner'>
									<cfif isdefined("form_company") and len(form_company) and len(form_authorized) and len(form_company_id)>#form_company_id#,<cfelse>NULL,</cfif>
									<cfif isdefined("form_company") and len(form_company) and len(form_authorized) and len(form_member_id)>#form_member_id#,<cfelse>NULL,</cfif>
									<cfif isdefined("form_company") and len(form_company) and len(form_authorized) and len(form_member_type)>#sql_unicode()#'#form_member_type#',<cfelse>NULL,</cfif>
								<cfelse>
									NULL,
									<cfif isdefined("form_authorized") and len(form_authorized) and len(form_member_id)>#form_member_id#,<cfelse>NULL,</cfif>
									<cfif isdefined("form_authorized") and len(form_authorized) and len(form_member_type)>#sql_unicode()#'#form_member_type#',<cfelse>NULL,</cfif>
								</cfif>
								<cfif isdefined("form_asset") and len(form_asset) and len(form_asset_id)>#form_asset_id#,<cfelse>NULL,</cfif>
								<cfif isdefined("form_project") and len(form_project) and len(form_project_id)>#form_project_id#,<cfelse>NULL,</cfif>
								<cfif isdefined("form_activity_type") and len(form_activity_type)>#form_activity_type#,<cfelse>NULL,</cfif>
								<cfif isdefined("form_total") and len(form_total)>#sql_unicode()#'#form_total#',<cfelse>NULL,</cfif>
								<cfif isdefined("form_kdv_total") and len(form_kdv_total)>#form_kdv_total#,<cfelse>NULL,</cfif>
								<cfif isdefined("form_net_total") and len(form_net_total)>#form_net_total#,<cfelse>NULL,</cfif>
								<cfif isdefined("form_money_1") and len(form_money_1)>#sql_unicode()#'#form_money_1#',<cfelse>NULL,</cfif>
								<cfif isdefined("form_money_2") and len(form_money_2)>#form_money_2#,<cfelse>NULL,</cfif>
								<cfif isdefined("form_money_3") and len(form_money_3)>#form_money_3#,<cfelse>NULL,</cfif>
								<cfif isdefined("form_tax_rate") and len(form_tax_rate)>#sql_unicode()#'#form_tax_rate#',<cfelse>NULL,</cfif>
								<cfif isdefined("form_other_net_total") and len(form_other_net_total)>#form_other_net_total#,<cfelse>NULL,</cfif>
								1,
								<cfif isdefined("form_product_id") and len(form_product_id) and len(form_product_name)>#form_product_id#<cfelse>NULL</cfif>,
								<cfif isdefined("form_stock_id") and len(form_stock_id) and len(form_product_name)>#form_stock_id#<cfelse>NULL</cfif>,
								<cfif isdefined("form_product_name") and len(form_product_name)>#sql_unicode()#'#left(form_product_name,40)#'<cfelse>NULL</cfif>,
								<cfif isdefined("form_quantity") and len(form_quantity)>#form_quantity#<cfelse>1</cfif>,
								<cfif isdefined("form_stock_unit") and len(form_stock_unit)>#sql_unicode()#'#form_stock_unit#'<cfelse>NULL</cfif>,
								<cfif isdefined("form_stock_unit_id") and len(form_stock_unit_id)>#form_stock_unit_id#<cfelse>NULL</cfif>,
								#session.ep.userid#,
								...
								#now()#,
                                <cfif isdefined("allowance_expense_type") and len(allowance_expense_type)><cfqueryparam cfsqltype="cf_sql_integer" value="#allowance_expense_type#"><cfelse>NULL</cfif>,
                                <cfif isdefined("allowance_day") and len(allowance_day)><cfqueryparam cfsqltype="cf_sql_integer" value="#allowance_day#"><cfelse>NULL</cfif>
							)
					</cfquery>
				</cfif>
			</cfloop>
...
<cf_workcube_process 
	...
	<cfif isdefined("attributes.process_cat") and len(attributes.process_cat)>
		<cfquery name="get_type" datasource="#dsn2#">
			SELECT PROCESS_TYPE,PROCESS_CAT_ID,IS_ACCOUNT,ACTION_FILE_NAME,ACTION_FILE_FROM_TEMPLATE FROM #dsn3#.SETUP_PROCESS_CAT WHERE PROCESS_CAT_ID = #attributes.process_cat#
		</cfquery>
...
<cfif attributes.cost_type eq 1>
    ...
    <script type="text/javascript">
        window.location.href = "<cfoutput>#request.self#?fuseaction=myhome.allowance_expense&event=upd&request_id=#attributes.request_id#</cfoutput>";
    </script>
<cfelse>
	<script type="text/javascript">
