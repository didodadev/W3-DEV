...
<cfif isDefined("departure_date") and len(attributes.departure_date)>
    <cf_date tarih="attributes.departure_date">
</cfif>
<cfif isDefined("departure_of_date") and len(attributes.departure_of_date)>
	<cf_date tarih="attributes.departure_of_date">
</cfif>
<cfif isDefined("activity_start_date") and len(attributes.activity_start_date)>
	<cf_date tarih="attributes.activity_start_date">
</cfif>
<cfif isDefined("activity_finish_date") and len(attributes.activity_finish_date)>
	<cf_date tarih="attributes.activity_finish_date">		
</cfif>
<cfif isDefined("flight_departure_date") and len(attributes.flight_departure_date)>	
	<cf_date tarih="attributes.flight_departure_date">
</cfif>
<cfif isDefined("flight_departure_of_date") and len(attributes.flight_departure_of_date)>
	<cf_date tarih="attributes.flight_departure_of_date">
</cfif>
<cfscript>	
	get_demands = createObject("component","V16.myhome.cfc.get_travel_demands");
	get_travel_demands = get_demands.travel_demands(travel_demand_id : attributes.travel_demand_id);
	if ((len(get_travel_demands.manager2_valid) and get_travel_demands.manager2_valid eq 1) OR (len(get_travel_demands.manager1_valid) and get_travel_demands.manager1_valid eq 1)) {
		upd_travel_demand_valid = get_demands.upd_travel_demand_valid
			(
		...
		upd_travel_demands = get_demands.upd_travel_demand
			(
				travel_demand_id : attributes.travel_demand_id,
				...
				city : sehir,
				is_country:'#IIf(IsDefined("attributes.is_country"),"attributes.is_country",DE(0))#',
...
<cfif listfirst(attributes.fuseaction,'.') is 'myhome'>
	<cfset attributes.demand_id= contentEncryptingandDecodingAES(isEncode:1,content:attributes.travel_demand_id,accountKey:'wrk')>
<cfelse>
	<cfset attributes.demand_id= attributes.travel_demand_id>
</cfif>
<cf_workcube_process 
	is_upd='1' 
	data_source='#dsn#' 
	...
	action_page='#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.list_travel_demands' 
	warning_description='Seyahat Talebi : #attributes.travel_demand_id#'>
