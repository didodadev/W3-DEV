...
<cfquery name="GET_PRO_TYPEROWS" datasource="#DSN#">
	...
	WHERE
		...
		PT.FACTION LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%crm.popup_add_postpone_debit%">
	ORDER BY 
		PTR.LINE_NUMBER
</cfquery>
	...
	<cfscript>
  	...
  	<cfquery name="get_company" datasource="#dsn#">
		...
		DISTINCT
			...
			CDP.COMPANY_DEBIT_POSTPONE_ID COMPANY_DEBIT_POSTPONE_ID,
			CDP.RECORD_DATE RECORD_DATE,
			CDP.PROCESS_CAT PROCESS_CAT,
			CDP.BRANCH_ID BRANCH_ID,
		...
		WHERE 
			...
			C.COMPANY_ID = CDP.COMPANY_ID AND
			CDP.BRANCH_ID = CBR.BRANCH_ID
			...
			<cfif len(attributes.is_active)>AND CDP.IS_ACTIVE = #attributes.is_active#</cfif>
			...
			<cfif len(attributes.branch_id)>AND CBR.BRANCH_ID = #attributes.branch_id#
			<cfelse>
				<cfif get_branch.recordcount>
					AND CBR.BRANCH_ID IN (#valuelist(get_branch.branch_id,',')#)
				</cfif>
			</cfif>
			<cfif len(attributes.pro_rows)>AND CDP.PROCESS_CAT = #attributes.pro_rows#</cfif>
			<cfif len(attributes.start_date)>AND CDP.RECORD_DATE >= #attributes.start_date#</cfif>
			<cfif len(attributes.finish_date)>AND CDP.RECORD_DATE < #DATEADD('d',1,attributes.finish_date)#</cfif>
		ORDER BY
			C.FULLNAME,
			CDP.RECORD_DATE
	</cfquery>
...
<div class="col col-12 col-xs-12">
	<cf_box>
		<cfform name="search_company" method="post" action="#request.self#?fuseaction=crm.list_postpone_debit">
	...
	<cf_box  title="#getLang('','Borç Erteleme Talebi',52188)#" uidrop="1" hide_table_column="1">
		<cf_flat_list>
			<thead>
				<tr>
					...
					<th width="20"><a href="javascript://" onClick="windowopen('<cfoutput>#request.self#?fuseaction=crm.popup_add_postpone_debit&is_page=1</cfoutput>','longpage','popup_add_postpone_debit')"><i class="fa fa-plus" title="<cf_get_lang dictionary_id='57582.Ekle'>"></i></a></th>
				</tr>
			</thead>
			<tbody>
				<cfif get_company.recordcount>
					...
					<cfoutput query="get_company" startrow="#attributes.startrow#" maxrows="#attributes.maxrows#">
						<tr>
							...
							<td width="20"><a href="javascript://" onClick="windowopen('#request.self#?fuseaction=crm.popup_upd_postpone_debit&debit_postpone_id=#COMPANY_DEBIT_POSTPONE_ID#','longpage','popup_upd_postpone_debit')"><i class="fa fa-pencil" title="<cf_get_lang dictionary_id='57464.Güncelle'>" border="0"></a></td>
						</tr>
					</cfoutput>
				<cfelse>
					<tr>
		...
		<cfif attributes.totalrecords gt attributes.maxrows>
			...
			<cf_paging 
				...
				adres="crm.list_postpone_debit#url_str#">
		</cfif>
	</cf_box>
</div>
