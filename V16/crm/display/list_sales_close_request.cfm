...
<cfquery name="GET_PRO_TYPEROWS" datasource="#DSN#">
	...
	WHERE
		...
		PT.FACTION LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%crm.popup_add_company_sale_close_request%">
	ORDER BY 
		PTR.LINE_NUMBER
</cfquery>
	...
	<cfscript>
  	...
  	<cfquery name="get_company" datasource="#dsn#">
		SELECT
			...
			CSCR.SALE_CLOSE_REQUEST_ID SALE_CLOSE_REQUEST_ID,
			CSCR.PROCESS_CAT PROCESS_CAT,
			CSCR.BRANCH_ID BRANCH_ID,
			CSCR.RECORD_DATE RECORD_DATE,
		...
		WHERE 
			...
			<cfif len(attributes.is_active)>AND CSCR.IS_ACTIVE = #attributes.is_active#</cfif>
			...
			<cfif len(attributes.city_id)>AND C.CITY = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.city_id#"></cfif>
			<cfif len(attributes.county_id)>AND C.COUNTY = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.county_id#"></cfif>
			<cfif len(attributes.branch_id)>
				AND CBR.BRANCH_ID =  <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.branch_id#">
			<cfelse>
				AND CBR.BRANCH_ID IN <cfif get_branch.recordcount>(#valuelist(get_branch.branch_id,',')#)<cfelse>0</cfif>
			</cfif>
			<cfif len(attributes.pro_rows)>AND CSCR.PROCESS_CAT = #attributes.pro_rows#</cfif>
			<cfif len(attributes.start_date)>AND CSCR.RECORD_DATE >= #attributes.start_date#</cfif>
			<cfif len(attributes.finish_date)>AND CSCR.RECORD_DATE < #DATEADD('d',1,attributes.finish_date)#</cfif>
		GROUP BY
			C.FULLNAME, 
			C.COMPANY_ID,
			C.COMPANY_TELCODE,
...
<div class="col col-12 col-md-12 col-sm-12 col-xs-12">
	<cf_box>
  		<cfform name="search_company" method="post" action="#request.self#?fuseaction=crm.list_sales_close_request">
			...
			<cf_box_search_detail>
				...
				<div class="col col-3 col-md-4 col-sm-6 col-xs-12" type="column" index="4" sort="true"></div>
			</cf_box_search_detail>
  		</cfform>
	</cf_box>
	<cf_box title="#getLang('','Satışa Kapama Talebi','52001')#" uidrop="1">
		<cf_grid_list>
			<thead>
				<tr>
					<th width="30"><cf_get_lang dictionary_id='57487.No'></th>
					<th width="140"><cf_get_lang dictionary_id='57750.İşyeri Adı'></th>
					<th width="130"><cf_get_lang dictionary_id='57570.Ad Soyad'></th>
					<th width="90"><cf_get_lang dictionary_id='58134.Mikro Bölge Kodu'></th>
					...
					<th width="50"><cf_get_lang dictionary_id='57483.Kayıt'></th>
					<th class="text-center" width="30"><a href="<cfoutput>#request.self#?fuseaction=crm.list_sales_close_request&event=add&is_page=1</cfoutput>"><i class="fa fa-plus" title="<cf_get_lang dictionary_id='57582.Ekle'>"></i></a></th>
				</tr>
			</thead>
			<tbody>
				<cfif get_company.recordcount>
					...
					<tr>
						...
						<td width="50">#dateformat(record_date,dateformat_style)#</td>
						<td><a href="#request.self#?fuseaction=crm.list_sales_close_request&event=upd&sale_request_id=#sale_close_request_id#"><i class="fa fa-pencil" title="<cf_get_lang dictionary_id='57464.Güncelle'>"></a></td>
					</tr>
					</cfoutput>
				</cfif>
			</tbody>
...
<cfif attributes.totalrecords gt attributes.maxrows>
	...
	adres="crm.list_sales_close_request#url_str#">
</cfif>
<!-- sil -->
