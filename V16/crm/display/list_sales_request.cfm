...
<cfquery name="GET_PRO_TYPEROWS" datasource="#DSN#">
	...
	WHERE
		...
		PT.FACTION LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%crm.popup_add_company_sale_request%">
	ORDER BY 
		PTR.LINE_NUMBER
</cfquery>
...
<cfquery name="get_company" datasource="#dsn#">
		...
		WHERE 
			...
			<cfif len(attributes.is_active)>AND CSR.IS_ACTIVE = #attributes.is_active#</cfif>
			...
			<cfelse>
				<cfif get_branch.recordcount>
					AND CSR.BRANCH_ID IN (#valuelist(get_branch.branch_id,',')#)
				</cfif>
			</cfif>
			<cfif len(attributes.pro_rows)>AND CSR.PROCESS_CAT = #attributes.pro_rows#</cfif>
			<cfif len(attributes.start_date)>AND CSR.RECORD_DATE >= #attributes.start_date#</cfif>
			<cfif len(attributes.finish_date)>AND CSR.RECORD_DATE < #DATEADD('d',1,attributes.finish_date)#</cfif>
		ORDER BY
			C.FULLNAME,
			CSR.RECORD_DATE DESC
</cfquery>
<cfelse>
...
<div class="col col-12 col-xs-12">
	<cf_box>
		<cfform name="search_company" method="post" action="#request.self#?fuseaction=crm.list_sales_request">
	...
	<cf_box  title="#getLang('','Satışa Açma Talebi',52000)#" uidrop="1" hide_table_column="1">
		<cf_flat_list>
			<thead>
				<tr>
					...
					<th width="20"><a href="javascript://" onClick="windowopen('<cfoutput>#request.self#?fuseaction=crm.popup_add_company_sale_request&is_page=1</cfoutput>','longpage','popup_add_company_sale_request')"><i class="fa fa-plus" title="<cf_get_lang dictionary_id='57582.Ekle'>"></i></a></th>
				</tr>
			</thead>
			<tbody>
				<cfif get_company.recordcount>
					...
					<cfoutput query="get_company" startrow="#attributes.startrow#" maxrows="#attributes.maxrows#">
						<tr>
							...
							<td width="20"><a href="javascript://" onClick="windowopen('#request.self#?fuseaction=crm.popup_upd_company_sale_request&sale_request_id=#sale_request_id#','longpage','popup_upd_sale_request')"><i class="fa fa-pencil" title="<cf_get_lang dictionary_id='57464.Güncelle'>" border="0"></a></td>
						</tr>
					</cfoutput>
				<cfelse>
					<tr>
		...
		<cfif attributes.totalrecords gt attributes.maxrows>
			...
			<cf_paging 
				...
				adres="crm.list_sales_request#url_str#">
		</cfif>
	</cf_box>
</div>
<!-- sil -->
