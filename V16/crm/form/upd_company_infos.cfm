<cfif not len(attributes.branch_id)>
	<script type="text/javascript">
		alert("<cf_get_lang dictionary_id='33652.Bu Müşteri İle Deponuzun İlişkisi Yok'>!");
...
<cfquery name="GET_BRANCH_RELATED" datasource="#dsn#">
	...
	WHERE 
		...
		COMPANY_BRANCH_RELATED.COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.cpid#"> AND 
...
<cfquery name="GET_CUSTS" datasource="#dsn#">
	SELECT COMPANYCAT_ID, COMPANYCAT FROM COMPANY_CAT WHERE COMPANYCAT_TYPE = <cfqueryparam cfsqltype="cf_sql_integer" value="0">
</cfquery>
...
<cfquery name="GET_SERVICE_INFO" datasource="#dsn#">
	SELECT COMPANY_ID, PC_NUMBER, NET_CONNECTION FROM COMPANY_SERVICE_INFO WHERE COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.cpid#">
</cfquery>
 <cf_box title="#getLang('','müşteri',57457)#" scroll="1" collapsable="1" resize="1" popup_box="#iif(isdefined("attributes.draggable"),1,0)#">
		<cfform name="form_add_company" method="post" action="#request.self#?fuseaction=crm.emptypopup_add_company_infos">
			...
			<input type="hidden" name="related_id" id="related_id" value="<cfoutput>#get_branch_related.related_id#</cfoutput>">
			<input type="hidden" name="is_control" id="is_control" value="1">
			<cfinput name="old_county_id" type="hidden" value="#get_company.county#">
			<cfinput name="old_ims_code" type="hidden" value="#get_company.ims_code_id#">
			<input type="hidden" name="old_branch_id" id="old_branch_id" value="<cfoutput>#attributes.branch_id#</cfoutput>">
		<cf_box_elements>
			<div class="col col-6 col-md-6 col-sm-6 col-xs-12" type="column" index="1" sort="true">
				<div class="form-group">
					...
					<div class="col col-8 col-md-8 col-sm-8 col-xs-12">
						...
						<cfinput type="hidden" name="old_fullname" value="#get_company.fullname#">
					</div>
				</div>			
				<div class="form-group">
					...
					<div class="col col-8 col-md-8 col-sm-8 col-xs-12">
						<select name="durum" id="durum">
							<cfoutput query="get_status">
								<option value="#tr_id#" <cfif get_branch_related.musteridurum eq tr_id>selected</cfif>>#tr_name#</option>
							</cfoutput>
						</select>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col col-8 col-md-8 col-sm-8 col-xs-12">										
						...
						<cfinput name="old_city" type="hidden" value="#get_company.city#">
					</div>
				</div>
				<div class="form-group">
        ...
        <cf_box_footer>
         	 <cfif get_company.ispotantial eq 1>
                <cfsavecontent variable="button_onay"><cf_get_lang_main no='1063.Onayla'></cfsavecontent>
                <cf_workcube_buttons is_upd='0' add_function="kontrol()" insert_info=" #button_onay# " insert_alert='Yaptığınız Değişiklikler Boyutu Etkileyecektir Emin misiniz!'>
            <cfelse>
                <cfsavecontent variable="button_onay"><cf_get_lang_main no='1063.Onayla'></cfsavecontent>
                <cf_workcube_buttons is_upd='1' add_function="kontrol()" is_delete='0' insert_info=" #button_onay# " insert_alert='Yaptığınız Değişiklikler Boyutu Etkileyecektir Emin misiniz!'>
            </cfif>
          </cf_box_footer>
		</cfform>
 </cf_box>
...
function kontrol()
{
	...
	for (var i = 1; i < form_add_company.tax_num.value.length; i++)
	{
		...
		if (check_char < 0)
		{
			alert("<cf_get_lang dictionary_id='31580.Vergi Numarası Sayısal OlmalıdırOlmalıdır'>!");
			return false;
		}
	}
