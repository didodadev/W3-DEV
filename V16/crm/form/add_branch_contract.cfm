...
<table width="100%" cellpadding="2" cellspacing="1" border="0" height="100%" class="color-border">
	<tr class="color-list">
		<td height="35" class="headbold"><cf_get_lang_main no='1556.Anlaşma Ekle'> (<cf_get_lang no='610.Eczane Adı'> : <cfoutput>#get_related.fullname#</cfoutput>)</td>
		<td><a href="javascript://" onClick="windowopen('<cfoutput>#request.self#?fuseaction=crm.popup_list_securefund_info&cpid=#attributes.company_id#</cfoutput>','page');"><img src="/images/notkalem.gif" title="<cf_get_lang no ='847.Müşteri Teminatları'>" border="0"></a></td>
	</tr>
	<tr class="color-row">
		<td valign="top">
		<table>
		...
		<cfif isdefined("attributes.is_normal_form")><input type="hidden" name="is_normal_form" id="is_normal_form" value="1"></cfif>
			...
			<tr>
				<td width="130"><cf_get_lang no='35.Müşteri Tipi'> *</td>
				<td width="175">
			...
			<tr>
				...
				<td>
					...
					<option value=""><cf_get_lang_main no='322.Seçiniz'></option>
						...
						<option value="2"><cf_get_lang no ='851.Reçete Tutarı'></option>
					</select>				
				</td>
			</tr>
			<tr>
...
function kontrol()
{
	...
	{ 
		alert ("<cf_get_lang no ='854.Lütfen Müşteri Tipi Seçiniz'> !");
		return false;
	}		
	...
	if(!date_check(document.branch_contract.start_date,document.branch_contract.finish_date,"<cf_get_lang_main no ='2326.Bitiş Tarihini Kontrol Ediniz'> !",1))
		return false;
	...
	if(value2>value1)
	{
		alert("<cf_get_lang no ='860.Hedeflenen Aylık Ciro Degeri, Aylık Alım Kapasitesinden Büyük Olamaz'> !");
		return false;
	}	
		
	// Eger varsa musteri tipine baglı olarak hedeflenen aylik ciro kontrolu
	if(is_control == 1)
	{
		control_value = (value1/100)*control_rate;
		if(value2>control_value)
		{
			alert('<cf_get_lang no ="861.Hedeflenen Aylık Ciro Degerini Kontrol Ediniz">. %'+control_value +'<cf_get_lang no ="862.Degerini Aşamaz">  !');
			return false;
		}
	}
	...
	{
		alert("<cf_get_lang no ='865.Şubenin Anlaşma Bilgileri Girilmemiş Kontrol Ediniz'> !");
		return false;
	}
