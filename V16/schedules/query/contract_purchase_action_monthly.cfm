...
<CFFUNCTION name="GET_TOTAL_FOR_PRODUCT" >
	...
	<cfquery name="GET_TOTAL_OF_PRODUCT" datasource="#DSN2#">
		...
		AND IR.PURCHASE_SALES=0	
		AND I.INVOICE_DATE >= #date1# 
		AND I.INVOICE_DATE <= #date2# 
	</cfquery>
	<cfreturn GET_TOTAL_OF_PRODUCT.TOTAL>
</CFFUNCTION>

<cfquery datasource="#DSN3#" name="CONTRACT_PURCHASE_ACTION_WORTH">
SELECT
	CONTRACT.COMPANY, CPAW.*
FROM
	CONTRACT,
	CONTRACT_PURCHASE_ACTION_WORTH AS CPAW
WHERE
	OUR_COMPANY_ID = #SESSION.EP.COMPANY_ID#
	AND
	CONTRACT.CONTRACT_ID = CPAW.CONTRACT_ID
	AND
	CONTRACT.STATUS = 1 
	AND
	CPAW.PERIOD = 1 <!--- 1:AYLIK, 2: 3 AYLIK, 3: YILLIK --->
</cfquery>

<cfoutput query="CONTRACT_PURCHASE_ACTION_WORTH">
	...
	<cfif primmiktar GT 0>
		...
		VALUES
			...
			'Ürün <cfif prod_or_prodcat IS 2> Kategorisi</cfif> Aylık ciro primi', 
