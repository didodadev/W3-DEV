...
<cfset wrk_id = dateformat(now(),'YYYYMMDD')&timeformat(now(),'HHmmss')&'_#session.ep.userid#_'&round(rand()*100)>
    ...
    <cfquery name="GET_YIELD_INTEREST_PLAN_ROWS" datasource="#dsn2#">
        SELECT YIELD_ID FROM INTEREST_YIELD_PLAN_ROWS WHERE YIELD_ROWS_ID IN (#attributes.currently_value#)
    </cfquery>
    <cfquery name="GET_YIELD_INTEREST_PLAN" datasource="#dsn2#">
        SELECT YIELD_ID, YIELD_AMOUNT, BANK_ACTION_ID FROM INTEREST_YIELD_PLAN WHERE YIELD_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#GET_YIELD_INTEREST_PLAN_ROWS.YIELD_ID#">
    </cfquery> 
    <cfscript>
        ...
        <cflock name="#createUUID()#" timeout="20">		
            <cftransaction>
                <cftry>
                    <cfquery name="CREATE_BUDGET_PLAN" datasource="#dsn2#" result="MAX_ID">
                        INSERT INTO
                            ...
                            (
                                ...
                                161,
                                109,
                                'VDM-BGP-#GET_YIELD_INTEREST_PLAN.BANK_ACTION_ID#',
                                #attributes.yield_valuation_date#,
                                'VADELİ MEVDUAT REESKONT TAHAKKUKU',
                                #reeskontVal#,
                                0,
                                #reeskontVal#,
                    ...
                    <cfloop from="1" to="#listlen(attributes.currently_value)#" index="i">
                        <cfquery name="GET_YIELD_INTEREST_PLAN_ROWS" datasource="#dsn2#">
                            SELECT YIELD_ID, EXPENSE_ITEM_TAHAKKUK_ID FROM INTEREST_YIELD_PLAN_ROWS WHERE YIELD_ROWS_ID = #listgetat(attributes.currently_value,i,',')#
                        </cfquery>
                        <cfquery name="GET_YIELD_INTEREST_PLAN" datasource="#dsn2#">
                            SELECT IYP.YIELD_ID, IYP.YIELD_AMOUNT, IYP.BANK_ACTION_ID, BA.EXPENSE_CENTER_ID FROM INTEREST_YIELD_PLAN as IYP
                            LEFT JOIN BANK_ACTIONS AS BA ON IYP.BANK_ACTION_ID = BA.ACTION_ID
                            WHERE YIELD_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#GET_YIELD_INTEREST_PLAN_ROWS.YIELD_ID#">
                        </cfquery> 
                        ...
                        <cfquery name="ADD_BUDGET_PLAN_ROW" datasource="#dsn2#" result="LAST_ID">
                            INSERT INTO
                                ...
                                (
                                    ...
                                    #attributes.yield_valuation_date#,
                                    'Getiri Reeskont',
                                    #GET_YIELD_INTEREST_PLAN.expense_center_id#, <!--- Masraf gelir merkezi --->
                        ...
                        <cfquery name="ADD_YIELD_VALUATION" datasource="#dsn2#">
                            INSERT INTO
                                INTEREST_YIELD_VALUATION
                                (
                                    YIELD_ROWS_ID,
                                    YIELD_VALUATION_DATE,
                        ...
                        <cfscript>
                            ...
                            for(j=1;j lte listlen(attributes.currently_value); j=j+1)
                                ...
                                get_yield_id = cfquery(datasource:"#dsn2#", sqlstring:"SELECT YIELD_ID, EXPENSE_ITEM_TAHAKKUK_ID FROM INTEREST_YIELD_PLAN_ROWS WHERE YIELD_ROWS_ID = #listgetat(attributes.currently_value,i,',')#");
                                get_yield_rows = cfquery(datasource:"#dsn2#", sqlstring:"SELECT IYP.YIELD_ID, IYP.YIELD_AMOUNT, IYP.BANK_ACTION_ID, BA.EXPENSE_ITEM_ID FROM INTEREST_YIELD_PLAN as IYP LEFT JOIN BANK_ACTIONS AS BA ON IYP.BANK_ACTION_ID = BA.ACTION_ID WHERE YIELD_ID = #get_yield_id.YIELD_ID#");
                                ...
                                get_acc_code_expense_acc_code = INTEREST_REVENUE.get_acc_code_exp( expense_item_id : get_yield_rows.EXPENSE_ITEM_ID );
                            ...
                            muhasebeci (
                                ...
                                belge_no : 'VDM-BGP-#GET_YIELD_INTEREST_PLAN.BANK_ACTION_ID#',
            ...
            <cftransaction>
                <cftry>
                    ...
                    <cfloop from="1" to="#listlen(attributes.currently_value)#" index="i">
                        ...
                        <cfquery name="GET_YIELD_INTEREST_PLAN" datasource="#dsn2#">
                            SELECT IYP.YIELD_ID, IYP.YIELD_AMOUNT, IYP.BANK_ACTION_ID, BA.EXPENSE_CENTER_ID, BA.PAPER_NO FROM INTEREST_YIELD_PLAN as IYP
                            LEFT JOIN BANK_ACTIONS AS BA ON IYP.BANK_ACTION_ID = BA.ACTION_ID
                            WHERE YIELD_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#GET_YIELD_INTEREST_PLAN_ROWS.YIELD_ID#">
                        </cfquery> 
                        <cfset get_acc_code_expense_tahakkuk_item = INTEREST_REVENUE.get_acc_code_exp( expense_item_id : GET_YIELD_INTEREST_PLAN_ROWS.expense_item_tahakkuk_id )>
                        ...
                        <cfscript>
                            ...
                            butceci(
                                ...
                                expense_center_id : GET_YIELD_INTEREST_PLAN.expense_center_id,
                                expense_item_id : get_yield_rows.EXPENSE_ITEM_ID,
                                detail : 'VADELİ MEVDUAT REESKONT',
                    ...
                    <cfscript>
                        muhasebeci (
                            ...
                            fis_satir_detay: satir_detay_list,
                            fis_detay : UCase(getLang('main',1853)),
                            belge_no : 'VDM-#GET_YIELD_INTEREST_PLAN.BANK_ACTION_ID#',
                            from_branch_id : branch_id_info,
                            to_branch_id : branch_id_info,
                            other_amount_borc : str_other_borc_tutar_list,
                            other_currency_borc : str_other_borc_currency_list,
                ...
                <cfcatch>
                    <script type="text/javascript">
                        alert("Gelirleştirme İşlemi oluşturulurken Bir Hata Oluştu!");
                        history.back();
                    </script>
                    <cfabort>
                </cfcatch>
