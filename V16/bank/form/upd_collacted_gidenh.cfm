...
<!--- toplu giden havale ile ilgili kapama isleminin olup olmadigina bakilir --->
<cfif len(get_action_detail.action_id) and len(get_action_detail.action_type_id)>
	<cfquery name="get_closed" datasource="#new_dsn2#">
		SELECT 
        ...
        WHERE 
            ...
            CARI_CLOSED_ROW.ACTION_TYPE_ID = 25
	</cfquery>
</cfif>
<cfscript>
    ...
    if(getXml.recordcount and getXml.property_value eq 1){
        if( len(get_action_detail.ACTION_EMPLOYEE_ID)){
            ...
            if(((not module_power_user_hr and listfind(hr_type_list,get_action_detail.acc_type_id)) or (not module_power_user_ehesap and listfind(ehesap_type_list,get_action_detail.acc_type_id))) ){
                writeOutput(getlang('','Yetkili Değilsiniz',57532));
                abort;
            }
        }
    }
...
<cfloop index="aaa" from="1" to="#arrayLen(sepet.satir)#">
	...
	<cfif len(sepet.satir[aaa].SPECIAL_DEFINITION_ID)>
		<cfset sepet.satir[aaa].SPECIAL_DEFINITION = '<option value="">#getLang("main",322)#</option>'>
	<cfelse>
		<cfset sepet.satir[aaa].SPECIAL_DEFINITION = '<option value="" selected="selected">#getLang("main",322)#</option>'>
	</cfif>
...
<cfset pageHead = getLang('main',1758)& ' : '& get_action_detail.multi_action_id><!--- Toplu Gelen Havale --->
<cf_catalystHeader>
		...
		<div id="basket_main_div">
			<cfform name="add_process" method="post" action="#request.self#?fuseaction=objects.emptypopup_basket_converter">
				<cf_basket_form id="collacted_gidenh">
					...
					<input type="hidden" id="form_action_address" name="form_action_address" value="<cfoutput>bank.emptypopup_upd_collacted_gidenh</cfoutput>" />
					...
					<cf_box_elements>
						<div class="col col-3 col-md-4 col-sm-6 col-xs-12" type="column" index="1" sort="true">
							<div class="form-group" id="item-action_date">
								...
								<div class="col col-8 col-xs-12">
									...
									<div class="input-group">
										<cfinput type="text" name="action_date" value="#dateformat(get_action_detail.action_date,dateformat_style)#" onBlur="change_money_info('add_process','action_date');" maxlength="10" validate="#validate_style#" required="Yes" message="#message#">
							...
							<div class="form-group" id="item-process_cat">
								...
								<div class="col col-8 col-xs-12">
									<cf_workcube_process_cat process_cat="#get_action_detail.process_cat#" action_db_type="#new_dsn3#" process_db3="#new_dsn3#">                               	
								</div>
							</div>
							<div class="form-group" id="item-to_account_id">
								...
								<div class="col col-8 col-xs-12">
									<cf_wrkBankAccounts call_function='kur_ekle_f_hesapla' selected_value='#get_action_detail.from_account_id#' is_upd='1'>
								</div>
							</div>
						</div>
						<div class="col col-3 col-md-4 col-sm-6 col-xs-12" type="column" index="2" sort="true">
							<cfif session.ep.isBranchAuthorization eq 0>
								<div class="form-group" id="item-to_branch_id">
									...
									<div class="col col-8 col-xs-12">
										<cf_wrkDepartmentBranch fieldId='branch_id' is_branch='1' selected_value='#get_action_detail.from_branch_id#'>
									</div>
								</div>
							</cfif>
							<div class="form-group" id="item-acc_department_id">
								...
								<div class="col col-8 col-xs-12">
									<cf_wrkDepartmentBranch fieldId='acc_department_id' is_department='1' width='250' selected_value='#get_action_detail.department_id#'>
								</div>
							</div>
						</div>
					</cf_box_elements>
						...
						<div class="col col-6 col-xs-12">
							<cfif not(session.ep.our_company_info.is_paper_closer eq 1 and isdefined("get_closed") and get_closed.recordcount neq 0)>
								<cfif isdefined("attributes.puantaj_id") and len(attributes.puantaj_id) and dsn2 eq new_dsn2>
									<cf_workcube_buttons is_upd='1' add_function='control_form()' update_status='#get_action_detail.upd_status#' del_function_for_submit='control_del_form()'
									delete_page_url='#request.self#?fuseaction=bank.form_add_gidenh&event=delMulti&is_virtual=#attributes.is_virtual_puantaj#&puantaj_id=#attributes.puantaj_id#&multi_id=#attributes.multi_id#&old_process_type=#get_action_detail.action_type_id#&active_period=#session.ep.period_id#'>
								<cfelseif (isdefined("attributes.puantaj_id") and len(attributes.puantaj_id))>
									<cf_workcube_buttons is_upd='1' add_function='control_form()' update_status='#get_action_detail.upd_status#' is_delete="0">
								<cfelse>
									<cf_workcube_buttons is_upd='1' add_function='control_form()' update_status='#get_action_detail.upd_status#' del_function_for_submit='control_del_form()'
									delete_page_url='#request.self#?fuseaction=bank.form_add_gidenh&event=delMulti&multi_id=#attributes.multi_id#&old_process_type=#get_action_detail.action_type_id#&active_period=#session.ep.period_id#'>
								</cfif>
							</cfif>
				...
				<cf_basket id="collacted_gidenh_bask">
					<cf_grid_list id="add_period" name="add_period" align="center" cellpadding="0" cellspacing="0" style="table-layout:fixed; width:100%;">
						<thead id="tblBasket">
							<tr>
								...
								<th style="min-width:150px;"><cf_get_lang dictionary_id='57519.Cari Hesap'> *</th>
								<th><cf_get_lang dictionary_id='57880.Belge No'>*</th>
								<th style="min-width:60px;text-align:right;"><cf_get_lang dictionary_id='57673.Tutar'>*</th>
								<th style="text-align:right;"><cf_get_lang dictionary_id='57489.Para Birimi'>*</th>
								<th style="text-align:right;"><cf_get_lang dictionary_id='49016.İşlem Dövizli Tutar'>*</th>
						...
						<tbody id="basketItemBase" style="display:none;">
							<tr ItemRow>
								...
								<td nowrap="nowrap"><div class="form-group"><input type="text" name="action_value_other" id="action_value_other" readonly="readonly" value="" maxlength="20" onkeyup="return(FormatCurrency(this,event));" class="moneybox"></div></td>
								<td><div class="form-group"><input type="hidden" name="deneme" id="deneme" value="" class="box" >
								<select name="money_id" id="money_id"></select></div></td>
								...
								<td nowrap>
									<div class="form-group">
										<div class="input-group">
											...
											<input name="project_head" id="project_head" value="">
										</div>
									</div>
								</td>
								<td nowrap>
									<div class="form-group">
										<div class="input-group">
											...
											<input type="text" name="subscription_no" id="subscription_no" value=""/>
										</div>
									</div>	
								</td>
								<cfif session.ep.our_company_info.asset_followup eq 1>
									<td>
										<div class="form-group">
											<div class="input-group">
												<input name="asset_id" id="asset_id" type="hidden" value="">
												<input id="asset_name" name="asset_name"  value="">
											</div>
										</div>
									</td>
								</cfif>
								...
								<td nowrap>
									<div class="form-group">
										<div class="input-group">
											...
											<input type="text"  name="expense_center_name" id="expense_center_name"  value="">
										</div>
									</div>
								</td>
								<td nowrap>
...
<script>
 	...
 	$(document).ready(function(){
		...
		//$("#btnNext, #btnPrev, #btnPrevLast, #btnLast").bind("click", showBasketItems);
	...
	function add_row()
	{
		...
		window.basket.items.push({
			...
			SPECIAL_DEFINITION :"<cfoutput>'<option value=''>Seçiniz</option>'<cfloop query='get_special_definition'>SPECIAL_DEFINITION=SPECIAL_DEFINITION+'<option value='#SPECIAL_DEFINITION_ID#'>#SPECIAL_DEFINITION#</option>'</cfloop></cfoutput>"
		});
		showBasketItems(); 
		toplam_hesapla();
	}
	...
	function formatField1(e)
	{
		...
		switch ($(e.target).attr("id")){
			...
			if($("#tblBasket tr[ItemRow]").eq(fixedRowNumber).find("#expense_amount").val() == '' || $("#tblBasket tr[ItemRow]").eq(fixedRowNumber).find("#expense_amount").val() == null || $("#tblBasket tr[ItemRow]").eq(fixedRowNumber).find("#expense_amount").val()  == 0)
	...
	{ 
		...
		{	
			...
			<cfif x_select_type_info eq 2>
				...
				{
					alert("<cf_get_lang dictionary_id='48792.Ödeme Tipi Seçmelisiniz'>!" +(a+1)+"");	
					return false;
				}
			</cfif>
		...
		if(!paper_no_control(document.getElementById('record_num').value,'<cfoutput>#dsn2#</cfoutput>','BANK_ACTIONS','ACTION_TYPE_ID*25','PAPER_NO','OUTGOING_TRANSFER','ROW_KONTROL','PAPER_NUMBER','ACTION_ID-ACT_ROW_ID')) return false;
