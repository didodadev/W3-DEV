...
<cfif isdefined("attributes.multi_id") and len(attributes.multi_id)><!--- kopyalama --->
	...
	<cfquery name="get_action_detail" datasource="#DSN2#">
		SELECT
			...
			BA.AVANS_ID,
			BA.BANK_ORDER_ID,
			ISNULL(C.MEMBER_CODE,ISNULL(CM.MEMBER_CODE,E.MEMBER_CODE)) AS MEMBER_CODE,
...
<cfscript>
	...
	if(IsDefined('get_action_detail.recordcount') and len(get_action_detail.recordcount)){
		for(i=1;i<=get_action_detail.recordcount;i++)
		{
			...
			sepet.satir[i].EXPENSE = get_action_detail.EXPENSE[i];
			sepet.satir[i].EXPENSE_ITEM_NAME = get_action_detail.EXPENSE_ITEM_NAME[i];
...
<cfset sepet.satir[aaa].FROM_BRANCH_ID = #to_branch_id#>
...
<cfset date_info = now()>
<cfif isdefined('get_action_detail') and get_action_detail.recordcount neq 0 and not isdefined("collacted_havale_list")>
...
<cfset pageHead = getLang('main',1750)><!--- Toplu Gelen Havale --->
<cf_catalystHeader>
	<cf_box>
		<div id="basket_main_div">
			<cfform name="add_process">
				<cf_basket_form id="collacted_gelenh">
					...
					<input type="hidden" id="form_action_address" name="form_action_address" value="<cfoutput>bank.emptypopup_add_collacted_gelenh</cfoutput>" />
					...
					<cf_box_elements>
						<div class="col col-3 col-md-4 col-sm-6 col-xs-12" type="column" index="1" sort="true">
							<div class="form-group" id="item-action_date">
								...
								<div class="col col-8 col-xs-12">
									...
									<div class="input-group">
										<cfinput type="text" name="action_date" value="#dateformat(date_info,dateformat_style)#" onBlur="change_money_info('add_process','action_date','#xml_money_type#');" maxlength="10" validate="#validate_style#" required="Yes" message="#message#">
							...
							<div class="form-group" id="item-to_account_id">
								...
								<div class="col col-8 col-xs-12">
									<cf_wrkBankAccounts call_function='kur_ekle_f_hesapla' selected_value='#to_account_id#' control_status='1'>
								</div>
							</div>
						</div>
						<div class="col col-3 col-md-4 col-sm-6 col-xs-12" type="column" index="2" sort="true">
							...
							<div class="form-group" id="item-acc_department_id">
								...
								<div class="col col-8 col-xs-12">
									<cf_wrkDepartmentBranch fieldId='acc_department_id' selected_value='#to_department_id#' is_department='1' is_deny_control='0'>
								</div>
							</div>
						</div>
					</cf_box_elements>
					...
					<cf_grid_list id="add_period" name="add_period" class="detail_basket_list">
						...
						<tbody id="basketItemBase" style="display:none; width:99%;">
							<tr ItemRow>
								...
								<td style="text-align:left;">
									...
									<div class="form-group">
										<div class="input-group medium">
											<input type="text" name="comp_name" id="comp_name" class="boxtext"  value="" >
										</div>
									</div>
								</td>
								<td>
									<div class="form-group medium">
										<input type="text" name="iban_no" id="iban_no" class="boxtext" value="">
									</div>
								</td>
								<cfif isdefined("is_copy")>
									<td><div class="form-group small"><input type="text" id="paper_number" name="paper_number" readonly="readonly" value=""></div></td>
								<cfelse>
									<td><div class="form-group small"><input type="text" id="paper_number" name="paper_number" readonly="readonly" value=""></div></td>
								</cfif>	
								<td nowrap="nowrap">
									<div class="form-group small">
										<input type="hidden" name="system_amount" id="system_amount" value="0">
										<input type="text" name="action_value" id="action_value" value="" onkeyup="return(FormatCurrency(this,event));" maxlength="20" class="moneybox">
									</div>
								</td>
								<td nowrap="nowrap">
									<div class="form-group small">
										<input type="text" name="tl_value" id="tl_value" value="" class="boxtext"  readonly="readonly" style="width:35px;">
									</div>
								</td>
								<td nowrap="nowrap">
									<div class="form-group small">
										<input type="text" name="action_value_other" id="action_value_other" value="" maxlength="20" onkeyup="return(FormatCurrency(this,event));" class="moneybox">
									</div>
								</td>
								<td>
									<div class="form-group small">
										<input type="hidden" name="deneme" id="deneme" value="" class="box">
										<select name="money_id" id="money_id"></select>
									</div>
								</td>
								<td><div class="form-group"><input type="text" name="action_detail" id="action_detail" value="" class="boxtext"></div></td>
								<td>
									<div class="form-group">
										<div class="input-group">
								...
								<cfif isDefined("x_select_type_info") and x_select_type_info neq 0><td><div class="form-group medium"><select name="special_definition_id" id="special_definition_id" class="boxtext"></select></div></td></cfif>
								<td>
									<div class="form-group small">
										<input type="text" name="expense_amount" id="expense_amount" value="" onkeyup="return(FormatCurrency(this,event));" maxlength="20" class="moneybox">
									</div>
								</td>
								<td>
									<div class="form-group medium">
										<div class="input-group">
											...
											<input type="text"  name="expense_center_name" id="expense_center_name" onFocus="exp_center(#currentrow#,1);" value="" class="boxtext">
										</div>
									</div>
								</td>
								<td>
									<div class="form-group medium">
										<div class="input-group">
											...
											<input type="text" name="expense_item_name" id="expense_item_name" value="" class="boxtext">
										</div>
									</div>
								</td>
							</tr>
		...
		for (var i = window.basket.scrollIndex; i < Math.min(window.basket.items.length, window.basket.scrollIndex + window.basket.pageSize); i++)
			{
				...
				if(data['ACTION_EMPLOYEE_ID'] != null && data['ACTION_EMPLOYEE_ID'] != ''){
				...
				if(data['ACTION_COMPANY_ID'] != null && data['ACTION_COMPANY_ID'] != ''){
				...
				if(data['ACTION_CONSUMER_ID'] != null && data['ACTION_CONSUMER_ID'] != ''){
				...
				fillArrayField('expense_item_name',data['EXPENSE_ITEM_NAME'],data['EXPENSE_ITEM_NAME'],i);
...
function add_row(amount,other_money,member_code,member_type,action_company_id,action_consumer_id,action_employee_id,comp_name,acc_code,paper_no,exp_center_id,exp_item_id,exp_center_name,exp_item_name,inc_center_id,inc_item_id,inc_center_name,inc_item_name,project_id,project_name,expense_amount,action_detail,employee_id,employee_name,asset_id,asset_name,special_definition_id,contract_id,contract_head,avans_id,other_amount,related_cari_action_id,related_action_id,related_action_type,subscription_id,subscription_no,is_import,iban_no)
{
	...
	if(document.getElementById('currency_id') != undefined)
		...
		special_defination = '<option value="">Seçiniz</option>' ;
		<cfoutput>
			
			<cfloop query='get_special_definition'>
				if('#SPECIAL_DEFINITION_ID#' == special_definition_id)
					special_defination += '<option value="#SPECIAL_DEFINITION_ID#" selected="selected">#SPECIAL_DEFINITION#</option>'
				else
					special_defination += '<option value="#SPECIAL_DEFINITION_ID#">#SPECIAL_DEFINITION#</option>'
			</cfloop>
		</cfoutput>
	...
	window.basket.items.push({
		...
		SPECIAL_DEFINATION :special_defination,
...
Number.prototype.format = function(n, x) {
	...
	function formatField1(e)
	{
			...
			switch ($(e.target).attr("id")){
				...
				if(window.basket.items[rowNumber]['ROW_EXPENSE_CENTER_NAME'] == null || window.basket.items[rowNumber]['ROW_EXP_CENTER_NAME'] == '')
...
function kur_ekle_f_hesapla(select_input,doviz_tutar,satir)
	...
	{ 
		...
		if(!window.basket.items.length)
			...
			if(!paper_no_control(document.getElementById('record_num').value,'<cfoutput>#dsn2#</cfoutput>','BANK_ACTIONS','ACTION_TYPE_ID*24','PAPER_NO','INCOMING_TRANSFER','ROW_KONTROL','PAPER_NUMBER')) return false;
...
function handlerPostBank(target, responseData, status, jqXHR){
	...
	*/
	...
	function open_file()
	{
		...
		AjaxPageLoad('<cfoutput>#request.self#?fuseaction=objects.popup_add_collacted_from_file&type=2<cfif isdefined("attributes.multi_id")>&multi_id=#attributes.multi_id#</cfif></cfoutput>','toplu_giden_file',1);
		return false;
	}
	</script>
