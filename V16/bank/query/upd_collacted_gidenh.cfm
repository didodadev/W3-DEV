...
<cfquery name="get_all_gelenh" datasource="#new_dsn2#">
...
<cfscript>
	...
	for(b=1; b lte attributes.record_num; b=b+1)
		...
		{
			'attributes.action_value#b#' = evaluate('attributes.action_value#b#');
			'attributes.action_value_other#b#' = evaluate('attributes.action_value_other#b#');
			'attributes.system_amount#b#' = evaluate('attributes.system_amount#b#');
			'attributes.expense_amount#b#' = evaluate('attributes.expense_amount#b#');
		}
...
<cflock name="#createUUID()#" timeout="60">			
	<cftransaction>
		<cfquery name="upd_bank_actions_multi" datasource="#new_dsn2#">
			UPDATE
				BANK_ACTIONS_MULTI
			SET				
		...
		<cfif isdefined("attributes.record_num") and len(attributes.record_num)>
			...
			<cfloop from="1" to="#attributes.record_num#" index="i">
					...
					<cfscript>
						...
						{	
							carici(
								...
								islem_detay : UCase(getLang('main',423)),//GİDEN HAVALE İŞLEMİ
				...
				<cfelseif isdefined("attributes.row_kontrol#i#") and evaluate("attributes.row_kontrol#i#") and isdefined("attributes.act_row_id#i#") and len(evaluate("attributes.act_row_id#i#"))><!--- güncellenecek satırlar --->
					<cfquery name="upd_gelenh" datasource="#new_dsn2#">
						UPDATE
							BANK_ACTIONS
						SET
							ACTION_TO_COMPANY_ID = <cfif len(evaluate("member_type#i#")) and evaluate("member_type#i#") eq 'partner' and len(evaluate("action_company_id#i#"))>#comp_id#,<cfelse>NULL,</cfif>
							ACTION_TO_CONSUMER_ID = <cfif len(evaluate("member_type#i#")) and evaluate("member_type#i#") eq 'consumer' and len(evaluate("action_consumer_id#i#"))>#cons_id#,<cfelse>NULL,</cfif>
							ACTION_TO_EMPLOYEE_ID = <cfif len(evaluate("member_type#i#")) and evaluate("member_type#i#") eq 'employee' and len(evaluate("action_employee_id#i#"))>#emp_id#,<cfelse>NULL,</cfif>
            ...
            <cfif  bank.recordcount>
            	<cfoutput query="bank">
            		<cfif isdefined("attributes.avans_id#i#") and len(evaluate("attributes.avans_id#i#"))>
					<cfquery name="upd_corr_payment" datasource="#new_dsn2#">
						UPDATE #dsn_alias#.CORRESPONDENCE_PAYMENT SET ACTION_ID = NULL,ACTION_TYPE_ID = NULL WHERE ID = #evaluate("attributes.avans_id#i#")# AND ACTION_ID = #action_id#
					</cfquery>
					</cfif>
                    <cfquery name="CHECK_BANK_ORDERS" datasource="#new_dsn2#">
                    ...
                    <cfif len(CHECK_BANK_ORDERS.BANK_ORDER_ID)>
                    	<cfquery name="UPDATE_BANK_ORDERS" datasource="#new_dsn2#">
					...
					<cfscript>
						cari_sil(action_id:action_id,process_type:25,cari_db : new_dsn2);
						butce_sil(action_id:action_id,process_type:25,muhasebe_db : new_dsn2);
					</cfscript>
            	</cfoutput>	
            </cfif>
            <cfquery name="deleteRows" datasource="#new_dsn2#">
		...
		<cfscript>
			if(get_process_type.is_account eq 1)
			{
				...
				if( isdefined("attributes.record_num") and attributes.record_num neq "")
				{
					for(j=1; j lte attributes.record_num; j=j+1)
						if( isdefined("attributes.row_kontrol#j#") and evaluate("attributes.row_kontrol#j#") eq 1)
						{
							...
							exp_center_id = exp_item_id = "";
							if(isDefined("attributes.row_exp_center_id#j#") and len(evaluate("attributes.row_exp_center_id#j#")) and isDefined("attributes.row_exp_center_name#j#") and len(evaluate("attributes.row_exp_center_name#j#"))) exp_center_id = evaluate("attributes.row_exp_center_id#j#");
							if(isDefined("attributes.row_exp_item_id#j#") and len(evaluate("attributes.row_exp_item_id#j#")) and isDefined("attributes.row_exp_item_name#j#") and len(evaluate("attributes.row_exp_item_name#j#"))) exp_item_id = evaluate("attributes.row_exp_item_id#j#");
				...
				muhasebeci (
					...
					workcube_old_process_type: attributes.old_process_multi_type,
		...
		<cfif (not isdefined("attributes.act_row_id#attributes.record_num#")) or (isdefined("attributes.act_row_id#attributes.record_num#") and not len(evaluate("attributes.act_row_id#attributes.record_num#")))>
        ...
        <cf_workcube_process_cat 
            ...
            action_page='#request.self#?fuseaction=#listGetAt(fuseaction,1,'.')#.form_add_gidenh&event=updMulti&multi_id=#attributes.multi_id#'
...
<cfelse>
    <script type="text/javascript">
    	window.location.href="<cfoutput>#request.self#?fuseaction=bank.form_add_gidenh&event=updMulti&multi_id=#attributes.multi_id#</cfoutput>";
	</script>
</cfif>
