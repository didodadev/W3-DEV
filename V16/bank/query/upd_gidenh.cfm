...
<!--- giden havale banka talimatından eklenmişse --->
...
<cfelseif is_account eq 1>
	<cfif len(attributes.employee_id)><!--- çalışanın muhasebe kodu--->
		<cfset MY_ACC_RESULT = GET_EMPLOYEE_PERIOD(employee_id:attributes.employee_id,acc_type_id:attributes.acc_type_id)>
...
<cflock name="#createUUID()#" timeout="60">
	<cftransaction>
		<cfquery name="UPD_GIDENH" datasource="#dsn2#">
			UPDATE
				BANK_ACTIONS
			SET
				...
				ACTION_TO_COMPANY_ID = <cfif len(ACTION_TO_COMPANY_ID) and (len(attributes.employee_id) eq 0)>#ACTION_TO_COMPANY_ID#<cfelse>NULL</cfif>,
				ACTION_TO_EMPLOYEE_ID = <cfif len(attributes.employee_id)>#attributes.employee_id#<cfelse>NULL</cfif>,
				ACTION_TO_CONSUMER_ID = <cfif len(ACTION_TO_CONSUMER_ID)>#ACTION_TO_CONSUMER_ID#<cfelse>NULL</cfif>,
				...
				ACTION_DETAIL = <cfif len(ACTION_DETAIL)><cfqueryparam cfsqltype="cf_sql_varchar" value="#ACTION_DETAIL#"><cfelse>NULL</cfif>,
		...
		<cfif len(get_process_type.action_file_name)>
			<cf_workcube_process_cat 
				...
				is_action_file = 1
				action_db_type = '#dsn2#'
				action_page='#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.form_add_gidenh&event=upd&id=#attributes.id#'
		...
		<cfif isDefined('attributes.process_stage')>
			<cf_workcube_process
				...
				action_column='ACTION_ID'
				action_id='#attributes.id#'
				action_page='#request.self#?fuseaction=bank.form_add_gidenh&event=upd&id=#attributes.id#'
				warning_description='Giden Havale : #attributes.id#'>
		</cfif>
        <cf_add_log employee_id="#session.ep.userid#" log_type="0" action_id="#attributes.id#" action_name= "#attributes.paper_number# Güncellendi" paper_no= "#attributes.paper_number#" period_id="#session.ep.period_id#" process_type="#get_process_type.PROCESS_TYPE#" data_source="#dsn2#">	
	</cftransaction>
</cflock>
...
<script type="text/javascript">
	<cfif session.ep.our_company_info.is_paper_closer eq 1 and (len(attributes.ACTION_TO_COMPANY_ID) or len(attributes.ACTION_TO_CONSUMER_ID) or len(attributes.employee_id))>
		...
		<cfelse>
			window.open('<cfoutput>#request.self#?fuseaction=finance.list_payment_actions&event=add&act_type=1&member_id=#attributes.ACTION_TO_COMPANY_ID#&consumer_id=#attributes.ACTION_TO_CONSUMER_ID#&employee_id_new=#attributes.employee_id#&acc_type_id=#attributes.acc_type_id#&money_type=#form.money_type#&row_action_id=#attributes.id#&row_action_type=#process_type#</cfoutput>','page');
		</cfif>			
	</cfif>
	window.location.href='<cfoutput>#request.self#?fuseaction=bank.form_add_gidenh&event=upd&ID=#attributes.id#</cfoutput>';
</script>
