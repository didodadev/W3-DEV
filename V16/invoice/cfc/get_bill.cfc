<cfcomponent>
        ...
        <cfquery name="GET_BILL" datasource="#this.DSN2#">
            ...
            SELECT
                ...
                <cfif isdefined('arguments.listing_type') and arguments.listing_type eq 2><!--- Eğer satır bazında listeleme yapılıyorsa --->
                    ...
                    ISNULL(INVOICE_ROW.BSMV_AMOUNT,0) BSMV_TOTAL,
                    ISNULL(INVOICE_ROW.OIV_AMOUNT,0) OIV_TOTAL,
                ...
                <cfelse>
                    INVOICE.NETTOTAL,
                    INVOICE.TAXTOTAL,
                    ISNULL(INVOICE.BSMV_TOTAL,0) BSMV_TOTAL,
                    ISNULL(INVOICE.OIV_TOTAL,0) OIV_TOTAL,
                    INVOICE.PROJECT_ID,
                </cfif>
                ...
                #dsn#.Get_Dynamic_Language(SP.PAYMETHOD_ID,'#session.ep.language#','SETUP_PAYMETHOD','PAYMETHOD',NULL,NULL,SP.PAYMETHOD) AS PAYMETHOD,
                CPT.CARD_NO,
                INVOICE.SALE_EMP,
                ...
                (SELECT STAGE FROM #this.dsn_alias#.PROCESS_TYPE_ROWS PTR WHERE PROCESS_ROW_ID = INVOICE.PROCESS_STAGE ) as STAGE,
                INVOICE.PROCESS_STAGE,                                      
                COMPANY.MEMBER_CODE,
                CASE
            ...
            WHERE
                ...
                <cfif isdefined("arguments.from_report") and arguments.from_report eq 1> <!---Cari faaliyet özeti için eklendi PY --->
                    SPC.IS_CARI = 1 AND
                    SPC.PROCESS_TYPE NOT IN (62) AND <!----ALIM İADE ---->
                </cfif>
                <cfif isdefined("arguments.invoice_id") and len(arguments.invoice_id)>
                    INVOICE.INVOICE_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.invoice_id#">
                <cfelse>
                ...
                <cfif isDefined('arguments.dept_id') and len(arguments.dept_id)>
                    AND  DEP_BRANCH.DEPARTMENT_ID = '#arguments.dept_id#'
                </cfif> 
                <cfif isDefined('arguments.BRANCH_ID') and len(arguments.BRANCH_ID)>
                    AND  DEP_BRANCH.BRANCH_ID = '#arguments.BRANCH_ID#'
                </cfif>  
                <cfif isdefined('arguments.process_stage') and len(arguments.process_stage)>
                    AND INVOICE.PROCESS_STAGE = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.process_stage#">
                </cfif>
                ...
                <cfif len(arguments.product_cat_code) and len(arguments.product_cat_name)>
                            <cfif isdefined('arguments.listing_type') and arguments.listing_type eq 2><!--- Satir bazinda listeleme yapiliyorsa --->
                                AND INVOICE_ROW.PRODUCT_ID IN (SELECT PRODUCT_ID FROM #this.dsn3_alias#.PRODUCT P WHERE P.PRODUCT_CODE LIKE  <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.product_cat_code#.%">)
                            <cfelse>
                                AND INVOICE.INVOICE_ID IN (SELECT INVOICE_ID FROM INVOICE_ROW IR, #this.dsn3_alias#.PRODUCT P WHERE P.PRODUCT_ID = IR.PRODUCT_ID AND P.PRODUCT_CODE LIKE  <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.product_cat_code#.%">)
                            </cfif>
                        </cfif>
                ...
                <cfif len(arguments.efatura_type) and arguments.efatura_type neq 5>
                    ...
                    <cfelseif arguments.efatura_type eq 6>
                        ...
                        (
                            (
                                (SELECT COUNT(SENDING_DETAIL_ID) FROM EINVOICE_SENDING_DETAIL ESD WHERE ESD.ACTION_ID = INVOICE.INVOICE_ID AND ESD.ACTION_TYPE = 'INVOICE' AND STATUS_CODE = 1) = 0 
                                AND (SELECT COUNT(SENDING_DETAIL_ID) FROM EINVOICE_SENDING_DETAIL ESD WHERE ESD.ACTION_ID = INVOICE.INVOICE_ID AND ESD.ACTION_TYPE = 'INVOICE' AND STATUS_CODE <> 1 AND COMPANY.USE_EFATURA = 1) > 0 
                            )
                        )
                ...
                <cfif isdefined("arguments.invoice_type") and len(arguments.invoice_type)>
                ...
                <cfif len(arguments.earchive_type)>
                    <cfif arguments.earchive_type neq 5> 
                    ...
                    <cfelseif arguments.earchive_type eq 5> <!---Alış faturalarından e-arşiv olanlar--->
                        AND INVOICE.IS_EARCHIVE = 1
                    </cfif>
                </cfif>
                <cfif isdefined("arguments.output_type") and len(arguments.output_type)>
            ...
            GROUP BY
                ...
                <cfif isdefined('arguments.listing_type') and arguments.listing_type eq 2><!--- Eğer satır bazında listeleme yapılıyorsa --->
                    ...
                    INVOICE_ROW.BSMV_AMOUNT,
                    INVOICE_ROW.OIV_AMOUNT,
                    INVOICE_ROW.PRODUCT_ID,
                    INVOICE_ROW.AMOUNT,
                    INVOICE_ROW.PRICE,
                    INVOICE_ROW.INVOICE_ROW_ID,
                ...
                INVOICE.DEPARTMENT_ID,
                INVOICE.PAY_METHOD,
                #dsn#.Get_Dynamic_Language(SP.PAYMETHOD_ID,'#session.ep.language#','SETUP_PAYMETHOD','PAYMETHOD',NULL,NULL,SP.PAYMETHOD),
                INVOICE.SUBSCRIPTION_ID,
                CPT.CARD_NO,
                INVOICE.SALE_EMP,
                INVOICE.SALE_PARTNER,
    ...
    <cffunction name="CHK_SEND_INV" returntype="query">
        <cfargument name="iid" default="">
        <cfquery name="CHK_SEND_INV" datasource="#dsn2#">
            SELECT COUNT(*) COUNT FROM EINVOICE_SENDING_DETAIL WHERE  ACTION_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.iid#"> AND ACTION_TYPE = 'INVOICE' AND STATUS_CODE = 1
        </cfquery>
        <cfreturn CHK_SEND_INV>
    </cffunction>
    <cffunction name="CHK_SEND_ARC" returntype="query">
        <cfargument name="iid" default="">
        <cfquery name="CHK_SEND_ARC" datasource="#dsn2#">
            SELECT COUNT(*) COUNT FROM EARCHIVE_SENDING_DETAIL WHERE  ACTION_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.iid#"> AND ACTION_TYPE = 'INVOICE' AND STATUS_CODE = 1
        </cfquery> 
        <cfreturn CHK_SEND_ARC>
    </cffunction>
    <cffunction name="CONTROL_EARCHIVE" returntype="query">
        <cfargument name="iid" default="">
        <cfquery name="CONTROL_EARCHIVE" datasource="#DSN2#">
            SELECT ISNULL(IS_CANCEL,0) IS_CANCEL FROM EARCHIVE_RELATION WHERE ACTION_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.iid#"> AND ACTION_TYPE = 'INVOICE'
        </cfquery>
        <cfreturn CONTROL_EARCHIVE>
    </cffunction>
    <cffunction name="CONTROL_EINVOICE" returntype="query">
        <cfargument name="iid" default="">
        <cfquery name="CONTROL_EINVOICE" datasource="#DSN2#">
            SELECT ACTION_ID FROM EINVOICE_RELATION WHERE ACTION_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.iid#"> AND ACTION_TYPE = 'INVOICE'
        </cfquery>
