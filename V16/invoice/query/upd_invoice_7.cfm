...
<cfif isdefined("attributes.company_id") and len(attributes.company_id)>
	...
	<cfquery name="get_invoice_payment_rows" datasource="#dsn2#">
		SELECT DUE_DATE,ACTION_VALUE,datediff(d,getdate(),DUE_DATE) as VADE_FARKI FROM #dsn3_alias#.INVOICE_PAYMENT_PLAN WHERE INVOICE_ID = #form.invoice_id# AND PERIOD_ID = #session.ep.period_id#
	</cfquery>
	<cfif get_invoice_payment_rows.recordcount and is_paymethod_based_cari eq 0>
		...
		<cfquery name="upd_cari_rows" datasource="#dsn2#">
			UPDATE CARI_ROWS SET DUE_DATE = #avg_duedate_new# WHERE ACTION_ID = #form.invoice_id# AND PROCESS_CAT = #form.process_cat#
		</cfquery>
	</cfif>
</cfif>
