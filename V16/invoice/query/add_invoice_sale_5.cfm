...
<cfif listfind('52,53,531,62,5311,533',invoice_cat,',')><!--- stok işlemi yapılabilir bir fatura ise --->
	...
	<cfelseif invoice_cat eq 531> <!--- ihracat faturası --->
        ...
        <cfset int_ship_process_cat = get_ship_process.get_ship_process_cat(process_cat:PROCESS_CAT,new_datasource:new_dsn2_group)>
	<cfelseif listfind('5311,533',invoice_cat,',')> <!--- ihraç Kayıtlı Satış veya KDV den Muaf Satış--->
	...
	<cfif (not included_irs) and (inventory_product_exists eq 1)><!--- fatura kendi irsaliyesini oluşturuyor --->
		<cfquery name="ADD_SALE" datasource="#new_dsn2_group#">
			INSERT INTO
				SHIP
			(
				WRK_ID,
		...
		<cfscript>
			if(get_process_type.IS_STOCK_ACTION eq 1)//Stok hareketi yapılsın
			{
			    ...
			    basket_kur_ekle(action_id:GET_SHIP_ID.MAX_ID,table_type_id:2,process_type:0,basket_money_db:new_dsn2_group);	
