<!--- loop index degiskeninin ismi "i"  olmali yoksa burasi calismaz!!!! --->
<cfif isdefined('attributes.row_ship_id#i#')><cfset ship_id = evaluate('attributes.row_ship_id#i#')><cfelse><cfset ship_id = 0></cfif>
...
<cfif isdefined('attributes.row_total#i#') and len(evaluate("attributes.row_total#i#"))>
	...
	<cfif isdefined('attributes.iskonto_tutar#i#') and len(evaluate('attributes.iskonto_tutar#i#'))>
		<cfset discount_amount = evaluate('attributes.amount#i#')*evaluate('attributes.iskonto_tutar#i#')>
		<cfset temp_row_total = evaluate("attributes.row_total#i#") - discount_amount>
		<cfif isdefined('attributes.promosyon_yuzde#i#') and len(evaluate('attributes.promosyon_yuzde#i#'))>
			<cfset discount_amount = discount_amount + (temp_row_total * evaluate('attributes.promosyon_yuzde#i#')/100)>
		</cfif>
		<cfset temp_row_total = evaluate("attributes.row_total#i#") - discount_amount>
	...
	<cfelse>
		...
		<cfif isdefined('attributes.promosyon_yuzde#i#') and len(evaluate('attributes.promosyon_yuzde#i#'))>
			<cfset discount_amount = evaluate("attributes.row_total#i#") * evaluate('attributes.promosyon_yuzde#i#')/100 >
		</cfif>
		<cfset temp_row_total = evaluate("attributes.row_total#i#") - discount_amount>
		<cfset discount_amount = discount_amount + (temp_row_total * indirim_carpan / 100000000000000000000) >
	</cfif> --->
