...
<cflock name="#createUUID()#" timeout="60">	
	...
	<cfloop list="#ches#" index="i">
		...
		<cfif ctr eq 0><!---çek tahsil bordrosundan çikarilmis,portföyde durumuna geri dönecek--->
			...
			<cfif GET_CHEQUE_STATUS.CHEQUE_STATUS_ID eq 13><!--- bankada icin --->
		...
		<cfif evaluate("attributes.row_kontrol#i#")>
			...
			<cfif ctr eq 0>
				<cfquery name="UPD_CHEQUES" datasource="#dsn2#">
					UPDATE CHEQUE SET CHEQUE_STATUS_ID=13 WHERE CHEQUE_ID= #evaluate("attributes.cheque_id#i#")#
				</cfquery>
	...
	<cfscript>
		...
		if(is_account eq 1)
		{
			if(session.ep.our_company_info.is_edefter eq 0)	/*standart muhasebe islemleri yapılıyor*/
			{
				...
				//20140520 Kullanilmadigi icin kapatildi.
				alacak_hesaplar = '';
				alacak_tutarlar = '';
				...
				get_bank_acc_code = cfquery(datasource:"#dsn2#",sqlstring:"SELECT CHEQUE_GUARANTY_CODE,ACCOUNT_ACC_CODE FROM #dsn3_alias#.ACCOUNTS AS ACCOUNTS WHERE ACCOUNT_ID =#attributes.account_id#");
						...
						*/
						...
						{ 
							if(attributes.x_detail_acc_card eq 1) //detaylı muhasebe seçilmişse çek bilgilerini yazıyoruz
								satir_detay_list[2][listlen(alacak_tutarlar)]='Ç.Ç.B.B.T.:#attributes.payroll_no#:#evaluate("attributes.bank_name#i#")#:#evaluate("attributes.bank_branch_name#i#")#:#evaluate("attributes.account_no#i#")#'; //satır acıklamaları borc acıklama aray e set edilir.
								...
								satir_detay_list[2][listlen(alacak_tutarlar)] = ' #evaluate("attributes.cheque_no#i#")# - ÇEK BANKA TEMİNAT ÇIKIŞ İŞLEMİ';
						}
						else
						{
							if(attributes.x_detail_acc_card eq 1) //detaylı muhasebe seçilmişse çek bilgilerini yazıyoruz
								...
								satir_detay_list[2][listlen(alacak_tutarlar)] = ' ÇEK BANKA TEMİNAT ÇIKIŞ İŞLEMİ';
						}
					}
				}
				if(len(get_bank_acc_code.CHEQUE_GUARANTY_CODE))
					acc = get_bank_acc_code.CHEQUE_GUARANTY_CODE;
				else
					acc = acc_general;
				...
				if(isDefined("attributes.action_detail") and len(attributes.action_detail))
					...
					satir_detay_list[1][1] = ' ÇEK BANKA TEMİNAT ÇIKIŞ İŞLEMİ';
				...
				{ /*<!--- masraf hesaplarini da muhasebe hesap ve tutarlara dahil edelim --->*/
					...
					{
						satir_detay_list[2][listlen(alacak_tutarlar)] = ' ÇEK BANKA TEMİNAT ÇIKIŞ İŞLEMİ';
						satir_detay_list[1][2] = ' ÇEK BANKA TEMİNAT ÇIKIŞ İŞLEMİ';
					}
				}
				muhasebeci (
					action_id: attributes.id,
					...
					currency_multiplier : currency_multiplier,
					fis_detay:'ÇEK BANKA TEMİNAT ÇIKIŞ İŞLEMİ',
			...
			else		/*e-deftere uygun muhasebe islemleri yapılıyor*/
			{
					...
					if (evaluate("attributes.row_kontrol#i#"))
					{
						...
						{ 
							if(isDefined("attributes.action_detail") and len(attributes.action_detail))
								...
								satir_detay_list[2][1] = ' #evaluate("attributes.cheque_no#i#")# - ÇEK BANKA TEMİNAT ÇIKIŞ İŞLEMİ';
						}
						else
						{
							if(isDefined("attributes.action_detail") and len(attributes.action_detail))
								...
								satir_detay_list[2][1] = ' ÇEK BANKA TEMİNAT ÇIKIŞ İŞLEMİ';
						}
						if(isDefined("attributes.action_detail") and len(attributes.action_detail))
							satir_detay_list[1][1] = ' #attributes.action_detail#';
						else
				...
				{ 
					...
					{
						satir_detay_list[2][1] = ' ÇEK BANKA TEMİNAT ÇIKIŞ MASRAFI';
						satir_detay_list[1][2] = ' ÇEK BANKA TEMİNAT ÇIKIŞ MASRAFI';
					}
					muhasebeci (
						action_id: attributes.id,
						workcube_process_type: process_type,
						...
						currency_multiplier : currency_multiplier,
						fis_detay:'ÇEK BANKA TEMİNAT ÇIKIŞ MASRAFI',
						fis_satir_detay:satir_detay_list,
						belge_no : form.payroll_no,
						from_branch_id : branch_id_info,
						workcube_process_cat : form.process_cat
		...
		<cf_workcube_process_cat 
			...
			is_action_file = 1
			action_db_type = '#dsn2#'
			action_page='#request.self#?fuseaction=cheque.form_add_payroll_bank_guaranty_tem&event=upd&ID=#attributes.id#'
...
<script type="text/javascript">
	window.location.href="<cfoutput>#request.self#?fuseaction=cheque.form_add_payroll_bank_guaranty_tem&event=upd&ID=#attributes.id#</cfoutput>";
</script>
