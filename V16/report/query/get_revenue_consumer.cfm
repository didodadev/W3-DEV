<cfif attributes.main_report_type eq 1>
	<cfset FROM_TO_CONSUMER_ID = 'FROM_CONSUMER_ID'>
    <cfset FROM_TO_ACCOUNT_ID = 'TO_ACCOUNT_ID'>
    <cfset FROM_TO_CASH_ID = 'TO_CASH_ID'>
<cfelse><!--- ödeme --->
	<cfset FROM_TO_CONSUMER_ID = 'TO_CONSUMER_ID'>
...
<cfquery name="GET_CARI_ROWS" datasource="#dsn2#" cachedwithin="#fusebox.general_cached_time#">
	<cfif attributes.report_type eq 1>
		...
		WHERE
			...
			CR.#FROM_TO_CONSUMER_ID# IS NOT NULL
		<cfif len(trim(attributes.company)) and len(attributes.consumer_id)>
			AND CR.#FROM_TO_CONSUMER_ID# = #attributes.consumer_id#
		</cfif>
		<cfif len(trim(attributes.pos_code_text)) and len(attributes.pos_code)>
			AND	CR.#FROM_TO_CONSUMER_ID# IN (SELECT CONSUMER_ID FROM #dsn_alias#.WORKGROUP_EMP_PAR WHERE POSITION_CODE = #attributes.pos_code# AND IS_MASTER = 1 AND OUR_COMPANY_ID = #session.ep.company_id# AND CONSUMER_ID IS NOT NULL)
		</cfif>
		<cfif len(attributes.customer_value_id)>
			AND CR.#FROM_TO_CONSUMER_ID# IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE CUSTOMER_VALUE_ID = #attributes.customer_value_id#)
		</cfif>
		<cfif isdefined ('attributes.consumercat_id') and len(attributes.consumercat_id)>
			AND CR.#FROM_TO_CONSUMER_ID# IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE CONSUMER_CAT_ID IN (#attributes.consumercat_id#))
		</cfif>
		<cfif len(attributes.ims_code_id) and len(attributes.ims_code_name)>
			AND CR.#FROM_TO_CONSUMER_ID# IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE IMS_CODE_ID = #attributes.ims_code_id# )
		</cfif>
		<cfif len(attributes.zone_id)>
			AND CR.#FROM_TO_CONSUMER_ID# IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE SALES_COUNTY = #attributes.zone_id# )
		</cfif>
		<cfif len(attributes.resource_id)>
			CR.#FROM_TO_CONSUMER_ID# IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE RESOURCE_ID = #attributes.resource_id# )
		</cfif>
		...
		FROM (
				<cfif attributes.main_report_type eq 2>
					...
					<cfelseif attributes.report_type eq 5><!--- Müşteri Bazında --->
						C.CONSUMER_NAME + ' ' + C.CONSUMER_SURNAME DSP_REPORT_TYPE,
					...
					<cfelseif attributes.report_type eq 4><!--- Müşteri Temsilcisi Bazında --->
						...
						AND CR.TO_CONSUMER_ID = WEP.CONSUMER_ID
					<cfelseif attributes.report_type eq 5><!--- Müşteri Bazında --->
						AND CR.TO_CONSUMER_ID = C.CONSUMER_ID
					<cfelseif attributes.report_type eq 6><!--- Bankalar Bazında --->
						AND
					...
					<cfif len(trim(attributes.pos_code_text)) and len(attributes.pos_code)>
						AND CR.TO_CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.WORKGROUP_EMP_PAR WHERE POSITION_CODE = #attributes.pos_code# AND IS_MASTER = 1 AND OUR_COMPANY_ID = #session.ep.company_id# AND CONSUMER_ID IS NOT NULL)
					</cfif>
					<cfif len(attributes.customer_value_id)>
						AND CR.TO_CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE CUSTOMER_VALUE_ID = #attributes.customer_value_id#)
					</cfif>
					<cfif isdefined ('attributes.consumercat_id') and len(attributes.consumercat_id)>
						AND CR.TO_CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE CONSUMER_CAT_ID IN (#attributes.consumercat_id#))
					</cfif>
					<cfif len(attributes.ims_code_id) and len(attributes.ims_code_name)>
						AND CR.TO_CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE IMS_CODE_ID = #attributes.ims_code_id#)
					</cfif>
					<cfif len(attributes.zone_id)>
						AND CR.TO_CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE SALES_COUNTY = #attributes.zone_id#)
					</cfif>
					<cfif len(attributes.resource_id)>
						AND CR.TO_CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE RESOURCE_ID = #attributes.resource_id#)
					</cfif>
					<cfif len(attributes.project_head) and len(attributes.project_id)>
						AND CR.PROJECT_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.project_id#">
					</cfif>
				...
				<cfif attributes.main_report_type eq 1>
					...
					WHERE
						CR.FROM_CONSUMER_ID IS NOT NULL AND
						CR.ACTION_TYPE_ID IN (30,32,33,34,35,37,38,39,31,310) AND
						CR.ACTION_DATE BETWEEN #attributes.date1# AND #attributes.date2#
					<cfif attributes.report_type eq 2><!--- üye kategorisi Bazında --->
						AND C.CONSUMER_CAT_ID = CT.CONSCAT_ID
						AND CR.FROM_CONSUMER_ID = C.CONSUMER_ID
					<cfelseif attributes.report_type eq 3><!--- Satış Bölgesi Bazında --->
						AND C.SALES_COUNTY = SZ.SZ_ID
						AND CR.FROM_CONSUMER_ID = C.CONSUMER_ID
					<cfelseif attributes.report_type eq 4><!--- Müşteri Temsilcisi Bazında --->
						AND WEP.POSITION_CODE = EP.POSITION_CODE AND WEP.IS_MASTER=1 AND WEP.OUR_COMPANY_ID = #session.ep.company_id#
						AND CR.FROM_CONSUMER_ID = WEP.CONSUMER_ID
					<cfelseif attributes.report_type eq 5><!--- Müşteri Bazında --->
						AND CR.FROM_CONSUMER_ID = C.CONSUMER_ID
					<cfelseif attributes.report_type eq 6><!--- Bankalar Bazında --->
						AND
					...
					<cfif len(trim(attributes.pos_code_text)) and len(attributes.pos_code)>
						AND CR.FROM_CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.WORKGROUP_EMP_PAR WHERE POSITION_CODE = #attributes.pos_code# AND IS_MASTER = 1 AND OUR_COMPANY_ID = #session.ep.company_id# AND CONSUMER_ID IS NOT NULL)
					</cfif>
					<cfif len(attributes.customer_value_id)>
						AND CR.FROM_CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE CUSTOMER_VALUE_ID = #attributes.customer_value_id#)
					</cfif>
					<cfif isdefined ('attributes.consumercat_id') and len(attributes.consumercat_id)>
						AND CR.FROM_CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE CONSUMER_CAT_ID IN (#attributes.consumercat_id#))
					</cfif>
					<cfif len(attributes.ims_code_id) and len(attributes.ims_code_name)>
						AND CR.FROM_CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE IMS_CODE_ID = #attributes.ims_code_id#)
					</cfif>
					<cfif len(attributes.zone_id)>
						AND CR.FROM_CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE SALES_COUNTY = #attributes.zone_id#)
					</cfif>
					<cfif len(attributes.resource_id)>
						AND CR.FROM_CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE RESOURCE_ID = #attributes.resource_id#)
					</cfif>
					<cfif len(attributes.project_head) and len(attributes.project_id)>
						AND CR.PROJECT_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.project_id#">
					</cfif>
	...
	<cfelseif attributes.report_type eq 8>
		<cfif attributes.main_report_type eq 1>
			...
			FROM
				...
				#dsn_alias#.CONSUMER C
			WHERE
				CP.CONSUMER_ID IS NOT NULL AND
			...
			<cfif len(trim(attributes.company)) and len(attributes.consumer_id)>
				AND CP.CONSUMER_ID = #attributes.consumer_id#
			</cfif>
			<cfif len(trim(attributes.pos_code_text)) and len(attributes.pos_code)>
				AND CP.CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.WORKGROUP_EMP_PAR WHERE POSITION_CODE = #attributes.pos_code# AND IS_MASTER = 1 AND OUR_COMPANY_ID = #session.ep.company_id# AND CONSUMER_ID IS NOT NULL)
			</cfif>
			<cfif len(attributes.customer_value_id)>
				AND CP.CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE CUSTOMER_VALUE_ID = #attributes.customer_value_id#)
			</cfif>
			<cfif isdefined ('attributes.consumercat_id') and len(attributes.consumercat_id)>
				AND CP.CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE CONSUMER_CAT_ID IN (#attributes.consumercat_id#))
			</cfif>
			<cfif len(attributes.ims_code_id) and len(attributes.ims_code_name)>
				AND CP.CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE IMS_CODE_ID = #attributes.ims_code_id#)
			</cfif>
			<cfif len(attributes.zone_id)>
				AND CP.CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE SALES_COUNTY = #attributes.zone_id#)
			</cfif>
			<cfif len(attributes.resource_id)>
				AND CP.CONSUMER_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE RESOURCE_ID = #attributes.resource_id#)
			</cfif>
			<cfif len(attributes.project_head) and len(attributes.project_id)>
		...
		<cfelseif attributes.main_report_type eq 2>
			...
			FROM
				...
				#dsn_alias#.CONSUMER C
			WHERE
				CP.CONS_ID IS NOT NULL AND
				CP.CONS_ID = C.CONSUMER_ID AND
				CP.OTHER_MONEY IS NOT NULL AND
				CP.OTHER_COST_VALUE IS NOT NULL AND
				CP.ACTION_DATE BETWEEN #attributes.date1# AND #attributes.date2#
			<cfif len(trim(attributes.company)) and len(attributes.consumer_id)>
				AND CP.CONS_ID = #attributes.consumer_id#
			</cfif>
			<cfif len(trim(attributes.pos_code_text)) and len(attributes.pos_code)>
				AND CP.CONS_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.WORKGROUP_EMP_PAR WHERE POSITION_CODE = #attributes.pos_code# AND IS_MASTER = 1 AND OUR_COMPANY_ID = #session.ep.company_id# AND CONSUMER_ID IS NOT NULL)
			</cfif>
			<cfif len(attributes.customer_value_id)>
				AND CP.CONS_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE CUSTOMER_VALUE_ID = #attributes.customer_value_id#)
			</cfif>
			<cfif isdefined ('attributes.consumercat_id') and len(attributes.consumercat_id)>
				AND CP.CONS_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE CONSUMER_CAT_ID IN (#attributes.consumercat_id#))
			</cfif>
			<cfif len(attributes.ims_code_id) and len(attributes.ims_code_name)>
				AND CP.CONS_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE IMS_CODE_ID = #attributes.ims_code_id#)
			</cfif>
			<cfif len(attributes.zone_id)>
				AND CP.CONS_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE SALES_COUNTY = #attributes.zone_id#)
			</cfif>
			<cfif len(attributes.resource_id)>
				AND CP.CONS_ID IN (SELECT CONSUMER_ID FROM #dsn_alias#.CONSUMER WHERE RESOURCE_ID = #attributes.resource_id#)
			</cfif>
			<cfif len(attributes.project_head) and len(attributes.project_id)>
				AND CP.PROJECT_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.project_id#">
			</cfif>
