...
<cfsavecontent  variable="head"><cf_get_lang dictionary_id ='39864.Kategorilere Göre Satışlar'></cfsavecontent>
<cf_seperator title="#head#" id="category1">
 <div id="category1"   style="padding: 10px; display: block;float: left; width: 100%">
		...
		<cf_grid_list>
			<thead>
				<tr>
					...
					<th nowrap><cf_get_lang_main no ='223.Miktar'></th>
					<th nowrap><cf_get_lang_main no ='261.Tutar'> <cfoutput>#session.ep.money#</cfoutput></th>
					<cfif len(session.ep.money2)>
					<th nowrap>Tutar <cfoutput>#session.ep.money2#</cfoutput></th>
					</cfif>
				</tr>
			</thead>
			<tbody>
				<cfif GET_PRODUCT_CAT.recordcount>
					...
					<tr height="20" onMouseOver="this.className='color-light';" onMouseOut="this.className='color-row';" class="color-row">
						...
						<td align="right" nowrap style="text-align:right;">&nbsp;#TLFormat(AMOUNT)#
							<cfif len(AMOUNT)>
					...
					<tr>
						...
						<td align="right" nowrap class="phead" style="text-align:right;">&nbsp;#TLFormat(total_amount)#</td>
						<td align="right" nowrap class="phead" style="text-align:right;">&nbsp;#TLFormat(product_cat_total)# #session.ep.money#</td>
						<cfif len(session.ep.money2)>
						<td align="right" nowrap class="phead" style="text-align:right;">&nbsp;#TLFormat(product_cat_total2)# #session.ep.money2#</td>
						</cfif>
					</tr>
					</cfoutput>
				<cfelse>
	...
	<div class="col col-6 col-md-6 col-sm-6 col-xs-12">
		<cfif GET_PRODUCT_CAT.recordcount>
			<cfif CONSUMER_PRODUCT_CAT_TOTAL.PRODUCT_CAT_TOTAL eq 0><cfset CONSUMER_PRODUCT_CAT_TOTAL.PRODUCT_CAT_TOTAL = 1></cfif>
			...
			<cfoutput query="GET_PRODUCT_CAT">
			  ...
			  <cfif len(PRODUCT_CAT) GT 15>
					...
					<cfset 'value_#currentrow#' = "#wrk_round(((TUTAR*100)/CONSUMER_PRODUCT_CAT_TOTAL.PRODUCT_CAT_TOTAL)/100)#">
				<cfelse>
			...
			<canvas id="myChartss17" style="float:left;max-width:320px;max-height:320px;"></canvas>
			<script>
				var ctx = document.getElementById('myChartss17').getContext('2d');
					var myChartss17 = new Chart(ctx, {
						type: '<cfoutput>#graph_type#</cfoutput>',
						data: {
							labels: [<cfloop from="1" to="#GET_PRODUCT_CAT.recordcount#" index="cat_index"><cfoutput>"#evaluate("item_#cat_index#")#"</cfoutput>,</cfloop>],
							datasets: [{
								label: "Kategorilere Göre Satışlar",
								...
								data: [<cfloop from="1" to="#GET_PRODUCT_CAT.recordcount#" index="cat_index"><cfoutput>"#evaluate("value_#cat_index#")#"</cfoutput>,</cfloop>],
							}]
						},
						options: {
							legend: {
