<!---Sayfada, satış iadeler haric tüm alış faturaları üzerinden en çok alınan ürünler listelenir... OZDEN20061006 --->
...
<cfsavecontent  variable="head"><cf_get_lang dictionary_id ='39871.Ürünlere Göre Alışlar'></cfsavecontent>
<cf_seperator title="#head#" id="pproducts1">
<div id="pproducts1"   style="padding: 10px; display: block;float: left; width: 100%">
		...
		<cfif GET_TOP_PRODUCT.recordcount>
			...
			<cfoutput query="GET_TOP_PRODUCT">
			  <cfif len(PRODUCT_NAME) GT 15>
				...
				<cfset 'value_#currentrow#'= "#((TUTAR*100)/GET_PRODUCT_TOTAL.ALL_PRODUCT_TOTAL)/100#">
			<cfelse>
				<cfset 'item_#currentrow#' = "#PRODUCT_NAME#...(%#wrk_round((TUTAR*100)/GET_PRODUCT_TOTAL.ALL_PRODUCT_TOTAL)#)">
				<cfset 'value_#currentrow#'= "#((TUTAR*100)/GET_PRODUCT_TOTAL.ALL_PRODUCT_TOTAL)/100#">
			  </cfif>
			</cfoutput> 
			<canvas id="myChartssss27" style="float:left;max-width:320px;max-height:100%;"></canvas>
				<script>
					var ctx = document.getElementById('myChartssss27');
						var myChartssss27 = new Chart(ctx, {
							type: '<cfoutput>#graph_type#</cfoutput>',
							data: {
								labels: [<cfloop from="1" to="#GET_TOP_PRODUCT.recordcount#" index="jj">
								...
								datasets: [{
									...
									data: [<cfloop from="1" to="#GET_TOP_PRODUCT.recordcount#" index="jj"><cfoutput>#evaluate("value_#jj#")#</cfoutput>,</cfloop>],
								}]
							},
							options: {
								legend: {
