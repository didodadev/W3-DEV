<!--- Secilen üye,tarih ve çalışana göre satış siparişlerini, sürec ve sipariş satır aşamalarına göre listeler. OZDEN20061013 --->
...
<cfquery name="GET_ORDERS" datasource="#dsn3#">
	SELECT
		(ORR.QUANTITY*(ORR.PRICE)*(100+ORR.TAX)/100) ROW_TOTAL,
		<cfif len(session.ep.money2)>
		((ORR.QUANTITY*(ORR.PRICE)*(100+ORR.TAX)/100)/(ORD_M.RATE2/ORD_M.RATE1)) ROW_TOTAL_2,
		</cfif>
		ORR.QUANTITY,
...
<cfquery name="GET_ORDER_STAGE" datasource="#dsn3#">
  ...
  <cfset item_name = "">
<cfsavecontent  variable="head"><cf_get_lang dictionary_id='58207.Satış Siparişleri'>
</cfsavecontent>
<cf_seperator title="#head#" id="siparis1">
	  <div id="siparis1" style="padding: 10px; display: block;float: left; width: 100%">
	...
	<cfelse>
		...
		<div class="col col-3 col-md-3 col-sm-6 col-xs-12">
			<cfif GET_ORDER_CURRENCY.recordcount>
					<cfoutput query="GET_ORDER_CURRENCY">
						...
						<cfif orders_total gt 0>
							<cfset 'item_#currentrow#'="#item_name# (%#wrk_round((CURRENCY_ROW_TOTAL*100)/orders_total)#)">
							<cfset 'value_#currentrow#' ="#wrk_round(((CURRENCY_ROW_TOTAL*100)/orders_total)/100)#">
						<cfelse>
							<cfset 'item_#currentrow#'="#item_name# (%0)">
							<cfset 'value_#currentrow#'="0">
						</cfif>
					</cfoutput>
					<script src="JS/Chart.min.js"></script> 	
					<canvas id="myChart4" style="float:left;max-width:320px;max-height:320px;"></canvas>
					<script>
							...
							<tr>
								...
								<td align="right" style="text-align:right;">&nbsp;#TLFormat(STAGE_TOTAL)# #session.ep.money#
									<cfif len(STAGE_TOTAL)>
										<cfset order_stage_total = wrk_round(order_stage_total + STAGE_TOTAL)>
									</cfif>
								</td>
								<cfif len(session.ep.money2)>
									<td align="right" style="text-align:right;">&nbsp;#TLFormat(STAGE_TOTAL_2)# #session.ep.money2#
										<cfif len(STAGE_TOTAL_2)>
											<cfset order_stage_total2 = wrk_round(order_stage_total2 + STAGE_TOTAL_2)>
										</cfif>
									</td>
								</cfif>
							</tr>
		...
		<div class="col col-3 col-md-3 col-sm-6 col-xs-12">
			<cfif GET_ORDER_STAGE.recordcount>
						<cfoutput query="GET_ORDER_STAGE">
							<cfif order_stage_total gt 0>
								<cfset 'item_#currentrow#'="#STAGE# (%#wrk_round((STAGE_TOTAL*100)/order_stage_total)#)">
								<cfset 'value_#currentrow#'="#wrk_round(((STAGE_TOTAL*100)/order_stage_total)/100)#">
							<cfelse>
								<cfset 'item_#currentrow#'="#STAGE# (%0)">
								<cfset 'value_#currentrow#'="0">
							</cfif>
						</cfoutput>
			</cfif>
					<canvas id="myChart5" style="float:left;max-width:320px;max-height:320px;"></canvas>
					<script>
		...
		<div class="col col-12 col-md-12 col-sm-12 col-xs-12">
			<cfif len(GET_TOTAL_ORDERS.GENERAL_TOTAL)>
				<cfif GET_TOTAL_ORDERS.GENERAL_TOTAL eq 0>
					<cfset customer_order_rate = 0>
				<cfelse>
					<cfset customer_order_rate = (order_stage_total*100)/GET_TOTAL_ORDERS.GENERAL_TOTAL>
				</cfif>
				<div class="col col-12">
				<canvas id="myChart3" style="float:left;max-width:320px;max-height:320px;"></canvas>
				</div>
				<script>
					var ctx = document.getElementById('myChart3').getContext('2d');
						var myChart3 = new Chart(ctx, {
							type: '<cfoutput>#graph_type#</cfoutput>',
							data: {
								labels: ["<cf_get_lang no ='1175.Toplam Sipariş'> (%)", "<cf_get_lang no ='1176.Müşteri Siparişi'> (%)"],
								datasets: [{
									label: "Satış Siparişleri",
									backgroundColor: ['rgb(255, 99, 132)','rgba(255, 99, 132, 0.2)'],
									borderColor: ['rgb(255, 99, 132)','rgba(255, 99, 132, 0.2)'],
									data: [<cfoutput>#wrk_round(100-customer_order_rate)#</cfoutput>, <cfoutput>#wrk_round(customer_order_rate)#</cfoutput>],
								}]
							},
							options: {
								legend: {
