...
<cfquery name="GET_ACCOUNT_CARD_ROWS_ALL" datasource="#DSN2#">
	SELECT 
		...
		AC.CARD_TYPE_NO, 
		AC.RECORD_DATE, 
		<!--- ACR.DETAIL AS DETAIL, belge no basa gelmesi adina calisma yapildi , birleştirilmiş fişler için account_card_rowsa paper no eklendi ordan da alınıyor.--->
		ISNULL(ISNULL(AC.PAPER_NO,ACR.ROW_PAPER_NO),'') + ' ' + REPLACE(ACR.DETAIL,ISNULL(ISNULL(AC.PAPER_NO,ACR.ROW_PAPER_NO),''),'') DETAIL,
		AC.ACTION_DATE, 
		ACR.AMOUNT ,
		<cfif isdefined('attributes.acc_code_type') and attributes.acc_code_type eq 1>
...
<cfloop from="1" to="#attributes.totalrecords#" index="I">
	...
	<cfif (isDefined("attributes.no_process") and get_account_card_rows.recordcount) or not isDefined("attributes.no_process")>
		...
		<cfif get_acc_total.recordcount or get_account_card_rows.recordcount>
			...
			<cfoutput>
			<cf_wrk_html_tr class="tr" valign="top">
				<cf_wrk_html_td colspan="6" align="left"><cfoutput>#session.ep.company# (#session.ep.period_year#)</cfoutput></cf_wrk_html_td>
			</cf_wrk_html_tr>
			<cf_wrk_html_tr class="tr">
				<cf_wrk_html_td class="headbold" colspan="10" align="center"><cf_get_lang dictionary_id ='39163.DEFTER-İ KEBİR'></cf_wrk_html_td>
			</cf_wrk_html_tr>
			<cf_wrk_html_tr class="tr">
			...
			<cfif get_acc_total.recordcount><!--- hesabın, baslangıc tarihinden onceki doneme ait bakiyesi varsa --->
				<cf_wrk_html_tr class="tr">
					<cfset row_count = row_count+1>
					<cf_wrk_html_td colspan="6" align="center" class="txtbold"><cf_get_lang dictionary_id='38878.Devreden Toplam'></cf_wrk_html_td>
					...
					<cf_wrk_html_td align="right" style="text-align:right;" class="txtbold" format="numeric"><cfif devir_borc neq 0><cfset borc_bakiye = borc_bakiye + devir_borc><cfoutput>#TLFormat(devir_borc)#</cfoutput><cfelse> </cfif></cf_wrk_html_td>
					<cf_wrk_html_td align="right" style="text-align:right;" class="txtbold" format="numeric"><cfif devir_alacak neq 0><cfset alacak_bakiye = alacak_bakiye + devir_alacak><cfoutput>#TLFormat(devir_alacak)#</cfoutput><cfelse> </cfif></cf_wrk_html_td>
			...
			<cfif get_account_card_rows.recordcount>
				<cfoutput query="get_account_card_rows">
					...
					<cfif len(attributes.pdf_page_row) and attributes.pdf_page_row neq 0 and (row_count neq 1 and ((row_count mod attributes.pdf_page_row) eq 0)) or currentrow eq get_account_card_rows.recordcount> <!--- yeni hesap baslayacaksa veya pdf sayfa satır sayı tamamlanmıssa yeni sayfaya gecilir --->
						<cf_wrk_html_tr class="tr">
							<!--- <cfset row_count = row_count+1> --->
							<cf_wrk_html_td colspan="6" align="center" class="txtbold"><cf_get_lang dictionary_id ='39183.Sayfa Toplam'></cf_wrk_html_td>
						...
						<cfif currentrow neq get_account_card_rows.recordcount><!--- son hesabın son kaydı degilse yeni sayfaya gecsin --->
							...
							<cf_wrk_html_tr class="tr">
								<cfset row_count = row_count+1>
								<cf_wrk_html_td colspan="6" align="center" class="txtbold"><cf_get_lang dictionary_id='38876.Önceki Sayfa Toplam'></cf_wrk_html_td>
								<cf_wrk_html_td align="right" style="text-align:right;" class="txtbold" format="numeric">#TLFormat(borc_bakiye)# </cf_wrk_html_td>
								<cf_wrk_html_td align="right" style="text-align:right;" class="txtbold" format="numeric">#TLFormat(alacak_bakiye)# </cf_wrk_html_td>
								<cf_wrk_html_td align="right" style="text-align:right;" class="txtbold" format="numeric">
								<cfif borc_bakiye gt alacak_bakiye>
		...
		<cfset file_name_list = listappend(file_name_list,'#filename#.xlsx')>	
            <cffile action="move" source="#upload_folder##dir_seperator#reserve_files#dir_seperator##drc_name_##dir_seperator##filename#.xlsx" destination="#upload_folder##dir_seperator#account#dir_seperator#fintab#dir_seperator##session.ep.userid#">			
		</cfif>
	</cfif>
</cfloop>
...
<cfelse>
	<script type="text/javascript">
		alert("<cf_get_lang dictionary_id='57936.Aradığınız Kriterlere Uygun Kayıt Bulunamadı'>..");
	</script>
</cfif>
