...
<cfoutput query="get_account_id">
	...
	<cfif isdefined("get_account_card_rows_all.recordcount") and get_account_card_rows_all.recordcount>
	<cfoutput query="get_account_card_rows_all" startrow="1">
		<cfif currentrow eq 1 or (currentrow neq 1 and (listfirst(get_account_card_rows_all.ACCOUNT_ID[currentrow],'.') neq listfirst(get_account_card_rows_all.ACCOUNT_ID[currentrow-1],'.')))>
			...
			<cf_wrk_html_tr class="tr">
				...
				<cf_wrk_html_td colspan="9" class="txtbold" align="center">
					<cf_get_lang dictionary_id='39371.Ana Hesap Kodu'> :  #listfirst(get_account_card_rows_all.ACCOUNT_ID[currentrow],'.')#  <cf_get_lang dictionary_id='39372.Ana Hesap Adı'> :<cfquery name="GET_ACCOUNT_NAME" dbtype="query">SELECT * FROM GET_ACCOUNT_NAME_ALL WHERE ACCOUNT_CODE = '#listfirst(get_account_card_rows_all.ACCOUNT_ID[currentrow],'.')#'</cfquery>	
		...
		<cf_wrk_html_tr>
			...
			<cfif BA> <!--- eq 1 : alacak --->
				...
				<cf_wrk_html_td align="right" style="text-align:right;" format="numeric" width="90">#TLFormat(AMOUNT)#</cf_wrk_html_td>
			<cfelse> <!--- borc --->
