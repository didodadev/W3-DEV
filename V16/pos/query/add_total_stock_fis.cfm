...
<cfif not(isdefined("attributes.del_file_id") and len(attributes.del_file_id))>
	<cfloop list="#attributes.file_id_list#" index="kk">
		...
		<cfloop from="1" to="#loop_count#" index="kkk">
			...
			<cfset attributes.islem_tarihi = dateformat(get_rows.process_date,dateformat_style)>
			<cfinclude template="add_file_import_total_sayim.cfm">
		</cfloop>
	</cfloop>
...
<cfelse>
	...
	<script type="text/javascript">
		alert("<cf_get_lang no ='41.Belgeye Ait Stok Fişleri Silindi'> !");
	</script>
</cfif>
<script type="text/javascript">
