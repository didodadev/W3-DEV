...
<cflock name="#createUUID()#" timeout="20">		
    <cftransaction>
        <cfquery name="get_row" datasource="#dsn2#">
            SELECT YIELD_TO_BANK, BANK_ACTION_ID FROM #dsn3_alias#.STOCKBONDS_YIELD_PLAN_ROWS WHERE YIELD_PLAN_ROWS_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.row_id#">
        </cfquery>
        <cfif get_row.YIELD_TO_BANK eq 1 and len(get_row.BANK_ACTION_ID) and isdefined("attributes.yield_to_bank") and attributes.yield_to_bank eq 1>
            <cfquery name="UPD_INTEREST_REVENUE" datasource="#dsn2#">
                ...
                SET
                    ...
                    ACTION_TO_ACCOUNT_ID = #attributes.from_account_id#,
                    ACTION_VALUE = #attributes.ACTION_VALUE - attributes.stopaj_total#,
                    ...
                    OTHER_CASH_ACT_VALUE = <cfif isDefined("attributes.OTHER_CASH_ACT_VALUE") and len(attributes.OTHER_CASH_ACT_VALUE)>#attributes.OTHER_CASH_ACT_VALUE - attributes.stopaj_total#<cfelse>NULL</cfif>,
                    OTHER_MONEY = <cfif isDefined("attributes.money_type") and len(attributes.money_type)><cfqueryparam cfsqltype="cf_sql_varchar" value="#form.money_type#"><cfelse>NULL</cfif>,
                    ...
                    UPDATE_DATE = #NOW()#,
                    TO_BRANCH_ID = #from_branch_id_info#,
                    SYSTEM_ACTION_VALUE = #wrk_round( (attributes.ACTION_VALUE - attributes.stopaj_total) * dovizli_islem_multiplier)#,
                    SYSTEM_CURRENCY_ID = <cfqueryparam cfsqltype="cf_sql_varchar" value="#session.ep.money#">
                    <cfif len(session.ep.money2)>
                        ,ACTION_VALUE_2 = #wrk_round(( (attributes.ACTION_VALUE - attributes.stopaj_total) * dovizli_islem_multiplier)/currency_multiplier,4)#
                        ,ACTION_CURRENCY_ID_2 = <cfqueryparam cfsqltype="cf_sql_varchar" value="#session.ep.money2#">
                    </cfif>

            </cfquery>
            <cfset BANK_ACT_ID = get_row.BANK_ACTION_ID>
        <cfelseif get_row.YIELD_TO_BANK neq 1 and not len(get_row.BANK_ACTION_ID) and isdefined("attributes.yield_to_bank") and attributes.yield_to_bank eq 1>
        ...
        <cfelseif get_row.YIELD_TO_BANK eq 1 and len(get_row.BANK_ACTION_ID) and not isdefined("attributes.yield_to_bank")> 
            <cfquery name="DEL_YIELD_PLAN_ROWS" datasource="#dsn2#">
                DELETE FROM BANK_ACTIONS WHERE ACTION_ID = #get_row.BANK_ACTION_ID#
            </cfquery>
        </cfif>
        ...
        <cfscript>
            if(is_account eq 1) {
                ...
                if( isdefined("get_com_exp_item_id") ) 
                {
                    ...
                    }else if( isdefined("attributes.yield_to_bank") and attributes.yield_to_bank eq 1){ /* Getiri Banka hesabına geçsin. */
                ...
                muhasebe_sil(action_id:attributes.row_id,process_type:form.old_process_type);
                muhasebeci(
            ...
            f_kur_ekle_action(action_id:attributes.row_id,process_type:0,action_table_name:'STOCKBONDS_SALEPURCHASE_MONEY',action_table_dsn:'#dsn3#',transaction_dsn:'#dsn2#');
             ...
             if(is_budget eq 1)
            {
                butce_sil(action_id:attributes.row_id,process_type:form.old_process_type);
                butceci(
                        action_id : attributes.row_id,
                        muhasebe_db : dsn2,
