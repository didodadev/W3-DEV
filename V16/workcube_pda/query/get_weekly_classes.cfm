<cfquery name="GET_WEEKLY_CLASSES" datasource="#dsn#">
	SELECT 
		 ...
		 (
		   (
		      TRAINING_CLASS.START_DATE >= #attributes.to_day#
		        AND
		      TRAINING_CLASS.START_DATE < #DATEADD("ww",1,attributes.to_day)#
			)
			  OR
			(
			  TRAINING_CLASS.FINISH_DATE >= #attributes.to_day#
		  	    AND
			  TRAINING_CLASS.FINISH_DATE < #DATEADD("ww",1,attributes.to_day)#
			) 
		 )
	    ORDER BY 
			TRAINING_CLASS.START_DATE
...
<!---<cfoutput>#GET_WEEKLY_CLASSES.RECORDCOUNT#  _ #SESSION.EP.USERID#</cfoutput><cfabort>--->
