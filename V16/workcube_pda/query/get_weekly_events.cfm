...
<cfquery name="GET_WEEKLY_EVENTS" datasource="#DSN#">
	...
	WHERE
		...
		(
			(
				(EVENT.STARTDATE >= #attributes.to_day#)
				AND
				(EVENT.STARTDATE < #DATEADD("ww",1,attributes.to_day)#)
			)
			OR
			(
				(EVENT.FINISHDATE >= #attributes.to_day#)
				AND
				(EVENT.FINISHDATE < #DATEADD("ww",1,attributes.to_day)#)
			)
		) 
		AND 
		(
