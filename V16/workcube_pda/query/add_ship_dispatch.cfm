...
<cfif not len(attributes.location_id_in)><cfset attributes.location_id_in = "NULL"></cfif>
<cfset wrk_id = dateformat(now(),'YYYYMMDD')&timeformat(now(),'HHmmss')&'_#session.pda.userid#_'&round(rand()*100)>
<cfset form.basket_money=session.pda.money2>
<cflock name="#CreateUUID()#" timeout="60">
	...
	<cfquery name="ADD_SALE" datasource="#DSN2#" result="MAX_ID">
		INSERT INTO
            ...
            (
                ...
                '#form.basket_money#',
                <cfif isDefined("form.basket_net_total")>#((form.basket_net_total*form.basket_rate1)/form.basket_rate2)#<cfelse>0</cfif>,
                #attributes.department_id#,
                #attributes.location_id#,
                #attributes.department_id_in#,
                #attributes.location_id_in#,
                ...
                <cfif isdefined("attributes.project_id_in") and len(attributes.project_id_in)>#attributes.project_id_in#<cfelse>NULL</cfif>,
                ...
                <cfif isDefined("attributes.is_delivered") and Len(attributes.is_delivered)>1<cfelse>NULL</cfif>
            )
	</cfquery>
	<cfloop from="1" to="#attributes.rows_#" index="i">
		<cfif isDefined("attributes.row_kontrol#i#") and Evaluate("attributes.row_kontrol#i#") eq 1>
			<!--- Spect bilgileri kullanilacaksa kontrol edilip duzenlenecek --->
			<cfinclude template="get_product_info.cfm">
			<cf_date tarih = 'attributes.deliver_date#i#'>
			<cfinclude template="get_dis_amount.cfm">
			<cfif isdefined('attributes.product_name_other#i#') and len(evaluate('attributes.product_name_other#i#'))><cfset product_name_other_ = evaluate('attributes.product_name_other#i#')></cfif>
			<cfquery name="ADD_SHIP_ROW" datasource="#DSN2#"> 
				INSERT INTO
                    ...
                    (
                        ...
                        #indirim10#,
                        #discount_amount#,
                        <cfif isDefined("attributes.row_lasttotal#i#") and Len(evaluate("attributes.row_lasttotal#i#"))>#evaluate("attributes.row_lasttotal#i#")#<cfelse>0</cfif>,
                        <cfif isDefined("attributes.row_nettotal#i#") and Len(evaluate("attributes.row_nettotal#i#"))> #evaluate("attributes.row_nettotal#i#")#<cfelse>0</cfif>,
                        <cfif isDefined("attributes.row_taxtotal#i#") and Len(evaluate("attributes.row_taxtotal#i#"))> #evaluate("attributes.row_taxtotal#i#")#<cfelse>0</cfif>,
						...
						<cfelseif isdefined('attributes.department_id_in') and len(attributes.department_id_in)>
                            #attributes.department_id_in#,
                        <cfelse>
                            NULL,
                        </cfif>
                        ...
                        <cfelseif isdefined('attributes.location_id_in') and len(attributes.location_id_in)>
                            #attributes.location_id_in#,
                        <cfelse>
                            NULL,
                        </cfif>
			...
			<cfif get_process_cat.is_stock_action eq 1><!--- Stok hareketi yapılsın --->
	...
	<cfif isdefined("attributes.is_delivered") and len(attributes.location_id_in) and attributes.is_delivered eq 1 and Get_Process_Cat.IS_STOCK_ACTION eq 1><!--- Stok hareketi yapılsın --->
		<cfloop from="1" to="#attributes.rows_#" index="i">
			<cfif  isDefined("attributes.row_kontrol#i#") and Evaluate("attributes.row_kontrol#i#") eq 1 and isDefined('attributes.is_inventory#i#') and evaluate('attributes.is_inventory#i#') eq 1><!--- urun envantere dahilse --->
	...
	<!---  //Sevk irsaliyesi teslim alan secilmis --->
	<cfscript>
		if( len(attributes.location_id_in) and len(attributes.department_id_in)) //giris deposu kontrol ediliyor
		{
			LOCATION_IN=cfquery(datasource:"#dsn2#",sqlstring:"SELECT SL.LOCATION_TYPE,D.BRANCH_ID FROM #dsn_alias#.STOCKS_LOCATION SL, #dsn_alias#.DEPARTMENT D WHERE D.DEPARTMENT_ID=SL.DEPARTMENT_ID AND SL.DEPARTMENT_ID=#attributes.department_id_in# AND SL.LOCATION_ID=#attributes.location_id_in#");
			location_type_in = LOCATION_IN.LOCATION_TYPE;
			branch_id_in=LOCATION_IN.BRANCH_ID;
		}
		else
		{
			location_type_in =''; branch_id_in='';
		}
		if( len(attributes.location_id) and len(attributes.department_id)) //cikis deposu kontrol ediliyor
		{	
			LOCATION_OUT=cfquery(datasource:"#dsn2#",sqlstring:"SELECT SL.LOCATION_TYPE,D.BRANCH_ID FROM #dsn_alias#.STOCKS_LOCATION SL, #dsn_alias#.DEPARTMENT D WHERE D.DEPARTMENT_ID=SL.DEPARTMENT_ID AND SL.DEPARTMENT_ID=#attributes.department_id# AND SL.LOCATION_ID=#attributes.location_id#");
			location_type_out = LOCATION_OUT.LOCATION_TYPE;
			branch_id_out= LOCATION_OUT.BRANCH_ID;
		}
		else
		{
			location_type_out = ''; branch_id_out='';
		}
			
		/*1-Hammadde,2-Mal,3-Mamul,4-Konsinye*/
		/*duzenlenecek fbs20111007
		if(Get_Process_Cat.is_account eq 1 and ((location_type_in neq location_type_out and location_type_in neq 1 and location_type_out eq 2) or is_dept_based_acc eq 1))
		{
			include('../../stock/query/ship_account_process.cfm');
			muhasebeci(
				action_id : MAX_ID.IDENTITYCOL,
				workcube_process_type : Get_Process_Cat.process_type,
	...
	<!--- Duzenlenecek 20111007
	<cf_workcube_process_cat 
		process_cat="#form.process_cat#"
		action_id = #MAX_ID.IDENTITYCOL#
		is_action_file = 1
		action_page='#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.upd_ship_dispatch&ship_id=#MAX_ID.IDENTITYCOL#'
		action_file_name='#Get_Process_Cat.action_file_name#'
		action_db_type = '#dsn2#'
		is_template_action_file = '#Get_Process_Cat.action_file_from_template#'> --->
	</cftransaction>
</cflock>
<!--- Serial ile ilgili talep gelirse duzenlenebilir simdilik kalsin
<!--- seri no--->
		...
		<cfif len(evaluate('attributes.guaranty_purchasesales#i#')) and (evaluate('attributes.is_serial_no#i#') eq 1) >
			 <cfscript>
				add_serial_no(
					...
					is_sale : true,
					dpt_id : attributes.department_id_in,
					loc_id : attributes.location_id_in
				);
			 </cfscript>
			 <cfscript>
				add_serial_no(
	...
	<cfquery name="UPD_PAPERS" datasource="#DSN3#">
		...
		WHERE
			<cfif isdefined('attributes.paper_printer_id') and len(attributes.paper_printer_id)>
                PRINTER_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.paper_printer_id#">
            <cfelse>
...
<script type="text/javascript">
	...
	document.location.href = '<cfoutput>#request.self#</cfoutput>?fuseaction=pda.form_add_ship_dispatch';
</script>
