...
<cfscript>
	...
	{
		get_money_history=cfquery(SQLString:"SELECT RATE2,RATE1,MONEY MONEY_TYPE FROM MONEY_HISTORY WHERE MONEY ='#get_setup_money.money_type[stp_mny]#' AND PERIOD_ID = #session.pda.period_id# ORDER BY VALIDATE_DATE ",Datasource:dsn,is_select:1);
	...
	for(row_i=1;row_i lte attributes.ship_row_count;row_i=row_i+1)
	{
		...
		if (evaluate('attributes.row_kontrol#row_i#'))
		{
			...
			else
			{
				...
				'attributes.wrk_row_id#row_i#' = "WRK#row_i##round(rand()*65)##dateformat(now(),'YYYYMMDD')##timeformat(now(),'HHmmssL')##session.pda.userid##round(rand()*100)##row_i#";
				...
				'attributes.price_other_indirimli#row_i#'= wrk_round(evaluate('attributes.price#row_i#')*indirim_carpan/100000000000000000000,4);
...
<cftry>
	<cfinclude template="../../stock/query/add_sale_pda.cfm">
	<script type="text/javascript">
		alert("İrsaliye <cfoutput>#ship_number#</cfoutput> Belge Numarası ile Oluşturulmuştur!");
		document.location.href = '<cfoutput>#request.self#?fuseaction=pda.form_add_sale&company_id=#attributes.company_id#</cfoutput>';
	</script>
	<cfcatch>
		<script type="text/javascript">
