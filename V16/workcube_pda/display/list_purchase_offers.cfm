...
<cfset attributes.is_purchase = 1>
...
<table cellpadding="0" cellspacing="0" align="center" style="width:98%">
	<tr style="height:35px;">
		<td class="headbold">Satınalma Tekliflerim</td>
	</tr>
</table>
<table cellpadding="2" cellspacing="1" border="0" align="center" style="width:98%">	
	<cfif isDefined('xml_offer_type') and xml_offer_type eq 1>
        <tr>
            <td align="right">
                <cfform name="get_purchase_offers" action="#request.self#?fuseaction=#attributes.fuseaction#">
                    ...
                    <select name="offer_status" id="offer_status" style="width:70px;">
                        <option value="" <cfif isDefined('attributes.offer_status') and not len(attributes.offer_status)>selected</cfif>>Tümü</option>                            	
                        <option value="1" <cfif isDefined('attributes.offer_status') and len(attributes.offer_status) and attributes.offer_status eq 1>selected</cfif>>Aktif</option>
                        <option value="0" <cfif isDefined('attributes.offer_status') and len(attributes.offer_status) and attributes.offer_status eq 0>selected</cfif>>Pasif</option>
                    </select>
		...
		<td class="color-row" align="center">
			<table cellpadding="2" cellspacing="1" border="0" style="width:100%" class="color-border" align="right">
				<tr class="color-header">
                    ...
                    <td class="form-title">Tedarikçi</td>
                    ...
                    <!---<td><cf_get_lang_main no='265.Döviz'><cfif attributes.listing_type eq 1><cf_get_lang_main no='261.Tutar'><cfelse><cf_get_lang_main no='672.Fiyat'></cfif></td> --->
                    <cfif Len(attributes.listing_type) and attributes.listing_type eq 1>
                        <!--- <td>Satış Calışan</td> --->
                        <td class="form-title"><cf_get_lang_main no='4.Proje'></td>
                        <!--- <td><cf_get_lang_main no='200.Fırsat'></td>       
                        <td><cf_get_lang_main no='1675.yontem'></td> --->
                    </cfif>
				...
				<cfif isdefined("attributes.form_varmi") and get_offer_list.recordcount>
                    ...
                    <cfoutput query="get_offer_list" startrow="#attributes.startrow#" maxrows="#attributes.maxrows#">
                        <tr class="color-row">
                            ...
                            <td width="150"><a href="#request.self#?fuseaction=pda.detail_offer&offer_id=#offer_id#" class="tableyazi">#left(offer_head,25)#<cfif len(offer_head) gt 25>...</cfif></a></td>
                           ...
                           <!--- <td align="right" style="text-align:right;">
                            ...
                            <cfelse>
                                ...
                                <cfif len(other_money_value)>
                                    <cfif session.pda.period_year gte 2009 and len(OTHER_MONEY) and OTHER_MONEY is 'YTL'>
                                        #session.pda.money#
                                    <cfelseif session.pda.period_year lt 2009 and len(OTHER_MONEY) and OTHER_MONEY is 'TL'>
                                        #session.pda.money#
                                    <cfelse>
                                        #OTHER_MONEY#
                                    </cfif>
                            ...
                            <cfif Len(attributes.listing_type) and attributes.listing_type eq 1>
                               ...
                               <!--- <td><cfif isdefined("get_offer_list.opp_id") and len(get_offer_list.opp_id)>
                                        <a href="#request.self#?fuseaction=sales.form_upd_opportunity&opp_id=#offer_opp.opp_id[listfind(opp_name_list,opp_id,',')]#" class="tableyazi" target="_blank">#offer_opp.opp_head[listfind(opp_name_list,opp_id,',')]#</a>
                                    </cfif>
                                </td>		
                                <td><cfif offer_zone eq 0><cf_get_lang_main no='1078.verilen'>
                                    <cfelseif offer_zone eq 1><cf_get_lang_main no='1473.Partner'>
                                    <cfelseif offer_zone eq 2><cf_get_lang no='45.public'>
                                    </cfif>
                                </td>--->
                            </cfif>
					...
					<cfif Len(attributes.listing_type) and attributes.listing_type eq 2 and len(attributes.member_name) and (len(attributes.company_id) or len(attributes.consumer_id))>
                        ...
                        <tr class="color-row">
                            <td colspan="17" style="text-align:left;"><cf_workcube_buttons is_upd='0' is_cancel='0' insert_info='Satış Siparişi Oluştur' insert_alert=''></td>
                        </tr>
                    </cfif>
					</cfform>
                <cfelse>
                    <tr class="color-row"> 
                        <td colspan="14" style="text-align:left;"><cfif len(attributes.form_varmi)><cf_get_lang_main no='72.Kayıt Yok'>!<cfelse><cf_get_lang_main no='289.Filtre Ediniz'>!</cfif></td>
                    </tr>
                </cfif>
			</table>
        </td>
