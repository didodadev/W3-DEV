...
<table border="0" cellpadding="0" cellspacing="0" align="center" style="width:98%;">
	<tr style="height:35px;">
		<td class="headbold">Üretim Emirleri</td>
	</tr>
</table>
	...
	<tr>
		...
		<cfform name="form_production" method="post" action="">
			<table border="0">
                ...
                <cfif isDefined('xml_search_type') and xml_search_type eq 1>
                    <tr>
                        <td>Ürün</td>
                        <td><cfoutput>
                ...
                <cfelseif isDefined('xml_search_type') and xml_search_type eq 2>
                     <tr>
                        <td>Sipariş Numarası</td>
                        <td><cfoutput>
                            <input type="text" name="order_no" id="order_no" value="" style="width:150px;" <cfif session.pda.use_onkeydown_enter>onkeydown<cfelse>onchange</cfif>="<cfif session.pda.use_onkeydown_enter>if(event.keyCode == 13)</cfif> {kontrol_prerecord('kontrol_prerecord_div');}">
                            </cfoutput>
                        </td>
                    </tr>              
                </cfif>
...
<script type="text/javascript">
	...
	function kontrol_prerecord(div_name)
	{
		...
		<cfelseif isDefined('xml_search_type') and xml_search_type eq 2>
			var form_values = form_values + '&order_no='+document.getElementById("order_no").value;		
		</cfif>
		...
		AjaxPageLoad(<cfoutput>'#request.self#</cfoutput>?fuseaction=pda.emptypopup_list_production_order'+form_values,div_name,1);
		return false;
	}
</script>
