...
SELECT BRAND_ID,BRAND_NAME FROM SETUP_BRAND WHERE MOTORIZED_VEHICLE = 1 ORDER
BY BRAND_NAME
</cfquery>
        ...
        <tr>
          <td>
            <table width="100%" cellpadding="2" cellspacing="1" border="0" height="100%">
              <tr valign="middle">
                <td class="txtbold" colspan="5"><cf_get_lang no='609.Satış-İade Talebi'></td>
              </tr>
...
<script type="text/javascript">
	...
	function sil(sy)
	{
		var my_element=eval("add_assetp_sales_request.row_kontrol"+sy);
		my_element.value=0;
	...
	function add_row()
	{
			...
			newCell = newRow.insertCell(newRow.cells.length);
			newCell.innerHTML = '<input  type="text" value=""  name="brand_name' + row_count +'" readonly=yes; style="width:120px">';
			
			newCell = newRow.insertCell(newRow.cells.length);
			newCell.innerHTML = '<input  type="text" value=""  name="brand_type' + row_count +'" readonly=yes; style="width:130px">';
	
			newCell = newRow.insertCell(newRow.cells.length);
			newCell.innerHTML = '<input  type="text" value=""  name="make_year' + row_count +'" readonly=yes; style="width:50px">';
			
			newCell = newRow.insertCell(newRow.cells.length);
			newCell.innerHTML = '<input  type="text"  value=""  name="quantity' + row_count +'" style="width:50px">';
	...
	{
			windowopen('<cfoutput>#request.self#</cfoutput>?fuseaction=objects.popup_list_ship_vehicles&field_id=add_assetp_sales_request.assetp_id' + no + '&field_name=add_assetp_sales_request.assetp_name' + no + '&brand_name=add_assetp_sales_request.brand_name' + no + '&model=add_assetp_sales_request.brand_type' + no + '&make_year=add_assetp_sales_request.make_year' + no ,'list');}
</script>
