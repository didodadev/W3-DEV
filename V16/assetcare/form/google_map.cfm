<!---
    Varlıklarda, koordinatların belirlenmesinin, koordinatları var olan varlıkların haritada gösterilmesinin,
    Google Map API kullanılarak yapılması için oluşturulmuştur.
    Alper ÇİTMEN - 24.03.2022
--->
<cfscript>
    if( isDefined("attributes.assetId") and len(attributes.assetId) ){
        removeChar = Replace(attributes.assetId, '[', '', 'ALL');
        assetIds = Replace(removeChar, ']', '', 'ALL');
    }
</cfscript>
<cf_box title="#getLang('','Fiziki Varlıklar',30004)# - #getLang('','Harita',34357)#" style="left: 0;top: 0;height: 80vh !important;" scroll="1" collapsable="1" resize="1" popup_box="#iif(isdefined("attributes.draggable"),1,0)#">
            ...
            <div id="pac-container">
                <div class="col col-2">
                    <table class="table">
                        <thead>
                            <tr>
                                <td style="background-color:#ccc;padding:5px 0 5px 7px;" colspan="2" class="bold"><cf_get_lang dictionary_id='57420.Varlıklar'></td>
                            </tr>
                        </thead>
                        <cfloop array="#listToArray(assetIds)#" index="i">
                            <cfquery name="get_assets" datasource="#dsn#">
                                ...
                                FROM
                                    ASSET_P
                                        INNER JOIN ASSET_P_SUB_CAT AC ON AC.ASSETP_SUB_CATID = ASSET_P.ASSETP_SUB_CATID,
                                    ASSET_P_CAT,
                                    DEPARTMENT,
                                    BRANCH
                                WHERE
                                    ASSETP_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#i#"> AND
                            ...
                            <cfoutput>
                                <tbody>
                                    <tr>
                                        <td>#get_assets.ASSETP#</td>
                                        <td>
                                            <a href="#request.self#?fuseaction=assetcare.list_assetp&event=upd&assetp_id=#get_assets.ASSETP_ID#" target="_blank">#get_assets.ASSETP_SUB_CAT#</a>
                                        </td>
                                    </tr>
                                </tbody>
                            </cfoutput>
                            <cfif len(get_assets.coordinate_1) and len(get_assets.coordinate_2)>
                                <cfset ArrayAppend(coordinates, "[{ lat: #get_assets.coordinate_1#, lng: #get_assets.coordinate_2# },  '#get_assets.ASSETP#<br>#get_assets.branch_name#<br>#get_assets.ASSETP_CAT#<br>#get_assets.ASSETP_SUB_CAT#'];") />
                            </cfif>
                        </cfloop>
                    </table>
                </div>
