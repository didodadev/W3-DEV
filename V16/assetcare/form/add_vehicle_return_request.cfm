...
<div class="col col-12 col-md-12 col-sm-12 col-xs-12">
	<cf_box>
		<form name="add_return_request" method="post" action="<cfoutput>#request.self#</cfoutput>?fuseaction=assetcare.emptypopup_add_vehicle_return_request">
			<cf_basket_form id="return_request">
				<cf_box_elements>
						...
						<div class="form-group" id="item-branch_id">
							...
							<div class="col col-8 col-sm-12">
								<div class="input-group">
									...
									<span class="input-group-addon icon-ellipsis" href="javascript://" onClick="openBoxDraggable('<cfoutput>#request.self#</cfoutput>?fuseaction=objects.popup_list_branches&field_branch_name=add_return_request.branch&field_branch_id=add_return_request.branch_id');"></span>
								</div>
							</div>                
						</div> 
						...
						<div class="form-group" id="item-employee_id">
							...
							<div class="col col-8 col-sm-12">
								<div class="input-group">
									...
									<span class="input-group-addon icon-ellipsis" href="javascript://" onClick="openBoxDraggable('<cfoutput>#request.self#?fuseaction=objects.popup_list_positions&field_emp_id=add_return_request.employee_id&field_name=add_return_request.employee&select_list=1&branch_related</cfoutput>')"></span>
								</div>
							</div>                
						</div> 
					</div>
			...
			<cf_basket id="return_request_bask">
				<cfinclude template="../form/add_return_row.cfm">
			</cf_basket>
			<cf_box_footer>
...
function sil(sy)
{
	var my_element=eval("document.add_return_request.row_kontrol"+sy);
	my_element.value=0;
...
function add_row()
{
	...
	document.add_return_request.record_num.value=row_count;
	...
	newCell = newRow.insertCell(newRow.cells.length);
	newCell.innerHTML = '<div class="form-group"><div class="input-group"><input type="hidden" value="1"  name="row_kontrol' + row_count +'"><input type="hidden" name="assetp_id'+ row_count +'" value=""><input type="text" name="assetp'+ row_count+'" value=""><span class="input-group-addon icon-ellipsis" href="javascript://" onClick="plaka_ac(' + row_count + ');"></span></div></div>';
...
{
	openBoxDraggable('<cfoutput>#request.self#</cfoutput>?fuseaction=objects.popup_list_ship_vehicles&field_id=add_return_request.assetp_id'+ no1 +'&field_name=add_return_request.assetp'+ no1 +'&field_brand_type_id=add_return_request.brand_type_id'+ no1 +'&field_brand_name=add_return_request.brand_name'+ no1 +'&field_make_year=add_return_request.make_year'+ no1);
}		
function kontrol()
/* Zorunlu alan kontrolleri için yazılan scriptler... Onur 01112004*/
{
	if(document.add_return_request.detail.value.length > 250)
	{
		alert("<cf_get_lang dictionary_id='57425.uyarı'>:<cf_get_lang dictionary_id='48082.en fazla 250 karakter'>!");
		return false;
	}
	
	if(document.add_return_request.branch_id.value == "")
	{
		alert("<cf_get_lang dictionary_id='58194.girilmesi zorunlu alan'>:<cf_get_lang dictionary_id='29532.Şube Adı'>!");
		return false;
	}
	
	if(document.add_return_request.employee.value == "")
	{
		alert("<cf_get_lang dictionary_id='58194.girilmesi zorunlu alan'>:<cf_get_lang dictionary_id='47953.Talep Eden'>!");
		return false;
	}
	
	if(document.add_return_request.request_date.value == "")
	{
		alert("<cf_get_lang dictionary_id='58194.girilmesi zorunlu alan'>:<cf_get_lang dictionary_id='47994.Talep Tarihi'>!");
		return false;
	}
	
	if(!CheckEurodate(add_return_request.request_date.value,"<cf_get_lang dictionary_id='47994.Talep Tarihi'>")) 
	{
		return false;
	}
	...
	{
		alert("<cf_get_lang dictionary_id='48464.En Az Bir Satır İade Kaydı Girmelisiniz'>!");
		return false;
	}

	if(document.add_return_request.record_num.value == 0)
	{
		alert("<cf_get_lang dictionary_id='58194.girilmesi zorunlu alan'>:<cf_get_lang dictionary_id='48094.En Az Bir Satır Talep Kaydı'>!");
		return false;
	}
		for(r=1;r<=add_return_request.record_num.value;r++)
		{
			deger_row_kontrol = eval("document.add_return_request.row_kontrol"+r);
			deger_make_year = eval("document.add_return_request.make_year"+r);
			if(deger_row_kontrol.value == 1)
			{
				if(eval("document.add_return_request.assetp"+r).value == "")
				{
					alert("<cf_get_lang dictionary_id='58194.girilmesi zorunlu alan'>:<cf_get_lang dictionary_id='29453.Plaka'>!");
					return false;
				}
				if(eval("document.add_return_request.brand_name"+r).value == "")
				{
					alert("<cf_get_lang dictionary_id='58194.girilmesi zorunlu alan'>:<cf_get_lang dictionary_id='30041.Marka Tipi'>!");
					return false;
				}
				x = deger_make_year.selectedIndex;
				if (deger_make_year[x].value == "")
				{
					alert("<cf_get_lang dictionary_id='58194.girilmesi zorunlu alan'>:<cf_get_lang dictionary_id='58225.Model'>!");
					return false;
				}
			}
			eval("document.add_return_request.make_year"+r).disabled = false
		}
