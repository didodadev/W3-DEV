...
<cf_grid_list>
	<thead>
        <tr>
            <th width="20">
                <input name="record_num" id="record_num" type="hidden" value="<cfif isDefined("get_care_report_row.recordcount") and get_care_report_row.recordcount><cfoutput>#get_care_report_row.recordcount#</cfoutput><cfelse>0</cfif>"> 
                <a onclick="add_row();"><i class="fa fa-plus" title="<cf_get_lang_main no ='170.Ekle'>"></i></a>
            </th>
            <th width="250"><cf_get_lang no='191.Bakım Kategorisi'> *</th>
            <th width="250"><cf_get_lang_main no='217.Açıklama'></th>
            <th width="100"><cf_get_lang_main no='223.Miktar'> *</th>
            <th width="85">&nbsp;<cf_get_lang_main no='224.Birim'> *</th>
            <th width="85"><cf_get_lang_main no='672.Fiyat'></th>
            <th width="85"><cf_get_lang_main no='77.Para Birimi'></th>
            <th width="65"><cf_get_lang_main no='227.kdv'></th>
            <th width="65"><cf_get_lang_main no='229.iskonto'></th>
            <th width="85"><cf_get_lang_main no='261.Tutar'></th>
		</tr>
    </thead>
	<cfif isDefined("attributes.care_report_id") and len(attributes.care_report_id)>
			...
			<cfif get_care_report_row.recordCount>
				<cfoutput query="get_care_report_row">
					<tr id="frm_row#currentrow#">
						<td nowrap><a onClick="sil_sor(#currentrow#);"><i class="fa fa-minus" title="<cf_get_lang dictionary_id='57463.Sil'>" alt="<cf_get_lang dictionary_id='57463.Sil'>"></i></a></td>
						<td nowrap>
							<div class="form-group">
								<div class="input-group">
									...
									<span class="input-group-addon icon-ellipsis" href="javascript://" onClick="openBoxDraggable('#request.self#?fuseaction=objects.popup_care_cat_names&field_id=asset_care.care_cat_id#currentrow#&field_name=asset_care.care_cat#currentrow#');"></span>
								</div>
							</div>
						</td>
						...
						<td>
							<div class="form-group">
								<select name="money_currency#currentrow#" id="money#currentrow#">
						...
						<td nowrap><div class="form-group"><input type="text" name="total_price#currentrow#" id="total_price#currentrow#" onBlur="fiyat_hesapla('#currentrow#',1);" value="#tlformat(get_care_report_row.total_price)#" class="moneybox" onKeyUp='return(FormatCurrency(this,event));'></div></td>
					</tr>
				</cfoutput>
			</cfif>
		</tbody>
...
<cfif isDefined("row") and len(row)>
...
<cfelse>
	...
	function add_row()
	{
		...
		newCell.innerHTML = '<a onClick="sil(' + row_count + ');"><i class="fa fa-minus" title="<cf_get_lang_main no ='51.sil'>" alt="<cf_get_lang_main no ='51.sil'>"></i></a>';							
		...
		newCell = newRow.insertCell(newRow.cells.length);
		newCell.innerHTML = '<input type="hidden" value="1" id="row_kontrol' + row_count +'" name="row_kontrol' + row_count +'"><input type="hidden" name="care_cat_id'+ row_count +'" value=""><div class="form-group"><div class="input-group"><input type="text" name="care_cat'+ row_count +'" value="" readonly="yes"><span class="input-group-addon icon-ellipsis" title="<cf_get_lang no ='702.Bakım Kalemi Ekle'>" href="javascript://" onClick="pencere_ac1('+ row_count +');"></span></div></div>';
	
		newCell = newRow.insertCell(newRow.cells.length);
		newCell.innerHTML = '<input type="text" name="detail'+ row_count +'" id="detail'+ row_count +'" maxlength="50" style="width:300px;">';
		...
		newCell = newRow.insertCell(newRow.cells.length);
		newCell.innerHTML = '<div class="form-group"><select name="unit' + row_count +'" id="unit' + row_count +'"><option value=""></option><cfoutput query="get_unit"><option value="#unit_id#">#unit#</option></cfoutput></select>';
		...
		newCell = newRow.insertCell(newRow.cells.length);
		newCell.innerHTML = '<div class="form-group"><input type="text" name="total_price' + row_count +'" id="total_price' + row_count +'" onBlur="fiyat_hesapla(' + row_count + ',1);"  onkeyup="return(FormatCurrency(this,event));" class="moneybox"></div>';
		
	}
	
	function pencere_ac1(no)
	{
		openBoxDraggable('<cfoutput>#request.self#</cfoutput>?fuseaction=objects.popup_care_cat_names&field_id=asset_care.care_cat_id'+ no +'&field_name=asset_care.care_cat'+ no);
	}
	...
	function sil(sy)
	{
		var my_element=eval("asset_care.row_kontrol"+sy);
		my_element.value=0;
	...
	{
		/*if(eval("asset_care.price"+satir).value.length != 0 && eval("asset_care.quantity"+satir).value.length != 0)
		{
			eval("asset_care.total_price" + satir).value =  parseFloat(filterNum(eval("document.asset_care.price"+satir).value) * filterNum(eval("document.asset_care.quantity"+satir).value));
			eval("asset_care.total_price" + satir).value = commaSplit(eval("asset_care.total_price" + satir).value);
		}*/
