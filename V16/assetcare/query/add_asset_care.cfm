<cfif (isDefined("attributes.expense_net") and len(attributes.expense_net))>
	<cfset attributes.expense_net = filterNum(attributes.expense_net)>
</cfif>
<cfif (isDefined("attributes.expense") and len(attributes.expense))>
	<cfset attributes.expense = filterNum(attributes.expense)>
</cfif>
<cfif isdefined("attributes.support_finish_date") and len(attributes.support_finish_date)>
<cf_date tarih='attributes.support_finish_date'>
<!---	<cfset care_start_date = attributes.support_finish_date>--->
	<cfset attributes.start_clock = "">
	<cfset attributes.start_minute = "">
	<!---<cf_date tarih='care_start_date'>--->
</cfif>
<cfif isdefined("attributes.care_start_date") and len(attributes.care_start_date)>
...
<cfelse>
	<cfif isdefined("attributes.care_start_date") and len(attributes.care_start_date)>
		<cfset attributes.calender_date = attributes.care_start_date>
    </cfif>
</cfif>
...
<cfset ADD_ASSET_CARE = login_act.addAssetpCareFnc( 
  		...
  		<cfquery name="GET_MAX_ID" datasource="#dsn#" maxrows="1">
			SELECT CARE_REPORT_ID FROM ASSET_CARE_REPORT ORDER BY CARE_REPORT_ID DESC
		</cfquery>
      ...
      <cfif isdefined('process_stage') and len(process_stage)>
		<cf_workcube_process
			...
			action_id='#get_max_id.care_report_id#'
			action_page='#request.self#?fuseaction=assetcare.list_asset_care&event=upd&care_report_id=#get_max_id.care_report_id#'
			warning_description='Bakım Sonucu : #get_max_id.care_report_id#'>
     </cfif>
	</cftransaction>
</cflock>
<script type="text/javascript">	
	window.location.href= <cfif isdefined("attributes.is_detail")>document.referrer;<cfelse>'<cfoutput>#request.self#?fuseaction=assetcare.list_asset_care&event=upd&care_report_id=#get_max_id.care_report_id#</cfoutput>';</cfif>	
</script>
