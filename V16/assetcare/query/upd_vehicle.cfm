	...
	<cftransaction>
		...
		<cfquery name="UPD_ASSETP" datasource="#dsn#">
			UPDATE
				ASSET_P
			SET
				DEPARTMENT_ID = <cfif len(attributes.department_id)>#attributes.department_id#</cfif>,
				DEPARTMENT_ID2 = <cfif isdefined('attributes.department_id2') and len(attributes.department_id2)>#attributes.department_id2#<cfelseif len(attributes.department_id)>#attributes.department_id#<cfelse>NULL</cfif>,
				EMPLOYEE_ID = <cfif len(attributes.emp_id)>#attributes.emp_id#<cfelse>NULL</cfif>,
				POSITION_CODE = <cfif len(attributes.position_code)>#attributes.position_code#<cfelse>NULL</cfif>,
				...
				STATUS = <cfif len(attributes.status) or not isDefined("attributes.status")>0<cfelse>#attributes.status#</cfif>,
				UPDATE_DATE = #now()#,
