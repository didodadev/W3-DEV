...
<cfset yakit_tarih = now()>
<cfif len(attributes.fuel_month) and len(attributes.fuel_year)>
  <cfset yakit_tarih = createodbcdatetime('#attributes.fuel_year#-#attributes.fuel_month#-01')>
  <cfset yakit_tarih = date_add('m', 1, yakit_tarih)>
</cfif>
  <cfquery name="get_fuel" datasource="#dsn#" maxrows="12">
	SELECT
	...
	WHERE 
		FUEL_DATE <  #yakit_tarih#
		<cfif len(attributes.department)>
		AND ASSET_P_FUEL.DEPARTMENT_ID = #attributes.department_id#
		</cfif>
		...
		AND ASSET_P_FUEL.DEPARTMENT_ID = DEPARTMENT.DEPARTMENT_ID
		</cfif>
		<cfif len(attributes.assetp) and (attributes.assetp_id)>
		AND ASSET_P_FUEL.ASSETP_ID = #attributes.assetp_id#
		AND ASSET_P_FUEL.ASSETP_ID = ASSET_P.ASSETP_ID
		</cfif> 
	GROUP BY
		<cfif database_type is 'MSSQL'>
...
<cfset sutun = get_fuel.recordCount>
	...
	<thead>
    	<tr>
			...
			WHERE
				...
				AND DEPARTMENT.DEPARTMENT_ID = ASSET_P_FUEL.DEPARTMENT_ID
				</cfif>
				<cfif len(attributes.depot)>
				AND BRANCH.BRANCH_ID = #attributes.depot_id#
				</cfif>
			...
			<cfif len(attributes.assetp)>
				...
				<cfsavecontent variable="ay"><cf_get_lang_main no ='180.Ocak'></cfsavecontent>
			...
			<cfif get_fuel.recordcount>
				<cfset aylar = "#ay#,#ay2#,#ay3#,#ay4#,#ay5#,#ay6#,#ay7#,#ay8#,#ay9#,#ay10#,#ay11#,#ay12#">
