...
<cfquery name="GET_WEEKLY_EVENTS" datasource="#DSN#">
	...
	WHERE
		...
		AND
		(
			((EVENT.STARTDATE >= #attributes.to_day#) AND (EVENT.STARTDATE < #DATEADD("ww",1,attributes.to_day)#)) OR
			((EVENT.FINISHDATE >= #attributes.to_day#) AND (EVENT.FINISHDATE < #DATEADD("ww",1,attributes.to_day)#))
		) 
		AND 
		(
			<cfif isDefined("session.agenda_userid")>
				...
				<cfif session.agenda_user_type is "P">
					<!--- PAR --->
					((EVENT.RECORD_PAR = #session.agenda_userid# OR EVENT.UPDATE_PAR = #session.agenda_userid#) AND EVENT_CAT.EVENTCAT_ID <> 1) OR
				...
				<cfelseif session.agenda_user_type is "E">
					<!--- EMP --->
					((EVENT.RECORD_EMP = #session.agenda_userid# OR EVENT.UPDATE_EMP = #session.agenda_userid#) AND EVENT_CAT.EVENTCAT_ID <> 1) OR
			...
			<cfif isdefined('attributes.view_agenda') and attributes.view_agenda eq 3><!--- herkes gorsun --->
                ...
                (
                		(
                    	EVENT.VIEW_TO_ALL = 0 
                        AND EVENT.IS_VIEW_COMPANY IS NOT NULL 
