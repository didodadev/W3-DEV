...
<cfquery name="GET_POSITION_CODE" datasource="#dsn#">
	...
	WHERE
		...
		LAST_RESPONSE_DATE	<= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#DateAdd('m',1,attributes.to_day)#">
	GROUP BY
	 	PARENT_ID,POSITION_CODE
</cfquery>

<cfquery name="GET_MONTHLY_PAGE_WARNINGS" datasource="#dsn#">
	SELECT 
		W_ID,
		SMS_WARNING_DATE,
		EMAIL_WARNING_DATE,
	...
	WHERE
		...
		IS_PARENT = 1 AND
		LAST_RESPONSE_DATE >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#attributes.to_day#"> AND 
		LAST_RESPONSE_DATE	<= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#DateAdd('m',1,attributes.to_day)#"> AND
		IS_ACTIVE = 1
