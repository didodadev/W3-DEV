<!---
File: add_budget_transfer_form.cfm
Author: Workcube - Melek KOCABEY <melekkocabey@workcube.com>
Date: 13.10.2020
Controller: WBO/controller/BudgetTransferDemandController.cfm
Description: Bütçe Aktarım Talepleri add sayfası.
---->
<cf_xml_page_edit fuseact="budget.budget_transfer_demand,myhome.budget_transfer_demand">
...
<cfparam name="attributes.demand_date" default="#now()#">
<cfset comp = createObject("component", "V16.budget.cfc.BudgetTransferDemand")>
<cfset get_money = comp.get_money()>
<cfset get_activity = comp.get_activity()>
<cfset det_demand = isdefined("attributes.budget_info") ?  attributes.budget_info : "">
<cf_papers paper_type="budget_transfer_demand">

<cfset attributes.responsible_emp_id= isdefined('x_to_position_code') and len(x_to_position_code) ? x_to_position_code : "">
<cfset attributes.responsible_emp= isdefined('x_to_position_code') and len(x_to_position_code)  ? get_emp_info(x_to_position_code,1,0) : "">
<cfif len(det_demand)>
    <cfset record_val = deserializeJSON(det_demand)>                                                                                
    <cfset attributes.project_head = len(record_val[1]["PROJECT_ID"])  ? get_project_name(record_val[1]["PROJECT_ID"]): "">
</cfif>
<cf_catalystHeader>
<div class="col col-12 col-md-12 col-sm-12 col-xs-12">
    <cf_box>
        <cfform name="add_budget_transfer" id="add_budget_transfer" method="post">
            <input type="hidden" name="demand_id" id="demand_id" value="">
            <cfif isdefined("attributes.budget_info")>
            ...
            <cfelseif isdefined("attributes.expense_") and len(attributes.expense_)>
                <input type="hidden" name="expense_" id="expense_" value="<cfoutput>#attributes.expense_#</cfoutput>">
            </cfif>
            <cf_box_elements>
                    ...
                    <div class="form-group" id="item-budget">
                        ...
                        <div class="col col-8 col-xs-12">
                            <div class="input-group">
                                ...
                                <span class="input-group-addon btnPointer icon-ellipsis" onclick="openBoxDraggable('<cfoutput>#request.self#</cfoutput>?fuseaction=objects.popup_list_budget&field_id=add_budget_transfer.budget_id&field_name=add_budget_transfer.budget_name&select_list=2');return false" title="<cfoutput>#getLang('budget','İlişkili Bütçe',49179)#</cfoutput>"></span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col col-4 col-md-4 col-sm-6 col-xs-12" type="column" index="2" sort="true">
                    <div class="form-group" id="item-demand_no">
                        ...
                        <div class="col col-8 col-xs-12">
                            <cfif isdefined("paper_code") and len(paper_code) and len(paper_number)>
                                <cfinput type="text" name="demand_no" id="demand_no" value="#paper_code & '-' & paper_number#" maxlength="40" readonly>
                            <cfelse>
                                <cfinput type="text" name="demand_no" id="demand_no" value="" maxlength="40">
                            </cfif>
                        </div>
                    </div>
                    <div class="form-group" id="item-demand_emp">
                        ...
                        <div class="col col-8 col-xs-12">
                            <div class="input-group">
                                ...
                                <span class="input-group-addon btnPointer icon-ellipsis" onclick="openBoxDraggable('<cfoutput>#request.self#</cfoutput>?fuseaction=objects.popup_list_positions&field_emp_id=add_budget_transfer.employee_id&field_name=add_budget_transfer.employee_name&select_list=1');"></span>
                            </div>
                        </div>
                    </div> 
                    <div class="form-group" id="item-responsible_emp">
                        ...
                        <div class="col col-8 col-xs-12"> 
                            <div class="input-group">
                                <cfoutput>
                                    <input type="hidden" name="responsible_emp_id" id="responsible_emp_id" value="<cfif len(attributes.responsible_emp_id)>#attributes.responsible_emp_id#</cfif>">
                                    ...
                                    <span class="input-group-addon icon-ellipsis btnPointer" onclick="openBoxDraggable('#request.self#?fuseaction=objects.popup_list_positions&field_code=add_budget_transfer.responsible_emp_id&field_name=add_budget_transfer.responsible_emp&select_list=1');" title="<cf_get_lang dictionary_id='57924.Kime'>"></span>
                                </cfoutput>
                            </div>
                        </div>
                    </div>               
                    ...
                    <div class="form-group" id="item-demand_date">
                        ...
                        <div class="col col-8 col-xs-12">
                            <div class="input-group">
                                ...
                                <cfinput type="text" name="demand_date" value="#dateformat(attributes.demand_date,dateformat_style)#" maxlength="10" required="Yes" message="#message#" validate="#validate_style#" onblur="change_money_info('add_budget_transfer','demand_date');">
...
<script type="text/javascript">
    <cfif len(det_demand)>
        $( document ).ready(function() {
            ...
            project_head = '<cfoutput>#attributes.project_head#</cfoutput>';
            processedBlocks = [];
            processedRows = [];

            //console.log(getJSON);
            $.each(getJSON, function( index, value ) {
                $("#reference_no").val(value.reference_no);
                if($.inArray(value.sayac,processedRows) == -1) { // İlk gelenden row alanlarını dolduruyoruz.
                    processedRows.push(value.sayac);
                    
                    addRow();
                    acc_input_hidden = $("#acc_control_" + $("#rowCount").val() + '_' + 2 );
                    acc_input = $("#acc_" + $("#rowCount").val() + '_' + 2 );
                    exp_item_input_hidden = $("#bud_cat_control_" + $("#rowCount").val() + '_' + 2 );
                    exp_item_input = $("#bud_cat_" + $("#rowCount").val() + '_' + 2 );
                    activity_type_input = $("#exp_act_type_" + $("#rowCount").val() + '_' + 2);

                    rate_input = $("#rate_" + $("#rowCount").val() + '_' + 1 );
                    serbest_input = $("#serbest_" + $("#rowCount").val() + '_' + 2 );
                    money_type = $("#money_type_" + $("#rowCount").val() + '_' + 2);
                    action_type_input_hidden = $("#action_type_hidden_" + $("#rowCount").val() + '_' + 2 );
                    action_type_input = $("#action_type_" + $("#rowCount").val() + '_' + 2 );
                    project_id_input1 = $("#proje_id_" + $("#rowCount").val() + '_' + 2 );
                    project_id_input_hidden1 = $("#proje_control_" + $("#rowCount").val() + '_' + 2 );

                    acc_input_hidden.val(value.expense_id);
                    acc_input.val(value.expense_name);
                    exp_item_input_hidden.val(value.expense_item_id);
                    exp_item_input.val(value.expense_item_name);
                    activity_type_input.val(value.activity_type);
                    project_id_input_hidden1.val(value.project_id);
                    project_id_input1.val(project_head);
                    
                    rate_input.val(value.amount);
                   
                    serbest_input.val(value.usable_money);
                    money_type.val(value.money_currency.toUpperCase());
    ...
    function addRow()
		{
			rowCount++;
			add_budget_transfer.rowCount.value = rowCount;
			var newRow;
			var newCell;
			newRow = $('<div />');
    ...
    function pencere_ac(acc_input, acc_cont_input) {
        ...
        openBoxDraggable("<cfoutput>#request.self#</cfoutput>?fuseaction=objects.popup_expense_center&field_id=add_budget_transfer." + acc_cont_input +"&field_name=add_budget_transfer." + acc_input + "&call_function=hesapla&call_function_parameter='"+acc_cont_input+"','"+ cat_cont_input +"','"+proje_cont_input+"',"+rowNum+","+i+"");
    }
    function pencere_item_ac(cat_input, cat_cont_input) {
        ...
        var proje_cont_input = 'proje_control_' + rowNum + '_' + i;
        openBoxDraggable("<cfoutput>#request.self#</cfoutput>?fuseaction=objects.popup_list_exp_item&field_id=add_budget_transfer." + cat_cont_input +"&field_name=add_budget_transfer." + cat_input+"&function_name=hesapla&function_name_parameter='"+acc_cont_input+"','"+ cat_cont_input +"','"+proje_cont_input+"',"+rowNum+","+i+"");
        }
    function pencere_proje_ac(proje_input, proje_cont_input) {
            ...
            var cat_cont_input = 'bud_cat_control_' + rowNum + '_' + i; 
            var acc_cont_input = 'acc_control_' + rowNum + '_' + i;
			openBoxDraggable("<cfoutput>#request.self#</cfoutput>?fuseaction=objects.popup_list_projects&project_id=add_budget_transfer." + proje_cont_input +"&project_cat_gln=1&project_head=add_budget_transfer." + proje_input+"&function_name=hesapla&function_param='"+acc_cont_input+"','"+ cat_cont_input +"','"+proje_cont_input+"',"+rowNum+","+i+"");
        }
    ...
    function createBlock(row_number) {
			block = $("#block_detail_" + row_number);
                ...
                acc_selector3.attr('title','Hesapla');
                ...
                budget_type_input.attr('class','col col-8 col-md-8 col-sm-8 col-xs-12');
