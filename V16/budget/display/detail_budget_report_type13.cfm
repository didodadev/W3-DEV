<cfquery name="get_cost" datasource="#dsn#"><!--- Planlanan --->
	...
	WHERE
		...
		WORK_GROUP.WORKGROUP_ID = BUDGET_PLAN_ROW.WORKGROUP_ID AND
		DATEPART(MM,BUDGET_PLAN_ROW.PLAN_DATE) IN (#month_list#) AND
		WORK_GROUP.STATUS = <cfqueryparam cfsqltype="cf_sql_smallint" value="1"> AND WORK_GROUP.IS_BUDGET = <cfqueryparam cfsqltype="cf_sql_smallint" value="1">
...
<cfquery name="get_cost2" datasource="#dsn2#"><!--- Gerçekleşen --->
	...
	(
		SELECT
			...
			WORK_GROUP.WORKGROUP_ID,
			WORK_GROUP.WORKGROUP_NAME,
			EXPENSE_ITEMS_ROWS.ACTIVITY_TYPE AS ACTIVITY_TYPE_ID
		FROM
			EXPENSE_ITEMS_ROWS,
			#dsn_alias#.WORK_GROUP WORK_GROUP
		WHERE
			EXPENSE_ITEMS_ROWS.WORKGROUP_ID = WORK_GROUP.WORKGROUP_ID AND
...
<cfquery name="get_other_exp" datasource="#dsn2#">
	SELECT 
		...
		WORKGROUP_NAME 
	FROM
		#dsn_alias#.WORK_GROUP
	WHERE
		WORKGROUP_ID NOT IN(SELECT WORKGROUP_ID FROM #dsn_alias#.BUDGET_PLAN_ROW WHERE WORKGROUP_ID IS NOT NULL AND DATEPART(MM,PLAN_DATE) IN (#month_list#)) AND 
		WORKGROUP_ID IN(SELECT WORKGROUP_ID FROM <cfif not fusebox.use_period>#dsn_alias#.</cfif>EXPENSE_ITEMS_ROWS WHERE WORKGROUP_ID IS NOT NULL AND DATEPART(MM,EXPENSE_DATE) IN (#month_list#) AND IS_INCOME = <cfqueryparam cfsqltype="cf_sql_smallint" value="1"> AND TOTAL_AMOUNT > 0) AND
		STATUS = <cfqueryparam cfsqltype="cf_sql_smallint" value="1"> AND IS_BUDGET = <cfqueryparam cfsqltype="cf_sql_smallint" value="1">
</cfquery>
...
<cfelse>
				...
				<cfloop list="#month_list#" index="k">
					<th colspan="#colspan_info#" align="center" class="form-title" nowrap><cfoutput>#listgetat(aylar,k,',')#</cfoutput></th>
					  <cfquery name="GET_COST_#k#" dbtype="query">
						SELECT * FROM GET_COST WHERE MONTH_VALUE = #k# ORDER BY WORKGROUP_ID
					  </cfquery>
					  ...
					  <cfif ay_que.recordcount>
						<cfset 'ay_cat_list_#k#' = listsort(valuelist(ay_que.WORKGROUP_ID,','),'numeric','ASC',',')>
					  </cfif>
					  <cfquery name="GET_COST_2_#k#" dbtype="query">
						SELECT * FROM GET_COST2 WHERE MONTH_VALUE = #k# ORDER BY WORKGROUP_ID
					  </cfquery>
					  ...
					  <cfif ay_que2.recordcount>
						<cfset 'ay_cat_list2_#k#' = listsort(valuelist(ay_que2.WORKGROUP_ID,','),'numeric','ASC',',')>
					  </cfif>
			...
			<tr>
				...
				<cfloop list="#month_list#" index="k">
					...
					<cfif isdefined("attributes.is_money") and len(session.ep.money2)>
						...
						<th align="center"><cf_get_lang dictionary_id='49176.Gerçekleşen'></th><cfset myCol = myCol+1>
						<th nowrap><cf_get_lang dictionary_id='57489.Para Birimi'></th><cfset myCol = myCol+1>
						<cfif isdefined("attributes.is_diff")><cfset myCol = myCol+1>
							<th width="110" align="center" nowrap><cf_get_lang dictionary_id='58583.Fark'></th><cfset myCol = myCol+1>
							<th nowrap><cf_get_lang dictionary_id='57489.Para Birimi'></th><cfset myCol = myCol+1>
						</cfif>
					</cfif>
		...
		<cfif get_expense_center.recordcount>
			<tbody>
				<cfoutput query="get_expense_center">	
					...
					<tr>
						<td nowrap>#GET_EXPENSE_CENTER.WORKGROUP_NAME#</td>
						<cfif isdefined("attributes.is_activity_type") and len(attributes.is_activity_type)>
							<td>
								<cfif len(GET_EXPENSE_CENTER.ACTIVITY_TYPE_ID)>
									<cfquery name="GET_ACTIVITY_TYPE" datasource="#DSN#">
										...
										FROM
											SETUP_ACTIVITY
										WHERE
											ACTIVITY_ID  = <cfqueryparam cfsqltype="cf_sql_integer" value="#GET_EXPENSE_CENTER.ACTIVITY_TYPE_ID#">
										ORDER BY
											ACTIVITY_NAME
									</cfquery>
						...
						<cfloop list="#month_list#" index="ay_ind">
							...
							<cfset cat_yeri = listfind(evaluate('ay_cat_list_#ay_ind#'),WORKGROUP_ID,',')>
							...
							<cfset cat_yeri2 = listfind(evaluate('ay_cat_list2_#ay_ind#'),WORKGROUP_ID,',')>
	...
	<cfif isdefined("attributes.is_expense")>
		...
		<cfquery name="get_other_exp" datasource="#dsn2#">
			...
			WHERE
				WORKGROUP_ID NOT IN(SELECT WORKGROUP_ID FROM #dsn_alias#.BUDGET_PLAN_ROW WHERE WORKGROUP_ID IS NOT NULL AND DATEPART(MM,PLAN_DATE) IN (#month_list#)) AND 
				WORKGROUP_ID IN(SELECT WORKGROUP_ID FROM <cfif not fusebox.use_period>#dsn_alias#.</cfif>EXPENSE_ITEMS_ROWS WHERE WORKGROUP_ID IS NOT NULL AND DATEPART(MM,EXPENSE_DATE) IN (#month_list#) AND IS_INCOME = <cfqueryparam cfsqltype="cf_sql_smallint" value="0"> AND TOTAL_AMOUNT > 0) AND
				STATUS = <cfqueryparam cfsqltype="cf_sql_smallint" value="1"> AND IS_BUDGET = <cfqueryparam cfsqltype="cf_sql_smallint" value="1">
		</cfquery>
		<cfquery name="GET_EXPENSE_CENTER" dbtype="query">
			SELECT DISTINCT 
