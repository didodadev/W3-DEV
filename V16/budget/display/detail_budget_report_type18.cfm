<!--- Tarih aralığına göre Fiziki Varlik Bazında --->
<cfquery name="get_act_types" datasource="#dsn2#">
	SELECT 
		DISTINCT ISNULL(PYSCHICAL_ASSET_ID,0) PYSCHICAL_ASSET_ID
	FROM
		EXPENSE_ITEMS_ROWS
	WHERE
		TOTAL_AMOUNT > 0					
...
<cfif not listlen(exp_inc_center_list)><cfset exp_inc_center_list = valuelist(get_act_types.PYSCHICAL_ASSET_ID)></cfif>
<cfquery name="GET_ASSETP" datasource="#DSN#">
	...
	WHERE
		...
		<cfif len(exp_inc_center_list)>
			AND ASSETP_ID IN (#exp_inc_center_list#)
		</cfif>
	ORDER BY
		ASSETP
</cfquery>
<cfoutput>
	<thead>	
 	...
 	<cfparam name="attributes.totalrecords" default='#get_assetp.recordcount#'>
	<cfif attributes.page neq 1>
		<cfoutput query="get_assetp" startrow="1" maxrows="#attributes.startrow-1#">
			<cfquery name="GET_EXPENSE_2" datasource="#DSN#">
				SELECT 
					...
					<cfif isdefined("attributes.is_activity_type") and len(attributes.is_activity_type)>
						<cfif len(get_assetp.activity_id)>
							,#get_assetp.activity_id# AS ACTIVITY_TYPE_ID
						<cfelse>
				...
				WHERE 
					...
					BUDGET_PLAN_ROW.ASSETP_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#ASSETP_ID#"> AND
				...
				<cfif isdefined("attributes.is_activity_type") and len(attributes.is_activity_type)>
					<cfif len(get_assetp.activity_id)>
						AND BUDGET_PLAN_ROW.ACTIVITY_TYPE_ID = #get_assetp.activity_id#
					<cfelse>
			...
			<cfquery name="GET_ROWS_2" datasource="#dsn2#">
				...
				(
					...
					FROM
						EXPENSE_ITEMS_ROWS
					WHERE
						PYSCHICAL_ASSET_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#ASSETP_ID#"> AND
						TOTAL_AMOUNT > 0					
					<cfif len(attributes.search_date1)>
						AND EXPENSE_DATE >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#attributes.search_date1#">
					</cfif>
					...
					<cfif isdefined("attributes.is_activity_type") and len(attributes.is_activity_type)>
						<cfif len(get_assetp.activity_id)>
							AND EXPENSE_ITEMS_ROWS.ACTIVITY_TYPE = #get_assetp.activity_id#
						<cfelse>
	...
	<cfif get_assetp.recordcount>
		<tbody>
			<cfoutput query="get_assetp" startrow="#attributes.startrow#" maxrows="#attributes.maxrows#">
				<cfquery name="GET_EXPENSE_2" datasource="#DSN#">
					SELECT 
						SUM(BUDGET_PLAN_ROW.ROW_TOTAL_INCOME) ROW_TOTAL_INCOME,
						SUM(BUDGET_PLAN_ROW.ROW_TOTAL_EXPENSE) ROW_TOTAL_EXPENSE,
		...
		<tfoot>		  
			<cfoutput>
				<tr>
					<td colspan="3" height="20" class="txtbold"><cf_get_lang dictionary_id='57492.Toplam'></td>
					<cfif isdefined("attributes.is_income")>					
