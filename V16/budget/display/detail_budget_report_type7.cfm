<cfquery name="get_cost" datasource="#dsn#"><!--- Planlanan --->
	...
	FROM
		BUDGET_PLAN,
		BUDGET_PLAN_ROW,
		<cfif fusebox.use_period>#dsn2_alias#.</cfif>EXPENSE_ITEMS EXPENSE_ITEMS
	WHERE
		...
		EXPENSE_ITEMS.EXPENSE_ITEM_ID = BUDGET_PLAN_ROW.BUDGET_ITEM_ID AND
...
<cfquery name="get_cost2" datasource="#dsn2#"><!--- Gerçekleşen --->
	...
	(
		...
		FROM
			EXPENSE_ITEMS_ROWS,
			<cfif not fusebox.use_period>#dsn_alias#.</cfif>EXPENSE_ITEMS EXPENSE_ITEMS
		WHERE
...
<cfquery name="get_other_exp" datasource="#dsn2#">
	SELECT 
		...
		ACCOUNT_CODE
	FROM
		<cfif not fusebox.use_period>#dsn_alias#.</cfif>EXPENSE_ITEMS
	WHERE
		EXPENSE_ITEM_ID NOT IN(SELECT BUDGET_ITEM_ID FROM #dsn_alias#.BUDGET_PLAN_ROW WHERE BUDGET_ITEM_ID IS NOT NULL AND DATEPART(MM,PLAN_DATE) IN (#month_list#) AND BUDGET_PLAN_ID IN(SELECT BUDGET_PLAN.BUDGET_PLAN_ID FROM #dsn_alias#.BUDGET_PLAN WHERE BUDGET_PLAN.BUDGET_ID IN (<cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.general_budget_id#" list="yes">))) AND 
		EXPENSE_ITEM_ID IN(SELECT EXPENSE_ITEM_ID FROM <cfif not fusebox.use_period>#dsn_alias#.</cfif>EXPENSE_ITEMS_ROWS WHERE EXPENSE_ITEM_ID IS NOT NULL AND DATEPART(MM,EXPENSE_DATE) IN (#month_list#) AND IS_INCOME = <cfqueryparam cfsqltype="cf_sql_smallint" value="1"> AND TOTAL_AMOUNT > 0)
...
UNION ALL
	...
	FROM 
		get_other_exp
	<cfif isdefined("attributes.expense_item_id") and len(attributes.expense_item_id)>
		WHERE EXPENSE_ITEM_ID IN (#attributes.expense_item_id#)
	</cfif>
</cfquery>
...
<cfelse>
	...
	<cfif isdefined("attributes.is_income")>
		<cfoutput>
			<thead>
				<tr>
					<th style="text-align:left;" colspan="80"><cf_get_lang dictionary_id='58089.Gelirler'></th>
				</tr>
				<tr>
					...
					<cfloop list="#month_list#" index="k">
						<th colspan="#colspan_info#" align="center" nowrap><cfoutput>#listgetat(aylar,k,',')#</cfoutput></th>
							<cfquery name="GET_COST_#k#" dbtype="query">
							SELECT * FROM GET_COST WHERE MONTH_VALUE = #k# ORDER BY EXPENSE_ITEM_ID
							</cfquery>
							...
							<cfset 'ay_cat_list_#k#' = listsort(valuelist(ay_que.EXPENSE_ITEM_ID,','),'numeric','ASC',',')>
							</cfif>
							<cfquery name="GET_COST_2_#k#" dbtype="query">
							SELECT * FROM GET_COST2 WHERE MONTH_VALUE = #k# ORDER BY EXPENSE_ITEM_ID
							</cfquery>
							...
							<cfset 'ay_cat_list2_#k#' = listsort(valuelist(ay_que2.EXPENSE_ITEM_ID,','),'numeric','ASC',',')>
							</cfif>
						
					</cfloop>
					<th align="center" colspan="<cfoutput>#colspan_info#</cfoutput>" nowrap><cf_get_lang dictionary_id='57492.Toplam'></th>
				</tr>
				<tr>
					<cfif isdefined('is_show_account_code') and is_show_account_code eq 1>
						<th nowrap><cf_get_lang dictionary_id='58811.Muhasebe Kodu'></th><cfset myCol = myCol + 1>
					</cfif>
					<th><cf_get_lang dictionary_id='58234.Bütçe Kalemi'></th><cfset myCol = myCol + 1>
			...
			<cfif get_expense_center.recordcount>
				<tbody>		
					<cfoutput query="GET_EXPENSE_CENTER">
						...
						<tr class="color-row">
						<cfif isdefined('is_show_account_code') and is_show_account_code eq 1>
							<td nowrap style="mso-number-format:\@;">#GET_EXPENSE_CENTER.ACCOUNT_CODE#</td>
						</cfif>
						<td nowrap>#GET_EXPENSE_CENTER.EXPENSE_ITEM_NAME#</td>
						
						<cfloop list="#month_list#" index="ay_ind">
							<cfset ay_que = evaluate('GET_COST_#ay_ind#')>
							<cfset cat_yeri = listfind(evaluate('ay_cat_list_#ay_ind#'),EXPENSE_ITEM_ID,',')>
							...
							<cfset cat_yeri2 = listfind(evaluate('ay_cat_list2_#ay_ind#'),EXPENSE_ITEM_ID,',')>
	...
	<cfif isdefined("attributes.is_expense")>	
		...
		<cfquery name="get_other_exp" datasource="#dsn2#">
			...
			WHERE
				EXPENSE_ITEM_ID NOT IN(SELECT BUDGET_ITEM_ID FROM #dsn_alias#.BUDGET_PLAN_ROW WHERE BUDGET_ITEM_ID IS NOT NULL AND DATEPART(MM,PLAN_DATE) IN (#month_list#) AND BUDGET_PLAN_ID IN(SELECT BUDGET_PLAN.BUDGET_PLAN_ID FROM #dsn_alias#.BUDGET_PLAN WHERE BUDGET_PLAN.BUDGET_ID IN (<cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.general_budget_id#" list="yes">))) AND 
				EXPENSE_ITEM_ID IN(SELECT EXPENSE_ITEM_ID FROM <cfif not fusebox.use_period>#dsn_alias#.</cfif>EXPENSE_ITEMS_ROWS WHERE EXPENSE_ITEM_ID IS NOT NULL AND DATEPART(MM,EXPENSE_DATE) IN (#month_list#) AND IS_INCOME = <cfqueryparam cfsqltype="cf_sql_smallint" value="0"> AND TOTAL_AMOUNT > 0)
				<cfif attributes.is_all_exp_center eq 2>
					AND 1 = 2
				</cfif>
		</cfquery>
        ...
        <thead>
            ...
            <tr>
                ...
                <th><cf_get_lang dictionary_id='58234.Bütçe Kalemi'></th><cfset myCol2 = myCol2+1>
