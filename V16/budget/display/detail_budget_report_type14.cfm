<cfquery name="get_cost" datasource="#dsn#"><!--- Planlanan --->
	SELECT
		...
		EXPENSE_CENTER2.EXPENSE,
		EXPENSE_CENTER2.EXPENSE_ID,
		EXPENSE_CENTER2.EXPENSE_CODE,
		BUDGET_PLAN_ROW.ACTIVITY_TYPE_ID AS ACTIVITY_ID
	FROM
		BUDGET_PLAN,
		BUDGET_PLAN_ROW,
		<cfif fusebox.use_period>#dsn2_alias#.</cfif>EXPENSE_CENTER EXPENSE_CENTER,
		<cfif fusebox.use_period>#dsn2_alias#.</cfif>EXPENSE_CENTER EXPENSE_CENTER2
	WHERE
		...
		EXPENSE_CENTER2.EXPENSE_CODE NOT LIKE '%.%' AND
		<!--- EXPENSE_CENTER2.HIERARCHY = <cfqueryparam cfsqltype="cf_sql_integer" value="1"> AND --->
		EXPENSE_CENTER.EXPENSE_ACTIVE = 1 AND
		(
			EXPENSE_CENTER.EXPENSE_CODE LIKE EXPENSE_CENTER2.EXPENSE_CODE+'.%' OR
			(
				EXPENSE_CENTER.EXPENSE_CODE = EXPENSE_CENTER2.EXPENSE_CODE AND
				EXPENSE_CENTER.EXPENSE_CODE NOT LIKE '%.%'
			)
		)
...
<cfquery name="get_cost2" datasource="#dsn2#"><!--- Gerçekleşen --->
	...
	(
		...
		FROM
			...
			<cfif not fusebox.use_period>#dsn_alias#.</cfif>EXPENSE_CENTER EXPENSE_CENTER2
		WHERE
...
<cfquery name="get_other_exp" datasource="#dsn2#">
	...
	WHERE
		EXPENSE_CENTER.EXPENSE_ACTIVE = 1 AND
		EXPENSE_ID NOT IN(SELECT EXP_INC_CENTER_ID FROM #dsn_alias#.BUDGET_PLAN_ROW WHERE EXP_INC_CENTER_ID IS NOT NULL AND DATEPART(MM,PLAN_DATE) IN (#month_list#)) AND 
...
<cfelse>
	...
	<cfif isdefined("attributes.is_income")>
		...
		<thead>
			<tr>
				<th style="text-align:left;" colspan="80" nowrap><cf_get_lang dictionary_id='58089.Gelirler'></th>
			</tr>
			<tr>
			...
			<tr>
				<cfif isdefined('is_show_exp_center_code') and is_show_exp_center_code eq 1>
					<th nowrap><cf_get_lang dictionary_id='49098.Gelir Merkezi Kodu'></th><cfset myCol = myCol+1>
				</cfif>
		...
		<cfif get_expense_center.recordcount>
            ...
            <tfoot>
				<tr>
					...
					<td class="txtbold" style="text-align:right;" colspan="#colspan#"><cf_get_lang dictionary_id='57492.Toplam'></td>
	...
	<cfif isdefined("attributes.is_expense")>
		...
		<cfquery name="get_other_exp" datasource="#dsn2#">
			...
			WHERE
				...
				AND EXPENSE_CENTER.EXPENSE_CODE NOT LIKE '%.%'
				<cfif attributes.is_all_exp_center eq 2>
					AND 1 = 2
				</cfif>
				<cfif isdefined("attributes.expense_center_id") and len(attributes.expense_center_id)>
		...
		<cfif get_expense_center.recordcount>
			...
			<cfoutput>
				<tr>
					...
					<td nowrap class="txtbold" style="text-align:right;" colspan="#colspan#"><cf_get_lang dictionary_id='57492.Toplam'></td>				
					<cfloop list="#month_list#" index="ay_ind">
						<td nowrap class="txtbold" style="text-align:right;" format="numeric"><cfoutput>#TlFormat(evaluate('toplam_ay_#ay_ind#'))#</cfoutput></td>
						<td class="txtbold">&nbsp;<cfoutput>#session.ep.money#</cfoutput></td>
						<td nowrap class="txtbold" style="text-align:right;" format="numeric"><cfoutput>#TlFormat(evaluate('toplam_ay2_#ay_ind#'))#</cfoutput></td>
