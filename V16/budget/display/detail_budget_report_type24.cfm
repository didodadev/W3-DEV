<cfquery name="get_act_types" datasource="#dsn2#">
	SELECT 
		DISTINCT ISNULL(PROJECT_ID,0) PROJECT_ID
	FROM
		EXPENSE_ITEMS_ROWS
	WHERE
		TOTAL_AMOUNT > 0					
...
<cfset project_list = valuelist(get_act_types.PROJECT_ID)>
...
<cfoutput>
	<thead>	
		...
		<tr class="color-header" height="20">
			...
			<th width="150" nowrap><cf_get_lang dictionary_id='31027.İş Grubu'></th>
 	...
 	<cfparam name="attributes.totalrecords" default='#GET_PROJECT.recordcount#'>
	<cfif attributes.page neq 1>
		<cfoutput query="GET_PROJECT" startrow="1" maxrows="#attributes.startrow-1#">
			<cfquery name="GET_EXPENSE_2" datasource="#DSN#">
				SELECT 
					...
					,#GET_PROJECT.activity_id# AS ACTIVITY_TYPE_ID
				FROM
					BUDGET_PLAN,
					BUDGET_PLAN_ROW 
				WHERE 
					...
					BUDGET_PLAN_ROW.PROJECT_ID IN (<cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.project_id#" list="yes">) AND
				...
				<cfif len(attributes.search_date2)>
                    ...
                    AND BUDGET_PLAN_ROW.ACTIVITY_TYPE_ID = #GET_PROJECT.activity_id#
			...
			<cfquery name="GET_ROWS_2" datasource="#dsn2#">
				...
				(
					...
					FROM
						EXPENSE_ITEMS_ROWS
					WHERE
						PROJECT_ID IN (<cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.project_id#" list="yes">) AND
						TOTAL_AMOUNT > 0					
					<cfif len(attributes.search_date1)>
						AND EXPENSE_DATE >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#attributes.search_date1#">
					</cfif>
					<cfif len(attributes.search_date2)>
                    	...
                    	AND EXPENSE_ITEMS_ROWS.ACTIVITY_TYPE = #GET_PROJECT.activity_id#
				)T1
				GROUP BY
					EXPENSE_CENTER_ID,
					IS_INCOME,
	...
	<cfif GET_PROJECT.recordcount>
		<tbody>
			<cfoutput query="GET_PROJECT" startrow="#attributes.startrow#" maxrows="#attributes.maxrows#">
				<cfquery name="GET_EXPENSE_2" datasource="#DSN#">
					SELECT 
						SUM(BUDGET_PLAN_ROW.ROW_TOTAL_INCOME) ROW_TOTAL_INCOME,
						SUM(BUDGET_PLAN_ROW.ROW_TOTAL_EXPENSE) ROW_TOTAL_EXPENSE,
