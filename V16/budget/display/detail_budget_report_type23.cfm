<cfquery name="GET_EXPENSE_ITEM" datasource="#dsn2#">
	...
	WHERE
		...
		<cfif len(exp_inc_center_list)>
			AND EXPENSE_ITEM_ID IN (#exp_inc_center_list#)
		</cfif>
	...
	<thead>
        <tr>
			...
			<cfif isdefined("attributes.ei_diff") and isdefined("attributes.is_income") and isdefined("attributes.is_expense")>				
	...
	<cfif attributes.page neq 1>
		<cfoutput query="get_expense_item" startrow="1" maxrows="#attributes.startrow-1#">
			<cfquery name="GET_EXPENSE_2" datasource="#DSN#">
				...
				WHERE 
					...
					BUDGET_PLAN_ROW.BUDGET_ITEM_ID IN (#expense_item_id#) AND
					...
					<cfif len(attributes.search_date2)>
						AND BUDGET_PLAN_ROW.PLAN_DATE < <cfqueryparam cfsqltype="cf_sql_integer" value="#DATEADD('d',1,attributes.search_date2)#">
					</cfif>
			...
			<cfquery name="GET_ROWS_2" datasource="#dsn2#">
				...
				(
					...
					FROM
						EXPENSE_ITEMS_ROWS
					WHERE
						EXPENSE_ITEM_ID IN (#expense_item_id#) AND
						TOTAL_AMOUNT > 0					
					<cfif len(attributes.search_date1)>
			...
			<cfoutput query="get_expense_item" startrow="#attributes.startrow#" maxrows="#attributes.maxrows#">
				<cfquery name="GET_EXPENSE_2" datasource="#DSN#">
					SELECT 
						...
						SUM(BUDGET_PLAN_ROW.ROW_TOTAL_INCOME_2) ROW_TOTAL_INCOME_2,
						SUM(BUDGET_PLAN_ROW.ROW_TOTAL_EXPENSE_2) ROW_TOTAL_EXPENSE_2
                        <cfif len(GET_EXPENSE_ITEM.activity_id)>
                            ,#GET_EXPENSE_ITEM.activity_id# AS ACTIVITY_TYPE_ID
                        <cfelse>
                    ...
                    <cfif len(GET_EXPENSE_ITEM.activity_id)>
                        AND BUDGET_PLAN_ROW.ACTIVITY_TYPE_ID = #GET_EXPENSE_ITEM.activity_id#
                    <cfelse>
				...
				<cfquery name="GET_ROWS_2" datasource="#dsn2#">
					...
					(
                        ...
                        <cfif len(GET_EXPENSE_ITEM.activity_id)>
                            AND EXPENSE_ITEMS_ROWS.ACTIVITY_TYPE = #GET_EXPENSE_ITEM.activity_id#
                        <cfelse>
	...
	<cfelse>
		<tbody>
			<tr>
				<td colspan="37"><cf_get_lang dictionary_id='57484.Kayıt Yok'>!</td>
			</tr>
		</tbody>
	</cfif>
