...
<cfquery name="CHECK_TABLE" datasource="#dsn2#">
	IF EXISTS (SELECT * FROM tempdb.sys.tables where name='####GET_EXPENSE_CENTER_CAT_#session.ep.userid#')
    DROP TABLE ####GET_EXPENSE_CENTER_CAT_#session.ep.userid#
</cfquery>
...
<cfquery name="GET_EXPENSE_CENTER_CAT" datasource="#dsn2#">
    SELECT 
    	EXPENSE.EXPENSE_ID,
        EXPENSE.EXPENSE,
        ...
        EXPENSE_CAT.EXPENSE_CAT_ID,
        EXPENSE_CAT.EXPENSE_CAT_NAME
    INTO ####GET_EXPENSE_CENTER_CAT_#session.ep.userid#
    FROM
	<cfif isdefined("attributes.is_activity_type") and len(attributes.is_activity_type)>
            (
            SELECT
    ...
    (	
        ...
        WHERE
            ...
            <cfif isdefined("attributes.expense_center_id") and len(attributes.expense_center_id) and attributes.expense_center_id neq 0>
    ...
    (
        ...
        WHERE
            ...
            <cfif len(exp_inc_center_list2)>
                AND EXPENSE_CAT_ID IN (SELECT EXPENSE_CATEGORY_ID FROM #db_alias_#.EXPENSE_ITEMS WHERE EXPENSE_ITEM_ID IN (#exp_inc_center_list2#))
            </cfif>        
    ) AS EXPENSE_CAT
</cfquery>
<cfquery name="GET_EXPENSE_BUDGET" datasource="#dsn2#">
    ...
    FROM
    	####GET_EXPENSE_CENTER_CAT_#session.ep.userid# AS GEC
    LEFT JOIN    
        (                
            SELECT 
     ...
     ON
     	PLANLANAN.EXP_INC_CENTER_ID = GEC.EXPENSE_ID AND PLANLANAN.EXPENSE_CAT_ID = GEC.EXPENSE_CAT_ID
     LEFT JOIN
     	(
        	SELECT
            ...
            (
                ...
                WHERE
                    TOTAL_AMOUNT > 0
                    AND EXPENSE_ITEMS_ROWS.EXPENSE_ITEM_ID IN(SELECT EXPENSE_ITEM_ID FROM #db_alias_#.EXPENSE_ITEMS WHERE EXPENSE_CATEGORY_ID = EXPENSE_CATEGORY.EXPENSE_CAT_ID)					
                    <cfif len(attributes.search_date1)>
     ...
     ON
     	   GERCEKLESEN.EXPENSE_CENTER_ID =  GEC.EXPENSE_ID AND GEC.EXPENSE_CAT_ID = GERCEKLESEN.EXPENSE_CAT_ID 
    WHERE 
		...
		(PLANLANAN.EXP_INC_CENTER_ID IS NOT NULL  OR GERCEKLESEN.EXPENSE_CENTER_ID IS NOT NULL)  
         ...
         CTE2 AS (
				SELECT
					CTE1.*,
						ROW_NUMBER() OVER (ORDER BY EXPENSE)
	...
	<thead>	
		<tr>
			...
			<cfif isdefined("attributes.is_income")>	
				<th colspan="#colspan_info#" align="center" nowrap><cf_get_lang dictionary_id='58677.Gelir'></th>
			</cfif>
			<cfif isdefined("attributes.is_expense")>
				
				<th colspan="#colspan_info#" align="center" nowrap><cf_get_lang dictionary_id='58678.Gider'></th>
			</cfif>
		...
		<tr>
			...
			<th nowrap><cf_get_lang dictionary_id='32999.Bütçe Kategorisi'></th>
			<cfif isdefined('is_show_exp_center_code') and is_show_exp_center_code eq 1>
	        ...
	        <cfoutput query="GET_EXPENSE_BUDGET">
				<tr>
					...
					<cfif isdefined("attributes.is_expense")>
						...
						<td width="5" align="center">
							<a href="#request.self#?fuseaction=cost.list_expense_management&form_exist=1<cfif len(attributes.project_id)>&project_id=#attributes.project_id#</cfif><cfif len(attributes.search_date1) and len(attributes.search_date2)>&search_date1=#dateformat(attributes.search_date1,dateformat_style)#&search_date2=#dateformat(attributes.search_date2,dateformat_style)#</cfif>&expense_center_id=#GET_EXPENSE_BUDGET.expense_id#&expense_cat=#GET_EXPENSE_BUDGET.EXPENSE_CAT_ID#<cfif len(attributes.process_cat)>&process_cat=#attributes.process_cat#</cfif>" class="tableyazi" target="blank_"><i class="icon-ellipsis"></i></a>																							
						</td>
						<td>&nbsp;#session.ep.money#</td>
						<cfif isdefined("attributes.is_diff")>
                ...
                <cfif attributes.maxrows*attributes.page gte attributes.totalrecords>
                    <tr>
                        ...
                        <cfif isdefined("attributes.durum") and isdefined("attributes.is_income") and isdefined("attributes.is_expense")>
                            ...
                            <td  class="txtbold" style="text-align:right;" format="numersic">#TLFormat(GET_EXPENSE_BUDGET.ALL_TOTAL_AMOUNT_BORC-GET_EXPENSE_BUDGET.ALL_TOTAL_AMOUNT_ALACAK)#</td>	
