<cftransaction>
	...
	<cfloop from="1" to="#attributes.record_num#" index="i">
		<cfif evaluate("attributes.row_kontrol#i#")>
			...
			<cfif isdefined("attributes.kdv_rate_#i#") and len(evaluate("attributes.kdv_rate_#i#"))>
			<cfset form_tax = evaluate("attributes.kdv_rate_#i#")>
			<cfelse>
				<cfset form_tax = ''>
			</cfif>
			<cfif isdefined("attributes.predicted_amount#i#") and len(evaluate("attributes.predicted_amount#i#"))>
				<cfset form_predicted_amount = evaluate("attributes.predicted_amount#i#")>
			<cfelse>
				<cfset form_predicted_amount = ''>
			</cfif>
			<cfif isdefined("attributes.amount#i#") and len(evaluate("attributes.amount#i#"))>
				<cfset form_amount = evaluate("attributes.amount#i#")>
                <cfset form_amount = replace(form_amount,',','.','all')>
			<cfelse>
				<cfset form_amount = ''>
			</cfif>
			...
			<cfquery name="ADD_SERVICE_OPERATION" datasource="#DSN3#">
				...
				(
					...
					'#form_product#',
					<cfif len(form_predicted_amount)>#form_predicted_amount#<cfelse>NULL</cfif>,
					...
					'#cgi.remote_addr#',
					#now()#,
					<cfif isdefined('form_tax') and len(form_tax)><cfqueryparam cfsqltype="cf_sql_float" value="#form_tax#"><cfelse>NULL</cfif>
			   )
			</cfquery>
		</cfif>
	</cfloop>
...
<script type="text/javascript">
	window.location.href='<cfoutput>#request.self#?fuseaction=service.list_service&event=upd&service_id=#attributes.service_id#</cfoutput>';
</script>
