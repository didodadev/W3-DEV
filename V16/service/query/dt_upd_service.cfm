...
<cfif isdefined("attributes.subscription_id") and len(attributes.subscription_id)>
	...
	<cfif get_system.recordcount and get_system.valid_days gte 1><!--- sistem var mi --->
		...
		<cfset deger_tarih_ilk_ = attributes.start_date1>
...
<cfquery name="UPD_SERVICE" datasource="#DSN3#">
	...
	SET
        ...
        SOLUTION = <cfif len(attributes.solution)>'#attributes.solution#'<cfelse>NULL</cfif>,
...
<cfquery name="GET_SERVICE1" datasource="#DSN3#">
	SELECT * FROM SERVICE WHERE SERVICE_ID = #url.id#
</cfquery>
...
<cfquery name="del_defected_code" datasource="#dsn3#">
	DELETE FROM SERVICE_CODE_ROWS WHERE SERVICE_ID=#url.id#
</cfquery>
...
<cf_workcube_process 
	...
	action_id='#url.id#' 
	action_page='#request.self#?fuseaction=service.list_service&event=upd&service_id=#url.id#' 
	warning_description='Servis No : #get_service1.service_no#'>
<script type="text/javascript">
	window.location.href='<cfoutput>#request.self#?fuseaction=service.list_service&event=upd&service_id=#url.id#</cfoutput>';
</script>
