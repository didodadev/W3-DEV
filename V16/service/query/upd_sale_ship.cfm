...
<cflock name="#CreateUUID()#" timeout="60">
	...
	<cfquery name="UPD_SALE" datasource="#DSN2#">
		...
		<cfif isdate(attributes.deliver_date_frm)>
			...
			DELIVER_EMP='#LEFT(TRIM(DELIVER_GET),50)#',
	...
	<cfloop from="1" to="#attributes.rows_#" index="i"><!--- Karma koli urunler icin --->
	 	...
	 	<cfquery name="ADD_SHIP_ROW" datasource="#DSN2#">
			...
			VALUES
				...
				<cfif isdefined('attributes.spect_id#i#') and len(evaluate('attributes.spect_id#i#'))>
					...
					<cfif isdefined('attributes.row_service_id#i#') and len(evaluate('attributes.row_service_id#i#'))>'#wrk_eval('attributes.row_service_id#i#')#'<cfelseif isdefined("attributes.service_id") and len(attributes.service_id)>#attributes.service_id#<cfelse>NULL</cfif>,
...
<cflocation url="#request.self#?fuseaction=service.popup_upd_sale_ship&ship_id=#attributes.UPD_ID#&service_id=#attributes.service_id#" addtoken="no">
