...
<cfif isdefined('attributes.appcat_id')>
    <cfif len(attributes.appcat_id)>
        <cfquery name="get_service_sub_appcat" datasource="#dsn3#">
             SELECT SERVICECAT_SUB_ID,#dsn#.Get_Dynamic_Language(SERVICECAT_SUB_ID,'#session.ep.language#','SERVICE_APPCAT_SUB','SERVICECAT_SUB',NULL,NULL,SERVICECAT_SUB) AS SERVICECAT_SUB FROM SERVICE_APPCAT_SUB WHERE SERVICECAT_ID = #attributes.appcat_id# ORDER BY SERVICECAT_SUB
        </cfquery>
...
<cfelse>
   <cfif len(attributes.appcat_sub_id)>
        <cfquery name="get_service_sub_status_appcat" datasource="#dsn3#">
             SELECT SERVICECAT_SUB_STATUS_ID,#dsn#.Get_Dynamic_Language(SERVICECAT_SUB_STATUS_ID,'#session.ep.language#','SERVICE_APPCAT_SUB_STATUS','SERVICECAT_SUB_STATUS',NULL,NULL,SERVICECAT_SUB_STATUS) AS SERVICECAT_SUB_STATUS FROM SERVICE_APPCAT_SUB_STATUS WHERE SERVICECAT_SUB_ID = #attributes.appcat_sub_id# ORDER BY SERVICECAT_SUB_STATUS
        </cfquery>
