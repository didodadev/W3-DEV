<cfif isdefined("attributes.is_form_submitted")>
	...
	<cfif isdefined('attributes.display_cost') and attributes.display_cost eq 1>
		<cfquery name="GET_ALL_PRODUCT_COST" datasource="#dsn3#" cachedwithin="#fusebox.general_cached_time#">
			SELECT 
				PRODUCT_ID, PURCHASE_NET_LOCATION PURCHASE_NET, PURCHASE_EXTRA_COST_LOCATION PURCHASE_EXTRA_COST, PURCHASE_NET_MONEY_LOCATION PURCHASE_NET_MONEY,START_DATE,RECORD_DATE,
	...
	<!--- urunlerin speclere gore DONEM SONU maliyet ve miktarlarını getiriyor --->
			...
			<cfquery name="GET_DS_SPEC_COST" datasource="#dsn1#" cachedwithin="#fusebox.general_cached_time#">
				SELECT  
					...
					ISNULL((SELECT
						...
						FROM 
							#dsn2_alias#.GET_PRODUCT_COST_PERIOD GPCP
						WHERE
		...
		<cfquery name="GET_ALL_STOCK" datasource="#dsn2#" cachedwithin="#fusebox.general_cached_time#">
			...
			<cfif listfind('1,2',attributes.report_type,',') and (isdefined('attributes.positive_stock') or isdefined('attributes.is_stock_action'))><!--- sadece pozitif stoklar veya sadece hareket gormus stokların gelmesi --->
				AND S.STOCK_ID IN (
						SELECT   
							SR.STOCK_ID
						FROM        
		...
		<cfif listfind('1,2',attributes.report_type)>
			<cfif len(attributes.process_type) and (listfind(attributes.process_type,4) or listfind(attributes.process_type,5) or listfind(attributes.process_type,12) or listfind(attributes.process_type,14))>
				<cfquery name="GET_STOCK_FIS" datasource="#dsn2#" cachedwithin="#fusebox.general_cached_time#">
					SELECT
						...
						GC.MALIYET MALIYET,
						(GC.MALIYET_2/(SF_M.RATE2/SF_M.RATE1)) MALIYET_2,
						<cfelse>
					...
					WHERE 
						...
						SF.FIS_TYPE  IN (110,111,112,115,119) AND 
			...
			<cfif len(attributes.process_type) and listfind(attributes.process_type,2) or listfind(attributes.process_type,3)>
				<cfquery name="GET_INV_PURCHASE" datasource="#dsn2#" cachedwithin="#fusebox.general_cached_time#">
					SELECT 
						...
						( (1- I.SA_DISCOUNT/(I.NETTOTAL-I.TAXTOTAL+I.SA_DISCOUNT)) * IR.NETTOTAL) BIRIM_COST,
						((IR.EXTRA_COST * IR.AMOUNT)/(IM.RATE2/IM.RATE1))EXTRA_COST_2,
						(( (1- I.SA_DISCOUNT/(I.NETTOTAL-I.TAXTOTAL+I.SA_DISCOUNT)) * IR.NETTOTAL)/(IM.RATE2/IM.RATE1)) BIRIM_COST_2
						<cfelse>
						((IR.EXTRA_COST * IR.AMOUNT)/(IM.RATE2/IM.RATE1))EXTRA_COST,
						(( (1- I.SA_DISCOUNT/(I.NETTOTAL-I.TAXTOTAL+I.SA_DISCOUNT)) * IR.NETTOTAL)/(IM.RATE2/IM.RATE1)) BIRIM_COST
						</cfif>
					FROM 
						INVOICE I,
						INVOICE_ROW IR,
			...
			<cfif len(attributes.process_type)>
				<cfquery name="GET_SHIP_ROWS" datasource="#dsn2#" cachedwithin="#fusebox.general_cached_time#">
					SELECT
						...
						<cfif isdefined('attributes.is_system_money_2')>
						((GC.MALIYET_2*ABS(GC.STOCK_IN-GC.STOCK_OUT))/(SM.RATE2/SM.RATE1)) AS TOTAL_COST_2,
				...
				UNION ALL
					SELECT
						...
						<cfif isdefined('attributes.is_system_money_2')>
						((GC.MALIYET_2*ABS(GC.STOCK_IN-GC.STOCK_OUT))/(IM.RATE2/IM.RATE1)) AS TOTAL_COST_2,
						(GC.MALIYET*ABS(GC.STOCK_IN-GC.STOCK_OUT)) AS TOTAL_COST,
						<cfelse>
		...
		<!---Dönem İçi Alış Miktar : Yurtiçi ve Yurtdışı Mal alım irsaliyelerindeki toplam miktar (Demirbaş, Konsinye, İade olmayacak)  --->
			<cfquery name="d_alis_miktar" dbtype="query">
				...
				FROM
					GET_SHIP_ROWS
				WHERE
					PROCESS_TYPE IN (76,80,811,84,87)
					...
					<cfscript>
						...
						if(isdefined('donem_sonu_#GET_ALL_STOCK.PRODUCT_ID#') and len(evaluate('donem_sonu_#GET_ALL_STOCK.PRODUCT_ID#')))
						{
							...
							if(isdefined('attributes.display_cost'))
							{
								...
								{
									if(isdefined("donem_sonu_specli_maliyet_#PRODUCT_GROUPBY_ID#") and len(evaluate("donem_sonu_specli_maliyet_#PRODUCT_GROUPBY_ID#")))
										donem_sonu_maliyet3=evaluate("donem_sonu_specli_maliyet_#PRODUCT_GROUPBY_ID#");
								
									if(isdefined("donem_sonu_specli_maliyet2_#PRODUCT_GROUPBY_ID#") and len(evaluate("donem_sonu_specli_maliyet2_#PRODUCT_GROUPBY_ID#")))
										ds_other_cost=evaluate("donem_sonu_specli_maliyet2_#PRODUCT_GROUPBY_ID#");
								}
								else
								{
