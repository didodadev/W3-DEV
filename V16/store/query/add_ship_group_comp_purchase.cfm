...
<cfquery name="get_ship" datasource="#old_dsn2#">
    ...
    FROM 
    	SHIP 
    WHERE 
    	SHIP_ID = #attributes.ship_id# AND IS_EXPORTED IS NULL 
</cfquery>
	 ...
	 <cfif get_ship_row.recordcount>
			...
			<cfif not control_other_comp_product.recordcount>
				...
				<script type="text/javascript">
					alert("<cf_get_lang no='46.Bu irsaliyedeki ürün, seçtiğiniz şirkette tanımlı deği'>.<cf_get_lang_main no ='245.Ürün'> : #get_prod_name.PRODUCT_NAME# " );
...
<cfelse>
	 <script type="text/javascript">
		alert("<cf_get_lang no='45.Bu İrsaliye Daha Önce Aktarılmış'>");
		wrk_opener_reload();
		window.close();
	</script>
	<cfabort>
