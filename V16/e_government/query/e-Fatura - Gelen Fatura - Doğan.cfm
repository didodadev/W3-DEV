		...
		<cftry>
			...
			<cfif len(invoice.InvoiceID)>
				...
				<cfquery name = "getPeriod" datasource = "#dsn#">
					SELECT * FROM SETUP_PERIOD WHERE OUR_COMPANY_ID = #get_einv_comp.comp_id# AND '#listFirst(invoice.issuetime,'+')#' BETWEEN START_DATE AND FINISH_DATE
				</cfquery>

				<cfset dsn2 = '#dsn#_#getPeriod.period_year#_#get_einv_comp.comp_id#'>
				
				<cffile action="write" file="#directory_name#/#invoice.uuid#.xml" output="#toString(tobinary(invoice.content))#" charset="utf-8" />
				...
				<cfif not invoice_control.recordcount>
					<cfscript>
						...
						add_invoice_receive_temp.add_received_invoices(service_result:'Successful',
																		...
																		issue_date:listFirst(invoice.issuetime,'+'),
