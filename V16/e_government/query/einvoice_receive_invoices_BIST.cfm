<!---
    ...
    Description:
        BIST için Dijital Planet gelen e-fatura aktarımı
    History:
                    ...
                    <cfif not invoice_control.recordcount>
						...
						<cfscript>
							if(get_received_invoice.recordCount){
								...
								department_id	= 46;//Raporlama ve Bütçe Servisi
								...
								if (fileExists(invoice_path)) {
									...
									if (isXML(invoice_file)) {
										...
										add_invoice_receive_temp.upd_received_invoice(dsn2_name:dsn2,receiving_id:get_received_invoice.RECEIVING_DETAIL_ID,process_stage:invoice_stage,department_id:department_id);
									}
								}
							}
						</cfscript>
