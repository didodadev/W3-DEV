...
<cfelse>
    ...
    <cfset LegalMonetaryTotal.LineExtensionAmount = get_producer.grosstotal>
    ...
    <cfset LegalMonetaryTotal.PayableAmount = get_producer.nettotal - temp_tax_amount>

    <cfset producer_receipt_number = "#session.ep.period_year##left('00000000',8-len(get_producer.invoice_id))##get_producer.invoice_id#">
    <cfset temp_eproducer_folder = 'eproducer_receipt_send'>
    ...
    <cfset preview_receipt_xml  = 'documents/eproducer_receipt_send/#session.ep.company_id#/#year(now())#/#numberformat(month(now()),00)#/#producer_receipt_number#.xml'>
    ...
    <cfinclude template="eproducer_receipt_digitalplanet.cfm" />
