<cfcomponent>
    ...
    <cffunction name="GetEInvoiceIntegrationInfo" access="public" hint="E-fatura entegrasyon bilgileri">
        <cfquery name="query_integrationinfo" datasource="#dsn2#">
            SELECT
                ...
                'https://authsoapapitest.superentegrator.com/' AS EINVOICE_TEST_URL,
                'https://authsoapapi.superentegrator.com/' AS EINVOICE_LIVE_URL
            FROM
    ...
    <cffunction name="GetAuthorizationVars" access="public" hint="Fatura entegrasyon doğrulama bilgileri">
        ...
        <cfquery name="get_authorizaton" datasource="#dsn2#">
            SELECT
                OCI.EINVOICE_COMPANY_CODE AS EFATURA_COMPANY_CODE,
                OCI.EINVOICE_USER_NAME AS EFATURA_USER_NAME,
                OCI.EINVOICE_PASSWORD AS EFATURA_PASSWORD,
                OC.TAX_NO 
            FROM
                #dsn#.OUR_COMPANY_INFO AS OCI
                JOIN #dsn#.OUR_COMPANY AS OC ON OCI.COMP_ID = OC.COMP_ID
            WHERE
                OCI.COMP_ID = #arguments.company_id#
        </cfquery>
