<cfscript>
    loadedJsonString = FileRead(ExpandPath( "./" ) & "/WDO/catalogs\sysparam.json");
...
<cfloop collection="#local.systemParam#" item="item">
    ...
    <cfif isStruct(local.systemParam[item])>

        <cfset counterExtraInfo = 1 />
        <cfset response[counter]["extra"] = arrayNew(1) />

        <cfloop collection="#local.systemParam[item]#" item="item_extra">
...
<!--- <cfoutput>
    #Replace(serializeJSON(response),"//","")#
</cfoutput>
<cfabort> --->
    ...
    <tbody>
        <cfif arrayLen(response)>
            <cfloop array = "#response#" item = "item"> 
                <tr>
                    <td><cfif not structKeyExists(item, "extra")><a href="javascript:void(0);" onclick="setValue('<cfoutput>#item.key#</cfoutput>','<cfoutput>#len(item.title) ? item.title : item.key#</cfoutput>');"><cfoutput>#item.key#</cfoutput></a><cfelse><cfoutput>#item.key#</cfoutput></cfif></td>
                            ...
                            <cf_grid_list>
                                ...
                                <tbody>
                                    <cfloop array = "#item['extra']#" item = "itemExtra">
                                        <tr>
                                            <td><a href="javascript:void(0);" onclick="setValue('<cfoutput>#item.key#</cfoutput>.<cfoutput>#itemExtra.key#</cfoutput>','<cfoutput>#len(itemExtra.title) ? itemExtra.title : item.key & "." & itemExtra.key #</cfoutput>');"><cfoutput>#itemExtra.key#</cfoutput></a></td>
...
<script type="text/javascript">
    function setValue(key, title) {
        window.opener.setFieldValue({ type: "SystemParam", formula: key, value: "SystemParam=>" + title, name: "systemparam_" + key });
        window.close();
    }
</script>