...
<cfquery datasource="#yeni_dsn#" name="column_length_all">
	SELECT name,xtype,isnullable,id,prec,colorder,colstat FROM syscolumns WHERE id IN (SELECT id FROM sysobjects WHERE xtype='U' AND name IN (#listqualify(attributes.tablename,"'")#) AND SUBSTRING(name,1,1) <> '_') AND name IN (#listqualify(attributes.columnnames,"'")#) AND SUBSTRING(name,1,1) <> '_' AND xtype = 99
</cfquery>
<cfquery datasource="#eski_dsn#" name="column_length_old">
	SELECT name,xtype,isnullable,id,prec,colorder,colstat FROM syscolumns WHERE id IN (SELECT id FROM sysobjects WHERE xtype='U' AND name IN (#listqualify(attributes.tablename,"'")#) AND SUBSTRING(name,1,1) <> '_') AND name IN (#listqualify(attributes.columnnames,"'")#) AND SUBSTRING(name,1,1) <> '_' AND xtype = 231
</cfquery>
<cfset sql_code = "">
<cfoutput query="table_length">
	...
	<cfif syscolumns_records.recordcount>
		<cfloop from="1" to="#syscolumns_records.recordcount#" index="i_column">
			...
			<cfif syscolumns_records_old.xtype[i_column] eq 231 and syscolumns_records.xtype[i_column] eq 99>
				<cfset sql_code = sql_code & "ALTER TABLE #table_length.name# #chr(13)#">
				<cfset sql_code = sql_code & "ALTER COLUMN #chr(13)#">
				<cfset sql_code = sql_code & "	#syscolumns_records.name[i_column]# NTEXT">
