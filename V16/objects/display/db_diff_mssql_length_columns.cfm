...
<cfquery datasource="#eski_dsn#" name="table_length_old">
	SELECT name,id FROM sysobjects WHERE xtype='U' AND name IN (#listqualify(attributes.tablename,"'")#) AND SUBSTRING(name,1,1) <> '_' and  schema_name(uid)='#eski_dsn#' ORDER BY name 
</cfquery>

<cfquery datasource="#yeni_dsn#" name="column_length_all">
	SELECT name,xtype,isnullable,id,prec,colorder,colstat FROM syscolumns WHERE id IN (SELECT id FROM sysobjects WHERE xtype='U' AND name IN (#listqualify(attributes.tablename,"'")#) and  schema_name(uid)='#yeni_dsn#' AND SUBSTRING(name,1,1) <> '_') AND name IN (#listqualify(attributes.columnnames,"'")#) AND SUBSTRING(name,1,1) <> '_' AND xtype = 231
</cfquery>
<cfquery datasource="#eski_dsn#" name="column_length_old">
	SELECT name,xtype,isnullable,id,prec,colorder,colstat FROM syscolumns WHERE id IN (SELECT id FROM sysobjects WHERE xtype='U' AND name IN (#listqualify(attributes.tablename,"'")#)  and  schema_name(uid)='#eski_dsn#' AND SUBSTRING(name,1,1) <> '_') AND name IN (#listqualify(attributes.columnnames,"'")#) AND SUBSTRING(name,1,1) <> '_' AND xtype = 231
</cfquery>
...
<cfoutput query="table_length">
	...
	<cfquery dbtype="query" name="syscolumns_records_old">
		SELECT * FROM column_length_old WHERE id = #table_length_old.id# ORDER BY NAME
	</cfquery>

	<cfif syscolumns_records.recordcount>
		<cfloop from="1" to="#syscolumns_records.recordcount#" index="i_column">
			<!--- Uzunluklari farkli ve tipi nvarchar ise --->
			<cfif syscolumns_records.prec[i_column] gt syscolumns_records_old.prec[i_column] and syscolumns_records.xtype[i_column] eq 231>
				<cfset sql_code = sql_code & "ALTER TABLE #table_length.name# #chr(13)#">
				<cfset sql_code = sql_code & "ALTER COLUMN #chr(13)#">
				<cfset sql_code = sql_code & "	#syscolumns_records.name[i_column]# NVARCHAR (#syscolumns_records.prec[i_column]#)">
