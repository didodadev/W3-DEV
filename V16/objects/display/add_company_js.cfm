...
GET_OUR_COMPANY_INFO = cfquery(SQLString:'SELECT ISNULL(IS_SELECT_RISK_MONEY,0) IS_SELECT_RISK_MONEY FROM OUR_COMPANY_INFO WHERE COMP_ID=#session.ep.COMPANY_ID#',Datasource:dsn);
GET_CREDIT_ALL = cfquery(SQLString:'SELECT PAYMETHOD_ID,REVMETHOD_ID,CARD_PAYMETHOD_ID,CARD_REVMETHOD_ID,SHIP_METHOD_ID,TRANSPORT_COMP_ID,TRANSPORT_DELIVER_ID,MONEY FROM COMPANY_CREDIT WHERE COMPANY_ID=#attributes.c_id# AND OUR_COMPANY_ID=#session.ep.COMPANY_ID#',Datasource:dsn);
if(fusebox.use_period eq 1)
	GET_CREDIT_ALL_MONEY = cfquery(SQLString:'SELECT PAYMETHOD_ID,REVMETHOD_ID,CARD_PAYMETHOD_ID,CARD_REVMETHOD_ID,SHIP_METHOD_ID,TRANSPORT_COMP_ID,TRANSPORT_DELIVER_ID,COMPANY_CREDIT.MONEY,RATE2,RATE1,PAYMENT_RATE_TYPE,RATE3,EFFECTIVE_SALE,EFFECTIVE_PUR FROM COMPANY_CREDIT,#dsn2_alias#.SETUP_MONEY SETUP_MONEY WHERE COMPANY_CREDIT.MONEY = SETUP_MONEY.MONEY AND COMPANY_CREDIT.COMPANY_ID=#attributes.c_id# AND COMPANY_CREDIT.OUR_COMPANY_ID=#session.ep.COMPANY_ID#',Datasource:dsn);
else
	GET_CREDIT_ALL_MONEY = cfquery(SQLString:'SELECT PAYMETHOD_ID,REVMETHOD_ID,CARD_PAYMETHOD_ID,CARD_REVMETHOD_ID,SHIP_METHOD_ID,TRANSPORT_COMP_ID,TRANSPORT_DELIVER_ID,COMPANY_CREDIT.MONEY,RATE2,RATE1,PAYMENT_RATE_TYPE,RATE3,EFFECTIVE_SALE,EFFECTIVE_PUR FROM COMPANY_CREDIT,#dsn_alias#.SETUP_MONEY SETUP_MONEY WHERE COMPANY_CREDIT.MONEY = SETUP_MONEY.MONEY AND COMPANY_CREDIT.COMPANY_ID=#attributes.c_id# AND COMPANY_CREDIT.OUR_COMPANY_ID=#session.ep.COMPANY_ID#',Datasource:dsn);
...
if(isdefined("attributes.field_member_account_code") and  isdefined("attributes.x_add_multi_acc") and attributes.x_add_multi_acc eq 1 and isdefined("attributes.is_multi_act") and attributes.is_multi_act eq 1)
	attributes.member_account_code=get_company_period(attributes.c_id,session.ep.period_id,dsn2,attributes.acc_type_id);
	...
	function add_user(p_id,c_id,name,c_name,long_address,address,postcode,semt,county,county_id,city,city_id,country,country_id,member_account_code,pay_method_id,pay_method,due_of_paymethod,shipmethod_id2,ship_method_text,rev_method_id,rev_method,due_of_revmethod,trans_comp_id,trans_comp_name,trans_deliver_id,trans_deliver_name,mail_adress,telephone_number,card_paymethod_id,card_revmethod_id,tel_code,tel_number,related_company,related_company_id,member_code,p_name,p_surname,tax_no,tax_office,mobile_number_full,mobile_number,mobile_code,address_id,commission,coordinate_1,coordinate_2,sz_id)
	{
			...
			<cfif get_our_company_info.is_select_risk_money eq 1 and get_credit_all_money.recordcount and len(get_credit_all_money.money) and not isdefined("attributes.is_rate_select")>
				...
				<cfelse>
					if(<cfif isdefined("attributes.draggable")>document<cfelse>window.opener.document</cfif>.all.kur_say != undefined)
					{
						...
						{
							if(eval('<cfif isdefined("attributes.draggable")>document<cfelse>window.opener.document</cfif>.all.hidden_rd_money_'+i) != undefined && eval('<cfif isdefined("attributes.draggable")>document<cfelse>window.opener.document</cfif>.all.hidden_rd_money_'+i).value != "<cfoutput>#session.ep.money#</cfoutput>")
							{
								if(<cfif isdefined("attributes.draggable")>document<cfelse>window.opener.document.all</cfif>.search_process_date_paper != undefined)
							...
							if( window.basketManager !== undefined ){
								basketManager.setBasketFooterItem("basket_money", new_money);
							}else{
								if(eval('<cfif isdefined("attributes.draggable")>document<cfelse>window.opener.document</cfif>.all.hidden_rd_money_'+i) != undefined &&
								eval('<cfif isdefined("attributes.draggable")>document<cfelse>window.opener.document</cfif>.all.rd_money') != undefined && 
								eval('<cfif isdefined("attributes.draggable")>document<cfelse>window.opener.document</cfif>.all.rd_money['+(i-1)+']').disabled != true && 
								eval('<cfif isdefined("attributes.draggable")>document<cfelse>window.opener.document</cfif>.all.hidden_rd_money_'+i).value == new_money)
								{
								...
								if(typeof selectedCurrency != "undefined")	
									selectedCurrency(new_money);
								if(typeof toplam_hesapla != "undefined")
									toplam_hesapla(0);		
								}
							}
		...
		<cfif isdefined("attributes.field_country_id")>
			<cfif isdefined("attributes.is_select") and attributes.is_select eq 2>
				<cfif not isdefined("attributes.draggable")>opener.</cfif>$("#<cfoutput>#listLast(field_country_id,".")#</cfoutput>").select2().val(country_id).trigger('change');
			<cfelse>
		...
		<cfif isdefined("attributes.field_city_id")>
			<cfif isdefined("attributes.is_select") and attributes.is_select eq 2>
				<cfif not isdefined("attributes.draggable")>opener.</cfif>$("#<cfoutput>#listLast(field_city_id,".")#</cfoutput>").select2().val(city_id).trigger('change');
			<cfelse>
		...
		<cfif isdefined("attributes.field_country_id2")>
			<cfif isdefined("attributes.is_select") and attributes.is_select eq 2>
				<cfif not isdefined("attributes.draggable")>opener.</cfif>$("#<cfoutput>#listLast(field_country_id2,".")#</cfoutput>").select2().val(country_id).trigger('change');
			<cfelse>
		...
		<cfif isdefined("attributes.field_city_id2")>
			<cfif isdefined("attributes.is_select") and attributes.is_select eq 2>
				<cfif not isdefined("attributes.draggable")>opener.</cfif>$("#<cfoutput>#listLast(field_city_id2,".")#</cfoutput>").select2().val(city_id).trigger('change');
			<cfelse>
		...
		<cfif isdefined("attributes.field_country_id3")>
			<cfif isdefined("attributes.is_select") and attributes.is_select eq 2>
				<cfif not isdefined("attributes.draggable")>opener.</cfif>$("#<cfoutput>#listLast(field_country_id3,".")#</cfoutput>").select2().val(country_id).trigger('change');
			<cfelse>
		...
		<cfif isdefined("attributes.field_city_id3")>
			<cfif isdefined("attributes.is_select") and attributes.is_select eq 2>
				<cfif not isdefined("attributes.draggable")>opener.</cfif>$("#<cfoutput>#listLast(field_city_id3,".")#</cfoutput>").select2().val(city_id).trigger('change');
			<cfelse>
...
<cfif fusebox.use_period and isdefined("attributes.c_id") and len(attributes.c_id)>
...
<script type="text/javascript">
	<cfif get_note.recordcount>
		openBoxDraggable('<cfoutput>#request.self#?fuseaction=objects.popup_detail_company_note&c_id=#attributes.c_id#</cfoutput>','','ui-draggable-box-small');
	</cfif>
	<cfif not isdefined("attributes.draggable")>self.close();<cfelseif isdefined("attributes.draggable")>closeBoxDraggable( '<cfoutput>#attributes.modal_id#</cfoutput>' );</cfif>
</script>
