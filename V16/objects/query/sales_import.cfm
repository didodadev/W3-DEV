...
<cfif listfind('-3,-1,-8,-82',get_import.source_system,',')><!--- NCR ve Wincor icin satislar barkod bilgisi ile, Genius da ise Hediye urunler barkod bilgisi ile donmekte --->
...
<cftry>
		...
		<cftransaction>
			<cfquery name="ADD_INVOICE" datasource="#DSN2#" result="MAX_ID">
				...
				(	
					...
					<cfelseif get_import.source_system eq -82><!--- Wincor E-İşlem--->
					'WINCOR_EI_#get_import.department_id#_#dateformat(now(),'yyyymmdd')##timeformat(now(),'HHMMSSS')#',
					</cfif>				
					'#get_import.department_id# - #dateformat(get_import.startdate,dateformat_style)# Satış İmportları',
					#now()#,
	...
	<cfelseif get_import.source_system eq -82>
		<cfinclude template="sales_import_wincor_eislem.cfm">
	</cfif>	
