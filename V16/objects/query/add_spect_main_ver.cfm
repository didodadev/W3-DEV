...
<cfscript>
	...
	for(i=1;i lte GET_TREE_STOCKS.RECORDCOUNT;i=i+1)
	{
		...
		if(len(GET_TREE_STOCKS.DETAIL[i])){
			detail_list_ = replace( GET_TREE_STOCKS.DETAIL[i], ",","|", "all");
			detail_list = listappend(detail_list,detail_list_,',');
		}
			else{
...
<cfif get_pro_tree_id.recordcount>
	...
	<cf_workcube_process is_upd='1' 
		...
		record_date='#now()#' 
		action_page='#request.self#?fuseaction=prod.list_product_tree&event=upd&stock_id=#attributes.STOCK_ID#' 
