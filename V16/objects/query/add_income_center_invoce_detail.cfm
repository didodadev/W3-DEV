<cflock name="#CreateUUID()#" timeout="60">
	<cftransaction>
		...
		<cfif isdefined("attributes.page_info")><!--- eğer detaylı dağılımdan gelmiyorsa ana dağıtım sayfasından geliyorsa değişkenler oluşturulacak --->
		...
		<cfloop from="1" to="#page_record_num#" index="kk">
			<cfif (isdefined("attributes.page_info") and len(evaluate('attributes.exp_template_id#kk#'))) or not isdefined("attributes.page_info")>
				<cfif isdefined("attributes.page_info")>
					...
					<cfoutput query="get_temp_rows">
						<cfscript>
							...
							'attributes.subscription_id#get_temp_rows.currentrow#' = evaluate('attributes.subscription_id#kk#');
					...
					<cfloop from="1" to="#record_num#" index="i">
						<cfif evaluate("attributes.row_kontrol#i#")>
							<cfscript>
								...
								if(isdefined('attributes.subscription_name#i#') and len(evaluate('attributes.subscription_name#i#')))
							...
							<cfquery name="ADD_EXPENSE_ROW" datasource="#dsn2#">
								...
								(
									...
									<cfif len(form_subscription_id)>#form_subscription_id#<cfelse>NULL</cfif>
							  )
							</cfquery>
						</cfif>
					</cfloop>
		...
		<cfif isdefined("attributes.discount_record_num") and len(attributes.discount_record_num) and attributes.discount_record_num neq "">
			<cfscript>
				if(session.ep.money neq session.ep.money2)
				{
					get_currency_rate_2 = cfquery(datasource : "#dsn2#", sqlstring : "SELECT (RATE2/RATE1) RATE FROM SETUP_MONEY WHERE MONEY ='#session.ep.money2#'");
			...
			<cfloop from="1" to="#attributes.discount_record_num#" index="i">
				<cfif evaluate("attributes.discount_row_kontrol#i#")>
					<cfscript>
					    
						form_expense_item_id = evaluate('attributes.discount_expense_item_id#i#');
						form_expense_center_ids = evaluate('attributes.discount_expense_center_ids#i#');
						form_member_id = evaluate('attributes.discount_member_id#i#');
						form_member_code = evaluate('attributes.discount_member_code#i#');
						form_member_type = evaluate('attributes.discount_member_type#i#');
						form_asset=evaluate("attributes.discount_asset#i#");
						form_asset_id=evaluate("attributes.discount_asset_id#i#");
						form_project=evaluate("attributes.discount_project#i#");
						form_project_id=evaluate("attributes.discount_project_id#i#");
						form_authorized = evaluate("attributes.discount_authorized#i#");
						form_total_expense_item = evaluate('attributes.discount_total_expense_item#i#');
						form_expense_item_value = evaluate('attributes.discount_expense_item_value#i#');
						if(isdefined("attributes.discount_subscription_name#i#") and len(evaluate('attributes.discount_subscription_name#i#')))
							form_subscription_id = evaluate('attributes.discount_subscription_id#i#');
						else
							form_subscription_id ="";
						form_activity_type = evaluate("attributes.discount_activity_type#i#");
						form_workgroup_id = evaluate("attributes.discount_workgroup_id#i#");
						
						form_rate_value_one = evaluate('attributes.discount_rate_value_one#i#');
						form_rate_value_two = evaluate('attributes.discount_rate_value_two#i#');
						
						form_gross_total = evaluate('attributes.discount_grosstotal#i#');
						form_net_total = form_gross_total;
						form_tax_total = 0;
						form_otv_total = 0;
						form_bsmv_total = 0;
                        form_oiv_total = 0;
                        form_tevkifat_total = 0;
						
						form_tax = 0;
						form_otv_oran = 0;
						form_bsmv_oran = 0;
                        form_oiv_oran = 0;
                        form_tevkifat_oran = 0;
												
						other_money_value = evaluate('attributes.discount_other_money_value#i#');
						other_money_grosstotal = evaluate('attributes.discount_other_money_grosstotal#i#');
						form_other_money = evaluate('attributes.discount_other_money#i#');
					...
					<cfquery name="ADD_EXPENSE_ROW" datasource="#dsn2#">
						...
						VALUES
                            ...
                            #form_tevkifat_oran#,
								...
								#attributes.invoice_id#,
								1,
								0,<!---iskontodan geldiği için gider gibi yaz --->
								<cfif len(form_other_money)>'#form_other_money#',<cfelse>NULL,</cfif>
								<cfif len(other_money_value_)>#other_money_value_#<cfelse>NULL</cfif>,
								<cfif len(other_money_grosstotal_)>#other_money_grosstotal_#<cfelse>NULL</cfif>,
								<cfif len(other_money_gross_total_2)>'#session.ep.money2#',<cfelse>NULL,</cfif>
