...
<cfloop from="1" to="#line_count#" index="k">
	...
	<cftry>
		...
		<cfif listlen(dosya[k],';') gte 4><cfset shelf_number = trim(ListGetAt(dosya[k],4,";"))><cfelse><cfset shelf_number = ""></cfif>
		<cfif listlen(dosya[k],';') gte 5><cfset to_shelf_number = trim(ListGetAt(dosya[k],5,";"))><cfelse><cfset to_shelf_number = ""></cfif>
	...
	<cfif len(miktar) and miktar neq 0>
		...
		<cfif get_product_main.recordcount>
			...
			<cfscript>
				...
				sepet.satir[i].net_maliyet = trim(ListGetAt(dosya[k],9,";"));
