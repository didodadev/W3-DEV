...
<cfloop from="1" to="#line_count#" index="k">
	...
	<cfif not (isdefined('attributes.row_stock_id') and len(attributes.row_stock_id))>
		...
		<cfif listlen(dosya[k],';') gte 3>
			<cfset price_other = trim(ListGetAt(dosya[k],3,";"))>
		<cfelse>
			<cfset price_other = 0>
		</cfif>
		<cfif listlen(dosya[k],';') gte 4>
			<cfset other_money = trim(ListGetAt(dosya[k],4,";"))>
		<cfelse>
			<cfset other_money = ''>
		</cfif>
		<cfif listlen(dosya[k],';') gte 5>
			<cfset discount_1 = trim(ListGetAt(dosya[k],5,";"))>
		<cfelse>
			<cfset discount_1 = 0>
		</cfif>
	</cfif>
	...
	<cfif len(miktar) and miktar neq 0>
		...
		<cfif get_product_main.recordcount>
			<cfif not len(other_money)><cfset other_money = get_product_main.MONEY></cfif>
			...
			<cfscript>
				...
				if(isdefined("price_other"))
				{
					sepet.satir[i].price = price_other*evaluate("rate2_#other_money#");	
					sepet.satir[i].price_other = price_other;
				}
				else if(len(trim(FIYAT)))
				{
				...
				sepet.satir[i].indirim1 =discount_1;
