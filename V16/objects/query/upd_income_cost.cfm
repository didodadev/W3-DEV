...
<cfquery name="GET_EXPENSE_ITEM_DETAIL" datasource="#dsn2#"><!---carici vs de kontrol için kullanılıyor silmeyiniz --->
...
<cfscript>
	...
	else if(isdefined("attributes.cash"))
	{
		...
		//get_money = cfquery(datasource : "#dsn2#", sqlstring : "SELECT ACCOUNT_900, ACCOUNT_950, RATE1, RATE2 FROM #dsn_alias#.SETUP_MONEY WHERE PERIOD_ID = #session.ep.period_id# AND MONEY='#get_cash_.cash_currency_id#'");
	...
	<cftransaction>
		<cfquery name="ADD_EXPENSE_TOTAL_COST" datasource="#dsn2#">
			...
			SET
				...
				EXPENSE_COST_TYPE = 121,<!--- gelir fişi işlem tipi --->
				...
				PAPER_NO = <cfif len(attributes.belge_no)><cfqueryparam cfsqltype="cf_sql_varchar" value="#attributes.belge_no#">,<cfelse>NULL,</cfif>
				...
				<cfelseif len(attributes.ch_member_type) and attributes.ch_member_type eq 'employee'>
					...
					CH_PARTNER_ID = NULL,
					CH_CONSUMER_ID = NULL,
					CH_EMPLOYEE_ID = <cfif len(attributes.ch_member_type) and len(attributes.emp_id)>#attributes.emp_id#<cfelse>NULL</cfif>,
				<cfelse>
					CH_COMPANY_ID = NULL,
					CH_PARTNER_ID = NULL,
					CH_CONSUMER_ID = NULL,
				...
				LAW_REQUEST_ID = <cfif IsDefined("attributes.law_request_id") and len(attributes.law_request_id)><cfqueryparam value = "#attributes.law_request_id#" CFSQLType = "cf_sql_integer" ><cfelse><cfqueryparam cfsqltype="cf_sql_float" value="" null="yes"></cfif>
			WHERE
		...
		<!--- iki işlem türüde seçilmedi ise işlem kayıtlarını siliyor--->
		<cfif not isdefined("attributes.bank") and not isdefined("attributes.cash")>
		...
		<cfscript>
			if((isDefined("attributes.invoice_payment_plan") and attributes.invoice_payment_plan eq 1) or not isDefined("attributes.invoice_payment_plan"))//odeme planini guncellememesi icin eklendi
			{
				if(is_cari and is_cari_islem eq 1)
				{
					...
					if(attributes.ch_member_type eq 'employee')
						...
						else if(GET_EXPENSE_ITEM_DETAIL.TOTAL_AMOUNT_KDVLI neq attributes.net_total_amount or GET_EXPENSE_ITEM_DETAIL.DUE_DATE neq income_due_date)//eğer vadesi veya tutarı değişmişse kapama işlemini de siliyor
						...
						if(is_paymethod_based_cari eq 1 and isdefined('attributes.paymethod') and len(attributes.paymethod) and isdefined('attributes.paymethod_name') and len(attributes.paymethod_name)) //Ödeme Yöntemi Bazında Cari İşlem yapılıyorsa ve odeme yontemi secilmisse
						{
							...
							if(get_paymethod_detail.recordcount neq 0)
							{
								...
								{
									row_duedate = date_add("d",0,income_due_date);
							...
							for(ind_t=1;ind_t lte listlen(row_duedate_list); ind_t=ind_t+1)
							{
								...
								carici(
									...
									islem_detay : UCase(getLang('main',653)), //GELİR FİŞİ
			...
			if ((isdefined("attributes.cash") or isdefined("attributes.bank") or is_cari_islem) and is_account eq 1) 
			{
					document_type_ = '';
					payment_type_ = '';
				...
				muhasebe_sil(action_id:action_id, process_type:attributes.old_process_type);
		</cfscript>
		<!--- money kayıtları --->
		...
		<!---<cfif len(get_process_type.action_file_name)>--->
			<cf_workcube_process_cat 
				...
				action_page='#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.upd_income_cost&expense_id=#attributes.expense_id#'
				action_db_type = '#dsn2#'
...
<script type="text/javascript">
    window.location.href="<cfoutput>#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.add_income_cost&event=upd&expense_id=#attributes.expense_id#</cfoutput>";
</script>
