<cfquery name="get_stocks" datasource="#dsn3#">
	...
	WHERE  
		...
		AND SB.STOCK_ID=S.STOCK_ID
		AND PU.PRODUCT_UNIT_ID=SB.UNIT_ID
		...
		AND PR.PRICE > 0 
		AND LEN(SB.BARCODE) > 6
		AND LEN(SB.BARCODE) < 14
		AND PR.PRICE < 10000000
		AND (PR.PRICE <> 0 OR PR.PRICE_KDV <> 0)
