...
<cfquery name="get_ship"  datasource="#dsn2#">
	SELECT DISCOUNTTOTAL FROM SHIP WHERE SHIP_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.ship_id#"> 
</cfquery>
...
<cfscript>
	...
	sepet.stopaj = 0;
	if(isdefined("get_ship_detail.general_prom_id") and len(get_ship_detail.general_prom_id) )
	{
		sepet.general_prom_id = get_ship_detail.general_prom_id;
		sepet.general_prom_limit = get_ship_detail.general_prom_limit;
		sepet.general_prom_discount = get_ship_detail.general_prom_discount;
		sepet.general_prom_amount = get_ship_detail.general_prom_amount;
	}
	if(isdefined("get_ship_detail.general_prom_id") and  len(get_ship_detail.free_prom_id) )
	{
		sepet.free_prom_id = get_ship_detail.free_prom_id;
		sepet.free_prom_limit = get_ship_detail.free_prom_limit;
		sepet.free_prom_amount = get_ship_detail.free_prom_amount;
		sepet.free_prom_cost = get_ship_detail.free_prom_cost;
		sepet.free_prom_stock_id = get_ship_detail.free_prom_stock_id;
		sepet.free_stock_price = get_ship_detail.free_stock_price;
		sepet.free_stock_money = get_ship_detail.free_stock_money;
	}
	if (isdefined("get_ship_detail.general_prom_id") and  isnumeric(get_ship_detail.sa_discount))
		sepet.genel_indirim = get_ship_detail.SA_DISCOUNT;
	if (isnumeric(get_ship.DISCOUNTTOTAL))
		sepet.genel_indirim = get_ship.DISCOUNTTOTAL;
</cfscript>
...
<cfscript>
	...
	if(sepet.genel_indirim neq 0 and sepet.toplam_indirim neq 0)
	sepet.genel_indirim = sepet.genel_indirim - sepet.toplam_indirim;
</cfscript>
