...
WITH CTE1 AS (
	...
	FROM 
		...
		JOIN GET_MY_CONSUMERCAT ON CONSUMER.CONSUMER_CAT_ID = GET_MY_CONSUMERCAT.CONSCAT_ID AND GET_MY_CONSUMERCAT.EMPLOYEE_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.ep.userid#"> AND GET_MY_CONSUMERCAT.OUR_COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.ep.company_id#">
	WHERE
        1 = 1
		...
		<cfelse>	
            ...
            <cfif isdefined('session.ep.our_company_info.sales_zone_followup') and session.ep.our_company_info.sales_zone_followup eq 1>
             ...
             CTE2 AS (
                    SELECT
                        CTE1.*,
                        ROW_NUMBER() OVER (	ORDER BY CONSUMER_NAME,CONSUMER_SURNAME ) AS RowNum,(SELECT COUNT(*) FROM CTE1) AS QUERY_COUNT
                    FROM
                        CTE1
                )
                SELECT
