	...
	<cfoutput query="Get_Order_Details" group="Member_Id">
		...
		<cfscript>
			...
			if(isdefined("attributes.ship_out_date") and isdate(attributes.ship_out_date))
				attributes.invoice_date=dateformat(attributes.ship_out_date,dateformat_style);
			else
				attributes.invoice_date=dateformat(Get_Order_Details.ORDER_DATE,dateformat_style);
