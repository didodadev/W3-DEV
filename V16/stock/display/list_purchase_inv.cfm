...
<table cellspacing="1" cellpadding="2" width="100%" border="0">
	...
	<tr height="20" onMouseOver="this.className='color-light';" onMouseOut="this.className='color-row';" class="color-row">
		...
		<td><!--- <cfset session.control=0> --->
		<cfif purchase.purchase_sales eq 0>
			<a href="#request.self#?fuseaction=stock.form_add_purchase&event=upd&ship_id=#ship_id#" class="tableyazi">#ship_number#</a>		
		<cfelseif purchase.purchase_sales eq 1>
			<cfif SHIP_TYPE eq 81>
			  <a href="#request.self#?fuseaction=stock.add_ship_dispatch&event=upd&ship_id=#ship_id#" class="tableyazi">#ship_number#</a>
			<cfelse>
			  <a href="#request.self#?fuseaction=stock.form_add_sale&event=upd&ship_id=#ship_id#" class="tableyazi">#ship_number#</a>
			</cfif>
		</cfif>
		</td>
		<td>
	<cfif SHIP_TYPE eq 76><cf_get_lang_main no='1784.Mal Alım İrsaliyesi'>
	  ...
	  <cfelseif SHIP_TYPE eq 75><cf_get_lang_main no='1343.Konsinye Çıkış İade İrsaliyesi'>
		...
		<td><cfif len(company_id) and company_id neq 0>#get_par_info(COMPANY_ID,1,1,0)#<cfelse>#get_cons_info(CONSUMER_ID,0,0)#</cfif></td>           
		<td>#department_head#</td>
		<td align="right" style="text-align:right;">#TLFormat(nettotal)# #session.ep.money#</td>
		<!-- sil -->
		  <td align="center">
		<cfif purchase.purchase_sales eq 1>
		<a href="javascript://" onClick="windowopen('#request.self#?fuseaction=objects.popup_operate_page&operation=emptypopup_temp_upd_sale&module=stock&id=#ship_id#&ship_id=#ship_id#&action=print,'page');"><img src="/images/print2.gif" title="<cf_get_lang no='172.İrsaliye Bas'>" border="0"></a>
		</cfif>			
		</td>
		<!-- sil -->
	</tr>
