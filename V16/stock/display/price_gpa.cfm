...
<cfif isDefined("attributes.stock_id")>
	...
	<cfoutput query="GET_INVOICE_ID_IN"> 
		<cfquery name="GET_INVOICE_ID_IN_PRICE" datasource="#DSN2#">
		SELECT
		<cfif attributes.tax IS 1>
			SUM(IR.AMOUNT*(((IR.PRICE*(100-IR.DISCOUNT1)*(100-IR.DISCOUNT2)*(100-IR.DISCOUNT3)*(100-IR.DISCOUNT4)*(100-IR.DISCOUNT5))/10000000000)+(((IR.PRICE*(100-IR.DISCOUNT1)*(100-IR.DISCOUNT2)*(100-IR.DISCOUNT3)*(100-IR.DISCOUNT4)*(100-IR.DISCOUNT5))/10000000000)*TAX/100))/PU.MULTIPLIER)/SUM(IR.AMOUNT) PRICE
		<cfelse>
			SUM(IR.AMOUNT*(((IR.PRICE*(100-IR.DISCOUNT1)*(100-IR.DISCOUNT2)*(100-IR.DISCOUNT3)*(100-IR.DISCOUNT4)*(100-IR.DISCOUNT5))/10000000000))/PU.MULTIPLIER)/SUM(IR.AMOUNT) PRICE
		</cfif>
		FROM
			INVOICE_ROW IR,
			#dsn3_alias#.PRODUCT_UNIT PU 
		WHERE 
			IR.PURCHASE_SALES=0 AND 		 
			 IR.STOCK_ID=#GET_INVOICE_ID_IN.STOCK_ID#  AND		
			 IR.PRODUCT_ID = PU.PRODUCT_ID  AND 
			 IR.UNIT = PU.ADD_UNIT	AND
			 IR.PRICE IS NOT NULL 			 
		</cfquery>
		<cfscript>
			PRICE2 = GET_INVOICE_ID_IN_PRICE.PRICE;
			PRICE_=PRICE2; // MULTIPLIER
	...
	<cfset OUT_LIST = "" >
	<cfset TOTAL_OUT = 0 >
	<cfif GET_INVOICE_ID_OUT.recordcount>
