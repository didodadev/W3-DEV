...
<cfquery name="GET_NUMBER" datasource="#dsn2#">
	SELECT SHIP_ID,SHIP_TYPE FROM SHIP WHERE SHIP_ID = #attributes.upd_id#
</cfquery>
...
<cfif form.del_ship eq 1>
	<cfinclude template="del_stock_in_from_customs.cfm">
	<cfabort>
</cfif>
...
<cftransaction>
	...
	<cfloop from="1" to="#attributes.rows_#" index="i">
	 	...
	 	<cfquery name="ADD_SHIP_ROW" datasource="#DSN2#">
			...
			VALUES
				...
				<cfif isdefined("attributes.row_nettotal#i#")>#evaluate("attributes.row_nettotal#i#")#,<cfelse>0,</cfif>
					...
					<cfelseif isdefined('attributes.int_loc_in') and len(attributes.int_loc_in)>
						#attributes.int_loc_in#,
					<cfelse>
						NULL,
					</cfif>
					<cfif isdefined('attributes.spect_id#i#') and len(evaluate('attributes.spect_id#i#'))>
					...
					#listfirst(import_invoice_id,';')#,
					#listlast(import_invoice_id,';')#,
		...
		<cfif not listfind(import_invoice_list,import_invoice_id,',')>
			<cfset import_invoice_list = listappend(import_invoice_list,import_invoice_id,',')>
		</cfif>
	</cfloop>
     <cfif isdefined("attributes.is_delivered") and attributes.is_delivered eq 1>
    	<cfquery name="upd_seri" datasource="#dsn2#">
        	SELECT * FROM #dsn3_alias#.SERVICE_GUARANTY_NEW WHERE PROCESS_ID = #attributes.upd_id# AND PERIOD_ID = #session.ep.period_id# AND PROCESS_CAT = 811 AND IN_OUT = 0 AND SERIAL_NO NOT IN (SELECT SERIAL_NO FROM #dsn3_alias#.SERVICE_GUARANTY_NEW WHERE PROCESS_ID = #attributes.upd_id# AND PERIOD_ID = #session.ep.period_id# AND PROCESS_CAT = 811 AND IN_OUT = 1)
        </cfquery>
 	...
 	<cfif not (isdefined("attributes.irsaliye_iptal") and (attributes.irsaliye_iptal eq 1))><!--- irsaliye iptal secili degilse--->
		<cfif len(import_invoice_list)>
			...
			<cfloop list="#import_invoice_list#" index="import_inv">
                                    ...
                                    <cfloop query="GET_SPEC_PRODUCT">
                                            <cfquery name="ADD_STOCK_ROW" datasource="#DSN2#">
                                                INSERT INTO 
                                                    STOCKS_ROW
                                                    (
                                                    <cfif len(attributes.DELIVER_DATE_FRM)>PROCESS_DATE,</cfif>
                                                        UPD_ID,
                                                        PRODUCT_ID,
                                                        STOCK_ID,
                                                        PROCESS_TYPE,
                                                ...
                                                VALUES
                                                    (
                                                    <cfif len(attributes.DELIVER_DATE_FRM)>#attributes.PROCESS_DATE#,</cfif>
                                                        #attributes.UPD_ID#,
                                           ...
                                           <cfquery name="ADD_STOCK_ROW" datasource="#DSN2#">
                                                INSERT INTO STOCKS_ROW
                                                    ...
                                                    <cfif isdefined('form_spect_main_id') and len(form_spect_main_id)>
                                                        ...
                                                        DELIVER_DATE,
                                                        SHELF_NUMBER <!--- giriş rafı yani basketteki raf_2 alanı yazıdırılır --->
	...
	<cfelse>
		...
		<cfquery name="UPD_SHIP_ROW" datasource="#dsn2#"> 
			UPDATE SHIP_ROW SET IMPORT_INVOICE_ID = 0, IMPORT_PERIOD_ID = 0 WHERE SHIP_ID = #attributes.UPD_ID#
		</cfquery>
	</cfif>
	<cfscript>
		...
		else
		{
			if(get_process_type.IS_ACCOUNT eq 1)
			{
				...
				muhasebeci(
					...
					from_branch_id:iif(isdefined('attributes.branch_id') and len(attributes.branch_id),attributes.branch_id,de('')),
					to_branch_id:iif(isdefined('attributes.branch_in_id') and len(attributes.branch_in_id),attributes.branch_in_id,de('')),
	...
	<cf_workcube_process_cat 
		...
		is_action_file = 1
		action_page='#request.self#?fuseaction=stock.upd_stock_in_from_customs&ship_id=#attributes.UPD_ID#'
...
<cfif session.ep.our_company_info.is_cost eq 1><!--- sirket maliyet takip ediliyorsa not js le yonlenioyr cunku cost_action locationda calismiyor --->
	<cfscript>
		if(not isdefined("attributes.irsaliye_iptal") and isdefined('attributes.is_delivered'))
			...
			cost_action(action_type:2,action_id:attributes.UPD_ID,query_type:3);//iptalse siliyor
	</cfscript>
</cfif>
<script language="javascript">
	window.location.href="<cfoutput>#request.self#?fuseaction=stock.add_stock_in_from_customs&event=upd&ship_id=#attributes.UPD_ID#</cfoutput>";
</script>
