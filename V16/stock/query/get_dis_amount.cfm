...
<cfif isdefined('attributes.row_ship_id#i#')>
	<!---irsaliyede row_ship_id tek elemanlı oldugunda, irsaliyeye cekilen order_id'yi tutar.
	row_ship_id 2 elemanlı ise irsaliyeye cekilen konsinye irsaliyeyi gosterir ve ship_id;ship_period_id olarak kullanılır. --->
	<cfif listlen(evaluate("attributes.row_ship_id#i#"),';') eq 1> 
	<!--- irs coklu siparis cekme. row_ship_id degiskeninde irsaliyeye siparis cekerken order_id'ler tutuluyor--->
		<cfset order_id= evaluate('attributes.row_ship_id#i#')>
		<cfset order_id_for_ship_row= evaluate('attributes.row_ship_id#i#')>
	<cfelse>
		<cfset order_id=0>
		<cfset order_id_for_ship_row = 0>
	</cfif>
	<cfset import_invoice_id = evaluate('attributes.row_ship_id#i#')> <!--- ithal mal girisi irsaliyesinde kullanılıyor, "invoice id;period" olarak set ediliyor ozden22032006--->
<cfelse>
	<cfset order_id=0>
	<cfset order_id_for_ship_row = 0>
	<cfset import_invoice_id = 0>
</cfif>
