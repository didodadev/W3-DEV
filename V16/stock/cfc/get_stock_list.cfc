...
<cffunction name="GET_PRODUCT_fnc" returntype="query">
	...
	<cfargument name="x_show_second_unit" default=""/>
    ...
    <cfargument name="manufacturer_code" default="">
    <cfargument name="lotno_filter" default="">
    ...
    <cfquery name="GET_PRODUCT" datasource="#this.dsn3#">
        WITH CTE1 AS (
            <cfif arguments.list_type eq 3>
                ...
                TABLE1.BARCOD, 
                TABLE1.STOCK_CODE, 
                TABLE1.MANUFACT_CODE,
                TABLE1.PROPERTY, 
                TABLE1.PRODUCT_STOCK,
                TABLE1.PACKAGE_TYPE_ID,
                ...
                <cfif isdefined('x_show_second_unit') and x_show_second_unit eq 1>
                    (SELECT TOP 1 PUNIT.ADD_UNIT FROM #this.dsn3_alias#.PRODUCT_UNIT PUNIT WHERE PUNIT.IS_ADD_UNIT = 1 AND PUNIT.PRODUCT_ID = SS.PRODUCT_ID) UNIT2,
                    (SELECT TOP 1 PT.MULTIPLIER FROM #this.dsn3_alias#.PRODUCT_UNIT PT WHERE PT.IS_ADD_UNIT = 1 AND PT.PRODUCT_ID = SS.PRODUCT_ID) MULTIPLIER,                
                </cfif>
            ...
            SELECT
                ...
                ST.MAXIMUM_STOCK,
                ST.REPEAT_STOCK_VALUE,
                ST.MINIMUM_STOCK,
                ST.PROVISION_TIME,
                ST.MINIMUM_ORDER_STOCK_VALUE,
                ST.BLOCK_STOCK_VALUE,
                ST.STOCK_ACTION_ID,
                </cfif>
                <cfif isdefined('x_show_second_unit') and x_show_second_unit eq 1>
                    (SELECT TOP 1 PUNIT.ADD_UNIT FROM #this.dsn3_alias#.PRODUCT_UNIT PUNIT WHERE PUNIT.IS_ADD_UNIT = 1 AND PUNIT.PRODUCT_ID = S.PRODUCT_ID) UNIT2,
                    (SELECT TOP 1 PT.MULTIPLIER FROM #this.dsn3_alias#.PRODUCT_UNIT PT WHERE PT.IS_ADD_UNIT = 1 AND PT.PRODUCT_ID = S.PRODUCT_ID) MULTIPLIER,                
                </cfif>
                0 HURDA_STOCK,
                <cfif listfind('1,2,3,4',arguments.list_type,',')>
                    <cfif arguments.list_type eq 1 and isdefined("arguments.amount_flag") and arguments.amount_flag eq 2>
                        <cfif isdefined("arguments.is_saleable_stock")>
                            ISNULL(SUM(SR.SALEABLE_STOCK),0) AS PRODUCT_STOCK, <!--- stok stratejisi satılabilir stoga gore calıstıgı icin listeye de satılabilir stok miktarı getiriliyor --->
                            0 AS POZ_PRODUCT_STOCK_2,
                            0 AS NEG_PRODUCT_STOCK_2,
                            SUM(SR.PRODUCT_STOCK) AS REAL_STOCK
                        <cfelse>
                            ISNULL(SUM(SR.PRODUCT_STOCK),0) AS PRODUCT_STOCK,
                            0 AS POZ_PRODUCT_STOCK_2,
                            0 AS NEG_PRODUCT_STOCK_2,
                        </cfif>
                    <cfelseif arguments.list_type eq 3>	
                        ISNULL(SUM(SR.STOCK_IN - SR.STOCK_OUT),0) AS PRODUCT_STOCK,
                        ( SELECT ISNULL(SUM(ST1.AMOUNT2),0) AMNT1 FROM #this.dsn2_alias#.STOCKS_ROW AS ST1 WHERE ST1.STOCK_ID = S.STOCK_ID AND ISNULL(ST1.STOCK_IN - ST1.STOCK_OUT,0) > 0 ) AS POZ_PRODUCT_STOCK_2,
                        ( SELECT ISNULL(SUM(ST2.AMOUNT2),0) AMNT2 FROM #this.dsn2_alias#.STOCKS_ROW AS ST2 WHERE ST2.STOCK_ID = S.STOCK_ID AND ISNULL(ST2.STOCK_IN - ST2.STOCK_OUT,0) < 0 ) AS NEG_PRODUCT_STOCK_2,
            ...
            FROM                
                ...
                <cfelse>
                   LEFT JOIN #this.dsn2_alias#.STOCKS_ROW SR ON SR.STOCK_ID = S.STOCK_ID
                </cfif>
                <cfif arguments.list_type eq 3>
            ...
            WHERE
                ...
                SR.SHELF_NUMBER IN (#arguments.shelf#) AND
                </cfif>
                <cfif arguments.list_type eq 5>
                S.IS_LOT_NO=1 AND         
                    <cfif isdefined('arguments.lotno_filter') and len(arguments.lotno_filter)>
                        SR.LOT_NO LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#arguments.lotno_filter#%"> and
                    </cfif>       
                </cfif>
              ...
              <cfif arguments.list_type neq 3><!--- Karma Koli İse Bu bloğa girmesin,buraya girerse karma koliyi oluşturan ürün özelliklerine göre arama yapmış olur bize gereken bu değil!Ana ürün İçin Yapması Gerekiyor Dolayısı ile bu bloklar aşağıda çalışacak.. --->
                  ...
                  <cfelseif isDefined("arguments.keyword") and len(arguments.keyword)>
                    (	
                      <cfif isnumeric(arguments.keyword) and int(arguments.keyword) gt 0>
                        (S.STOCK_ID = <cfqueryparam cfsqltype="cf_sql_decimal" value="#left(arguments.keyword,38)#">) OR
                      </cfif>
                     (
                      <cfif len(arguments.keyword) lte 3>
                        (','+S.PRODUCT_NAME+',' LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#arguments.keyword#%"> COLLATE SQL_Latin1_General_CP1_CI_AI)	
                        ...
                        (
                            (
                              <cfloop from="1" to="#listlen(arguments.keyword,'+')#" index="pro_index">
                                S.PRODUCT_NAME LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#ListGetAt(arguments.keyword,pro_index,'+')#%"> COLLATE SQL_Latin1_General_CP1_CI_AI
                                <cfif pro_index neq listlen(arguments.keyword,'+')>AND</cfif>
                              </cfloop>
                            )		
                            OR S.STOCK_CODE_2 LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#arguments.keyword#%"> COLLATE SQL_Latin1_General_CP1_CI_AI
                        )				
                      </cfif>
                        )
                        OR S.MANUFACT_CODE = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.keyword#"> COLLATE SQL_Latin1_General_CP1_CI_AI
                    ) AND
                  </cfif>
                  <cfif arguments.list_type eq 2 and isdefined("arguments.spect_main_id") and len(arguments.spect_main_id)>
                    SR.SPECT_VAR_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.spect_main_id#"> AND
                  </cfif>
				  ...
				  <cfif isDefined("arguments.manufacturer_code") and len(arguments.manufacturer_code)>
                    S.MANUFACT_CODE LIKE '<cfif len(arguments.manufacturer_code) gt 3>%</cfif>#arguments.manufacturer_code#%' AND
                  </cfif>
              </cfif>
                ...
                <cfif isDefined("arguments.amount_flag") and  arguments.amount_flag eq 2>
                    S.PRODUCT_ID = ST.PRODUCT_ID AND
                    S.STOCK_ID = ST.STOCK_ID AND
                    ST.MAXIMUM_STOCK IS NOT NULL AND 
		            ST.MINIMUM_STOCK IS NOT NULL AND
                    <cfif isdefined('arguments.department_id') and len(arguments.department_id)>
                        ST.DEPARTMENT_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.department_id#"> AND
                    <cfelse>
                        ST.DEPARTMENT_ID IS NULL AND 
                    </cfif>
                </cfif>
            ...
            GROUP BY            
                ...
                S.PACKAGE_TYPE_ID,
                S.PROPERTY
                <cfif listfind('1,2,3,4,5',arguments.list_type,',')>
                    <cfif len(arguments.department_id)>
                            <cfif (arguments.amount_flag eq 2 and arguments.list_type eq 1)>
                                ,SR.DEPARTMENT_ID
                            <cfelse>
                ...
                <cfelseif isDefined("arguments.amount_flag") and  arguments.amount_flag eq 2>
                    <cfif isdefined("arguments.is_saleable_stock")> <!---strateji satılabilir stok uzerinden hesaplanır --->
                        <cfif isdefined('arguments.strategy_type') and arguments.strategy_type eq 1><!--- fazla stok --->
                            HAVING SUM(SALEABLE_STOCK) >= ST.MAXIMUM_STOCK
                        <cfelseif isdefined('arguments.strategy_type') and arguments.strategy_type eq 2><!--- eksik stok --->
                            HAVING SUM(SALEABLE_STOCK) <= ST.MINIMUM_STOCK	
                        <cfelseif isdefined('arguments.strategy_type') and arguments.strategy_type eq 3><!--- yeniden siparis noktasına gelen urunler --->
                            HAVING SUM(SALEABLE_STOCK) <= ST.REPEAT_STOCK_VALUE AND SUM(SALEABLE_STOCK) > ST.MINIMUM_STOCK
                        </cfif>
                    <cfelseif not len(arguments.department_id)>
                        <cfif isdefined('arguments.strategy_type') and arguments.strategy_type eq 1><!--- fazla stok --->
                            HAVING
                                SUM(PRODUCT_STOCK) >= ST.MAXIMUM_STOCK AND
                                SUM(PRODUCT_STOCK) > 0 AND
                                SUM(PRODUCT_STOCK) >= ST.MINIMUM_STOCK
                        <cfelseif isdefined('arguments.strategy_type') and arguments.strategy_type eq 2><!--- eksik stok --->
                            HAVING SUM(PRODUCT_STOCK) <= ST.MINIMUM_STOCK	
                        <cfelseif isdefined('arguments.strategy_type') and arguments.strategy_type eq 3><!--- yeniden siparis noktasına gelen urunler --->
                            HAVING SUM(PRODUCT_STOCK) <= ST.REPEAT_STOCK_VALUE AND SUM(PRODUCT_STOCK) > ST.MINIMUM_STOCK
                        </cfif>
                    <cfelse>
                        <cfif isdefined('arguments.strategy_type') and arguments.strategy_type eq 1><!--- fazla stok --->
                            HAVING SUM(PRODUCT_STOCK) >= ST.MAXIMUM_STOCK
                        <cfelseif isdefined('arguments.strategy_type') and arguments.strategy_type eq 2><!--- eksik stok --->
                            HAVING SUM(PRODUCT_STOCK) <= ST.MINIMUM_STOCK	
            ...
            <cfelse>
                ...
                <cfelseif isDefined("arguments.amount_flag") and  arguments.amount_flag eq 2 >
                    <cfif isdefined('arguments.strategy_type') and arguments.strategy_type eq 2><!--- eksik stok --->
                        HAVING ROUND(SUM(SR.STOCK_IN - SR.STOCK_OUT),2) <= ST.MINIMUM_STOCK	
                    <cfelseif isdefined('arguments.strategy_type') and arguments.strategy_type eq 3><!--- yeniden siparis noktasına gelen urunler --->
                        HAVING ROUND(SUM(SR.STOCK_IN - SR.STOCK_OUT),2) <= ST.REPEAT_STOCK_VALUE AND SUM(SR.STOCK_IN - SR.STOCK_OUT) > ST.MINIMUM_STOCK
                    <cfelseif isdefined('arguments.strategy_type') and arguments.strategy_type eq 1><!--- fazla stok --->
                        HAVING ROUND(SUM(SR.STOCK_IN - SR.STOCK_OUT),2) >= ST.MAXIMUM_STOCK
                    </cfif>
                </cfif>
            </cfif>            
            <cfif arguments.list_type eq 3>
                ...
                (	
                <cfif isnumeric(arguments.keyword) and int(arguments.keyword) gt 0>
                    (SS.STOCK_ID = <cfqueryparam cfsqltype="cf_sql_decimal" value="#left(arguments.keyword,38)#">) OR
                </cfif>
                    (
                <cfif len(arguments.keyword) lte 3 >
                    (SS.PRODUCT_NAME LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.keyword#%"> COLLATE SQL_Latin1_General_CP1_CI_AI)	
                <cfelseif len(arguments.keyword) gt 3 or listlen(arguments.keyword,"+")>
                    (
                        (
                        <cfloop from="1" to="#listlen(arguments.keyword,'+')#" index="pro_index">
                            SS.PRODUCT_NAME LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#ListGetAt(arguments.keyword,pro_index,'+')#%"> COLLATE SQL_Latin1_General_CP1_CI_AI
                            <cfif pro_index neq listlen(arguments.keyword,'+')>AND</cfif>
                        </cfloop>
                        )		
                        OR SS.STOCK_CODE_2 LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#arguments.keyword#%"> COLLATE SQL_Latin1_General_CP1_CI_AI
                    )				
                </cfif>
                    )
                    OR SS.MANUFACT_CODE = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.keyword#"> COLLATE SQL_Latin1_General_CP1_CI_AI
                ) AND
                </cfif>
                <cfif isDefined("arguments.stock_code") and len(arguments.stock_code)>
                ...
                <cfif isDefined("arguments.manufacturer_code") and len(arguments.manufacturer_code)>
                    SS.MANUFACT_CODE LIKE '<cfif len(arguments.manufacturer_code) gt 3>%</cfif>#arguments.manufacturer_code#%' AND
                </cfif>
