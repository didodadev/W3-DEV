                        ...
                        <cf_box_elements id="is_plani" vertical="1">              
                            ...
                            <div class="col col-4 col-md-4 col-sm-6 col-xs-12 padding-0" type="column" index="7" sort="true">      
                                ...
                                <div class="form-group" id="form_ul_sale_contract_amount">
                                    ...
                                    <div class="col col-12 col-xs-12">
                                        <cfinput type="text" name="sale_contract_amount" value="" passthrough="onkeyup=""return(formatcurrency(this,event));"""  class="moneybox">
                                    </div>
                                </div>
...
function estimated_finishdate(add_time,type)
{
	...
	{
		alert("<cf_get_lang dictionary_id='60617.Lütfen 60dan Küçük Bir Değer Giriniz'>!");
		return false;
	}
...
{	
    ...
    if(document.getElementById('startdate_plan') != undefined && document.getElementById('finishdate_plan') != undefined)
    {
        ...
        <cfif isdefined("attributes.work_id") and len(attributes.work_id) and isdefined('attributes.id') and len(attributes.id)>
            if(tarih1_ < tarih6_)
            {
                alert("<cf_get_lang dictionary_id ='61643.Girdiğiniz Alt İşin Başlangıç Tarihi Bağlı Olduğu Üst İşin Başlangıç Tarihinden Önce Gözüküyor.Lütfen Düzeltiniz'>");
                return false;
            }
            /* if(tarih3_ > tarih5_)
            {
        ...
        <cfif isDefined('xml_pro_work_date') and xml_pro_work_date eq 1>
            if(document.getElementById('project_ids').value != ""  &&  document.getElementById('project_ids').value != -1)   
            {
                ...
                if(tarih3_ > tarih4_)
                {
                    alert("<cf_get_lang dictionary_id ='38210.Girdiğiniz İşin Hedef Bitiş Tarihi Bağlı Olduğu Projenin Hedef Bitiş Tarihinden Sonra Gözüküyor Lütfen Düzeltin'>");
                    return false;
                }  
            }	
        </cfif>
	...
	{ 
		alert ("<cf_get_lang dictionary_id='58194.Zorunlu Alan'> : <cf_get_lang dictionary_id='38177.İş Kategorisi'>");
		return false;
	}
    ...
    if(document.getElementById('warning') != undefined)
    {
        ...
        {
            alert("<cf_get_lang dictionary_id='38278.Tekrar Periyodu Seçiniz'>!");
            return false;
        }
        if((document.add_work.warning[x].value==1) && (document.add_work.warning_count.value==""))
        {
            alert("<cf_get_lang dictionary_id='38135.Tekrar Sayısı Girmelisiniz'>!");
            return false;
        }
    }
	...
	document.add_work.appendChild(document.all.process_stage);
	unformat_fields();
    return process_cat_control();
    
    return true;
	//if(document.add_work.workgroup.value == 1) return process_cat_control();
