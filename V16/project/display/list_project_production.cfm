...
<!--- Uretimden Cikis--->
<cfquery name="GET_PROJECT_PRODUCTION_OUTPUT" datasource="#DSN2#">
WITH CTE1 AS (
    SELECT
    	SF.FIS_ID,
    	SF.PROJECT_ID,
    ...
    WHERE 
		...
		<cfelse>
			<cfif attributes.line_based eq 1>
                 AND SFR.ROW_PROJECT_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#URL.ID#">
            <cfelse>
                AND (SF.PROJECT_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#URL.ID#">       
                OR SF.PROJECT_ID_IN = <cfqueryparam cfsqltype="cf_sql_integer" value="#URL.ID#">) 
            </cfif>
		</cfif> 
        <cfif attributes.line_based eq 1>
...
<cfif GET_PROJECT_PRODUCTION_OUTPUT.recordcount>
    <cfparam name="attributes.totalrecords" default="#GET_PROJECT_PRODUCTION_OUTPUT.query_count#">
<cfelse>
    <cfparam name="attributes.totalrecords" default="0">
</cfif>
<cfif attributes.line_based eq 1>
	<cfset fis_id_list = listdeleteduplicates(ValueList(GET_PROJECT_PRODUCTION_OUTPUT.FIS_ID,','))>
	<cfif len(fis_id_list)>
        ...
        <cfif GET_ALL_USED_AMOUNT.recordcount>
            <cfscript>
                ...
                'used_amount_#replace(GET_ALL_USED_AMOUNT.FROM_WRK_ROW_ID[uai],"-","_","ALL")#' = GET_ALL_USED_AMOUNT.AMOUNT[uai];//kullanılan miktarları benzersiz wrk_row_id'ye göre belirledik.
            </cfscript>
        </cfif>
    </cfif>
	<cfset stock_id_list = ValueList(GET_PROJECT_PRODUCTION_OUTPUT.STOCK_ID,',')>
...
<div id="div_project_production_output">
<cf_grid_list>
	<thead>
        <tr>
     ...
     <tbody>
	<cfif GET_PROJECT_PRODUCTION_OUTPUT.recordcount>
		<cfoutput query="GET_PROJECT_PRODUCTION_OUTPUT">
            <tr>
                <td>#rownum#</td>
                <td><a href="#request.self#?fuseaction=stock.form_add_fis&event=upd&upd_id=#FIS_ID#"  target="_new" class="tableyazi">#FIS_NUMBER#</a></td>
                <td>#PROCESS_CAT#</td>
                ...
                <cfif attributes.line_based eq 1>
                    ...
                    <input type="text"  name="#var_price_##STOCK_FIS_ROW_ID#" id="#var_price_##STOCK_FIS_ROW_ID#" style="width:80;"  value="#tlformat(PRICE)#" onKeyup="return(FormatCurrency(this,event,6));" onBlur="#var_calc_marj_function#('price',#STOCK_FIS_ROW_ID#);">#session.ep.money#</td>
                    <cfif attributes.line_based eq 1 and isdefined("attributes.from_paper") and len(attributes.from_paper)> 
...
<cfset adres="project.popup_ajax_list_project_production_output&id=#attributes.id#&line_based=#attributes.line_based#">
...
<cfif attributes.totalrecords gt attributes.maxrows>
    <cf_paging
        name="project_production_output"
        ...
        adres="#adres#"
        isAjax="1"
        target="div_project_production_output"
        is_iframe="1"
        >
</cfif>
</div> 
