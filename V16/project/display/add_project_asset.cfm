	...
	<cf_grid_list>
		<thead>
			<tr>
				...
				<th width="20"><a href="javascript://" onClick="windowopen('<cfoutput>#request.self#?fuseaction=objects.popup_list_pro_assets&project_id=#get_project.project_id#&startdate_temp=#get_project.target_start#&finishdate_temp=#get_project.target_finish#</cfoutput>','list');"><i class="fa fa-plus" title="<cf_get_lang dictionary_id='58496.Olay Ekle'>"></i></a></th>
			</tr>
		</thead>
		<tbody>
			<cfif get_asset_pro.recordcount>
				<cfoutput query="get_asset_pro">
					<tr>
						...
						<td><a href="javascript:windowopen('#request.self#?fuseaction=objects.popup_form_upd_assetp_reserve&assetp_resid=#assetp_resid#&assetp_id=#assetp_id#&project_id=#attributes.id#','small');"><i class="fa fa-pencil" title="<cf_get_lang dictionary_id='57464.Güncelle'>"></i></a></td>
						...
						<td><a href="##" onClick="javascript:if (confirm('#message#')) windowopen('#request.self#?fuseaction=project.emptypopup_del_asset_project&id=#url.id#&assetp_id=#get_assetp_name.assetp_id #','small'); else return;"><i class="fa fa-minus" title="<cf_get_lang dictionary_id='38180.Notu Sil'>"></i></a></td>
					</tr>
				</cfoutput>
			<cfelse>
				<tr>
