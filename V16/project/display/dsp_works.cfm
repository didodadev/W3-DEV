						...
						<select name="department" id="department">
							...
							<cfif isdefined('attributes.branch_id') and isnumeric(attributes.branch_id)>
								<cfquery name="GET_DEPARTMANT" datasource="#DSN#">
									SELECT
										#dsn#.Get_Dynamic_Language(DEPARTMENT_ID,'#session.ep.language#','DEPARTMENT','DEPARTMENT.HEAD',NULL,NULL,DEPARTMENT_HEAD) AS department_head,
										DEPARTMENT_ID
									FROM 
										DEPARTMENT 
									WHERE 
										DEPARTMENT_STATUS = 1 AND BRANCH_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.branch_id#"> 
									ORDER BY 
										DEPARTMENT_HEAD
								</cfquery>
			...
			<cf_grid_list>
				...
				<thead>
					<tr>
						...
						<!---					<cfoutput><a href="javascript://" onclick="windowopen('#request.self#?fuseaction=project.popup_add_work&work_fuse=#attributes.fuseaction#','wwide1');"><img src="/images/plus_list.gif" align="absbottom" alt="İş Ekle" title="<cf_get_lang dictionary_id='521.is ekle'>"></a></cfoutput>
						--->					
						...
						<cfoutput query="get_works" startrow="#attributes.startrow#" maxrows="#attributes.maxrows#">
							<cfif isdefined("attributes.project_emp_id") and len(attributes.project_emp_id) and attributes.project_emp_id neq project_emp_id><cfset _row_font_color_ ='FF6633'><cfelse><cfset _row_font_color_ =''></cfif>
								<tr>
									...
									<td>#work_id#</td>
									<!---<td><a href="javascript://" onclick="windowopen('#request.self#?fuseaction=project.works&event=upd&id=#work_id#','wwide1');">--->
									...
									<!-- sil -->
									<!---<td><a href="javascript://" onclick="windowopen('#request.self#?fuseaction=project.works&event=upd&id=#work_id#','wwide1');"><img src="/images/update_list.gif" title="<cf_get_lang dictionary_id='61.Tarihe'>"></a></!---<td>--->
