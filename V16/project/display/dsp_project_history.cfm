	...
	<!---	</cfoutput>--->
        ...
        <table id="history_#temp_#" style="display:none;">
            ...
            <tr>
				...
				<cfif len(get_pro_history.UPDATE_PAR)><a href="javascript://" class="tableyazi" onclick="windowopen('#request.self#?fuseaction=objects.popup_par_det&par_id=#get_update_par_name.partner_id[ListFind(update_par_list,update_par,',')]#','medium');">#get_update_par_name.name_surname[ListFind(update_par_list,update_par,',')]#</a></cfif>
			</td>
                <td class="txtbold"><cf_get_lang dictionary_id='57574.Şirket'></td>
              <td>
			  			<cfif len(get_pro_history.PARTNER_ID)>
					<a href="javascript://" class="tableyazi" onclick="windowopen('#request.self#?fuseaction=objects.popup_par_det&par_id=#get_company_partner_name.partner_id[ListFind(company_partner_list,partner_id,',')]#','medium');">#get_company_partner_name.name_surname[ListFind(company_partner_list,partner_id,',')]#</a>
				<cfelseif len(get_pro_history.CONSUMER_ID)>
					<a href="javascript://" class="tableyazi" onclick="windowopen('#request.self#?fuseaction=objects.popup_con_det&con_id=#get_consumer_name.consumer_id[ListFind(consumer_list,consumer_id,',')]#','medium');">#get_consumer_name.name_surname[ListFind(consumer_list,consumer_id,',')]#</a>
				</cfif>
               </td>
            </tr>
            <tr>
                ...
                <td>
					...
					<a href="javascript://" class="tableyazi" onclick="windowopen('#request.self#?fuseaction=objects.popup_emp_det&emp_id=#get_project_emp_name.employee_id[ListFind(project_emp_list,project_emp_id,',')]#','medium');">#get_project_emp_name.name_surname[ListFind(project_emp_list,project_emp_id,',')]#</a>
				</cfif>
				<cfif (get_pro_history.OUTSRC_PARTNER_ID NEQ 0) and len(get_pro_history.OUTSRC_PARTNER_ID)>
					<a href="javascript://" class="tableyazi" onclick="windowopen('#request.self#?fuseaction=objects.popup_par_det&par_id=#get_outsrc_partner_name.partner_id[ListFind(oursrc_partner_list,partner_id,',')]#','medium');">#get_outsrc_partner_name.name_surname[ListFind(oursrc_partner_list,partner_id,',')]#</a>
				</cfif>
                </td>
