...
<cfif attributes.line_based eq 1><!--- SATIR BAZINDA İSE --->
	<script type="text/javascript">
		...
		{
			<cfoutput query="GET_PROJECT_EXPENSE">
				<cfif isDefined("xml_") and ListGetAt(xml_,2,';') eq 1>//Kalan Miktarlar Aktarılsın
				...
				if(type=='select')
				{
					...
					{
						<cfif isDefined("xml_") and ListGetAt(xml_,2,';') eq 1>//Kalan Miktarlar Aktarılsın evet secilirse 0 oldugunda dusurulmez
						if(row_amount > 0)
						</cfif>
