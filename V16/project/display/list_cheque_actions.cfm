...
<!--- Proje İlişkili çek İşlemleri--->	
<cfquery name="GET_PAYROLL" datasource="#action_dsn2#">
	SELECT * FROM PAYROLL WHERE PROJECT_ID = #URL.ID#
</cfquery>
...
<cf_grid_list>
    ...
    <tbody>
		<cfif GET_PAYROLL.recordcount>
            <cfoutput query="GET_PAYROLL">
                <tr>
                    <td>#GET_PAYROLL.currentrow#&nbsp;</td>
                    <td>#GET_PAYROLL.PAYROLL_NO#</td>
                    <!---<td>#GET_PAYROLL.cheque_code#</td>--->
                    <td>
                    <cfset type=GET_PAYROLL.PAYROLL_TYPE>
                        <cfswitch expression="#type#">
                            ...
                            <cfcase value="91">
                                ...
                                <cfset Xurl="form_upd_payroll_endorsement">
                            </cfcase>
                            <cfcase value="92">
                                <cfset act="Çek Çıkış Bordrosu-Tahsil">
                                <cfset Xurl="form_upd_payroll_bank_revenue">
                            </cfcase>
                            <cfcase value="93">
                                <cfset act="Çek Çıkış Bordrosu-Banka">
                                <cfset Xurl="form_upd_payroll_bank_guaranty">
                            </cfcase>
                            <cfcase value="94">
                                <cfset act="Çek İade Çıkış Bordrosu">
                                <cfset Xurl="form_upd_payroll_endor_return">
                            </cfcase>
                            <cfcase value="95">
                                <cfset act="Çek İade Giriş Bordrosu">
                                <cfset Xurl="form_upd_payroll_entry_return">
                            </cfcase>
                            <cfcase value="105">
                                <cfset act="Çek İade Giriş Bordrosu-Banka">
                                <cfset Xurl="form_upd_payroll_bank_guaranty_return">
                            </cfcase>
                            <cfcase value="106"><!--- 20040112 Önceki Dönemden Aktarılan Çeklerin Bordrosunun Update i Yok Olmamalı --->
                                <cfset act="">
                                <cfset Xurl="">
                            </cfcase>
                            <cfdefaultcase>
                        ...
                        <a href="#request.self#?fuseaction=cheque.#Xurl#&ID=#GET_PAYROLL.ACTION_ID#" class="tableyazi">#act#</a>
                    </td>
                    <td>#dateformat(GET_PAYROLL.PAYROLL_REVENUE_DATE,dateformat_style)#&nbsp;</td>
                    <td>
                    <cfif len(GET_PAYROLL.COMPANY_ID)>
                        #get_company.FULLNAME[listfind(company_id_list,COMPANY_ID,',')]#
                    <cfelseif len(GET_PAYROLL.CONSUMER_ID)>
                    ...
                    <td>#dateformat(GET_PAYROLL.PAYROLL_AVG_DUEDATE,dateformat_style)#</td>
                    <td style="text-align:right;">#TLFormat(GET_PAYROLL.PAYROLL_TOTAL_VALUE)# #GET_PAYROLL.CURRENCY_ID# </td>
                    <!-- sil -->
                    <td>
                        <cfset my_flag=0>
                        <cfswitch expression="#type#">
