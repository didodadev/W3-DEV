...
<cf_grid_list>
	<thead>
		 <tr>
			...
			<th width="20">
				<cfif isdefined ("attributes.project_id") and len(attributes.project_id)>
					<a href="javascript://" onClick="windowopen('<cfoutput>#request.self#?fuseaction=call.list_service&event=add&project_id=#attributes.project_id#</cfoutput>','longpage');"><i class="fa fa-plus" alt="<cf_get_lang dictionary_id='57582.Ekle'>" title="<cf_get_lang dictionary_id='57582.Ekle'>"></i></a>
				</cfif>	
				<cfif isdefined("attributes.is_from_sales")>
					<a href="javascript://" onClick="windowopen('<cfoutput>#request.self#?fuseaction=call.list_service&event=add&subscription_id=#attributes.subscription_id#</cfoutput>','longpage');"><i class="fa fa-plus" alt="<cf_get_lang dictionary_id='57582.Ekle'>" title="<cf_get_lang dictionary_id='57582.Ekle'>"></i></a>
				</cfif>
            </th> 
		</tr>
	</thead>
	...
	<cfif get_g_service.recordcount>
		<cfoutput query="get_g_service">
			<tr>
				...
				<td width="80"><a href="#request.self#?fuseaction=call.list_service&event=upd&service_id=#get_g_service.service_id#&service_no=#get_g_service.service_no#" class="tableyazi">#service_no#</a></td>
				<td width="60">#dateformat(record_date,dateformat_style)#</td>
				<td width="200">#service_head#</td>z
				<td>#servicecat#</td>
				<td>#stage#</td>
                <td><a href="javascript://" onClick="windowopen('#request.self#?fuseaction=call.list_service&event=upd&service_id=#get_g_service.service_id#','longpage');"> <i class="fa fa-pencil" alt="<cf_get_lang dictionary_id='58718.Düzenle'>" title="<cf_get_lang dictionary_id='58718.Düzenle'>"></i></a></td>
			</tr>
		</cfoutput>
	<cfelse>
		<tr>
