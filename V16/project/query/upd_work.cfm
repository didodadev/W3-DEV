...
<cftransaction>
	...
	<cfquery name="ADD_WORK_HISTORY" datasource="#DSN#">
		...
		(
			...
			<cfif isdefined('attributes.completed_amount') and len(attributes.completed_amount)>#filternum(attributes.completed_amount)#<cfelse>NULL</cfif>,
			<cfif isdefined('attributes.average_amount') and len(attributes.average_amount)>#filternum(attributes.average_amount)#<cfelse>NULL</cfif>,
	...
	<cfquery name="UPD_PRO_WORKS" datasource="#DSN#">
		...
		SET 
			...
			EXPECTED_BUDGET=<cfif isDefined("form.expected_budget") and len(form.expected_budget)>#replace(expected_budget,',','','all')#<cfelse>NULL</cfif>,
		...
		<cfelseif len(form.task_partner_id)>
			...
			COMPLETED_AMOUNT = <cfif isdefined('attributes.completed_amount') and len(attributes.completed_amount)>#filternum(attributes.completed_amount)#<cfelse>NULL</cfif>,
			...
			AVERAGE_AMOUNT = <cfif isdefined('attributes.average_amount') and len(attributes.average_amount)>#filternum(attributes.average_amount)#<cfelse>NULL</cfif>,
			...
			SALE_CONTRACT_AMOUNT = <cfif isdefined('attributes.sale_contract_amount') and len(attributes.sale_contract_amount)>#filternum(attributes.sale_contract_amount)#<cfelse>NULL</cfif>,
			PURCHASE_CONTRACT_AMOUNT = <cfif isdefined('attributes.purchase_contract_amount') and len(attributes.purchase_contract_amount)>#filternum(attributes.purchase_contract_amount)#<cfelse>NULL</cfif>,
	...
	<cfset attributes.mail_content_from = '#session.ep.company#<#session.ep.company_email#>'><!--- Ortak Mail From --->
	...
	<cfif len(task_user_email)>
		...
		<cfsavecontent variable="message">
				<cfif mail_type_id eq 1><cf_get_lang no='362.iş kaydı'><cfelseif mail_type_id eq 2><cf_get_lang_main no='1745.bilgilendirme'></cfif>
		</cfsavecontent>
		...
		<cfsavecontent variable="subject_info">
			<cfif mail_type_id eq 1>
				<cf_get_lang_main no="1728.Görevlendirme">
			<cfelseif mail_type_id eq 2>
				<cf_get_lang_main no='1745.Bilgilendirme'>
			</cfif>
		</cfsavecontent>
				...
				<cfloop list="#attributes.cc_par_ids#" index="pid">
					...
					<cfif isdefined("attributes.is_mail")>
						...
						<!---    CC Mail Template Ekliyorum --->
							<cfsavecontent variable="subject_info"><cf_get_lang_main no='1745.Bilgilendirme'></cfsavecontent>
							...
							<cfsavecontent variable="attributes.mail_content_info"><cf_get_lang_main no='1745.bilgilendirme'>
							</cfsavecontent>
							<cfif len(mail_emp_id)>
								<cfset attributes.mail_content_link = '#user_domain#/#request.self#?fuseaction=project.works&event=det&id=#attributes.work_id#'>
							<cfelse>
...
<script type="text/javascript">
	...
	<cfelseif isdefined("attributes.is_det") and attributes.is_det eq 1>		
		refresh_box('workUpd','<cfoutput>#request.self#?fuseaction=project.emptypopup_upd_work_ajax&id=#attributes.work_id#</cfoutput>','0');
