<cfquery name="GET_TMARKET_CONSUMER" datasource="#DSN3#" result="xx">
	SELECT  DISTINCT
		C.CONSUMER_ID
	FROM 
	...
	WHERE
        ...
        <cfelseif TMARKET.cons_want_email eq 0>
            AND C.WANT_EMAIL = 0
        <cfelse>
            AND (C.WANT_EMAIL = 1 OR C.WANT_EMAIL = 0)
        </cfif>
		<cfif ListLen(TMARKET.CONS_REL_BRANCH) or ListLen(TMARKET.CONS_REL_COMP)>
		...
		<cfif isdefined('TMARKET.REQ_CONS') and len(TMARKET.REQ_CONS)> 
			<cfquery name="GET_REQ_PEOPLE_CONS" datasource="#DSN3#">
				SELECT 
		...
		<cfif len(TMARKET.TMARKET_MEMBERSHIP_FINISHDATE)>AND C.START_DATE <=#createodbcdatetime(TMARKET.TMARKET_MEMBERSHIP_FINISHDATE)#</cfif>
...
<cfset GET_TMARKET_CONSUMER_COUNT.TOTAL=GET_TMARKET_CONSUMER.RECORDCOUNT>
