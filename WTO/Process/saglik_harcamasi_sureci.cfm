...
--->
<cfset onay_asama_id = "2256"> <!--- Sağlık harcamasının onaylandı olarak işaretleneceği aşama id --->
<cfset red_asama_id = "2254"> <!--- Sağlık harcamasının red olarak işaretleneceği aşama id --->
<cfset odeme_asama_id = "2258"> <!--- Sağlık harcamasının ödendi olarak işaretleneceği aşama id --->
...
<cfif caller.attributes.fuseaction eq 'myhome.emptypopup_upd_list_warning'> <!--- Workflowdan tetiklendiğinde çalışacak --->
    ...
    <cfif caller.get_warnings.recordcount and caller.get_warnings.ACTION_STAGE_ID eq 171> <!--- Buraya süreç id'si yazılmalı --->
        ...
        <cfif len(id_list) >
            <cfquery name="upd_payment_date" datasource="#caller.dsn2#">
                UPDATE
    ...
    <cfif caller.get_warnings.recordcount and caller.get_warnings.ACTION_STAGE_ID eq 233> <!--- Buraya süreç id'si yazılmalı --->
        <!--- Toplu onay kontrolü - toplu onay varsa çalışacak --->
        <cfif len(getGeneralPaper.ACTION_LIST_ID) and actionListCounter eq listLen(getGeneralPaper.ACTION_LIST_ID)>
            <cfset id_list = getGeneralPaper.ACTION_LIST_ID>
        <cfelse>
        ...
        <cfif len(id_list)>
            ...
            <cfquery name="cari_control" datasource="#caller.dsn2#">
                SELECT 
                    INVOICE_NO 
                FROM 
                    EXPENSE_ITEM_PLAN_REQUESTS 
            ...
            <cfif len(type) and type eq 1>
                <cfquery name="getExpenses" datasource="#caller.dsn2#">
                    SELECT 
