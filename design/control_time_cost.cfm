<!--- 
	Zaman Harcamasi Kontrolu
	Sirket Akis Parametrelerinden Gunluk veya Haftalik Olarak Secilir
	Sureler, Calisan Maliyet Tanimlarindan Girilen Calisma Saatleri Uzerinden Kontrol Edilir
	1: Haftalik	: Aylik Calisma Saati Uzerinden Hesaplanir,
				  AylikSaat/4 ile Haftalik Calisma Saati Bulunur, Girilen Haftalik Toplam Harcama ile Karsilastirilir.
				  (BURADA DAHA DOGRU BIR DUZENLEME YAPILABILIR, AY TOPLAM SURESINDE KONTROLDE EKSIKLIKLER OLABILIR!)
	2: Gunluk	: Gunluk Calisma Uzerinden Uzerinden Hesaplanir,
				  Bulunulan Ay Icerisinde Ilk Is Gununden Itibaren Bugune Kadarki IsGunleri Bulunur, Gun Bazinda Girilen Aylik Toplam Harcama ile Karsilastirilir.
	
	Haftanin Gunlerinin Rakamla Siralamasi;
	1-Pazar,	2-Pazartesi,	3-Salı,		4-Çarşamba,		5-Perşembe,		6-Cuma,		7-Cumartesi
 --->
<cfif fusebox.fuseaction is not 'upd_myweek' and fusebox.fuseaction is not 'add_my_week_timecost' and fusebox.fuseaction is not 'my_offtimes'>
	...
	<cfif not isdefined("session.ep.time_cost_control")>
		<cfquery name="GET_OUR_COMPANY_CONTROL" datasource="#DSN#">
	...
	<cfif session.ep.time_cost_control eq 1 and ListFind("1,2",session.ep.time_cost_control_type,",")>
		<cfquery name="GET_EMPLOYEE_HOUR_CONTROL" datasource="#DSN#">
		...
		<cfif GET_EMPLOYEE_HOUR_CONTROL.TIME_COST_CONTROL eq 0><!--- Eğer pozisyon detayda zaman harcaması kontrolü yapılmasın seçili ise kontrol yapılacak --->
            ...
            <cfif DateDiff('d',get_employee_hour_control.position_startdate,WStart) lt 0>
            	<cfset WStart = get_employee_hour_control.position_startdate>
            </cfif>
			...
			<cfif Get_Employee_Hour_Control.On_Hour gt 0 and len(Get_Employee_Hour_Control.On_Hour_Daily)>
				...
				<cfif session.ep.time_cost_control_type eq 2>
					...
					<cfif Get_Employee_Hour_Control.On_Hour_Daily gt 0>
						...
						<cfif Len(All_WorkDay_List)>
							<cfquery name="GET_FULL_TIME_COST_WORKDAY" datasource="#DSN#">
								SELECT
						...
						<cfif ListLen(InComplete_WorkDay_List)>
							<script type="text/javascript">
								alert("Zaman Harcamanızda <cfoutput>#Get_Employee_Hour_Control.On_Hour_Daily#</cfoutput> Saate Tamamlanmamış Gün Bulunmaktadır. \n Lütfen Kontrol Ediniz ! \n <cfoutput>İlgili Günler : #InComplete_WorkDay_List#</cfoutput>");
				...
				<cfelse>
					<cfscript>
						...
						if (fark eq 1) fark = -6;
						last_week_start = date_add('d',fark-7,tarih);
						first_day = date_add('d',fark,tarih);
					</cfscript>
					<cfquery name="GET_TIME" datasource="#DSN#">
						SELECT 
					...
					<cfif total_calisma lt control_time>
						<script type="text/javascript">
							alert("Haftalık Zaman Harcamasını Doldurmadınız! Sistem İşlem Yapmanıza İzin Vermeyecektir!");
							window.location.href = '<cfoutput>#request.self#?fuseaction=myhome.upd_myweek</cfoutput>';
						</script>
						<cfabort>
					<cfelse>
