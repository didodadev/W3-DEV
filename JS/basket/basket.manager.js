var basketManager = function() {
    ...
    self.activePopupHeaderName = ko.observable('');
    ...
    self.basketItemsView = ko.computed( function(){
        ...
        for (var i = startIndex; i < endIndex; i++) {
            if (i < self.basketItems().length)
                result.push({items: self.basketItems()[i], index: i })
        }
        return result;
    });
    ...
    return {
        ...
        setActivePopup: function(name, data,headername) {
            self.activePopupTemplate(name);
            self.activePopupData(data);
            self.activePopupHeaderName(headername);
        },
        getBasketJSON() {
