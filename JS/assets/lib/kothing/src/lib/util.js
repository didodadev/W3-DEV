...
const util = {
  ...
  _tagConvertor: function(text) {
    ...
    return text.replace(
      /(<\/?)(b|strong|var|i|em|u|ins|s|strike|del)\b\s*(?:[^>^<]+)?\s*(?=>)/gi,
      function(m, t, n) {
  ...
  convertHTMLForCodeView: function(wysiwygDiv) {
    ...
    (function recursionFunc(element) {
      ...
      for (let i = 0, len = children.length, node; i < len; i++) {
        node = children[i];
        if (
          /^(BLOCKQUOTE|TABLE|THEAD|TBODY|TR|OL|UL|FIGCAPTION)$/i.test(
            node.nodeName
          )
        ) {
          ...
          html +=
            node.outerHTML.match(reg("<" + tag + "[^>]*>", "i"))[0] + "\n";
          recursionFunc(node);
          html += "</" + tag + ">\n";
        } else {
          html +=
  ...
  isFormatElement: function(element) {
    if (
      element &&
      element.nodeType === 1 &&
      /^(P|DIV|H[1-6]|LI|TH|TD)$/i.test(element.nodeName) &&
  ...
  isRangeFormatElement: function(element) {
    if (
      element &&
      element.nodeType === 1 &&
      (/^(BLOCKQUOTE|OL|UL|PRE|FIGCAPTION|TABLE|THEAD|TBODY|TR|TH|TD)$/i.test(
        element.nodeName
      ) ||
        element.getAttribute("data-format") === "range")
    )
      return true;
    return false;
  },
  ...
  /**
   * @description It is judged whether it is the component(img, iframe cover, table, hr) element - ".ke-component"
   * @param {Element} element The element to check
   * @returns {Boolean}
   */
  ...
  getListChildren: function(element, validation) {
    ...
    (function recursionFunc(current) {
      if (
        (element !== current && validation(current)) ||
        /^BR$/i.test(element.nodeName)
      ) {
  ...
  getOffset: function(element, wysiwygFrame) {
    ...
    return {
      ...
      top:
        offsetTop - wysiwyg.scrollTop + (iframe ? wysiwygFrame.offsetTop : 0),
    };
  },
  ...
  removeEmptyNode: function(element) {
    ...
    (function recursionFunc(current) {
      if (
        ...
        !/^BR$/i.test(current.nodeName) &&
        (!current.firstChild || !/^BR$/i.test(current.firstChild.nodeName)) &&
        !inst.isComponent(current)
      ) {
  ...
  isIgnoreNodeChange: function(element) {
    return (
      element.nodeType !== 3 &&
      !/^(span|font|b|strong|var|i|em|u|ins|s|strike|del|sub|sup|mark)$/i.test(
        element.nodeName
      )
    );
  },
  ...
  cleanHTML: function(html) {
    const tagsAllowed = new this._w.RegExp(
      "^(meta|script|link|style|[a-z]+:[a-z]+)$",
      "i"
    );
    const domTree = this._d.createRange().createContextualFragment(html)
      .childNodes;
    let cleanHTML = "";
    ...
    for (let i = 0, len = domTree.length; i < len; i++) {
      if (!tagsAllowed.test(domTree[i].nodeName)) {
        cleanHTML +=
          domTree[i].nodeType === 1
            ? domTree[i].outerHTML
            : domTree[i].nodeType === 3
            ? domTree[i].textContent
            : "";
      }
    }
    ...
    cleanHTML = cleanHTML
      ...
      .replace(/(<[a-zA-Z0-9]+)[^>]*(?=>)/g, function(m, t) {
        const v = m.match(
          /((?:colspan|rowspan|target|href|src|class|data-file-size|data-file-name|data-origin|origin-size|data-format)\s*=\s*"[^"]*")/gi
        );
        if (v) {
