	...
	if ((unicode >= 48 && unicode <= 57) || (unicode >= 65 && unicode <= 90) || (unicode >= 96 && unicode <= 105) || unicode == 8 || unicode == 46 || unicode == 32 || tr_unicode || other_unicode){
		...
		if (arrAutocomplete[AutocompleteId]["obj_text_input"].value.length>2){
			...
			if(dizi[1])
			{
				...
				{
					document.getElementById(arrAutocomplete[AutocompleteId]["divName"]).style.display='block';					
					AjaxPageLoad('index.cfm?fuseaction=objects2.emptypopup_autocomplete&mask='+urlencode(dizi[1])+'&'+arrAutocomplete[AutocompleteId]["address"],arrAutocomplete[AutocompleteId]["divName"],1);
				}
			}
			else
			{				
				AjaxPageLoad('index.cfm?fuseaction=objects2.emptypopup_autocomplete&mask='+urlencode(dizi[0])+'&'+arrAutocomplete[AutocompleteId]["address"],arrAutocomplete[AutocompleteId]["divName"],1);					
			}
...
function AutoComplete_GetLeft2(element)
{
	var curNode = element;
	var left    = 0;
	do
	{
		alert("1."+curNode.tagName.toLowerCase());
		if(curNode.tagName.toLowerCase() != 'div')//div ise kaymalara sebebe oluyor
			left += curNode.offsetLeft;
		else if(curNode.offsetParent.tagName.toLowerCase() == 'div')
			left += curNode.offsetLeft;
		alert("Left:"+left);
		alert("2.curNode:"+curNode);
		curNode = curNode.offsetParent;
		alert("3.curNode:"+curNode);
	}
	while(curNode.tagName.toLowerCase() != 'body' && curNode.tagName.toLowerCase() != 'html');
	left = left - 1;
	return left;
