...
;(function ( $, window, undefined ) {
	...
	Plugin.prototype = {
		init: function (element) {
			...
			var slidesCount = this.element.slidesContainer.slides.length - 3;
			var slidesContainerWidth = (slidesCount + 2) * 100;
			//var slideWidth = 100 / (slidesCount + 1);

			slidesContainerCss.width = slidesContainerWidth + '%';
			//slideCss.width = slideWidth + '%';

			this.padding = 0//this.element.width() / 2;
		...
		setOffset:function(){
			this.index = $("#slider").attr("data-id");
		},
		slideNext: function() {
		...
		_slide: function () {
			...
			console.log('total tab:' + this.maxIndex);
			...
			$("div#Downtab button").removeClass("active");	
			$("div#Downtab button:eq("+this.index+")").addClass("active");
			$("#slider").attr({"data-id": + this.index});
