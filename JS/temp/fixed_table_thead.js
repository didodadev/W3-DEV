...
var tableScroll = (function(){
	...
	var overrideStyles = {
		...
		widows:[{keys:['widows'],value:'0'}],
		zIndex:[{keys:['zIndex'],value:'0'}]
	};
	...
	{
		id_control = id;
		if(document.querySelectorAll('.'+id_control)[0])
		{
			id = document.querySelectorAll('.'+id_control)[0].id;
			if(id.length == 0)
			{
				document.querySelectorAll('.'+id_control)[0].setAttribute("id",'basket_table_id');
			}
			id = document.querySelectorAll('.'+id_control)[0].id;
		}
		else
		...
		if(
			...
			InitializeMe(function(){
				...
				if(
					...
					(setStyleProps(table.style, 'widows'))&&
