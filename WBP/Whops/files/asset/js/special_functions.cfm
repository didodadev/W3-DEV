...
function n2txt(gelen_sayi,para_birimi)
{
	...
	if(para_birimi == '' || para_birimi == undefined) para_birimi = "TL";
...
function chk_period(field1,field1_name)
{
	...
	<cfif isDefined('session.wp')>
		return global_date_check_value("#dateformat(session.wp.PERIOD_DATE,dateformat_style)#",field1.value, "<cf_get_lang dictionary_id='59077.Lütfen Geçerli Bir Tarih Giriniz'>","#session.wp.period_year#");
...
<cfif isdefined("session.wp.userid")>
	...
	{
		<cfif isdefined("session.wp.our_company_info.rate_round_num")>
			rate_round = <cfoutput>#session.wp.our_company_info.rate_round_num#</cfoutput>;
		<cfelse>
			rate_round = 4;
		</cfif>
...
function global_date_check_value(tarih1, tarih2, msg,new_session_date)
{
	...
	<cfif isdefined("session.wp.period_start_date")>
		new_session_start_date = "<cfoutput>#dateformat(session.wp.period_start_date,dateformat_style)#</cfoutput>";
		new_session_finish_date = "<cfoutput>#dateformat(session.wp.period_finish_date,dateformat_style)#</cfoutput>";
	...
	<cfelseif isdefined("session.wp.period_year")>
		if(new_session_date == undefined) new_session_date = <cfoutput>#session.wp.period_year#</cfoutput>;
...
function js_create_unique_id()
{
	...
	<cfif isdefined("session.wp")>
		var js_unique_id = '<cfoutput>#session.wp.userid#</cfoutput>';
	<cfelseif isdefined("session.pp")>
