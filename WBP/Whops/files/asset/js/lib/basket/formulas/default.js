...
function amount_hesapla( fieldName, basketRow ) {
    ...
    if( basketManager.hasShownItem("is_use_add_unit") ){
        if ( basketRow.unit2 != '' ) {
...
function control_row_prom( basketRow ) {
    ...
    if ( get_row_proms.recordcount ) {
        ...
        if (get_row_proms.PROM_ID != row_prom_id) {
            viewModel.row_promotion_id( get_row_proms.PROM_ID );
        }
        if (free_prom_row != 0) {
            //
        } else {
...
function add_basket_row(product_id, stock_id, stock_code, barcod, manufact_code, product_name, unit_id_, unit_, spect_id, spect_name, price, price_other,tax, duedate, d1, d2, d3, d4, d5, d6, d7, d8, d9, d10, deliver_date, deliver_dept, department_head, lot_no, money, row_ship_id,amount_, product_account_code, is_inventory,is_production,net_maliyet,flt_marj,extra_cost,row_promotion_id,promosyon_yuzde,promosyon_maliyet,iskonto_tutar,is_promotion,prom_stock_id,otv,product_name_other,amount_other,unit_other,ek_tutar,shelf_number,row_unique_relation_id,row_catalog_id,toplam_hesap_yap,is_commission,basket_extra_info,prom_relation_id,reserve_date,list_price,number_of_installment,price_cat,karma_product_id,row_service_id,ek_tutar_price,wrk_row_relation_id,related_action_id,related_action_table,row_width,row_depth,row_height,to_shelf_number,row_project_id,row_project_name,row_otv_amount,action_window_name,row_paymethod_id,special_code,basket_employee_id,basket_employee,row_work_id,row_work_name, row_exp_center_id, row_exp_center_name, row_exp_item_id, row_exp_item_name, row_acc_code,select_info_extra,detail_info_extra,gtip_number,row_activity_id,row_subscription_id,row_subscription_name,row_assetp_id,row_assetp_name,row_bsmv_rate,row_bsmv_amount,row_bsmv_currency,row_oiv_rate,row_oiv_amount,row_tevkifat_rate,row_tevkifat_amount,reason_code_info,row_tevkifat_id)
{
    ...
    if (basketManager.hasShownItem("otv_from_tax_price")) {
...
function row_object(rowObj){
    ...
    if( data.length == undefined){
        ...
        data.forEach( elm => {
            add_basket_row(elm.PRODUCT_ID, elm.STOCK_ID, elm.STOCK_CODE, elm.BARCOD, elm.MANUFACT_CODE, elm.PRODUCT_NAME, elm.UNIT_ID, elm.UNIT, elm.SPECT_ID, elm.SPECT_NAME, elm.PRICE, elm.PRICE_OTHER, elm.TAX, elm.DUEDATE, elm.D1, elm.D2, elm.D3, elm.D4, elm.D5, elm.D6, elm.D7, elm.D8, elm.D9, elm.D10, elm.DELIVER_DATE, elm.DELIVER_DEPT, elm.DEPARTMENT_HEAD, elm.LOT_NO, elm.MONEY, elm.ROW_SHIP_ID, elm.AMOUNT_, elm.PRODUCT_ACCOUNT_CODE, elm.IS_INVENTORY, elm.IS_PRODUCTION, elm.NET_MALIYET, elm.MARJ, elm.EXTRA_COST, elm.ROW_PROMOTION_ID, elm.PROMOSYON_YUZDE, elm.PROMOSYON_MALIYET, elm.ISKONTO_TUTAR, elm.IS_PROMOTION, elm.PROM_STOCK_ID, elm.OTV, elm.PRODUCT_NAME_OTHER, elm.AMOUNT_OHTER, elm.UNIT_OTHER, elm.EK_TUTAR, elm.SHELF_NUMBER, elm.ROW_UNIQUE_RELATION_ID, elm.ROW_CATALOG_ID, elm.TOPLAM_HESAP_YAP, elm.IS_COMMISSION, elm.BASKET_EXTRA_INFO, elm.PROM_RELATION_ID, elm.RESERVE_DATE, elm.LIST_PRICE, elm.NUMBER_OF_INSTALLMENT, elm.PRICE_CAT, elm.KARMA_PRODUCT_ID, elm.ROW_SERVICE_ID, elm.EK_TUTAR_PRICE, elm.WRK_ROW_RELATION_ID, elm.RELATED_ACTION_ID, elm.RELATED_ACTION_TABLE, elm.ROW_WIDTH, elm.ROW_DEPTH, elm.ROW_HEIGHT, elm.TO_SHELF_NUMBER, elm.ROW_PROJECT_ID, elm.ROW_PROJECT_NAME, elm.ROW_OTV_AMOUNT, elm.ACTION_WINDOW_NAME, elm.ROW_PAYMETHOD_ID, elm.SPECIAL_CODE, elm.BASKET_EMPLOYEE_ID, elm.BASKET_EMPLOYEE, elm.ROW_WORK_ID, elm.ROW_WORK_NAME, elm.ROW_EXP_CENTER_ID, elm.ROW_EXP_CENTER_NAME, elm.ROW_EXP_ITEM_ID, elm.ROW_EXP_ITEM_NAME, elm.ROW_ACC_CODE, elm.SELECT_INFO_EXTRA, elm.DETAIL_INFO_EXTRA, elm.GTIP_NUMBER, elm.ROW_ACTIVITY_ID, elm.ROW_SUBSCRIPTION_ID, elm.ROW_SUBSCRIPTION_NAME, elm.ROW_ASSETP_ID, elm.ROW_ASSETP_NAME, elm.ROW_BSMV_RATE, elm.ROW_BSMV_AMOUNT, elm.ROW_BSMV_CURRENCY, elm.ROW_OIV_RATE, elm.ROW_OIV_AMOUNT, elm.ROW_TEVKIFAT_RATE, elm.ROW_TEVKIFAT_AMOUNT, elm.REASON_CODE_INFO, elm.ROW_TEVKIFAT_ID );
        });
    }
}
/**
...
function open_product_popup(viewModel) {
    if (basketManager.hasShownItem("product_name")) {
        ...
        let stock_id = viewModel.stock_id();
        let product_id = viewModel.product_id();
        let spect_id = viewModel.spect_id();
        let spect_name = viewModel.spect_name();
...
function basket_exp_center_popup(satir){
    windowopen('/index.cfm?fuseaction=objects.popup_expense_center&satir='+satir, 'list');
}
...
function open_subscription_popup(satir){
    windowopen('/index.cfm?fuseaction=objects.popup_list_subscription&satir='+satir, 'list');
}
...
function basket_exp_item_popup(satir){
    windowopen('/index.cfm?fuseaction=objects.popup_list_exp_item&satir='+satir, 'list');
}
...
function open_assetp_popup(satir){
	windowopen('/index.cfm?fuseaction=assetcare.popup_list_assetps&event_id=0&satir='+satir, 'list');
}
/**
 * Satır Proje
 * @param {int} satir satır no 
...
function open_basket_project_popup(satir){
    openBoxDraggable('index.cfm?fuseaction=objects.popup_list_projects&satir='+satir);
}
/**
 * Satır İş
 * @param {int} satir satır no
...
function open_basket_work_popup(satir){
    windowopen('/index.cfm?fuseaction=objects.popup_add_work&satir='+satir, 'list');
 }
...
function open_basket_employee_popup(satir){
    if(basketManager.hasShownItem("basket_employee")){
        ...
        windowopen('/index.cfm?fuseaction=objects.popup_list_positions&select_list=1&field_emp_id='+field_basket_emp_id_+'&field_name='+field_basket_emp_name_+'&satir='+satir, 'list');
    }
}
...
function open_basket_locations(satir){
    ...
    windowopen('/index.cfm?fuseaction=objects.popup_list_stores_locations&form_name=form_basket&field_name='+field_name+'&field_id='+field_id+'&row_id='+satir+'&satir='+satir,'list');
}
...
var toplam_hesapla = function( from_sa_amount, from_new_row, is_from_upd ) {
    ...
    if (basketService.wmo_module() == 'invoice' || [1,2,3,4,5,10,14,15,20,21,33,38,42,43,46,51,52].indexOf(basketService.basket_id()) >= 0 ) {
        if (basketManager.hasShownItem("is_promotion")) {
    ...
    if ( basketManager.hasShownItem("is_risc") ) {
...
function control_comp_selected(update_product_row_id, isUpdOrAdd,prod_config) {
    ...
    if ( basketManager.hasShownItem("is_member_selected") ) {
    ...
    if ( (sepet_process_type == -1 || sepet_process_type != 52) && sepet_process_type != 115 ) {
        if( ( $("#basket_main_div #project_id").length > 0 && $("#basket_main_div #project_head").length > 0 ) || $("#basket_main_div #project_id_in").length > 0 && $("#basket_main_div #project_head_in").length > 0 ) {
            if ( ( ($("#basket_main_div #project_id").val().length == 0 || $("#basket_main_div #project_head").val().length == 0 ) && $("#basket_main_div #project_id_in").length == 0) || ( ($("#basket_main_div #project_id").val().length == 0 || $("#basket_main_div #project_head").val().length == 0) && ($("#basket_main_div #project_id_in").length != 0 && ($("#basket_main_div #project_id_in").length == 0 || $("#basket_main_div #project_id_in").val().length == 0)) ) ) {
                if ( basketManager.hasShownItem("is_project_selected") ) {
...
function open_lot_no_popup(viewModel) {
    ...
    let from_stock_code = viewModel.stock_code();
    let product_id = viewModel.product_id();
    let stock_id = viewModel.stock_id();
...
function open_price(viewModel) {
    if (viewModel.product_id() == "") return;

    let urlparams = {};
    let satir = basketManagerObject.basketItems().indexOf(viewModel);

    urlparams.product_id = viewModel.product_id();
    urlparams.stock_id = viewModel.stock_id();
    urlparams.product_name = viewModel.product_name();
    urlparams.unit = viewModel.unit();
 ...
 */
    ...
    if( $("input#data_action").val() != undefined && $("input#next_page").val() != undefined ) {
        ...
        var data = new FormData();
            ...
            data.append('form_data', encodeURIComponent(JSON.stringify(basketJSON)) );
            AjaxControlPostDataJson('../../../../../app/component/datagate.cfc?method=basketConverter', data, function(response) {
                if( response.STATUS ){
                    cfmodalx({e:'close',id:'pay_modal'});

                    $("<label>").addClass("col col-3 col-md-3 col-xs-2").append(
                        $("<a>").attr({ "href" : "javascript://", "onclick" : "printInvoice('"+response.INVOICE_ID+"')" }).text("Print")
                    ).appendTo('div#footerElement');

                    $("<label>").addClass("col col-3 col-md-3 col-xs-2").attr({ "id" : "invoiceSend" }).append(
                        $("<a>").attr({ "href" : "javascript://", "onclick" : "sendInvoice('"+response.INVOICE_ID+"')" }).text("e-Fatura Gönder")
                    ).appendTo('div#footerElement');

                    $("<label>").addClass("col col-3 col-md-3 col-xs-2").append(
                        $("<a>").attr({ "href" : "/index.cfm?wo=invoice.whops" }).text("Yeni Kayıt")
                    ).appendTo('div#footerElement');

                    alert("Fatura Başarılı Şekilde Oluşturuldu. Aşağıdaki Menü'den e-fatura gönderebilir, print edebilir ya da yeni kayıt oluşturabilirsiniz.");
                }else{
        ...
        callURLBasket("/index.cfm?fuseaction=objects.emptypopup_basket_converterv2&ajax=1&xmlhttp=1&_cf_nodebug=true",handlerPostBasket,{ basket: encodeURIComponent(JSON.stringify(basketJSON)) });
        return false;
    }
}
...
function popup_factory(fieldName, fieldData, viewModel) {
    switch (fieldName) {
        case 'other_money':
            basketManager.setActivePopup("template_other_money", viewModel);
            break;
        case 'reason_code':
            basketManager.setActivePopup("template_reason_code", fieldData);
            break;
        case 'otv_type':
            basketManager.setActivePopup("template_otv_type", fieldData);
            break;
        case 'delivery_condition':
            basketManager.setActivePopup("template_delivery_condition", fieldData);
            break;
        case 'container_type':
            basketManager.setActivePopup("template_container_type", fieldData);
            break;
        case 'delivery_type':
            basketManager.setActivePopup("template_delivery_type", fieldData);
            break;
        ...
        case 'basket_acc_code_popup':
            open_basket_acc_code_popup(basketManagerObject.basketItems().indexOf(fieldData));
            break;
        case 'lot_no_popup':
            open_lot_no_popup(fieldData);
            break;
        case 'unit2':
            basketManager.setActivePopup("template_unit2", viewModel);
            break;
        case 'row_tevkifat_id':
            basketManager.setActivePopup("template_tevkifat_rates", viewModel);
            break;
        case 'department_popup':
            open_basket_locations(basketManagerObject.basketItems.indexOf(fieldData));
            break;
        case 'order_currency':
            basketManager.setActivePopup("template_order_currency", fieldData);
            break;
        case 'reserve_type':
            basketManager.setActivePopup("template_reserve_type", fieldData);
            break;
        case 'row_activity_id':
            basketManager.setActivePopup("template_activity_id", fieldData);
            break;
        case 'basket_extra_info':
            basketManager.setActivePopup("template_basket_extra_info", fieldData);
            break;
        case 'select_info_extra':
            basketManager.setActivePopup("template_select_info_extra", fieldData);
            break;
        case 'basket_exp_center_popup':
            basket_exp_center_popup(basketManagerObject.basketItems().indexOf(fieldData));
            break;
        case 'basket_exp_item_popup':
            basket_exp_item_popup(basketManagerObject.basketItems().indexOf(fieldData));
            break;
        case 'row_subscription_popup':
            open_subscription_popup(basketManagerObject.basketItems().indexOf(fieldData));
            break;
        case 'row_assetp_popup':
            open_assetp_popup(basketManagerObject.basketItems().indexOf(fieldData));
            break;
        case 'row_project_popup':
            open_basket_project_popup(basketManagerObject.basketItems().indexOf(fieldData));
            break;
        case 'row_work_popup':
            open_basket_work_popup(basketManagerObject.basketItems().indexOf(fieldData));
            break;
        case 'basket_employee_popup':
            open_basket_employee_popup(basketManagerObject.basketItems().indexOf(fieldData));
            break;
        case 'reserve_date_popup':
            get_basket_date_reserve('reserve_date', basketManagerObject.basketItems().indexOf(fieldData));
            break;
        case 'deliver_date_popup':
            get_basket_date_deliver('deliver_date', basketManagerObject.basketItems().indexOf(fieldData));
            break;
        case 'spect_name_popup':
            open_spec(basketManagerObject.basketItems().indexOf(fieldData));
            break;
        case 'shelf_list':
            open_shelf_list(basketManagerObject.basketItems().indexOf(fieldData), basketManagerObject.basketItems().indexOf(fieldData), 0, 'shelf_number', 'shelf_number_txt');
            break;
        case 'shelf_list_info':
            open_shelf_list(basketManagerObject.basketItems().indexOf(fieldData), basketManagerObject.basketItems().indexOf(fieldData), 1, 'shelf_number', 'shelf_number_txt');
            break;
        case 'to_shelf_list':
            open_shelf_list(basketManagerObject.basketItems().indexOf(fieldData), basketManagerObject.basketItems().indexOf(fieldData), 0, 'to_shelf_number', 'to_shelf_number_txt');
            break;
        default:
            break;
    }
...
function tripoint_factory(fieldName, viewModel) {
    switch (fieldName) {
        case 'product_name':
            return ['product_detail_popup', 'product_price_history_popup'];
        case 'basket_acc_code':
            return ['basket_acc_code_popup'];
        case 'basket_exp_item':
            return ['basket_exp_item_popup'];
        case 'basket_exp_center':
            return ['basket_exp_center_popup'];
        case 'lot_no':
            return ['lot_no_popup'];
        case 'price':
            return ['price_popup'];
        case 'deliver_dept':
            return ['department_popup'];
        case 'row_subscription_name':
            return ['row_subscription_popup'];
        case 'row_assetp_name':
            return ['row_assetp_popup'];
        case 'basket_project':
            return ['row_project_popup'];
        case 'basket_work':
            return ['row_work_popup'];
        case 'basket_employee':
            return ['basket_employee_popup'];
        case 'reserve_date':
            return ['reserve_date_popup'];
        case 'deliver_date':
            return ['deliver_date_popup'];
        case 'spec':
            return ['spect_name_popup'];
        case 'shelf_number_txt':
            return ['shelf_list', 'shelf_list_info'];
        case 'to_shelf_number_txt':
            return ['to_shelf_list'];
        default:
            break;
    }
}
...
function lv_factory(fieldName, value) {
    switch (fieldName) {
        ...
        case 'select_info_extra':
            return value() == '' ? basketService.get('select_info_extra_list')[0].TEXT : basketService.get('select_info_extra_list').filter(m => m.ID == value())[0];
        case 'basket_extra_info':
