...
<cf_report_list_search title="#getLang('','	Ürün Bazlı Sayım Karşılaştırma Raporu',62762)#">
    <cf_report_list_search_area>
        <cfform action="#request.self#?fuseaction=retail.list_stock_count_orders_report_product" method="post" name="search_depo">
            <input name="is_form_submitted" id="is_form_submitted" type="hidden" value="1">
            <input type="hidden" name="maxrows" id="maxrows" value="<cfoutput>#attributes.maxrows#</cfoutput>" required="yes" validate="integer">
            <div class="row">
                            ...
                            <div class="col col-3 col-md-3 col-sm-6 col-xs-12">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col col-12 col-xs-12">
                                            <div class="input-group">
                                                <cfinput type="text"  id="limit" name="limit" value="#attributes.limit#" class="moneybox" onkeyup="return(FormatCurrency(this,event,#session.ep.our_company_info.purchase_price_round_num#));">
                                ...
                                <div class="col col-12 col-xs-12">
                                    <div class="form-group">
                                        ...
                                        <div class="col col-12 col-xs-12" >
                                            <div class="input-group">
                                                <select name="siralama_tipi" id="siralama_tipi" >                
                                                ...
                                                <select name="siralama" id="siralama" >                  
                                                    ...
                                                    <option value="7" <cfif attributes.siralama eq 7>selected</cfif>><cf_get_lang dictionary_id='61642.Alt Grup'> 2</option>                                
                                                </select>  
                                            </div>
                                        </div>
                                    </div>
...
<cfelse>
    ...
    (
        SELECT
            TOPLAM_STOCK,        
            TOTAL_COUNT_AMOUNT,
            (TOTAL_COUNT_AMOUNT - TOPLAM_STOCK) AS MIKTAR_FARK,
            (TOPLAM_STOCK * LISTE_FIYATI) AS TOPLAM_STOCK_TUTAR,
            (TOTAL_COUNT_AMOUNT * LISTE_FIYATI) AS TOPLAM_COUNT_TUTAR,
            ((TOTAL_COUNT_AMOUNT * LISTE_FIYATI) - (TOPLAM_STOCK * LISTE_FIYATI)) AS TUTAR_FARK,
            T2.PRODUCT_CAT,
            T2.STOCK_NAME,
            T2.STOCK_ID,
            T2.BARCOD,
                ...
                SELECT
                    ...
                    WHERE
                        ...
                        OR
                            ...
                            SR.PROCESS_TYPE NOT IN (-1002)
                            )
                        )
                    ),0)
                    AS TOPLAM_STOCK,
                    SUM(AMOUNT) TOTAL_COUNT_AMOUNT,
                    T1.PRODUCT_CAT,
                    T1.STOCK_NAME,
                    T1.STOCK_ID,
                    T1.BARCOD,
                ...
                FROM
                    (            
                     SELECT
                        ...
                        T2.STOCK_ID,
                        T2.BARCOD,
                        CASE WHEN (T2.UCUNCU_SAYIM_AMOUNT IS NOT NULL AND T2.UCUNCU_SAYIM_AMOUNT > -1000000)
                    		THEN T2.UCUNCU_SAYIM_AMOUNT / T2.STOCK_SAYISI
                        ELSE 
                        	T2.AMOUNT_ASIL END AS AMOUNT,
                        T2.IS_UPDATE,
                        T2.ORDER_DATE,
                        T2.DEPARTMENT_ID,
                        T2.LISTE_FIYATI
                     FROM
                     	(
                            SELECT
                                ...
                                SCOR.STOCK_ID,
                                S.BARCOD,
                                SCOR.AMOUNT AS AMOUNT_ASIL,
                                (SELECT COUNT(SCOR_IC.STOCK_ID) FROM #dsn_dev_alias#.STOCK_COUNT_ORDERS_ROWS SCOR_IC WHERE SCOR_IC.ORDER_ID = SCO.ORDER_ID AND SCOR_IC.STOCK_ID = SCOR.STOCK_ID) AS STOCK_SAYISI,
                                (
                                SELECT 
                                    SUM(SCOR3.AMOUNT) AS TOTAL 
                                FROM 
                                    #dsn_dev_alias#.STOCK_COUNT_ORDERS_ROWS SCOR3,
                                    #dsn_dev_alias#.STOCK_COUNT_ORDERS SCO3
                                WHERE 
                                    SCO3.ORDER_ID = SCOR3.ORDER_ID AND
                                    SCO3.COUNT_TYPE = 3 AND
                                    SCO3.MAIN_ORDER_ID = SCO.ORDER_ID AND
                                    SCOR3.ORDER_ID = SCO.ORDER_ID + 2 AND 
                                    SCOR3.STOCK_ID = SCOR.STOCK_ID
                                ) AS UCUNCU_SAYIM_AMOUNT,
                                SCOR.IS_UPDATE,
                                SCO.ORDER_DATE,
                                SCO.DEPARTMENT_ID,
                                ISNULL(( 
                          ...
                          UNION ALL
                            ...
                            WHERE	
                                ...
                                PS.PRICESTANDART_STATUS = 1 AND
                                SCO.STATUS = 1 AND
                                S.STOCK_ID NOT IN (SELECT SCOR2.STOCK_ID FROM #dsn_dev_alias#.STOCK_COUNT_ORDERS_ROWS SCOR2 WHERE SCOR2.ORDER_ID = SCO.ORDER_ID AND SCOR2.STOCK_ID IS NOT NULL) AND
                                S.STOCK_ID IN 
                                	(
                                    SELECT 
                                    	SCOR4.STOCK_ID 
                                    FROM 
                                    	#dsn_dev_alias#.STOCK_COUNT_ORDERS_ROWS SCOR4,
                                        #dsn_dev_alias#.STOCK_COUNT_ORDERS SCO4
                                    WHERE 
                                    	SCOR4.ORDER_ID = SCO.ORDER_ID + 2 AND
                                        SCOR4.ORDER_ID = SCO4.ORDER_ID AND
                                        SCO4.COUNT_TYPE = 3 AND
                                        SCO4.MAIN_ORDER_ID = SCO.ORDER_ID AND 
                                        SCOR4.STOCK_ID IS NOT NULL
                                    ) AND
                                S.PRODUCT_STATUS = 1 AND
                                S.STOCK_STATUS = 1 AND
                                S.PRODUCT_CATID = PC.PRODUCT_CATID 
        ...
        ORDER BY        	
            ...
            <cfelseif attributes.siralama eq 6>MIKTAR_FARK
            <cfelseif attributes.siralama eq 7>PRODUCT_CAT</cfif>
        ...
        <tfooter>
            <cfoutput>
                <tr>
                    ...
                    <td></td>
                    <td></td>
                    <td style="text-align:right;"><cf_get_lang dictionary_id='40302.Toplamlar'></td>
                    ...
                    <td style="text-align:right;">#tlformat(miktar_fark_,2)#</td>
                </tr>
             </cfoutput>
        </tfooter>
    </cf_grid_list>
