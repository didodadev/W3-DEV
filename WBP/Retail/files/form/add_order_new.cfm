<cfset new_basket = DeserializeJSON(URLDecode(attributes.basket, "utf-8"))>
<cfset satir_sayisi = form.ROWCOUNT>
<cfset department_count = listlen(attributes.department_list)>
...
<cfset attributes.order_code = new_number>
<cfloop from="1" to="#8-len(new_number)#" index="ccc">
    <cfset attributes.order_code = "0" & attributes.order_code>
</cfloop>
...
<cfif myQuery.recordcount>
    ...
    <cfif len(order_id_list)>
        ...
        <cfoutput>#order_id_list#</cfoutput>
    </cfif>
<cfelse>
	0
</cfif>
<cfcatch type="any">
	<cfsavecontent variable="aaa"><cfdump var="#cfcatch#"></cfsavecontent>
    <cffile action="write" file="#upload_folder#siparis_olusturma_hata.txt" nameconflict="overwrite" output="#aaa#">
</cfcatch>
</cftry>