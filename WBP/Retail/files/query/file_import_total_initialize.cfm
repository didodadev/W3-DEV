...
<cfquery name="get_stock" datasource="#dsn2#">
	...
	WHERE 
		...
		<cfif len(attributes.product_catid)>
			AND STOCK_ID IN(SELECT S.STOCK_ID FROM #dsn3_alias#.STOCKS S WHERE S.STOCK_CODE LIKE (SELECT PC.HIERARCHY FROM #dsn3_alias#.PRODUCT_CAT PC WHERE PC.PRODUCT_CATID = #attributes.product_catid#)+'.%')
		</cfif>	
