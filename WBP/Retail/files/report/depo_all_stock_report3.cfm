...
<cf_report_list_search title="#getLang('','Depo Stok Satış Raporu',61872)#">
    <cf_report_list_search_area>
        <cfform action="#request.self#?fuseaction=retail.depo_all_stock_report3" method="post" name="search_depo">
...
<cfif isdate(attributes.startdate)><cf_date tarih = "attributes.startdate"></cfif>
	<cfif isdate(attributes.finishdate)><cf_date tarih = "attributes.finishdate"></cfif>
<cfquery name="get_internaldemand" datasource="#DSN2#" result="query_r">
SELECT
	*
FROM
	(
	SELECT
        ISNULL((SELECT SUM(STOCK_IN - STOCK_OUT) FROM STOCKS_ROW SR WHERE SR.PROCESS_DATE <= #dateadd('d',1,attributes.finishdate)# AND S.STOCK_ID = SR.STOCK_ID AND SR.STORE IN (#attributes.search_department_id#)),0) AS TOPLAM_STOCK,
		<cfif listlen(attributes.search_department_id)>
    		<cfloop list="#attributes.search_department_id#" index="dd">
            	ISNULL((SELECT SUM(STOCK_IN - STOCK_OUT) FROM STOCKS_ROW SR WHERE SR.PROCESS_DATE < #dateadd('d',1,attributes.finishdate)# AND S.STOCK_ID = SR.STOCK_ID AND SR.STORE = #dd#),0) AS MAGAZA_STOCK_#dd#,
            </cfloop>
            <cfloop list="#attributes.search_department_id#" index="dd">
        		ISNULL((
                	SELECT
                	...
                	SELECT 
                    	PA.BIRIMIK  
                    FROM 
                    	#dsn_dev_alias#.PRODUCT_ACTIONS AS PA 
                    WHERE
                    	PA.STOCK_ID = S.STOCK_ID AND 
                        PA.DEPARTMENT_ID = #dd# AND 
                        PA.HAREKET_TIP_KOD IN (33,55,57) AND 
                        TARIH BETWEEN <cfqueryparam cfsqltype="cf_sql_timestamp" value="#attributes.startdate#"> AND <cfqueryparam cfsqltype="cf_sql_timestamp" value="#attributes.finishdate#">
                UNION ALL
     				SELECT 
                    	(ISNULL(SR.STOCK_OUT,0) - ISNULL(SR.STOCK_IN,0)) BIRIMIK
                    FROM
                        STOCKS_ROW AS SR
                    WHERE
                        ...
                        SR.STORE = #dd# AND 
                        SR.PROCESS_TYPE IN (#satis_karakterli_processler#) AND
                        SR.PROCESS_DATE >= #attributes.startdate# AND 
                        SR.PROCESS_DATE < #dateadd('d',1,attributes.finishdate)#
                	) T2
        ...
        <cf_grid_list>
            <thead>
                <tr>
                    ...
                    <th style="text-align:right;" colspan="2"><cf_get_lang dictionary_id='53263.Toplamlar'></th>
                </tr>
            </thead>
                    <thead>
                <tr>
                    ...
                    <th style="text-align:right;"><cf_get_lang dictionary_id='54587.Toplam Satış'></th>
                </tr>
            </thead>
            <tbody>
                <cfoutput>
