...
<cf_report_list_search title="#getLang('','Stok Kontrol Raporu',61873)#">
    <cf_report_list_search_area>
        <cfform action="#request.self#?fuseaction=retail.depo_stock_report" method="post" name="search_depo">
    ...
    <cfquery name="get_stocks_status_all" datasource="#dsn3#" >
        ...
        FROM
       	...
       	WHERE
            ...
            <cfif listlen(p_cat_list)>
                <cfif (isdefined("attributes.HIERARCHY1") and len(attributes.HIERARCHY1)) OR
					(isdefined("attributes.HIERARCHY2") and len(attributes.HIERARCHY2)) or
					(isdefined("attributes.HIERARCHY3") and len(attributes.HIERARCHY3))
				>
                AND
                PC.PRODUCT_CATID IN (#p_cat_list#)
                <cfelse>
                AND
                PC.PRODUCT_CATID IN (0)
                </cfif>
            </cfif>
    </cfquery>
            ...
            <cf_grid_list id="manage_table">   
                ...
                <tbody>
                    <cfoutput query="get_cat_status">
                        <tr>
                            <td>#currentrow#</td>
                            <td><a href="#request.self#?fuseaction=retail.depo_stock_report&event=reportCat&p_cat_list=#PRODUCT_CATID#&search_department_id=#attributes.search_department_id#" class="tableyazi" target="blank">#PRODUCT_CAT#</a></td>
