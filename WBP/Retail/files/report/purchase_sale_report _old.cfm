...
FROM
	...
	SELECT
        ...
        <cfoutput query="get_list_departments">
            ...
            ISNULL((SELECT SUM(SALES_COUNT_#DEPARTMENT_ID#) FROM #dsn_dev_alias#.DAILY_ACTIONS DAS WHERE DAS.STOCK_ID = S.STOCK_ID AND DAS.S_TARIH >= #attributes.startdate# AND DAS.S_TARIH <= #attributes.finishdate#),0) AS SATIS_#department_id#,
            <cfelse>
            0 AS SATIS_#department_id#,
            </cfif>
        	...
        	ISNULL((SELECT SUM(NETTOTAL) FROM #dsn_dev_alias#.GET_SHIP_ROWS GSR WHERE GSR.SHIP_DATE >= #attributes.startdate# AND GSR.SHIP_DATE <= #attributes.finishdate# AND GSR.STOCK_ID = S.STOCK_ID AND GSR.DEPARTMENT_IN = #department_id#),0) AS ALIM_TUTAR_#department_id#,
            ISNULL((SELECT SUM(PRODUCT_STOCK) FROM #DSN2_ALIAS#.GET_STOCK_PRODUCT WHERE S.PRODUCT_ID = GET_STOCK_PRODUCT.PRODUCT_ID AND GET_STOCK_PRODUCT.DEPARTMENT_ID = #department_id#),0) AS STOCK_#department_id#,
    ...
    <table class="big_list" id="manage_table">
    	<thead>
        	<tr>
                ...
                <cfoutput query="get_list_departments"><th style="text-align:center;color:black; display:none;" colspan="9" rel="depo_cols">#DEPARTMENT_HEAD#</th></cfoutput>
                <th style="text-align:center;color:black;" colspan="11"><a href="javascript://" onclick="get_depts();" class="form-title">Toplamlar</a></th>
            </tr>
        </thead>
            	<thead>
        	<tr>
        ...
        <cfoutput query="get_internaldemand">
        	<cfloop from="1" to="#get_list_departments.recordcount#" index="dept">
				...
				<cfset satis_ = evaluate("SATIS_#dept_#")>
				<cfset g_total_row_total_satis_tutar = g_total_row_total_satis_tutar + (satis_ * liste_fiyati_satis)>
                <cfset t_total_row_total_satis_tutar = t_total_row_total_satis_tutar + (satis_ * liste_fiyati_satis)>
            </cfloop>
        </cfoutput>
        	...
        	<cfoutput query="get_internaldemand">
                ...
                <tr>
                    ...
                    <cfloop from="1" to="#get_list_departments.recordcount#" index="dept">
                        ...
                        <cfset satis_ = evaluate("SATIS_#dept_#")>
                        <cfset satis_tutar_ = (satis_ * LISTE_FIYATI_SATIS)>
                    ...
                    <td style="text-align:right;">#tlformat(row_total_alim_tutar)#</td>
