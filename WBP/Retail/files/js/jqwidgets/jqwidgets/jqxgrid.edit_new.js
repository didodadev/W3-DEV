                            ...
                            if (c != null) {
                                if (w.selectcell) 
								{
                                    if (this.editcell == null) 
									{
										...
										{
											next_row_ = c.rowindex + 2;
											r_type = rows[next_row_].row_type;
											...
											else if(next_row_ < eleman_sayisi)
											{
												if (this.sortcolumn != c.datafield) 
												{
													var d = this.getrowvisibleindex(c.rowindex);
													var F = this.dataview.loadedrecords[d + 2];
													if (F) {
														if (!this.pageable || (this.pageable && d + 2 < (this.dataview.pagenum + 1) * this.pagesize)) 
														{
															this.clearselection(false);
															this.selectcell(this.getboundindex(F), c.datafield);
															var c = this.getselectedcell();
												...
												{
													if (s != null) {
														var E = this.dataview.loadedrecords[v + 2];
														if (E) {
															if (!this.pageable || (this.pageable && v + 2 < this.pagesize)) {
																this.clearselection(false);
																this.selectcell(this.getboundindex(E), c.datafield)
															} else {
																if (this.pageable && v + 2 >= this.pagesize) {
																	this.clearselection(false);
																	var E = this.dataview.loadedrecords[v];
																	this.selectcell(this.getboundindex(E), c.datafield)
																}
            ...
            switch (G.columntype) {
                case "numberinput":
                    if (this.host.jqxNumberInput) 
					{
						...
						d = N == undefined ? $("#numbereditor_special_" + D) : N;
						
						//d = $("#numbereditor_special_" + D);
						//N = undefined;
						d.show();
