...
<cfif isdefined("attributes.is_submitted")>
	<cfif isdefined("attributes.list_type") and len(attributes.list_type) and attributes.list_type eq 2>
		<cfinclude template="./get_order_detail_for_material_from_file.cfm">
	<cfelse>
...
<cf_box title="#getLang('prod',250)#"> 
    ...
    <div class="row">
        <div class="col col-12 form-inline ">
            ...
            <div class="form-group">
            	<cf_get_lang no='283.Tüm Gelen İhtiyaçlar'> <input type="checkbox" value="0" name="row_demand_all" id="row_demand_all" checked>
            </div>
            </cfif>
        	<div class="form-group">
				<cfinput type="text" name="demand_no" id="demand_no" value="#attributes.demand_no#" maxlength="250" style="width:180px;" placeholder="#getLang('assetcare',164)#">
            </div>
            <div class="form-group">
            	<cfinput type="text" name="production_order_no" id="production_order_no" value="#attributes.production_order_no#" maxlength="250" style="width:180px;" placeholder="#getLang('prod',33)#">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
            	<cfif isdefined("x_is_import_file") and x_is_import_file eq 1>
					<a href="javascript://" onclick="open_file();"><img src="images/file.gif" border="0" title="CSV İmport"></a>
	...
	<cf_box_search_detail>
		<cfset is_purchase = 1>
        <cfinclude template="/V16/production_plan/query/get_price_cat.cfm">
            <div class="row" type="row">
                <div class="col col-3 col-md-4 col-sm-6 col-xs-12" type="column" index="1" sort="true">
                    <div class="form-group" id="item-list_type">
                        <label class="col col-12"><cf_get_lang no ='278.Listeleme'></label>
                        <div class="col col-12">
                        <select name="list_type" id="list_type">
                            <option value="1" <cfif attributes.list_type eq 1>selected</cfif>><cf_get_lang no ='280.Üretim Emirlerinden'>(<cf_get_lang_main no='235.Spec'>)</option>
                            <option value="3" <cfif attributes.list_type eq 3>selected</cfif>><cf_get_lang no ='280.Üretim Emirlerinden'>(<cf_get_lang no ='52.Ürün Ağacı'>)</option>
                            <option value="4" <cfif attributes.list_type eq 4>selected</cfif>><cf_get_lang no ='280.Üretim Emirlerinden'>(<cf_get_lang no ='164.Alternatifli'>)</option>
                            <cfif isdefined("x_is_import_file") and x_is_import_file eq 1>
                                <option value="2" <cfif attributes.list_type eq 2>selected</cfif>><cf_get_lang no ='281.Dosyadan'></option>
                            </cfif>
                        </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group" id="item-price_cat">
                        <label class="col col-12"><cf_get_lang_main no='1310.Standart Alış'></label>
                        <div class="col col-12">
                            <select name="price_cat" id="price_cat" style="width:130px;">
                                 <option value="-1" <cfif (-1 is attributes.price_cat)> selected</cfif>><cfoutput>#getLang('main',322)#</cfoutput></option>
                    ...
                    <div class="form-group" id="item-sort_type">
                        <label class="col col-12"><cfoutput>#getLang('main',570)#</cfoutput></label>
                        <div class="col col-12">
                            <select name="sort_type" id="sort_type" <cfif attributes.sort_type eq 1></cfif>style="width:130px;">
                                ...
                                <option value="1" <cfif attributes.sort_type eq 1>selected</cfif>><cf_get_lang no='295.Üretilen'></option>
                                <option value="0" <cfif attributes.sort_type eq 0>selected</cfif>><cf_get_lang no='285.Üretilmeyen'></option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group" id="item-station_id">
                        <label class="col col-12"><cf_get_lang_main no='1422.İstasyon'></label>
                        <div class="col col-12">
                ...
                <div class="col col-3 col-md-4 col-sm-6 col-xs-12" type="column" index="3" sort="true">
                    <div class="form-group" id="item-department_id">
                        <label class="col col-12"><cf_get_lang_main no='1351.Depo'></label>
                        <div class="col col-12">
...
<cfif isDefined("attributes.req_id") and len(attributes.req_id)>
    ...
    <cf_box id="sample_request" closable="0" unload_body="0">
...
<table class="detail_basket_list" width="100%">
	<thead>
			...
			<tr>
				<th colspan="15">
					<select id="matgroup">
						...
						<option value="0">Kumaş</option>
						<option value="1">Aksesuar</option>
						<option value="2">İşçilik</option>
					</select>
					<script type="text/javascript">
						$("#matgroup").change(function() {
							$('[data-reqtype]').show();
							$('#all_conv_product').attr('checked', false);
							$('._conversion_product').attr('checked', false);
							if ($(this).val() != "") {
								$('tr[data-reqtype]').not('[data-reqtype="'+$(this).val()+'"]').hide();
							}
						});
					</script>
				</th>
		...
		<tr>
			...
			<th width="1%"><cf_get_lang_main no ='1165.Sıra'></th>
			<th><cf_get_lang_main no ='106.Stok Kodu'></th>
			<th><cf_get_lang_main no ='245.Ürün'></th>
            <th>BOY</th>
            <th>BEDEN</th>
			<th width="50" style="text-align:right;"><cf_get_lang dictionary_id='36437'></th>
			<cfset var_class = 'txtbold'>
			<cfif ListLen(attributes.DEPARTMENT_ID,',')>
				<cfloop list="#attributes.DEPARTMENT_ID#" index="_depID_" delimiters=",">
					<cfif var_class is 'form-title'><cfset var_class = 'txtbold'><cfelse><cfset var_class = 'form-title'></cfif> 
					<cfoutput>
						<th class="#var_class#" style="text-align:right;" title="<cf_get_lang dictionary_id='58120'>" ><cf_get_lang dictionary_id="54858.G"></th>
						<th class="#var_class#" style="text-align:right;" title="<cf_get_lang dictionary_id='36600'> (Kendi Dışındaki Üretim Emirleri)" ><cf_get_lang dictionary_id="29684.A"></th>
						<cfif x_is_alternative_stock eq 1>
						...
						<th class="#var_class#" style="text-align:right;" title="<cf_get_lang dictionary_id='36601'>"><cf_get_lang dictionary_id="42499.M"></th>
					</cfoutput>
				</cfloop>
			<cfelse>
				<cfif var_class is 'form-title'><cfset var_class = 'txtbold'><cfelse><cfset var_class = 'form-title'></cfif> 
				<cfoutput>
					<th class="#var_class#" style="text-align:right;" title="<cf_get_lang dictionary_id='58120'>" ><cf_get_lang dictionary_id="34287.G"></th>
					<th class="#var_class#" style="text-align:right;" title="<cf_get_lang dictionary_id='36600'>  (Kendi Dışındaki Üretim Emirleri)" ><cf_get_lang dictionary_id="29684.A"></th>
					<cfif x_is_alternative_stock eq 1>
						<th class="#var_class#" style="text-align:right;" title="Alternatif Ürünlerin Satılabilir Stok Toplamı"><cf_get_lang dictionary_id="54866.A.S."></th>
					</cfif>
					<th class="#var_class#" style="text-align:right;" title="<cf_get_lang dictionary_id='36601'>"><cf_get_lang dictionary_id="34285.M"></th>
				</cfoutput>
			</cfif>
			<cfif isdefined("x_is_price") and x_is_price eq 1><th style="text-align:right;"><cf_get_lang_main no='672.Fiyat'></th></cfif>
			<th title="<cf_get_lang dictionary_id='36603'>" style="text-align:right;"><cf_get_lang dictionary_id='36602'></th>
			<th title="Üretim Emirleri Beklenen" style="text-align:right;"><cf_get_lang dictionary_id="34284.Ü.E."> <cf_get_lang_main no="707.Beklenen"></th>
			<th title="<cf_get_lang dictionary_id='36607'>" style="text-align:right;"><cf_get_lang_main no ='1032.Kalan'></th>
			<th title="<cf_get_lang_main no ='224.Birim'>"><cf_get_lang_main no ='224.Birim'></th>
			<th width="1%" style="text-align:right;"><cf_get_lang dictionary_id='57635'></th>
			<cfif isdefined("x_is_price") and x_is_price eq 1>
	...
	<cfif isdefined("attributes.is_submitted")>
        ...
        <cfif get_metarials.recordcount>
			...
			<cfif isDefined("attributes.req_id") and len(attributes.req_id)>
				<cfquery name="query_supplier" datasource="#dsn3#">
					SELECT * FROM TEXTILE_SR_SUPLIERS
					WHERE REQ_ID = #attributes.req_id#
				</cfquery>
			</cfif>
			<cfoutput query="get_metarials" startrow="#attributes.startrow#" maxrows="#attributes.maxrows#">
				<cfif isDefined("attributes.req_id") and len(attributes.req_id)>
					<cfquery name="qoq_supplier" dbtype="query">
						SELECT * FROM query_supplier
						WHERE STOCK_ID = #get_metarials.STOCK_ID#
					</cfquery>
					<cfif qoq_supplier.recordcount>
						<cfset reqtype = qoq_supplier.REQUEST_TYPE>
					<cfelse>
						<cfset reqtype = 2>
					</cfif>
				<cfelse>
                ...
                <tr id="row-#currentrow#" data-reqtype="#reqtype#">
					<cfif isdefined("attributes.list_type") and attributes.list_type eq 4>
						<!-- sil -->
					...
					<cfelse>
						...
						<cfelse>
							...
							<input type="hidden" name="row_list_price_#stock_id#" id="row_list_price_#stock_id#" value="#tlformat(list_price)#">
							<input type="hidden" name="row_money_type_#stock_id#" id="row_money_type_#stock_id#" value="#money_type#">
                        ...
                        <!-- sil -->
							<cfif not (isdefined("attributes.list_type") and attributes.list_type eq 4) and (isdefined("attributes.is_submitted") eq 1)>
                                <td width="1%" align="center">
                                    <input type="checkbox" name="conversion_product_#stock_id#" value="#stock_id#" id="_conversion_product_#currentrow#" class="_conversion_product"><!--- onClick="aktif_yap();" --->
                                </td>
                            </cfif>
						<!-- sil -->
					</cfif>
		...
		<tfoot>
			<tr>
				<td colspan="<cfoutput>#colspan_#</cfoutput>" align="right" style="text-align:right;">
					...
					<input type="button" value="<cf_get_lang dictionary_id='36897'>" name="sevk_irsaliyesi" id="sevk_irsaliyesi" onClick="kota_kontrol(1);" style="width:140px;">
					<input type="button" value="<cf_get_lang dictionary_id='36898'>" name="ambar_fisi" id="ambar_fisi" onClick="kota_kontrol(2);" style="width:140px;">
					...
					<input type="button" value="<cf_get_lang dictionary_id="38539">" name="tam_satin_alma_talebi" id="tam_satin_alma_talebi" onClick="kota_kontrol(4);" style="width:140px;">
				</td>
			</tr>
		</tfoot>
		</cfif>
        <cfelse>
            <tr>
                <td colspan="<cfoutput>#colspan_#</cfoutput>" class="color-row" height="20">
                    <cfif isdefined("attributes.is_submitted")><cf_get_lang_main no ='72.Kayıt Yok'><cfelse><cf_get_lang_main no ='289.Filtre Ediniz'></cfif>!
                </td>
            </tr>
        </cfif>
	<cfelse>
...
<cfif attributes.page eq 1 and ((isdefined("stock_id_list_info1") and len(stock_id_list_info1)) or (isdefined("stock_id_list_info2") and len(stock_id_list_info2)))>
	<br/><table width="99%" border="0" cellspacing="1" cellpadding="2" align="center">
		<tr>
			<td class="headbold">
				<a href="javascript:connectAjax();">&raquo;<cf_get_lang no ='301.Bulunamayan Kayıtlar'></a>
			</td>
		</tr>
		<tr>
			<td>
...
<form name="aktar_form" method="post">
	...
	<input type="hidden" name="convert_list_price" id="convert_list_price" value="">
	<input type="hidden" name="convert_money_type" id="convert_money_type" value="">
	<input type="hidden" name="project_id" id="project_id" value="<cfoutput>#attributes.projectid#</cfoutput>">
</form>
<script type="text/javascript">
	function kontrol()
	{
		...
		{
			alertObject({message: "<cfoutput><cf_get_lang_main no ='243.Başlama Tarihi'></cfoutput>"})    
			return false;
		}
		if(!$("#finish_date").val().length)
		{
			alertObject({message: "<cfoutput><cf_get_lang_main no ='288.Bitiş Tarihi'></cfoutput>"})    
			return false;
		}
		...
		*/
	{
		 ...
		 var convert_list_list_price = "";
		 var convert_list_money_type = "";
		 //
		 <cfif isdefined("attributes.is_submitted")>
			 <cfoutput query="get_metarials" startrow="#attributes.startrow#" maxrows="#attributes.maxrows#">
				 if(document.all.conversion_product_#stock_id#.checked && filterNum(document.getElementById('row_total_need_#stock_id#').value) > 0)
				 {
					...
					convert_list_list_price += list_getat(document.getElementById('row_list_price_#stock_id#').value.replace(',','.'),1,',')+',';
					convert_list_money_type += list_getat(document.getElementById('row_money_type_#stock_id#').value,1,',')+',';
				 }
			 </cfoutput>
		</cfif>
		...
		document.getElementById('convert_list_price').value=convert_list_list_price;
		document.getElementById('convert_money_type').value=convert_list_money_type;
		if(convert_list)//Ürün Seçili ise
		{
			 ...
			 if (type==4) {
				aktar_form.action="<cfoutput>#request.self#</cfoutput>?fuseaction=purchase.list_purchasedemand&event=add&type=convert&ref_no="+encodeURIComponent(document.getElementById('production_order_no').value);
				aktar_form.target='_blank';
			 }
			 aktar_form.submit();
		 }
...
function wrk_select_all2(all_conv_product,_conversion_product_,number,strttw)
{
	...
	if ($("#matgroup").length > 0) {
		matgroupval = $("#matgroup").val();
	}
	for(var cl_ind=strttw; cl_ind <= number; cl_ind++)
	{
		if(document.getElementById(all_conv_product).checked == true)
		{
			if(document.getElementById('_conversion_product_'+cl_ind) != undefined && document.getElementById('_conversion_product_'+cl_ind).checked == false && ( matgroupval == "" || $('#_conversion_product_'+cl_ind).closest("tr").attr("data-reqtype") == matgroupval ) )
