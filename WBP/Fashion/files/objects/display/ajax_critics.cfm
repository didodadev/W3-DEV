...
select
	*from
	...
	WHERE
		...
		CONTENT_RELATION.ACTION_TYPE_ID=<cfif isdefined("attributes.company_id") and len(attributes.company_id)><cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.company_id#"><cfelse>null</cfif>
	...
	<cfif isDefined('attributes.period_id') and len(attributes.period_id)>
		...
		as T 
			ORDER BY T.TYPE ASC, T.RECORD_CONS , record_date desc
</cfquery>



<!---<cfdump var="#GET_NOTE#" abort>---->
        ...
        <cfoutput query="get_note">
            <tr id="notes_#currentrow#">
            	<td>
            		...
            		<cfif type eq 1>
						<cfif isdefined('is_open_det') and attributes.is_open_det eq 1><a href="javascript://" onClick="openBoxDraggable('#request.self#?fuseaction=textile.popup_form_upd_critic&note_id=#note_id#&is_delete=#attributes.is_delete#','','ui-draggable-box-medium')" class="tableyazi"><font><cfif len(note_head) gt 25>#left(note_head, 25)#...<cfelse>#note_head#</cfif></font></a><cfelse><cfif len(note_head) gt 25>#left(note_head, 25)#...<cfelse>#note_head#</cfif></cfif>
					<cfelse>
