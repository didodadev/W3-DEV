<cfscript>
    ...
    if ( mh_id eq "0" )
    {
        mh_id = measure.insert_measure_header( req_id, form["ERATE"], form["BRATE"], form["PROCESS_STAGE"], form["STRETCHING_GROUP"] );
    }
    else 
    {
        measure.update_measure_header( mh_id, form["ERATE"], form["BRATE"], form["STRETCHING_GROUP"] );    
    }
...
location.href = '<cfoutput>#request.self#?fuseaction=#attributes.fuseaction#&event=upd&req_id=#attributes.req_id#&pid=#attributes.pid#&mh_id=#mh_id#</cfoutput>';
</script>