...
<cfsavecontent variable="title">
	Kumaş Fİyat Talep No
</cfsavecontent>
<cfset referel_page="fabric_price">
<cf_catalystHeader>
<div class="row">
...
<cfif len(attributes.project_emp_id)>
    <div class="col col-12">
    <cfsavecontent variable="message1">Kumaş Fiyat Talepleri</cfsavecontent>
                    <cf_box id="list_supplier_k"
                        box_page="#request.self#?fuseaction=textile.emptypopup_ajax_req_supplier&tableid=list_supplier_k&req_id=#attributes.req_id#&req_type=0&default_station=3&price_plan=1&plan_id=#attributes.plan_id#"
                        title="#message1#"
                        closable="0">
                    </cf_box>
    </div>
...
<!--- Necip Menekşe--->
			<script type = "text/javascript">
				...
				if ($('#process_stage').val()=='1311'){
				$('#table1 input[type="file"]').hide();
				}
				else
				{
				$('#table1 input[type="file"]').show();
				}
				...
				console.log($('#process_stage').val());
				if ($('#process_stage').val()=='1311'){
				setTimeout(()=>
				$('#table1 input[type="file"]').hide(),2000);
				}
