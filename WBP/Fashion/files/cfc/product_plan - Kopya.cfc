<cfcomponent>
    ...
    <cffunction name="list_productplan" access="public" returntype="query">
        ...
        <cfquery name="query_list_productplan" datasource="#dsn3#">
			...
			FROM 
				...
				LEFT OUTER JOIN #dsn#.SETUP_PRIORITY SPR ON SR.REQ_TYPE_ID = SPR.PRIORITY_ID
            WHERE
				...
				<cfif len(arguments.date_start)>AND PLAN_DATE >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#arguments.start_date#"></cfif>
				<cfif len(arguments.date_end)>AND PLAN_DATE <= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#arguments.date_end#"></cfif>
				...
				<cfif isDefined("arguments.access") and arguments.access eq 1> 
							AND SR.REQ_ID IN(
								...
								where
									PCP.PRODUCT_CAT_ID=TS.PRODUCT_CATID 
									<!--- AND PCP.POSITION_CODE=#session.ep.position_code# --->
							)
				<cfelseif isDefined("arguments.access") and arguments.access eq 2> 
							AND SR.REQ_ID IN(
								<!----
								SELECT TS.REQUEST_ID 
								FROM 
								...
								SELECT TS.REQUEST_ID 
								FROM 
									#dsn#_product.PRODUCT P,
									#dsn3#.TEXTILE_SR_PROCESS TS,
									#dsn#_product.PRODUCT_CAT_POSITIONS PCP
								where
									P.PRODUCT_CATID=TS.PRODUCT_CATID AND PCP.PRODUCT_CAT_ID=P.PRODUCT_CATID
									AND 	PCP.POSITION_CODE=#session.ep.position_code#
    ...
    <cffunction name="add_productplan" access="public" returntype="any">
		...
		<cfargument name="is_revision" type="any" default="">
		<cfargument name="task_emp" type="any" default="">
		<cfargument name="is_plain_id" default="0">
		<cfargument name="is_copy" default="0">
		<cfargument name="operation_relations" default="0">
        ...
        <cftransaction>
			<cfquery name="query_add_productplan" datasource="#dsn3#" result="add_productplan">
				...
				VALUES
					...
					,<cfif arguments.task_emp eq "">NULL<cfelse>#arguments.task_emp#</cfif>
					)
			</cfquery>
			<cfif ListFind('2,3',arguments.plan_type_id)><!---kumas veya aksesuar talep--->	   
				<cfquery name="update_suplier" datasource="#dsn3#">
						...
						WHERE 
							...
							<cfif arguments.is_plain_id eq "0">
							AND PLAN_ID IS NULL
							</cfif>
			...
			<cfelseif listFind('5,7', arguments.plan_type_id)><!---iscilik fiyat talebi--->
				<cfquery name="update_suplier" datasource="#dsn3#">
					...
					WHERE 
						...
						<cfif len(arguments.operation_relations) and arguments.operation_relations neq "0">
							AND OPERATION_ID IN (#arguments.operation_relations#)
						</cfif>
						<cfif arguments.is_revision eq 1>
							AND IS_REVISION=1
						</cfif>
    ...
    <cffunction name="update_productplan" access="public" returntype="any">
		...
		<cfargument name="discount_rate" type="any">
		<cfargument name="total_price" type="any">
        <cflock name="#createUUID()#" timeout="20">
        <cftransaction>
        ...
        <cfquery name="query_update_productplan" datasource="#dsn3#" result="update_productplan">
			...
			<cfif isDefined("arguments.total_price") and len(arguments.total_price)>
				,TOTAL_PRICE = <cfqueryparam cfsqltype='CF_SQL_FLOAT' value='#arguments.total_price#'>
			</cfif>
			<cfif isDefined("arguments.discount_rate") and len(arguments.discount_rate)>
				,DISCOUNT_RATE = <cfqueryparam cfsqltype='CF_SQL_FLOAT' value='#arguments.discount_rate#'>
			</cfif>
	...
	<cffunction name="update_productplan_prices" access="public">
		<cfargument name="plan_id">
		<cfargument name="discount_rate">
		<cfargument name="total_price">

		<cfquery name="query_productplan_prices" datasource="#dsn3#">
			UPDATE TEXTILE_PRODUCT_PLAN
            SET 
				DISCOUNT_RATE = <cfqueryparam cfsqltype='CF_SQL_FLOAT' value='#arguments.discount_rate#'>,
				TOTAL_PRICE = <cfqueryparam cfsqltype='CF_SQL_FLOAT' value='#arguments.total_price#'>
			WHERE PLAN_ID = <cfqueryparam cfsqltype='CF_SQL_INTEGER' value='#arguments.plan_id#'> 
		</cfquery>

		
		<cfquery name="query_product_process_prices" datasource="#dsn3#">
			UPDATE TEXTILE_SR_PROCESS
			SET
				PRICE = <cfqueryparam cfsqltype='CF_SQL_FLOAT' value='#arguments.total_price#'>
			WHERE PLAN_ID = <cfqueryparam cfsqltype='CF_SQL_INTEGER' value='#arguments.plan_id#'>
				AND OPERATION_ID = 3
		</cfquery>
	...
	<!--- process type içerisinde Opertaion type alanı textile_sr_process deki operation ilişkisini tutmak içindir --->
	...
	<cffunction name="getSrProcess" access="public" returntype="any">
		<cfargument name="req_id">
		<cfargument name="opertaion_id">
		<cfquery name="query_sr_process" datasource="#dsn3#">
			SELECT * FROM TEXTILE_SR_PROCESS 
			WHERE REQUEST_ID = <cfqueryparam cfsqltype='CF_SQL_INTEGER' value='#arguments.req_id#'>
				AND OPERATION_ID IN (#arguments.opertaion_id#)
		</cfquery>
		<cfreturn query_sr_process>
	</cffunction>
