...
<div class="col col-12 col-md-12 col-sm-12 col-xs-12">
    <cf_box title="Process Templates">
        <cfform name="add_process_templates" method="post" action="#request.self#?fuseaction=dev.emptypopup_add_process_templates">
            <cf_box_elements>
                <div class="col col-4 col-md-4 col-sm-6 col-xs-12" index="1" type="column" sort="true">
                    <div class="form-group" id="item-process-template">
                        <label class="col col-4 col-md-4 col-sm-4 col-xs-12">Name *</label>
                        <div class="col col-8 col-md-8 col-sm-8 col-xs-12">
                            <input type="text" id="process_template_name" name="process_template_name" value="" required>
                        </div>
                    </div>
...
<script>
    ...
    {   $('#wo').show();	
        ...
        if(document.getElementById("related_wo").value=="")
            AjaxPageLoad('<cfoutput>#request.self#?fuseaction=process.popup_dsp_faction_list&draggable=1&field_name=add_process_templates.related_wo&is_upd=0</cfoutput>','wo');
        else
            AjaxPageLoad('<cfoutput>#request.self#?fuseaction=process.popup_dsp_faction_list&draggable=1&field_name=add_process_templates.related_wo&is_upd=1</cfoutput>','wo');
        return false;
    }
</script>
