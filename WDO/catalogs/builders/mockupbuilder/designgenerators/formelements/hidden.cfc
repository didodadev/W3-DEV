<cfcomponent extends="WDO.catalogs.builders.mockupbuilder.mockup">
    ...
    <cffunction name="generate" access="public" returntype="string">
        ...
        <cfscript>
            ...
            //template
            template = '<input type="hidden" name="%s" id="%s" value="%s">';
            ...
            result = iif(len(precodeList), DE(precodeList & crlf() & ident()), DE("")) & super.stringFormat(template, [element.label & this.index, stck.name & "_" & element.label & this.index, valueformat]);
        </cfscript>
        <cfreturn result>
    </cffunction>
