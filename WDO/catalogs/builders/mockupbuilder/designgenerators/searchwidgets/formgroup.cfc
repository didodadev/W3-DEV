<cfcomponent extends="WDO.catalogs.builders.mockupbuilder.designgenerators.formgroupwidget">
    ...
    <cffunction name="generatesearchelements" access="public" returntype="string">
        ...
        <cfscript>
            ...
            else 
            {
                ...
                template = template & crlf() & ident() & '<div style="margin: 7px 5px;" id="item-%s">' & crlf() & ident( this.identcount + 1 ) & '%s' & crlf() & ident( this.identcount - 1 ) & '</div>';
                ...
                inputwidget = createObject("component", "WDO.catalogs.builders.mockupbuilder.designgenerators.inputfactory").create( type:'hidden input', data:this.data, domain:this.domain, ident:this.identcount + 1, group:this.elementgroup, eventtype:this.eventtype );
