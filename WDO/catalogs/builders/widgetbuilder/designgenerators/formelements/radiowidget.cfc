    ...
    <cffunction name="radios" access="public" returntype="string">
        ...
        <cfscript>
            ...
            else
            {
                ...
                return "<cfoutput><c" & 'floop condition="##' & iteratorname & '.hasiter()##">' & crlf() & ident(this.identcount + 1) & '<div><input type="radio" style="float: none; display: inline;" name="' & arguments.element.label & this.index & '" ##''value="''&' & iteratorname & '.get(' & arguments.element.listValueIndex & ')&''"''## ##iif(' & arguments.element.label & '_defaultValue eq ' & iteratorname & '.get(' & arguments.element.listValueIndex & '), DE(''checked="checked"''), DE(""))## %s> ##' & iteratorname & '.get(' & arguments.element.listDisplayIndex & ')##' & crlf() & ident() & '</div><cfset ' & iteratorname & '.next()>' & crlf() & ident() & "</cfloop></cfoutput>";
            }
        </cfscript>
    </cffunction>
