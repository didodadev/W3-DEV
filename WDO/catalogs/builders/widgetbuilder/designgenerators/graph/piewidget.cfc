<cfcomponent extends="WDO.catalogs.builders.widgetbuilder.widget">
    ...
    <cffunction name="generate_query" returntype="string">
        <cfargument name="queryparams">
        <cfscript>
            masterquery = crlf() & ident() & '<c' & 'fset piequery_master_' & this.uniqueid & ' = ' & this.data.listOfSummaries[1].struct & "_query( [%s] )>";
            ...
            queryformat = crlf() & ident() & '<c' & 'fquery name="piequery_' & this.uniqueid & '" dbtype="query">' & crlf() & ident(this.identcount + 1) & 'SELECT %s FROM %s GROUP BY %s' & crlf() & ident() & '</cfquery>';
            ...
            return masterquery & stringFormat( queryformat, [ groups & "," & summaries, "piequery_master_" & this.uniqueid, groups ] );
        </cfscript>
    </cffunction>
    ...
    <cffunction name="getlabels" returntype="string">
        <cfscript>
            ...
            result = result & crlf() & ident( this.identcount + 2 ) & 'for( i=1; i<=piequery_' & this.uniqueid & '.recordCount; i++ ) {';
            result = result & crlf() & ident( this.identcount + 3 ) & 'arrayAppend(axislabels, ##piequery_' & this.uniqueid & '["' & this.data.listOfAxis[1].label & '"][i]##);';
    ...
    <cffunction name="getdatasets" returntype="string">
        <cfscript>
            ...
            axiscolors = arrayMap( this.data.listOfSummaries, function( elm ) {
                ...
                axiscolorformat = axiscolorformat & crlf() & ident( this.identcount + 4 ) & 'for( i=1; i<=piequery_' & this.uniqueid & '.recordCount; i++ ) {';
            ...
            axisdatas = arrayMap( this.data.listOfSummaries, function( elm ) {
                ...
                axisdataformat = axisdataformat & crlf() & ident( this.identcount + 4 ) & 'for( i=1; i<=piequery_' & this.uniqueid & '.recordCount; i++ ) {';
                axisdataformat = axisdataformat & crlf() & ident( this.identcount + 5 ) & 'arrayAppend(axisdatas, ##piequery_' & this.uniqueid & '["' & elm.label & '"][i]##);';
