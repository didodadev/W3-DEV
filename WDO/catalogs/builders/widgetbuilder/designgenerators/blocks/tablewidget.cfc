        ...
        <cfscript>
            ...
            for (element in this.data.listOfElements) {
                ...
                if (getRowType(element) eq "Add Row" && this.data.editMode neq "none")
                {
                    ...
                    {
                        eventattrbuilder = createObject("component", "WDO.catalogs.builders.widgetbuilder.designgenerators.events.fieldeditorevent");
                        eventattr = eventattrbuilder.getAttributes(element, this.domain, this.data.editMode);
                    }
                    
                    inputwidget = createObject("component", "WDO.catalogs.builders.widgetbuilder.designgenerators.inputfactory").create(modelElement.fieldType, element, this.domain, this.eventtype, this.identcount + 4, eventattr, '##' & tableStruct.name & '_index##', "formelements", 0);
            ...
            {
                summaryWidget = createObject("component", "WDO.catalogs.builders.widgetbuilder.designgenerators.blocks.summarywidget").init(tableStruct.listOfSummaries, this.domain, this.identcount);
    ...
    <cffunction name="template" access="public" returntype="string">
        <cfscript>
            ...
            for (element in this.data.listOfElements) {
                ...
                inputwidget = createObject("component", "WDO.catalogs.builders.widgetbuilder.designgenerators.inputfactory").create(modelElement.fieldType, element, this.domain, this.eventtype, 0, eventattr, "{index}");
