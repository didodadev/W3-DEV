...
<cfif len( attributes.formsubmited )>
    ...
    <cfset query_bp = list_bp(attributes.keyword)>
    <cfset attributes.totalrecords = query_bp.recordcount>
</cfif>
<cfset attributes.startrow=((attributes.page-1)*attributes.maxrow)+1>
<cf_box id="best_practice_search_area">
    <cfform name="listBP" id="listBP" action="#request.self#?fuseaction=dev.bestpractice">
        ...
        <cf_box_search more="0">
            ...
            <div class="form-group">
                <cfoutput>
                    <a href="#request.self#?fuseaction=dev.bestpractice&event=add" class="ui-btn ui-btn-gray"><i class="fa fa-plus"></i></a>
                </cfoutput>
            </div>
        </cf_box_search>
    </cfform>
</cf_box>
<cf_box title="Best Practice" id="bp_list" uidrop="1" hide_table_column="1">
    <cf_ajax_list>
        <thead>
            <tr>
                ...
                <th width="20"><a href="<cfoutput>#request.self#?fuseaction=dev.bestpractice&event=add&type=bp&mode=add</cfoutput>"><i class="fa fa-plus"></i></a></th>
            </tr>
        </thead>
        <tbody>
            <cfif len( attributes.formsubmited ) and isDefined( "query_bp" ) and query_bp.recordcount gt 0>
            <cfoutput query="query_bp"  startrow="#attributes.startrow#" maxrows="#attributes.maxrow#">
            <tr>
                <td>#currentrow#</td>
                <td>
                    <a href="#request.self#?fuseaction=dev.bestpractice&event=upd&mode=upd&id=#BESTPRACTICE_ID#">#BESTPRACTICE_NAME#</a>
                </td>
                <td>#BESTPRACTICE_AUTHOR#</td>
                <td>#BESTPRACTICE_PRODUCT_CODE#</td>
                <td><a href="#request.self#?fuseaction=dev.bestpractice&event=upd&mode=upd&id=#BESTPRACTICE_ID#"><i class="fa fa-pencil"></i></a></td>
            </tr>
            </cfoutput>
            <cfelse>
    ...
    <cfif attributes.totalrecords gt attributes.maxrow>
        <cfset adres="dev.bestpractice&formsubmited=1">
