...
<cfif isDefined("attributes.oldFile") and len(attributes.oldFile)>
    <cfset fileName = listLast("#attributes.oldFile#","/")>
</cfif>
...
<cfif isdefined('attributes.image') and len(attributes.image)>
    ...
    <cfelse>
        <cftry>
            ...
            <cffile action = "delete" file = "#upload_folder##attributes.oldFile#">
...
<cfset response = wex.update(
        wex_id          :   '#attributes.wxid#',
...
<cfif response.status>
    <script type="text/javascript">
        <cfoutput>
            window.location.href = '#request.self#?fuseaction=dev.wex&event=upd&wxid=#attributes.wxid#';
        </cfoutput>
    </script>
<cfelse>
    <script>
