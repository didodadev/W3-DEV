...
<cfif isdefined("attributes.is_del") and attributes.is_del eq 1>
    <cfset comp = getComponent.del_output_template(
        output_template_id : '#attributes.output_template_id#'
    )>
    <script>
        location.href="<cfoutput>#request.self#?fuseaction=dev.output_templates</cfoutput>";
    </script>
<cfelse>
    <cfset comp = getComponent.upd_output_template(
    output_template_id : '#attributes.output_template_id#',
