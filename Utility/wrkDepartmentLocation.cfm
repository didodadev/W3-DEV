...
<cfparam name="attributes.title" default="#caller.lang_array_main.item[1112]#"><!--- Divin üzerinde gelen başlık.. ---><!---Departman  - Lokasyon--->
...
<cfscript>
	defaultColumnList="LOCATION_NAME@#caller.lang_array_main.item[2234]#";
	columnList='';
...
<input type="text" name="#attributes.fieldName#" <cfif len(attributes.data_msg)>data-msg="#attributes.data_msg#" required="yes"</cfif> id="#attributes.fieldName#"  autocomplete="off" style="width:#attributes.width#px" onblur="compenentInputValueEmptyinglocation_#attributes.line_info#(this);" value="#attributes.location_name#" onkeypress="if(event.keyCode==13) {compenentAutoCompletelocation_#attributes.line_info#(this,'#evaluate("comp_div_id_#attributes.line_info#")#','#compenent_url#'); return false;}"> 
...
<div id="#evaluate("comp_div_id_#attributes.line_info#")#" style="z-index:1;position:absolute;display:none;width:#attributes.boxwidth#px;height:#attributes.boxheight#px;margin-left:0px;"></div>
	...
	function compenentAutoCompletelocation_#attributes.line_info#(object_,div_id,comp_url){
		other_parameters = 'is_delivery*#attributes.is_delivery#/location_type*#attributes.location_type#/sistem_company_id*#attributes.sistem_company_id#/is_ingroup*#attributes.is_ingroup#/user_level_control*#attributes.user_level_control#/is_store_module*#attributes.is_store_module#/status*#attributes.status#';//alt+789=§ ve alt+987=█
		 ...
		 if(keyword_.length < 3 && object_ != ""){
			alert('#caller.lang_array_main.item [2152]#');
			return false;}
		 else{
			document.getElementById(div_id).style.display='';
			comp_url = comp_url+'&keyword='+keyword_+'';
			AjaxPageLoad('#request.self#?fuseaction=objects.popup_wrk_list_comp&comp_div_id=#evaluate("comp_div_id_#attributes.line_info#")#&other_parameters='+other_parameters+''+comp_url+'',div_id,1);
			return false;	
		 }
		 return false;	
	}
