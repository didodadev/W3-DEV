...
<div class="row">
    ...
    <div class="row formContent">
					...
					<tr id="frm_row#currentrow#" <cfif TREE_TYPE is 'P'>class="phantom" title="Phantom Ağaçtan Ürünler"<cfelseif TREE_TYPE is 'O'>class="operasyon" title="Operasyondan Gelen Ürünler"</cfif>>
						...
						<cfif x_is_fire_product eq 1>
							<td>
								...
								<input type="text" class="moneybox" name="fire_amount_#currentrow#" id="fire_amount_#currentrow#" value="#TlFormat(wrk_round(get_row_enter.fire_amount,round_number,1),round_number)#" style="width:60px;" onkeyup="return(FormatCurrency(this,event,8));" onblur="hesapla_deger(#currentrow#,3);aktar('#p_order_id#','#currentrow#');">
							</td>
						</cfif>
						<td>
		...
		<table id="table3" name="table3" class="basket_list" style="margin-bottom:50px; display:none; width:100%;">
            <cfif x_pro_add_barkod_and_seri_no eq 1>
				<iframe name="add_prod" id="add_prod" frameborder="0" vspace="0" hspace="0" scrolling="no" src="<cfoutput>#request.self#?fuseaction=prod.popup_add_production_result&iframe=1</cfoutput>&type=outage" width="100%" height="25"></iframe>
			</cfif>
			<thead>
				<tr>
	...
	<cfquery name="get_company_id" datasource="#dsn3#">
		SELECT COMPANY_ID FROM workcube_akarteks_1.TEXTILE_SAMPLE_REQUEST WHERE PROJECT_ID=#GET_DETAIL.PROJECT_ID#
	</cfquery>
