...
<cfquery name="GET_ROW_RESULT" datasource="#DSN3#">
	SELECT
		...
		PRODUCTION_ORDER_RESULTS.IS_STOCK_FIS,
			PRODUCTION_ORDER_RESULTS.PARTY_ID,
				PRODUCTION_ORDER_RESULTS_ROW.P_ORDER_ID ROW_P_ORDER_ID
	FROM
...
<cfif get_row_result.is_stock_fis eq 1><!--- fis kesildi ise geri geldiginde tekrar kayıt yapmasın diye --->
    <cflocation url="#request.self#?fuseaction=prod.upd_prod_order_result_tex&party_id=#attributes.party_id#&pr_order_id=#attributes.pr_order_id#" addtoken="no">
	<cfabort>
</cfif>
...
<cfif year(get_row_result.finish_date) neq session_period_year>
	<script type="text/javascript">
...
<cflock name="#CreateUUID()#" timeout="60">
        ...
        <cfif get_row.recordcount>
                ...
                <cfquery name="ADD_FIS_MONEY" datasource="#DSN3#">
                    INSERT INTO
                        ...
                        (
                            ...
                            #get_money_fis.rate2#,
                            #get_money_fis.rate1#,
                            <cfif get_money_fis.money eq session_money2>1<cfelse>0</cfif>
                        )
                </cfquery>
            </cfoutput>
            <cfoutput query="get_row">
                <cfscript>
					...
					sub_p_order_id=get_row.row_p_order_id;
                </cfscript>
				...
				<cfif len(sub_p_order_id)>
					<cfquery name="GET_RESULT_AMOUNT" datasource="#DSN3#">
							...
							WHERE
								...
								AND POR_.P_ORDER_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#sub_p_order_id#">
								AND POR_.TYPE = 1
						...
						<cfquery name="UPD_PROD_ORDERS" datasource="#DSN3#">
							UPDATE PRODUCTION_ORDERS SET RESULT_AMOUNT=#get_result_amount.RESULT_AMOUNT# WHERE P_ORDER_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#sub_p_order_id#">
						</cfquery>
				</cfif>
						<cfquery name="GET_RESULT_AMOUNT_party" datasource="#DSN3#">
							SELECT
								ISNULL(SUM(POR_.AMOUNT),0) RESULT_AMOUNT
							FROM
								PRODUCTION_ORDER_RESULTS_ROW POR_,
							...
							WHERE
								POR_.PR_ORDER_ID = POO.PR_ORDER_ID
								AND POO.PARTY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.party_id#">
								AND POR_.TYPE = 1
						</cfquery>
						<cfquery name="UPD_PROD_ORDERS" datasource="#DSN3#">
							UPDATE TEXTILE_PRODUCTION_ORDERS_MAIN SET RESULT_AMOUNT=#get_result_amount_party.RESULT_AMOUNT# WHERE PARTY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.party_id#">
						</cfquery>
        ...
        <cfif len(get_row.production_dep_id) and len(get_row.enter_dep_id) and get_row.recordcount>
            ...
            <cfquery name="GET_ROW_RESULT_COUNT" datasource="#DSN3#">
                ...
                FROM
                    PRODUCTION_ORDER_RESULTS
                WHERE
                    PARTY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#get_row.party_id#">
                ORDER BY
                    PR_ORDER_ID
            </cfquery>
					...
					VALUES
						(
						   '#left(form_name_product,75)#',
							...
							<cfif len(form_spect_id)>#form_spect_id#,<cfelse>NULL,</cfif>
							<cfif len(form_spect_name)>'#form_spect_name#',<cfelse>NULL,</cfif>
						    <cfif isdefined("attributes.x_lot_no_in_stocks_row") and attributes.x_lot_no_in_stocks_row eq 1>'#get_row.lot_no#',</cfif>
						   	'#form_other_money_currency#',
							#form_other_money#,
	...
	<cfif not (IsDefined("attributes.party_id") and len(attributes.party_id))>
        <cfquery name="GET_RESULT_AMOUNT" datasource="#DSN3#">
            SELECT
                ISNULL(SUM(POR_.AMOUNT),0) RESULT_AMOUNT
            FROM
        ...
        <cf_workcube_process 
            ...
            record_date='#now()#' 
            data_source='#dsn3#'
            action_page='#request.self#?fuseaction=prod.upd_prod_order_result&p_order_id=#get_row_result.p_order_id#&pr_order_id=#get_row_result.pr_order_id#' 
            action_id='#get_row_result.p_order_id#'
            action_table='PRODUCTION_ORDER_RESULTS'
            action_column='PR_ORDER_ID'
            old_process_line='#get_row_result.p_order_id#' 
            warning_description = 'Üretim Sonucu : #get_row_result.production_order_no#'>
  	</cftransaction>
</cflock>
...
<cfquery name="GET_P_O_R" datasource="#DSN3#">
	...
	WHERE
		...
		PRODUCTION_ORDER_RESULTS.PR_ORDER_ID=PRODUCTION_ORDER_RESULTS_ROW.PR_ORDER_ID AND
		PRODUCTION_ORDER_RESULTS.PARTY_ID=#GET_ROW_RESULT.PARTY_ID# AND
...
<cfelse>
	<cfif isDefined("session.ep")><!--- Pda de Gelmemesi Icin --->
		<cfif not isdefined('attributes.is_multi')><!--- Çoklu sayfalardan geliyorsa bu bloğa girmesin! --->
			<cflocation url="#request.self#?fuseaction=textile.list_results&event=upd&party_id=#attributes.party_id#&pr_order_id=#attributes.pr_order_id#" addtoken="no">
		</cfif>
	</cfif>
</cfif>
