...
<cfobject name="operation_process" component="addons.n1-soft.textile.cfc.operation_process">

<cfif cgi.REQUEST_METHOD eq "POST" and isDefined( "attributes.mode" ) and attributes.mode eq "save_list">
    <cfset predefined_list = operation_process.get_predefined_byname( attributes.listname )>
    <cfset predefined_current_id = 0>
    <cfif predefined_list.recordcount>
        <cfset predefined_current_id = predefined_list['PREDEFINED_ID'][1]>
        <cfset operation_process.clear_predefined_rows( predefined_current_id )>
    <cfelse>
        <cfset predefined_current_id = operation_process.insert_predefined( attributes.listname )>
    </cfif>
    <cfloop index="item" list="#attributes.listitems#">
        <cfset operation_process.insert_predefined_row( predefined_current_id, item )>
    </cfloop>
