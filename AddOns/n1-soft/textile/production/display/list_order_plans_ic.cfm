<cf_get_lang_set module_name="prod">
	...
	<cf_big_list id="list_order_big_list">
		<thead>
				<tr>
				<th></th>
				<th>S<cf_get_lang_main no='75.No'></th>
				<th>Resim</th>
				<th><cf_get_lang_main no='199.Sipariş'></th>
		...
		<cfif get_orders.recordcount>
			...
			<cfoutput query="get_orders" startrow="#attributes.startrow#" maxrows="#attributes.maxrows#">
				...
				<tr>
					...
					<td>#project_head# <input type="hidden" name="project_id#currentrow#" id="project_id#currentrow#" value="#project_id#"></td>
					...
					<td>#unit#</td>
					<td><input type="text" name="marj_amount#currentrow#" id="marj_amount#currentrow#" style="width:50px;float:right;" class="moneybox" onblur="amount_marj('#currentrow#');" value="#tlformat(marj)#"></td>
					...
					<!-- sil -->
						...
						<td>
							<input type="checkbox" name="row#currentrow#" id="row#currentrow#" value="#order_id#-#req_id#">
	...
	function demand_convert_to_production(type)
	{
		...
		{
					if(document.getElementById('row'+i)!=undefined && document.getElementById('row'+i).checked==true)
					{
				...
				window.location.href="<cfoutput>#request.self#?</cfoutput>fuseaction=textile.tracking&event=material&amounts="+amounts_+"&orderids="+orderids_+"&pids="+pids_+"&projectid="+projectid;
