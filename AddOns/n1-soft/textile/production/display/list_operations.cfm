                	...
                	<div class="col col-12 form-inline ">
                    	<div class="form-group">
	                        <div class="input-group x-15">
	                                ...
	                                <cfsavecontent variable="key_title"><cf_get_lang_main no='1677.Emir No'> ,<cf_get_lang_main no ='1622.Operasyon'> ,Numune No,<cf_get_lang_main no='799.Siparis No'> ve  <cf_get_lang no ='385.Lot No'>,<cf_get_lang_main no ='1372.Referans'>,<cf_get_lang_main no='245.Ürün'> Alanlarında Arama Yapabilirsiniz!</cfsavecontent>
								<cfset checked_list = "">
	                            ...
	                            <cf_wrk_search_input name="keyword" id="keyword" title="#key_title#" style="width:80px;" check_column="#checked_list#" checkbox="Üretim Emri No,Talep No,Lot No,Referans No,Numune Numarası,Sipariş Numarası" columnlist="P_ORDER_NO1,DEMAND_NO1,LOT_NO1,REFERENCE_NO1,REQ_NO1,ORDER_NUMBER1" placeholder="#getLang('assetcare',48)#">
                        ...
                        <div class="form-group">
                            <select name="result" id="result" style="width:100px;" title="Üretim Sonucu">
                                <option value="" <cfif not len(attributes.result)>selected</cfif>><cf_get_lang_main no='1854.Üretim Sonucu'></option>
...
<cf_big_list>
    <thead>
        <tr>
            ...
            <th style="width:50px;"><cf_get_lang_main no='799.Siparis No'></th>
            <!---<th style="width:80px;">Ürün Kodu</th>--->
            <th style="width:250px;"><cf_get_lang_main no='245.Ürün'></th>
                <th style="width:100px;"><cf_get_lang_main no='4.Proje'></th>
                <th style="text-align:left;width:100px;"><cf_get_lang_main no='1622.Operasyon'></th>         
                <th style="width:30px;text-align:right;">Planlanan Operasyon <cf_get_lang_main no='223.Miktar'></th>
                <th style="width:60px;text-align:right;">Gerçekleşen Operasyon Miktarı</th>
                <th style="width:30px;text-align:right;"><cf_get_lang no='295.Üretilen'></th>
                    <th style="width:30px;text-align:right;"><cf_get_lang_main no='1032.Kalan'></th>
            <!-- sil -->
            <cfif not (isdefined('attributes.is_excel') and attributes.is_excel eq 1)>
                        ...
                        <tr <cfif line eq 1>bgcolor="##FADBD8"</cfif>>
                            <cfif not (isdefined('attributes.is_excel') and attributes.is_excel eq 1)>
                                ...
                                <td align="center" id="order_row#currentrow#"  onclick="gizle_goster(operation_detail#currentrow#);connectAjax('#currentrow#','#main_operation_id#','#p_operation_id#','#operation_type_id#','#line#','#req_id#','#order_id#');gizle_goster_nested('siparis_goster#currentrow#','siparis_gizle#currentrow#');">
                                        <i id="siparis_goster#currentrow#" class="fa fa-chevron-circle-right fa-2x" aria-hidden="true" title="<cf_get_lang_main no ='1184.Göster'>"></i>	
                                        <i id="siparis_gizle#currentrow#" class="fa fa-chevron-circle-down fa-2x"  title="<cf_get_lang_main no ='1216.Gizle'>" style="display:none;"></i>
                                </td>
                                <!-- sil -->
                            </cfif>
                            <td>
