...
<cffunction name="addProduct"  access="remote" returntype="any" returnFormat="json">
		...
		<cfscript>
            attributes.sales_emp_id =arguments.sales_emp_id;
            attributes.sales_emp = arguments.sales_emp;
			attributes.product_status=1;
			FORM.PRODUCT_CATID=arguments.product_cat_id;
			//attributes.PRODUCT_NAME=arguments.product_name;
			attributes.short_code=arguments.short_code;
			attributes.short_code_id=arguments.short_code_id;
			FORM.TAX=8;					
			FORM.TAX_PURCHASE=8;
			FORM.BARCOD="";
		...
		<cfset attributes.brand_name="">
		<cfset attributes.brand_id="">
		<cfset FORM.PRODUCT_CODE="">
		<cfset FORM.MANUFACT_CODE="">
		<cfset FORM.SHELF_LIFE="">
		
		<cfset attributes.process_stage="">
		...
		<cfset attributes.product_code_2="">
		<cfset UNIT_ID="1,Adet">
		<cfset FORM.DIMENTION="">
		<cfset FORM.WEIGHT="">
		<cfset attributes.VOLUME="">
		<cfset attributes.PURCHASE=0>
		<cfset attributes.IS_TAX_INCLUDED_PURCHASE=1>
		<cfset attributes.tax_purchase=8>
		<cfset FORM.MONEY_ID_SA='#session.ep.money#'>
		<cfset attributes.price=0>
		<cfset attributes.IS_TAX_INCLUDED_SALES=0>
		<cfset attributes.tax=8>
			<cfset attributes.PRODUCT_CATID=arguments.product_cat_id>
						<cfset attributes.BRAND_CODE="">
		...
		<cfset attributes.fuseaction="textile.list_sample_request">
...
<cf_date tarih='bugun_00'>
	...
	<cfif len(GET_PRODUCT_CAT.PRODUCT_CAT)>
		...
		<cfif GET_CODES.recordcount> 
			<cfset attributes.acc_code_cat=GET_CODES.PRO_CODE_CATID>
		</cfif>
		<cfquery name="GET_OTHER_PERIOD" datasource="#DSN3#">
			SELECT PERIOD_ID FROM #dsn_alias#.SETUP_PERIOD WHERE OUR_COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#SESSION.EP.COMPANY_ID#">
		</cfquery>
	...
	--->
		...
		<cfset attributes.PRODUCT_NAME=pname>
	...
	<cftransaction>
		<cfquery name="ADD_PRODUCT" datasource="#DSN3#">
			...
			(
				...
				<cfif isDefined('attributes.MIN_MARGIN') and len(attributes.MIN_MARGIN)><cfqueryparam cfsqltype="cf_sql_float" value="#attributes.min_margin#"><cfelse>0</cfif>,
				<cfif isDefined('attributes.MAX_MARGIN') and len(attributes.MAX_MARGIN)><cfqueryparam cfsqltype="cf_sql_float" value="#attributes.max_margin#"><cfelse>0</cfif>,
				<cfif isDefined('attributes.OTV') and len(attributes.OTV)><cfqueryparam cfsqltype="cf_sql_float" value="#attributes.OTV#"><cfelse><cfqueryparam cfsqltype="cf_sql_float" value="" null="yes"></cfif>,
       ...
       <!--- <cfif GET_OUR_COMPANY_INFO.IS_PRODUCT_COMPANY EQ 1>
		...
		<cfscript>
			if (isnumeric(attributes.PURCHASE))
				...
				{
					purchase_kdvsiz = (attributes.PURCHASE*100/(attributes.tax_purchase+100));
				...
				else
				{
					...
					purchase_kdvli =  attributes.PURCHASE*(1+(attributes.tax_purchase/100));
		...
		<cfscript>
			if (isnumeric(attributes.PRICE))
				...
				{
					price_kdvsiz = attributes.PRICE*100/(attributes.tax+100);
				...
				else
				{
					...
					price_kdvli = attributes.PRICE*(1+(attributes.tax/100));
