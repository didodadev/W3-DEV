...
<cf_xml_page_edit fuseact="product.form_add_product">
	...
	<cfif not get_color_property.recordcount or not get_size_property.recordcount or not get_len_property.recordcount>
		<script type="text/javascript">
			alert("<cf_get_lang no='1002.Ürün Renk ve Beden ve Boy Özelliklerini Giriniz !'> !");
			window.close();
		</script>
		<cfabort>
	</cfif>
												  ...
												  <table class="table table-hover" >
													  ...
													  <input type="hidden" name="recordnum_size" id="recordnum_size" value="<cfoutput>#stokOzellikBeden.recordcount#</cfoutput>">
														...
														<cfif stokOzellikRenBoy.recordcount gt 0>
															...
															<cfelse>
																<tr><td colspan="4"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i>Sipariş oluşturulabilecek kayıtlı asorti stoğu bulunamadı! </td></tr>
															</cfif>
...
function addsales(tip)
		{
				...
				<cfelse>
					if(tip==0)
						add_part.action="<cfoutput>#request.self#?fuseaction=sales.list_order&event=add&order_id=&assorment=&req_id=#attributes.req_id#&ref_no=#attributes.req_id#&convert_products_id=1&convert_stocks_id=1</cfoutput>";
					else
