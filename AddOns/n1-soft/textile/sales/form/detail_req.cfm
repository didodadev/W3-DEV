    ...
    <div class="row">
		...
		<div class="col col-12 col-md-12 col-xs-12" id="divTabMenu" style="padding:0;overflow:hidden;">
			<div id="slider" style="position:relative;">
				<div>
                <cfsavecontent variable="message1">KUMAŞ DETAYLARI</cfsavecontent>
                ...
                <cfsavecontent variable="message2">AKSESUAR DETAYLARI</cfsavecontent>
                ...
                <cfsavecontent variable="message2">İŞÇİLİK FİYAT TALEPLERİ</cfsavecontent>
                <cf_box id="list_process"
                    box_page="#request.self#?fuseaction=textile.emptypopup_ajax_req_process_list&req_id=#attributes.req_id#&request_plan=1&req_stage=#get_opportunity.req_stage#&product_catid=#get_opportunity.PRODUCT_CATID#"
                    title="#message2#"
                    closable="0">
				</cf_box>
				<cfsavecontent variable="message2">DİĞER İŞÇİLİKLER</cfsavecontent>
		...
		<!---<cf_get_workcube_asset_textile id="olcu" title="ÖLÇÜ TABLOSU" company_id="#session.ep.company_id#" asset_cat_id="-3" module_id='99' action_section='OLCU_TABLO' action_id='#attributes.req_id#'>
				...
				<cf_box style="text-align:right;">
					    <button type="button" class="btn btn-success" id="btnstockcreate">Stok Oluştur</button>
						<button type="button" class="btn btn-success" id="btnorderadd">Sipariş Oluştur</button>
						<button type="button" class="btn btn-success" id="btnkanban">Kanban</button>
				</cf_box>
...
// jQuery
        ...
        $(document).ready(function () {
				...
				$('#btnstockcreate').click(function(){
					windowopen('<cfoutput>#request.self#?fuseaction=textile.list_sample_request&event=add_stock&pid=#get_opportunity.product_id#&pcode=#get_opportunity.PRODUCT_CODE#&req_id=#req_id#</cfoutput>','list');
				});
				$('#btnkanban').click(function(){
					window.open('<cfoutput>#request.self#?fuseaction=project.projects&event=kanban&id=#get_opportunity.project_id#</cfoutput>','page');
				});
