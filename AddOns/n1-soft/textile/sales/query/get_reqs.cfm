...
<cfquery name="GET_SAMPLE_REQ" datasource="#DSN3#">
	...
	<cfif isdefined("attributes.is_detail_stage")>
		,
			(select TOP 1 STAGE_ID from TEXTILE_PRODUCT_PLAN WHERE PLAN_TYPE_ID=2 AND REQUEST_ID=SAMPLE_REQ.REQ_ID ORDER BY PLAN_ID DESC) AS FABRIC_STAGE,
			(select TOP 1 STAGE_ID from TEXTILE_PRODUCT_PLAN WHERE PLAN_TYPE_ID=3 AND REQUEST_ID=SAMPLE_REQ.REQ_ID ORDER BY PLAN_ID DESC) AS ACCESSORY_STAGE,
			(select TOP 1 STAGE_ID from TEXTILE_PRODUCT_PLAN WHERE PLAN_TYPE_ID=4 AND REQUEST_ID=SAMPLE_REQ.REQ_ID ORDER BY PLAN_ID DESC) AS PATTERN_STAGE,
			(select TOP 1 STAGE_ID from TEXTILE_PRODUCT_PLAN WHERE PLAN_TYPE_ID=5 AND REQUEST_ID=SAMPLE_REQ.REQ_ID ORDER BY PLAN_ID DESC) AS WORKSHIP_STAGE,
			(select TOP 1 STAGE_ID from TEXTILE_PRODUCT_PLAN WHERE PLAN_TYPE_ID=7 AND REQUEST_ID=SAMPLE_REQ.REQ_ID ORDER BY PLAN_ID DESC) AS WASH_STAGE,
			(select TOP 1 STAGE_ID from TEXTILE_PRODUCT_PLAN WHERE PLAN_TYPE_ID=8 AND REQUEST_ID=SAMPLE_REQ.REQ_ID ORDER BY PLAN_ID DESC) AS MOLD_STAGE,
			(select TOP 1 STAGE_ID from TEXTILE_PRODUCT_PLAN WHERE PLAN_TYPE_ID=9 AND REQUEST_ID=SAMPLE_REQ.REQ_ID ORDER BY PLAN_ID DESC) AS MARKER_STAGE
		</cfif>
	...
	FROM
        ...
        LEFT JOIN SETUP_OPPORTUNITY_TYPE SOT ON SAMPLE_REQ.REQ_TYPE_ID = SOT.OPPORTUNITY_TYPE_ID
