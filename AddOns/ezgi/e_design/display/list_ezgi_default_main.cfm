		...
		FROM            
      	...
      	WHERE 
            ...
            <cfif len(attributes.keyword)>
            	AND 
                (
                	MAIN_ROW_SETUP_NAME LIKE '%#attributes.keyword#%' OR
                    MAIN_ROW_SETUP_CODE LIKE '%#attributes.keyword#%'
                )
            </cfif>
      	...
      	ORDER BY
            ...
            <cfelseif attributes.oby eq 2>
            	MAIN_ROW_SETUP_NAME
            <cfelse>
            	MAIN_ROW_SETUP_ID desc
            </cfif>
    </cfquery>
	<cfparam name="attributes.totalrecords" default='#get_main.recordcount#'>
<cfelse>
	<cfset get_main.recordcount = 0>
...
<cfsavecontent variable="header_">
	<cfoutput>#getLang('settings',1133)# #getLang('prod',429)# #getLang('product',92)#</cfoutput>
</cfsavecontent>
...
<cf_big_list>
    <thead>
        <tr>
            ...
            <th style="width:60px;"><cfoutput>#getLang('prod',429)#</cfoutput> ID</th>
            <th style="width:90px;"><cfoutput>#getLang('prod',429)# #getLang('objects',256)#</cfoutput></th>
           	<th><cfoutput>#getLang('settings',1133)# #getLang('prod',429)# #getLang('main',485)#</cfoutput></th>
    ...
    <tbody>
        <cfif len(attributes.is_submitted)>
            <cfif get_main.recordcount>
                    <cfoutput query="get_main" startrow="#attributes.startrow#" maxrows="#attributes.maxrows#">
                        <tr>
                        	<td style="text-align:right">#currentrow#</td>
                            <td style="text-align:center">#MAIN_ROW_SETUP_ID#</td>
                            <td style="text-align:center">
                            	<a href="#request.self#?fuseaction=prod.upd_ezgi_default_main&main_id=#MAIN_ROW_SETUP_ID#" class="tableyazi">
                					#MAIN_ROW_SETUP_CODE#
               					</a>
                            </td>
                            ...
                            <!-- sil -->
                            <td style="text-align:center;">
                            	<a href="#request.self#?fuseaction=prod.upd_ezgi_default_main&main_id=#MAIN_ROW_SETUP_ID#">
		...
		<tr>
			<td>
				<cf_pages 
					...
					totalrecords="#get_main.recordcount#" 
