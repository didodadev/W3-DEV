...
<cfif isdefined("attributes.is_submitted") and len(attributes.is_submitted)>
		...
		FROM            
      	...
      	WHERE 
            ...
            <cfif len(attributes.keyword)>
            	AND 
                (
                	COLOR_NAME LIKE '%#attributes.keyword#%' OR
                    PROPERTY_DETAIL_CODE LIKE '%#attributes.keyword#%'
                )
            </cfif>
	...
	<cfparam name="attributes.totalrecords" default='#get_color.recordcount#'>
<cfelse>
	<cfset get_color.recordcount = 0>
...
<cfsavecontent variable="header_">
	<cfoutput>#getLang('settings',1133)# #getLang('prod',241)# #getLang('product',92)#</cfoutput>
</cfsavecontent>
<cfform name="search_list" action="#request.self#?fuseaction=prod.list_ezgi_default_color" method="post">
...
<cf_big_list>
    <thead>
        <tr>
            ...
            <th style="width:60px;"><cfoutput>#getLang('prod',241)#</cfoutput> ID</th>
            <th style="width:90px;"><cfoutput>#getLang('prod',241)# #getLang('main',1173)#</cfoutput></th>
           	<th><cfoutput>#getLang('settings',1133)# #getLang('prod',241)# #getLang('main',485)#</cfoutput></th>
            ...
            <th style="text-align:center;width:10px;">
             	<cfoutput>
                	<a href="#request.self#?fuseaction=prod.add_ezgi_default_color">
                		<img src="/images/plus_list.gif" style="text-align:center" title="#getLang('settings',1133)# #getLang('main',2836)#">
               		</a>
              	</cfoutput>
            </th>
            <!-- sil -->
    ...
    <tbody>
        <cfif len(attributes.is_submitted)>
            <cfif get_color.recordcount>
                    <cfoutput query="get_color" startrow="#attributes.startrow#" maxrows="#attributes.maxrows#">
                        <tr>
                        	<td style="text-align:right">#currentrow#</td>
                            <td style="text-align:center">#COLOR_ID#</td>
                            <td style="text-align:center">
                            	<a href="#request.self#?fuseaction=prod.upd_ezgi_default_color&color_id=#COLOR_ID#" class="tableyazi">
                					#PROPERTY_DETAIL_CODE#
               					</a>
                            </td>
                            <td>#COLOR_NAME#</td>
                            ...
                            <!-- sil -->
                            <td style="text-align:center;">
                            	<a href="#request.self#?fuseaction=prod.upd_ezgi_default_color&color_id=#COLOR_ID#">
		...
		<tr>
			<td>
				<cf_pages 
					...
					totalrecords="#get_color.recordcount#" 
