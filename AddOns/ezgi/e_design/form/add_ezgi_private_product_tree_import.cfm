...
<cfquery name="get_all_material" datasource="#dsn3#">
	SELECT DESIGN_PACKAGE_ROW_ID FROM EZGI_DESIGN_ALL_MATERIAL WHERE DESIGN_ID = #attributes.design_id# AND DESIGN_PACKAGE_ROW_ID IS NOT NULL
</cfquery>
<cfif get_all_material.recordcount> <!---Paket Sepetindeki Ürün Kontrolü--->
	<script type="text/javascript">
		alert("<cf_get_lang_main no='2959.Paket İçindeki Malzeme Sepetinde Malzeme Mevcut Lütfen Düzenleyip Yeniden Deneyin'> !");
		window.close();
	</script>
  	<cfabort>
</cfif>
    ...
    <cf_big_list id="list_product_big_list">
        <thead>
            <tr>
                ...
                <th style="text-align:left; width:60px"><cfoutput>#getLang('main',245)# #getLang('product',154)#</cfoutput></th>
				<th style="text-align:left;"><cfoutput>#getLang('main',809)#</cfoutput></th>
			...
			<cfoutput query="get_satirlar">
           		<tr id="row_#id#_#main_id#_#currentrow#">
                  	...
                  	<td style="text-align:left;" nowrap="nowrap">&nbsp;
                    	<cfif id eq 1>
                        	<cfoutput>#getLang('main',2848)#</cfoutput>
                        <cfelseif id eq 2>
                        	<cfoutput>#getLang('main',2903)#</cfoutput>
                        <cfelseif id eq 3>
                        	<cfoutput>#getLang('prod',429)#</cfoutput>
                        </cfif>
                    </td>
                	<td style="text-align:left;" nowrap="nowrap">&nbsp;#NAME#</td>
                  	<td style="text-align:center;" nowrap="nowrap">
                        ...
                        <cfelse>
                            ...
                            <cfelse>
                                <img src="images/c_ok.gif" title="#getLang('product',551)#" />
                            </cfif>
                        </cfif>
                    </td>
				</tr>
        	...
        	<tfoot>
            	<tr>
               		<td colspan="4" height="20" style="text-align:right">
						<cfinput type="button" value="  #getLang('main',1371)# #getLang('main',1156)#  " name="buton" onClick="transfer_product_tree()">              
                  	</td>
            	</tr>
       		</tfoot>
    	<cfelse>
...
<script language="javascript">
	...
	{
		sor=confirm('<cf_get_lang_main no='2971.Seçtiğiniz Kayıtların Workcube İmport İşlemini Onaylıyor musunuz'>?');
		if (sor == true)
		{
