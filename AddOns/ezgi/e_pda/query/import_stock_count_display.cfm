...
<table width="98%" border="0" cellspacing="0" cellpadding="0" height="35" align="center">
	<tr><td class="headbold"><cf_get_lang_main no='3196.Stok Sayım Sonuçları'> (<cfoutput>#form.store#-#dateformat(get_stock_date,'dd/mm/yyyy')#</cfoutput>)</td></tr>
</table>
<cfif isdefined('error_flag') and error_flag eq 1>
	<script type="text/javascript">
		alert('<cf_get_lang_main no='3195.Belgenizin Miktarlarını Kontrol Ediniz!'>');
		window.history.back();
	</script>
	<cftry>
			<cffile action="delete" file="#upload_folder##file_name#">
		<cfcatch type="Any"><cf_get_lang_main no='3194.Dosya Silinemedi'></cfcatch>  
	</cftry>
	<cfabort>
</cfif>
<cfif not listlen(barcod_id_list)>
	<script type="text/javascript">
		alert('Belgenizde '+ <cfif attributes.stock_identity_type eq 1>'Barkod Listesi'<cfelseif attributes.stock_identity_type eq 3>'Özel Kod Listesi'<cfelse>'Stok Kodu Listesi'</cfif>  + ' Bulunamadı. <cf_get_lang_main no='3193.Lütfen Belgenizi Kontrol Ediniz!'>');
		window.history.back();
	</script>
	<cfabort>
</cfif>
...
<script type="text/javascript">
	<cfif isdefined('error_flag')>
		alert('<cf_get_lang_main no='3192.Bazı Ürünler Kayıtlı Değil veya Ürün Bilgisi İle Eşlenen Birden Fazla Ürün Var!'>');
	</cfif>
	<cfif not isDefined("session.ep")>
	wrk_opener_reload();
	</cfif>
