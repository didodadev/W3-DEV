...
<cfform name="form_basket">
  ...
  <table cellpadding="2" cellspacing="1" align="left" class="color-border" width="99%">
      ...
      <td colspan="4">
      	<table border="0" cellpadding="0" cellspacing="0" width="100%" class="color-border">
           <tr class="color-list" height="15px">
            ...
            <td align="center" width="50%"><cfoutput>#getLang('prod',193)#</cfoutput></td>
           </tr>
           <tr class="color-list" height="20px">
            <td>
    ...
    <tr class="color-list" height="15px">
      ...
      <td width="55" align="left"><cf_get_lang_main no='809.Ürün Adı'></td>
      <td width="25" align="right"><cf_get_lang_main no='223.Miktar'></td>
      <td width="50" align="left"><cfoutput>#getLang('prod',401)#</cfoutput></td>
    </tr>
    <tr class="color-list" height="20px">
	...
	{
		{
			  ...
			  if(amount == 0)
			  {
				alert('<cf_get_lang_main no='3147.Miktar 0 dan Büyük Olmalıdır'>.');
				document.getElementById('shelf_select_td').style.display='none';
				return false;
			  }
			  add_amount();
