...
<cfcomponent accessors="true" extends="parameter">
    ...
    <cfset wbpPath = '#ReplaceNoCase(Replace(GetDirectoryFromPath(GetBaseTemplatePath()),'\','/','ALL'),"workcloud/cfc/","")#WBP' />
    ...
    <cffunction  name="controlWBPLicense" returnType = "any" returnformat="JSON" access = "remote" hint = "WBP License Control">
        ...
        <cfscript>
            get_license_information = workcube_license.get_license_information();
            if( len(get_license_information.PROD) ){
                ...
                if( isStruct(license_prod) ){ //Satın alınmış
                    response = { subscription_status: true };
                }else{//Satın alınmamış
                    response = { subscription_status: false };
                }
            }else response = { subscription_status: false };
        </cfscript>
    ...
    <cffunction name = "complete_wbp_upload" returnType = "any" returnformat="JSON" access = "remote" hint = "Completed WBP Upload">
        ...
        <cftry>
            <cfquery name="updateOurCompanyInfo" datasource="#getDatasource()#">
                UPDATE OUR_COMPANY_INFO SET PROD = '#getWbp_code()#,' WHERE COMP_ID IN (#getOur_company_ids()#)
            </cfquery>
    ...
    <cfscript>
        ...
        public any function querySettings( queryContent ){
            ...
            if( LCase( getSchema_type() ) eq 'main' || LCase( getSchema_type() ) eq 'product' ){
                ...
                periodQuery = new Query(datasource=getDatasource(), sql="SELECT OUR_COMPANY_ID,PERIOD_YEAR FROM SETUP_PERIOD ORDER BY PERIOD_YEAR,OUR_COMPANY_ID").execute().getResult();
			    companyQuery = new Query(datasource=getDatasource(), sql="SELECT DISTINCT OUR_COMPANY_ID FROM SETUP_PERIOD").execute().getResult();
        ...
        public any function execute_script_buffer() returnformat="JSON" {
            ...
            wbpInfo = this.get();
            if( not wbpInfo.recordcount or ( wbpInfo.recordcount and wbpInfo.BPDL_STATUS[1] eq 0 ) ){
                if( FileExists( filePath ) ){
                    fileInfo = GetFileInfo( filePath );
                    if( len(fileInfo.size) and fileInfo.size gt 0 ){
                        queryContent = fileRead( filePath, 'UTF-8' );
        ...
        public any function get() {
            ...
            return new Query(datasource = getDatasource(), sql = "SELECT BPDL_ID, BPDL_STATUS FROM WRK_BESTPRACTICE_DATA_LIBRARY WHERE WBP_CODE = '#getWbp_code()#' AND COMPANY_ID = #getOur_company_ids()# AND BPDL_FILE = '#getSchema_type()#/#getwbp_type()#/#getObject_type()#.txt'").execute().getResult();
